...
{Stochastic Partial Differential Equations: Some Applications and Theoretical Results}
...
\begin{frame}{Some of the Questions}
	...
	\rdot We want to model the evolution of heat of a piece of an initially heated wire. How can we account for random outside sources of heat that affect the wires temperature? (SHE) 
...
\begin{frame}{Macroscopic Cell Growth}
	...
	\includegraphics[scale=.2]{maccell1.png} \qquad \includegraphics[scale=.2]{maccell2.png} \vfill
...
\begin{frame}{Microscopic Cell Growth}
	...
	\includegraphics[scale=.2]{miccell1.png} \qquad \includegraphics[scale=.2]{miccell2.png} \vfill
...
%	\end{frame}
	...
	\begin{frame}[t]{Random Walk $\to$ Brownian Motion}
		\begin{center}
			\includegraphics[scale=.55]{BrownianMotion.png}
		\end{center}
	\rdot Property of Brownian Motion: $B_t \sim B_{t - \Delta t} +  \text{Normal}(0,\Delta t)$. \vspace{.1in} \\
	\rdot Recall that $\text{Normal}(0,\Delta t) = \sqrt{\Delta t} \: \text{Normal}(0, 1)$
	\end{frame}
	...
	\end{frame}
		...
		\centering
		\includegraphics[width=13.5cm, height=8.5cm]{whitenoise.png}
	\end{frame}
...
\begin{frame}
	\begin{center}
		\includegraphics[scale=.55]{geometricbmotion.png}
	\end{center}
\end{frame}

\begin{frame}{Black-Scholes-Merton Option Model}
	\begin{center} 
	\includegraphics[scale=.5]{black.png} \hfill \includegraphics[scale=.542]{scholes.png} \hfill  \includegraphics[scale=.5]{merton.png} \vspace{.2in} \\
	\end{center}

\rdot Scholes and Merton received the Nobel Memorial Prize in Economics - 1997. \vspace{.1in} \\
\rdot Unfortunately Black passed away due to cancer before the award was presented. 
\end{frame}
...
%		\end{frame}
			...
			\[
			\begin{cases}
			\frac{\partial}{\partial t} u(t,x) - \frac{1}{2}\frac{\partial^2}{\partial x^2} u(t,x) = \lambda u(t,x) \frac{\partial^2}{\partial t \partial x}W(t,x) & t \ge 0, \; x\in \R \\
			u(0, x) = f(x) & x \in \R
			\end{cases}
			\]
			\vspace{.1 in}
...
%		\end{frame}
	...
	\begin{frame}{Random vs Deterministic}
			\rdot The solution to the stochastic heat equation is a stochastic process. 
			\[
				u(t,x)(\omega_1), \quad u(t,x)(\omega_2), \quad \cdots, \quad u(t,x)(\omega_n).
			\]
			 \rdot The random solution only exists implicitly through the integral equation. \vspace{.3in} \\
			\rdot $\frac{\partial}{\partial x} u(t,x)$ and $\frac{\partial}{\partial t} u(t,x)$ does not exists in the random case with space-time white noise. \vspace{.3in} \\
			\rdot Interesting 'blow up' happens in the random case. This actually has real-world application. 
	\end{frame}
...
\rdot This is a hard question and a current open problem is calculate the following limit
\[
\lim_{t \to \infty} \frac{\log|u(t,x)|}{t^{K}} = 
	\begin{cases}
		0 & t \mapsto u(t,x) \text{ grows slower than } e^{t^K} \\
		C & t \mapsto u(t,x) \text{ grows the same as } e^{C \: t^K} \\
		\infty & t \mapsto u(t,x) \text{ grows faster than } e^{t^K}
	\end{cases}.
\] \vspace{.15in}

\rdot The following has been calculated for the SHE and SWE (and also other SPDEs which we will see below)
\[
...
\]
	...
	\begin{equation*}
	\begin{cases}
	\left(\partial^b_t  -\Delta_x \right)\: u(t,x) =  u(t,x)\: \dot W(x) & \text{$x\in \R^d$, $t>0$} \\
	...
	\vspace{.5in}
		...
		\[
		\mathbb{E}(W(\phi)W(\psi)) = \int_{\R^{2d}} \ud x \ud y \; \phi(x) |x-y|^{-\alpha} \psi(y)
		\]
\end{frame}% }}}
	...
	% \end{equation*}
	\begin{center}
	{\raggedleft \rdot}	For the SHE, i.e., \textcolor{red}{$b=1$} \\
		{\small [X. Chen '17]}
		\[
	...
	\vspace{.3in}
	\begin{center}
	{\raggedleft \rdot}	For the SWE, i.e., \textcolor{red}{$b=2$} \\
...
%		\end{frame}
	...
	\rdot  $\partial^b_t$ is the {\it Caputo} fractional derivative of order $b > 0$ \vspace{.15in} \\
...
\begin{frame}
	\frametitle{Local Solution: {\small $\left(\partial^b_t + \frac{\nu}{2}(-\Delta)^{a/2}\right) u(t,x) = I^r_t \left[\sqrt{\theta}\: u(t,x)\: \dot W(x) \right]$}}
	\begin{definition}
	...
	\begin{theorem}[{\small Chen-E. '21+}]
		...
		\]
		In this case, a unique local $L^2(\Omega)$ solution exists for $t \in (0,T_2)$ where
		\[
		T_p := \dfrac{\nu^{\alpha/a}}{2 \theta \mathcal{M}_a^{(2a-\alpha)/a}}
		\]
...
\begin{frame}{Special Form of the Solution}
	\rdot The time independent noise and the constant initial conditions allows us to prove the following explicit representation of the solution:
...
\begin{frame}{Sketch}
	...
	\]
	so there exists a unique solution that is continuous, which we denote as $u_N := \left\{ u_N(t,x) : (t,x) \in [0,T] \times \R \right\}$. \vspace{.2in}
