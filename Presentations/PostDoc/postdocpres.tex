...
\begin{frame}{The Questions}
	  \rdot We want to model the evolution of heat of a piece of initially heated wire. How can we account for random outside sources of heat that affect the wires temperature? (SHE) \vspace {.5in} \\
	  ...
	  \rdot We are investors looking to make estimations on the return rate of our current investments. Can we make legitimate assumptions on the "randomness" of our returns? (Ornstein-Uhlenbeck process)
...
%	\end{frame}
  ...
  \rdot $S_0 = \text{stock price at entry}$ \quad \quad $S_t = \text{stock price after } t \text{ time}$. 
	\[
...
\rdot We can model this with a stochastic differential equation.
	\[
	R_t =	\frac{dS_t}{S_t} = \mu \ud t + \sigma \ud B_t, \quad \text{where } dS_t = S_{t + \Delta t} - S_t
	\]
\vspace{.1in} \\ 
\rdot It can be shown that $\mu \ud t + \sigma \ud B_t \sim \text{Normal}(\mu \cdot \Delta t,\; \sigma^2 \cdot \Delta t)$
\end{frame}
...
%		\end{frame}
		...
		\begin{frame}{Approximation of a Space-Time White Noise Differential}
			\centering
			\includegraphics[scale=.5]{spacetimewhite.png}
		\end{frame}
...
\rdot This limit has been calculated in the following sense for the SHE
\[
...
\end{frame}
	...
	\begin{frame}{The Case of Space-time White Noise}
	\begin{equation}
	\begin{cases}
...
\begin{frame}
	\rdot Really the only issue is the super-linear drift term. 
\end{frame}
