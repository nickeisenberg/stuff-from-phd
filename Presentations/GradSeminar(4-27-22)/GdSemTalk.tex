...
\begin{frame}
		...
		\]
	\end{theorem}
For example, if we apply the above to $f(x) = x^2 / 2$ then we have 
	\[
		\int_a^t B_s \; \ud B_s = \frac{B_t^2}{2} - \frac{B_a^2}{2} - \frac{t-a}{2},
	\]
which is close but not quite the same thing as 
	\[
		\int_a^t x \; \ud x = \frac{t^2}{2} - \frac{a^2}{2}.
	\]
\end{frame}
...
The notation 
	\[
		dS_t = \mu S_t \ud t + \sigma S_t \ud B_t, \quad \text{where } B_tQ \text{ is a Brownian motion}
	\]
