...
{%
	...
	\insertframenumber\,/\,\inserttotalframenumber\kern1em\vskip40pt%
}
...
{An introduction to stochastic calculus \\[0.3em]
  with applications to\\[0.3em]
finance \& SPDEs}

\author[Arthur, Doe] % (optional, for multiple authors)
{Nicholas Eisenberg\\[4em]}


\date[VLC 2021] % (optional)
{
  Graduate Seminar \\[1em] \small
  Auburn University\\
  March 29, 2022
}
...
<<<<<<< HEAD
\begin{frame}[t]{Proving the fundamental theorem of algebra using Brownian motion} 
	\begin{theorem}
		For any non-constant polynomial in $\mathbb{C}$, there exists a $z \in \mathbb{C}$ such that $p(z) = 0$.
	\end{theorem}
\begin{proof}
	\begin{itemize}
		\item If $p(z) \not = 0$ for and $z \in \mathbb{C}$ then $f = 1/p$ is analytic and tends to $0$ at infinity as $p$ is entire and thus unbounded.
		\item Let $\alpha < \beta$ and $\Lambda_1 = \{ \Re(f) \le \alpha\}$ and $\Lambda_2 = \{ \Re(f) \ge \beta \}$. Both $\Lambda_1$ and $\Lambda_2$ contain an open set due to continuity.
		\item $f(B_t)$ is a continuous local martingale and Doob's martingale convergence theorem implies $\lim_{t \to \infty }f(B_t)(\omega)$ exists with probability $1$.
		\item But, for all $T>0$, $B_t$ will revisit $\Lambda_1$ and $\Lambda_2$ after $T$ with probability $1$.
		\item So with probability $1$, $\liminf_{t \to \infty} f(B_t) \le \alpha < \beta \le \limsup_{t \to \infty} f(B_t)$. This contradicts the fact that $\lim_{t \to \infty }f(B_t)(\omega)$ exists with probability $1$.
	\end{itemize} 
\end{proof}
\end{frame}
...
=======
\begin{frame}[t]{The It\^o's integral \small (a special case -- {\it Wiener integral})}
>>>>>>> 76f3ad3de0fe1a53062d9ca99acbd47ec557b94e
...
\]
is only symbolic as $dB_t$ does not exist. \pause The above can be legitimately
...
\textcolor{magenta}{\it Geometric Brownian Motion}. \pause Notice that we have
...
\begin{frame}
  ...
  $\beta_t$ at time $t$. \pause Similarly...
		\[
