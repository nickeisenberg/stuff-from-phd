...
\begin{remark}
	...
	For example, one may choose $\rho$ to be a nonnegative function with compact support, or to be
	$G(1,\cdot)$. In both cases, $\rho$ is smaller than those in \eqref{E:admRho} (up to a
	constant). However, one can easily check that the admissible condition in \eqref{E:aw} exclude these
	two cases.
\end{remark}
...
\begin{lemma}
	\label{L:admRho}
	For any $a>0$ and $b\in (0,2)$, the following weight function	 is admissible:
	\begin{equation*}
