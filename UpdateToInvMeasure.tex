...
We note that in order to prove \eqref{E:RhoDom}, it suffices to show that there exists some $K'_a \in \mathbb{N}$ such that
	...
	\textcolor{blue}{The following is incorrect because $\rho$ does not need to be bounded. Continuity and integrability do not imply boundedness on the whole space. If the limit in \eqref{E:awbd3} in positive then the result will work. I am unsure if the limit even need to exist though, can the numerator in \eqref{E:awbd3} have an infinite number of growing spikes?}
...
\textcolor{red}{The following needs to be investigated more. It is used in the first couple lines of Step 1 in the proof of theorem 3.1 in their paper. Its used to prove that \eqref{E:fop} is compact. This is only stated but not proven in the Tessitore paper}
