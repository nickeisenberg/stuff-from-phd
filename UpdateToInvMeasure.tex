...
\end{align}
It is known that in most cases, solutions to \eqref{E:gspde} are {\it intermittent}, namely, the
probability moments to \eqref{E:gspde} have exponential growth in $t$. In order to have
\eqref{E:BddMnt}, one has to first identify the exact conditions, under which the moments to
\eqref{E:gspde} do not possess exponential growth. This question has been answered in \cite{CK19},
where necessary and sufficient conditions are given. In \cite{AM03,MSY15,MSY16}, a drift term
$f(x,u)$ is included in \eqref{E:gspde} in a crucial way to help to cancel the otherwise exponential
growing moments. The absence of such a drift term in this paper makes the problem more challenging.
According to part (3) of Theorem 1.3 of \cite{CK19}, in order to have \eqref{E:BddMnt}, one has to
have the Lyapunov exponent of $b$ is small enough and
\begin{equation}
	\label{E:U0finite}
	\Upsilon(0) <\infty,
\end{equation}
...
a more broad range of initial measures which include the Dirac delta
distribution $\delta_0$ and for more
...
\begin{assumptions}
	...
	\begin{enumerate}[(i)]
		...
		\begin{equation}
			\label{A:dalcon}
			\Upsilon(\beta) := (2\pi)^{-d}\int_{\R^d}\dfrac{\hat f(\ud \xi)}{\beta + |\xi|^2} < \infty
		...
		\begin{equation}
		|b(x,u)-b(x,v)|< L |u-v| \quad \text{and} \quad |b(x,0)| \le L_0 \quad \text{for all } u,v \in \R, \; x \in \R^d \label{A:lipcon}.
		\end{equation}
	\end{enumerate}
\end{assumptions}
...
\begin{theorem}
	...
	\end{align*}
	and $u^{\hat\varphi}(\cdot,x)$ is bounded in probability in $L^2_{\hat\rho}(\R^d)$.
...
\end{equation}
It is shown in the proof of Theorem 3.1 \cite{TZ98} that $F_\alpha$ is compact from $L^q\big((0,t_0),L^2_{\hat\rho} \big)$ to
