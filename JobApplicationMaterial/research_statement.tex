...
\thispagestyle{fancy}
\fancyhf{}
\rhead{Email : \href{mailto:nickeisenberg@gmail.com}{nickeisenberg@gmail.com}, \\
...
\section{Overview} I am currently a PhD student studying at Auburn University under the supervision of \href{http://webhome.auburn.edu/~lzc0090/}{Dr. Le Chen} and I will be defending my thesis in August 2022. Overall, my research deals with a space-time fractional stochastic partial differential equation driven by some type of Gaussian noise which varies depending on the problem in question. I will refer this equation as the {\it Interpolated Stochastic Heat and Wave Equation} (ISHWE),
	\begin{equation}
	\label{E:ISHWE}
		\begin{cases}
		...
		\end{cases}
	\end{equation}
The ISHWE interpolates the well known {\it Stochastic Wave and Stochastic Heat Equations}, SWE and SHE respectively, in the sense that when we let $\alpha = \beta =2$ and $\gamma =0$ then we recover the SWE and when $\alpha =2$ and $\beta=1$ and $\gamma =0$ then we recover the SHE. I have worked on problems that have dealt with proving the existence and uniqueness of a solution, deriving the exact blow-up time for a local solution, proving the existence of invariant measures of the solution, deriving exact moment asymptotics, and proving regularity of the solution. 
...
Assume $d \ge 3$ and consider any pair of admissible weights $\rho$ and $\hat{\rho}$ such that
	\[
		\int_{\R^d}\dfrac{\rho(x)}{\hat\rho(x)}\: \ud x < \infty.
	\]
Also, suppose that the initial measure $\mu$ satisfies the following integrability condition:
		\begin{equation}
		\label{A:InvAssumpMeas}
			 \limsup_{t \ge 0}	\int_{\R^d} \ud x \hat{\rho}(x) \left[\int_{\R^d} |\mu|(\ud y) G(t,x-y)\right]^2 < \infty,
		\end{equation}
and that the spectral measure $\hat{f}$ satisfies both 
		\begin{equation}
		\label{A:InvAssumpSpec}
			\Upsilon(0) := (2\pi)^{-d}\int_{\R^d}\dfrac{\hat f(\ud \xi)}{|\xi|^2} < \infty, \quad \text{and} \quad \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \hat f(\ud \xi) \exp\left(-(s-r)|\xi|^2 \right) < \infty.
		\end{equation}
...
\hspace{\parindent} In this project, we studied the the ISHWE with constant one initial condition:
	\begin{equation}
	\label{E:ISHWEone}
		\begin{cases}
...
\label{S:GlobalSupLinWhite}
My current project deals with proving the existence and uniqueness a of global solution to \eqref{E:ISHWE} with a super-linear diffusion term $\sigma$ that satisfies the following growth condition: as $|x| \to \infty$,
\begin{equation}
\label{C:SigSupLin}
|\sigma(x)| \le \sigma_1 + \sigma_2|x|[\ln(|x|)]^{\delta},
\end{equation}
where $\sigma_1 = c(\sigma) = \sigma(0)$ and $\delta >0$. The work is an extension of Millet and Sanz-Sol\'e \cite{SoleMillet2021}. It is often the case that blow of the solution to SPDE's will occur when considering a super-linear diffusion coefficient. For example, Asogwa {\it et al} \cite{AMN19} study \eqref{E:ISHWE} where $\beta \in (0,1]$, $\alpha \in (0,2)$, and $\gamma = 1- \beta$. They prove in Theorem 1.2 that blow up will occur in finite time when the diffusion coefficient has super linear growth and is bounded below by a non-zero constant. The techniques in their paper can also be easily adapted \eqref{E:ISHWE} for $\beta \in (0,2)$, $\alpha \in (0,2)$ and $\gamma >0$.

In this project, we require the initial conditions, $u_0=1$ and $v_0=0$, and the noise $\dot{W}$ may either be space-time white or white in time and colored in space. When the noise is colored in space, we take the correlation function to be the Riesz kernel. As of now, I have more or less completed the case of space-time white noise and the same techniques should also apply to the colored noise case as well. 

I will now outline the steps which prove the existence of a global solution to \eqref{E:ISHWE} driven by space-time white noise. We first consider the case where the diffusion coefficient $\sigma$ is globally Lipschitz. In this case, existence and uniqueness is proven in \cite[Theorem 3.1]{CHN19}. The next step was to prove the Proposition \ref{P:Holder} stated below. We note that this H\"older regularity proven in this result recovers \cite[Theorem 1.9]{FN2016}.
	...
	\]
	where $K$ is a specific constant, whose definition at the moment is unimportant, then we have that for any $R >0$, 
	\[
...
After proving this proposition, we can consider proving the existence of a global solution to \eqref{E:ISHWE} with a super-linear diffusion coefficient satisfying \eqref{C:SigSupLin}. For simplicity, suppose $d=1$. What we do is truncate $\sigma$ in following sense:
	\[
...
then through a bit of analysis we deduce that $\tau_N \uparrow \infty$ as $N \to \infty$ and that on $\{t < \tau_N\}$, $u_N(t,x) = u_{N+k}(t,x)$ for any $k \in \mathbb{N}$. Lastly, we define on each $\{ t < \tau_N \}$, $(u(t,x) : (t,x) \in [0,T) \times \R)$ by $u(t,x) = u_N(t,x)$. A little more analysis shows that $u$ is a global solution to \eqref{E:ISHWE}. Thus we have our next theorem. 
	...
	\begin{theorem}
		Consider the initial conditions $u_0=1$ and $v_0=0$ and suppose that $\sigma$ satisfies \eqref{C:SigSupLin}. Then for any $M>0$ and $T>0$, there exists a random field solution to \eqref{E:ISHWE} denoted as $\{ u(t,x): (t,x) \in [0,T]\times B_M(0) \}$. This solution satisfies 
	\begin{equation}
		\sup_{t\in[0,T],\; |x| \le M} |u(t,x)| < \infty, a.s.
	\end{equation}
	\end{theorem}

One last thing to note here is that the solution $u$ constructed in the above theorem is not defined in $L^2(\Omega)$. In order to define $u$, we use an extension of the Walsh integral which can be found in \cite{DalangSoleBook}

\section{Future Projects}
In this section I will introduce four possible future projects. 

\subsection{Sharp Moment Bounds for the Solution to the ISHWE Driven by a Spatially Colored Gaussian Noise}

This project will try to generalize the following lemma to the ISHWE. 

	\begin{lemma}[\normalfont {\cite[Lemma 2.2]{CH16Comparison}}]
Suppose $\alpha=2$, $\beta=1$, $\gamma=0$ and $\sigma(x) = \lambda x$ in \eqref{E:ISHWE}. Suppose that the measure $\mu$ is such that $\int_{\R^d} \exp(-a|x|^2) |\mu|(\ud x) < \infty$. Denote $J_0(t,x)$ as the solution to the deterministic homogeneous SHE. Let $g:[0,\infty) \times \R^d \to [0,\infty)$ satisfy 
	\[
		\int_0^t \ud s \iint_{\R^{2d}} \ud y_1 \ud y_2 G(t-s,x-y_1)G(t-s,x-y_2)f(y_1-y_2) g(s,y_1)g(s,y_2) < \infty.
	\]
In addition suppose,
	\[
		g^2(t,x) \le J_0^2(t,x) + \lambda^2\int_0^t \ud s \iint_{\R^{2d}} \ud y_1 \ud y_2 G(t-s,x-y_1)G(t-s,x-y_2)f(y_1-y_2) g(s,y_1)g(s,y_2).
	\]
Then,
	\[
		g(t,x) \le (|\mu|*G(t,\cdot))(x)H(t;2\lambda^2)^{1/2}.
	\]
	\end{lemma}

There are two issues that arise. First is the loss of the following identity for the fundamental solution of the SHE: 
\[
G(s,x)G(t-s,y) = G\left( \dfrac{s(t-s)}{t}, \dfrac{sy -(t-s)x}{t} \right) G(t,x+y).
\] 
Second is the spatially colored noise. Because of the noise, we have that 
	\[
		\mathbb{E}(|u(t,x)|^2) = J_0^2(t,x) + \lambda^2\int_0^t \ud s \iint_{\R^{2d}} \ud y_1 \ud y_2 G(t-s,x-y_1)G(t-s,x-y_2)f(y_1-y_2) \mathbb{E}[u(s,y_1)u(s,y_2)].
	\]
In the space-time white noise case, we have that $f(x-y) = \delta_0(x-y)$, which simplifies the expression above. 
\subsection{Solvability of the ISHWE With Superlinear Diffusion Term Driven by a Spatially Colored Gaussian Noise}
This project will be a natural extension to my current project, which was explained in Section \ref{S:GlobalSupLinWhite} above. The goal will be to prove the following results.

	\begin{theorem}
For $p \ge 2$, $T>0$ and $R>0$, there exists a series of constants $K_1$, $K_2$ and $K_3$ such that 
	\[
		\frac{\Norm{u(t,x)-u(s,z)}}{|x-z|^{v_1} + |t-s|^{v_2}} \le K_1\left(K_2 + K_2\exp(T a) \mathcal{N}_{a,p}(u)\right)
	\]
where $v_1$ and $v_2$ are to be determined and $\mathcal{N}_{a,p} = \sup_{t \ge 0, \; x \in \R^d} \exp(-at)\Norm{u(t,x)}_p$.
	\end{theorem}

	\begin{theorem}
Consider the initial conditions $u_0=1$ and $v_0=0$ and suppose that $\sigma$ satisfies \eqref{C:SigSupLin}. Then for any $M>0$ and $T>0$, there exists a random field solution to \eqref{E:ISHWE} driven by a spatially colored noise denoted as $\{ u(t,x): (t,x) \in [0,T]\times B_R(0) \}$. This solution satisfies 
	\begin{equation}
	\sup_{t\in[0,T],\; |x| \le R} |u(t,x)| < \infty, a.s.
	\end{equation}
\end{theorem}

To overcome the introduction of the spatially correlation, we have the following lemma which will be useful.  
	\begin{lemma}
	\label{L:intYfYbdd}
	Suppose that the spatial correlation function is given to be the Riesz Kernel, $f(x) = x^{-\lambda}$, and that $\lambda < 2\alpha$. Then there exists a constant $C(\beta,\gamma) =: C >0$ such that for any $x,y \in \R^d$ we have that 
	\begin{equation}
	\label{E:YYfBdd}
...
Using this lemma we will be able to prove an analogous result as \cite[Proposition 4.4]{SoleMillet2021} which was key to complete the project in the space-time white noise case. 
\subsection{ISHWE Driven by a L\'evy Noise}
This project will aim to generalize the recent results due to Balan \cite{Balan22} to the ISHWE. The goal will be to show existence of the solution of \eqref{E:ISHWE} driven by a L\'evy white noise, $L(A)$, defined as follows:
	\[
		L(A) = b|A| + \int_{A \times \{|z| \le 1\}} z \tilde{J}(\ud t, \ud x, \ud z) +    \int_{A \times \{|z| > 1\}} z J(\ud t, \ud x, \ud z)\,
	\]
where $A \in \mathcal{B}_0(\R_{+}\times \R^d)$ is the class of finite Lebesgue-measured sets, $|\cdot|$ denotes the Lebesgue measure and $b \in \R$. $J$ is a Poisson random measure on $\R_+ \times \R^d \times \R$ and $\tilde{J}$ is the compensated version of $J$.

Under the assumption that there exists $0<q\le p$ such that 
	\[
		\gamma_1 := \int_{|z| \le 1} |z|^p \nu(\ud z) < \infty, \quad \text{and} \quad \gamma_2 := \int_{|z|>1} |z|^q \nu(\ud z) < \infty,
	\]
Balan \cite{Balan22} proved the following result for the SWE.
	\begin{theorem}
		Assume that $\gamma_1$ and $\gamma_2$ are finite and if $d=2$ then $ p <2$ . Let $h(x) = 1 + |x|^\eta$ for some $d/q < \eta < (2-d(p-1))/(p-q)$. Then the truncated SWE driven by a truncated L\'evy measure,
			\[
				L_N(A) := b|A| + \int_{A \times \{|z| \le 1\}} z \tilde{J}(\ud t, \ud x, \ud z) +    \int_{A \times \{1 < |z| \le Nh(x)\}} z J(\ud t, \ud x, \ud z),
			\]
		has a unique solution denoted as $\{ u_N(t,x): t \ge 0, x\in\R^d \}$. Moreover,	
			\[
				\sup_{0 \le t \le T}\sup_{|x| \le R} \mathbb{E}|u_N(t,x)|^p < \infty, 
			\]
		for any $T>0$ and $R>0$. In $d=1$, the above supremum holds for $p \le 2$. 
	\end{theorem}

\subsection{Spatial Integral of the Solution of the ISHWE Driven by a Time-Independent Noise}

This project will aim to generalize the recent results due to Balan and Yuan \cite{BY22} to the ISHWE. In this project we will consider \eqref{E:ISHWE} driven by a time-independent Gaussian noise as was done in my previous work with Dr. Le Chen in \cite{CE21}. We will need to consider the constant 1 initial condition with 0 initial velocity: $u_0 =1$ and $v_0 =0$. We also take $\sigma(u)=u$. As was proven in \cite[Theorem 1.6 and 3.3]{CE21}, under these conditions, there exists a unique solution that can be expressed through the following Wiener expansion:
	\[
		u(t,x) = 1 + \sum_{k \ge 1} I_k(f_k(\cdot,x,t)),
	\]
where $B_R(0)$ is the ball centered at the origin. We will want to investigate the quantity 
	\[
		F_R(t) := \int_{B_R(0)} u(t,x) -1 \; \ud x.
	\]
Balan and Yuan \cite{BY22} proved the following theorem, which will be the main focus of this future project. 

	\begin{theorem}[\normalfont {\cite[Theorem 1.3]{BY22}} ]
If $d=1$, then we take the noise to be white. If $ d >1$, then the noise will be colored with correlation function, $\gamma \in L^1(\R^d)$ which satisfies Dalang's condition. We have the following:
	\begin{enumerate}
		\item for any $t>0$ and $s>0$,	
			\[
				\mathbb{E}[F_R(t)F_R(s)] \sim K(t,s)R^d \text{ as } R\to \infty, \text{ where } K(t,x): = \omega_d \int_{\R^d} \rho_{t,s}(z) \ud z < \infty,
			\]
			and in particular, $\sigma_R^2(t) \sim K(t,t) R^d$ as $R \to \infty$ where $\sigma_R^2 = \mathbb{E}(|F_r(t)|^2)$ and $\rho_{t,s} := \mathbb{E}\left[ (u(t,x)-1)(u(s,y)-1) \right]$,
		\item for any $t>0$,
			\[
				d_{TV}\left( \frac{F_R(t)}{\sigma_R(t)}, Z \right) \le C_t R^{-d/2},
			\]
		where $d_{TV}$ is the total variation distance and $Z$ is a standard normal random variable,
		\item there exists a continuous modification of the process $\{R^{-d/2}F_R(t)\}_{t \ge 0}$ which converges in distribution in $C[0,\infty)$ as $R \to \infty$, to a zero-mean Gaussian processes with covariance $K(t,s)$.
	\end{enumerate}
	\end{theorem}
...
\begin{thebibliography}{999}
	...
	\bibitem{Balan22}
	Balan, R.
	\newblock Stochastic wave equation with L\'evy white noise
	\newblock {\it arXiv:2111.14242}
	
	\bibitem{BY22}
	Balan, R. and Yuan W. 
	\newblock Spatial integral of the solution to hyperbolic Anderson model with time-independent noise
	\newblock {\it arXiv:2201.02319}
