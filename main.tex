...
\begin{NickQuestion}[(Law of the solution)]
 ...
 However there is a minor discrepancy here when comparing the Law defined the way I just had defined
 it above and how \cite{DZ96} defines it. Again on page 89 of \cite{DZ96} they define the law to be
 a measure on $\mathcal B(H)$ but does not define what $\mathcal B(H)$ is. Im guessing that it means
 the space of bounded linear functionals on some hilbert space $H$. The Law I defined above would be
 defined on $H=L^2_\rho$ not $\mathcal B(H)=\mathcal B(L^2_\rho)$. We are looking for a measure on
 $L^2_\rho$ and so I believe the way I had defined it above is how it is supposed to be defined in
 the Tessitore paper \cite{TessitoreZabczyk98M}. 
\end{NickQuestion}
