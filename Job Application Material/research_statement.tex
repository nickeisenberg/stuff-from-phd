...
This project will try to generalize \cite[Lemma 2.2]{CH16Comparison} to the ISHWE. This issue that arises is the loss of the following identity: 
	\[
		G(s,x)G(t-s,y) = G\left( \dfrac{s(t-s)}{t}, \dfrac{sy -(t-s)x}{t} \right) G(t,x+y)
	\] 
The above identity is only true in the case of the SHE. 

\subsection{Solvability of the ISHWE With Superlinear Diffusion Term Driven by a Spatially Colored Gaussian Noise}
This project will be a natural extension to my current project. 

\subsection{ISHWE Driven by a Levy Noise}
It has often been the case that results proven for the SWE have been able to be extended to the ISHWE. This project will aim to generalize the recent results due to Balan \cite{Balan22} to the ISHWE.

\subsection{Spatial Integral of the Solution of the ISHWE Driven by a Time-Independent Noise}
This project will aim to generalize the recent results due to Balan and Yuan \cite{BY22} to the ISHWE.
...
\begin{thebibliography}{999}
	...
	\bibitem{Balan22}
	Balan, R.
	\newblock Stochastic wave equation with Lévy white noise
