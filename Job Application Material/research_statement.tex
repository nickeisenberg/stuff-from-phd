...
\subsection{Sharp Moment Bounds for the Solution to the ISHWE}
...
The issue that arises is the loss of the following identity for the fundamental solution of the SHE: 
\[
...
Balan \cite{Balan22} proved the following result for the SWE.
	\begin{theorem}
		Assume that $\gamma_1$ and $\gamma_2$ are finite and $ p <2$ if $d=2$. Let $h(x) = 1 + |x|^\eta$ for some $d/q < \eta < (2-d(p-1))/(p-q)$. Then the truncated SWE driven by a truncated L\'evy measure,
			\[
...
This project will aim to generalize the recent results due to Balan and Yuan \cite{BY22} to the ISHWE. In this project we will consider \eqref{E:ISHWE} driven by a time-independent Gaussian noise as was done in my previous work with Dr. Le Chen in \cite{CE21}. We will need to consider the constant 1 initial condition with 0 initial velocity: $u_0 =1$ and $v_0 =0$. As was proven in \cite[Theorem 1.6 and 3.3]{CE21}, under these conditions, there exists a unique solution that can be expressed through the following Wiener expansion:
	\[
...
If $d=1$, then we take the noise to be white. If $ d >1$, then the noise will be colored with correlation function, $\gamma \in L^1(\R^d)$ which satisfies Dalang's condition. We have the following:
	\begin{enumerate}
		\item for any $t>0$ and $s>0$,	
			...
			\]
			and in particular, $\sigma_R^2(t) \sim K(t,t) R^d$ as $R \to infty$ where $\sigma_R^2 = \mathbb{E}(|F_r(t)|^2)$ and $\rho_{t,s} := \left[ (u(t,x)-1)(u(s,y)-1) \right]$,
		\item for any $t>0$,
			\[
