...
\begin{NickQuestion}[Exercise 3.4]
	Let $M = \sum_{t\in [0,1]} X_t$. The hint says to approximate this by taking the sup over a finite set. We know that since $X_t$ is continuous in $t$ then $M=\sup_{t\in \mathbb A}X_t$ where $A=\mathbb Q \cap [0,1]$. Let $\{a_n\}$ be an enumeration of $A\setminus \{0,1\}$ and let $S_k = \{0,a_1,...,a_k,1\}$ then each $S_k$ are finite and $M_k := \sup_{t \in S_k}X_t$ is an approximation of $M$ since $M_k \to M$ almost surely. The also tells use to use Corollary 4.2.5 but that requires us to find $D_tM_k$ which I am unsure of how to do. This corollary would also require us to show that $M_k \xrightarrow[]{L^2(\Omega)} M$ which I have been unable to show either.
\end{NickQuestion}
