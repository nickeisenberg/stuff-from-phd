...
\begin{align*}
	...
	\\& = \sum_{i=1}^n \lim_{\epsilon \to 0} \dfrac{f(W(h_1),...,W(h_i)+\epsilon \langle h_i ,h \rangle, ,...,W(h_n))-F}{\epsilon } 
\end{align*}
...
\begin{NickQuestion}[The formula  $$I_n(f)I_m(g) = \sum_{r=0}^{n \wedge m}r! {n \choose r}{m \choose r}I_{n+m -2r}(f \otimes_r g)$$ on page 64:] $\;$
	...
	This also relates to exercise 2. I tried testing this formula out for the most basic example, for $f,g \in L^2(\R_+)$ and $I_1(f)I_1(g)$. 
	
	Using the formula we would need to get 
	\begin{align}
	I_1(f)I_1(g) &= I_2(f\otimes_0 g)+ I_0(f \otimes_1 g) \label{eqn}
	\\&= \int_0^\infty \int_0^{t_2}f(t_1)g(t_2)\ud B_{t_1}\ud B_{t_2} +  \int_0^\infty f(x)g(x) \ud x
	\end{align}
	...
	\end{align}
	where the last line follows that $I_n(f) = I_n(\tilde{f})$ for any $f \in L^2(\R_+^n)$. However \eqref{eqn} and \eqref{cont} give us a contradiction. 
\end{NickQuestion}
...
\end{align*}
I am not sure what $\ud G_k$ is equal too. I don't believe that it is equal to $g(k,\xi)\ud B_k$
