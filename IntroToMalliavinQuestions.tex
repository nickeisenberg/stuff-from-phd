...
\[
	\tau_g(B(h))=B(h) + g
\]
but also $B(h) \not \in \Omega$ where $\Omega$ is the Weiner space so why does $\tau_g(B(h))$ exist?
...
\]
$\href{http://www.math.wisc.edu/~kurtz/NualartLectureNotes.pdf}{\text{In this pdf}}$ they claim that when we have $DF = \sum_{i=1}^n\partial_i f(W(h_1),...,W(h_n))h_i$ then 
\[
...
\item What is the definition of $F^g$ on the top of page 70? 
	\end{enumerate}
\end{NickQuestion}
...
\item This is a hard proof to follow 
\end{enumerate}
\end{NickQuestion}
\end{document}
