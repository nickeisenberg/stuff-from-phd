...
One question by Le:
\begin{itemize}
	\item Can this paper be generalized to the following SPDE
	\begin{equation}
		\label{E:FSHE}
		\begin{cases}
			\dfrac{\partial u}{\partial t}(t,x)+\frac{1}{2}(-\Delta)^{a/2} u(t,x) = b(x,u(t,x))\dot W(t,x) & \text{$x\in \R^d$ , $t>0$} \\
		u(0,\cdot) = \mu(\cdot)                                                                          &
		\end{cases}
	\end{equation}
	where $a\in (1,2]$?
	\item Can you show me what is new in the paper on the meeting Aug. 31st 2021?
\end{itemize}
\newpage
...
\begin{proof}
		\textcolor{red}
	{
		I remember discussing this awhile back. Is the following correct? 
		\\
		Since $\rho$ is admissible, then for any $T>0$, we have a constant $C_T$ such that 
		\[
		(2 \pi t )^{-d/2} \int_{\R^d} \exp\left(-\frac{|x-y|^2}{2t}\right)\rho(y) \ud y \le C(T) \rho(x) \quad \forall t \in [0,T].
		\]
		Then for a given $a>0$, let $T=2a$, $x=0$ and $t=1/(2a)$. Then because $\rho$ is continuous, we see that 
		\[
		\int_{\R^d} \exp\left(-a|y|^2\right)\rho(y) \ud y \le C(2a) \rho(0) < \infty.
		\]
		This is condition \eqref{A:icon}.
		\\
		This seems more clear then the following proof I gave awhile back.
	}
...
\textcolor{red}{The following needs to be investigated more. I do not believe I mentioned in the proof of the main result where this is needed to be used. See the Tessitore paper to see where they used it.}
...
\textcolor{red}{What is the range of the function $\varphi$ ? eg. is it real valued?}
...
We first show that $u(t_0,x)\in L^2_\rho$ for any admissible weight.
\begin{align*}
...
\begin{theorem}
	...
	\eqref{E:gspde} in $L^2_\rho$ for $t>l>0$. In particular, for any $l>0$ there exists a measure that is
