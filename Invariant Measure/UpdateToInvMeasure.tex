...
\begin{equation}
  \label{E:gspde}
	...
	\end{cases}
\end{equation}
where $b(x,u)$ is assumed to be a globally Lipschitz continuous function in $u$ with Lipschitz
constant independent of $x$.  In particular, our assumption allows the linear case $b(x,u) = \lambda
u$, which is usually referred to as the \textit{parabolic Anderson model} \cite{CarMol94}. In this
equation, $\dot{W}(t,s)$ is a centered Gaussian noise that is white in time and homogeneously
correlated in space. The initial condition $\mu$ is a deterministic, locally finite, regular, signed
Borel measure that satisfies \eqref{A:icon} below. The aim of the paper is to investigate the
conditions required to guarantee the existence of invariant measures for the solution to
\eqref{E:gspde}, which is a crucial step towards the study of the ergodicity of the system.


The question of the existence of invariant measures in unbounded domains has been studied in few
papers such as \cite{AM03, MSY15, MSY16,TZ98}. Since the spatial domain is unbounded, one needs to
introduce a weight function $\rho$ and the corresponding weighted space $L_\rho^2(\R^d)$ as usual;
see Section \ref{S:weightedspace} below for more details. Following the standard procedure as laid
out by Da Prato and Zabczyk \cite{DZp92}, the main effort is to show that
\begin{align}
	\label{E:BddMnt}
...
\eqref{E:gspde} with sharp moment estimates; see \cite{CK19,CH19,CD15}.
\begin{assumptions}
	\label{A:General}
	\begin{enumerate}[(i)]
		...
		\begin{equation}
			|b(x,u)-b(x,v)|< L_b |u-v| \quad \text{and} \quad |b(x,0)| \le L_0 \quad \text{for all } u,v \in \R, \; x \in \R^d \label{A:lipcon}.
		\end{equation}
	\end{enumerate}
\end{assumptions}

Moreover, this solution is usually {\it intermittent}, namely, the probability moments to
\eqref{E:gspde} have exponential growth in $t$. In order to have \eqref{E:BddMnt}, one has to first
identify the exact conditions, under which the moments to \eqref{E:gspde} do not possess exponential
growth. This question has been answered in \cite{CK19}, where necessary and sufficient conditions
are given. In \cite{AM03,MSY15,MSY16}, a drift term $f(x,u)$ is included in \eqref{E:gspde} in a
crucial way to help to cancel the otherwise exponential growing moments. The absence of such a drift
term in this paper makes the problem more challenging.  According to part (3) of Theorem 1.3 of
\cite{CK19}, in order to have \eqref{E:BddMnt}, one has to have $L_b$ in \eqref{A:lipcon} --- the
Lyapunov exponent of $b$ --- small enough and
\begin{equation}
	\label{E:U0finite}
	\Upsilon(0) <\infty,
\end{equation}
...
equivalently \eqref{E:U0Eqv}) gives the admissible correlation functions, which is weaker
than that in \cite{TZ98}; see Remark \ref{R:Cor} below.

\begin{remark}
	\label{R:Cor}
	The assumption on the correlation function in \cite{TZ98} is given through the spectral density
	$\widehat{f}$ as follows:
	\begin{equation}
		\label{E:CorTZ}
		\widehat{f} \in L^p(\R^d) \quad \text{for some $p\in\left(d/(d-2),\infty\right]$};
	\end{equation}
	...
	to be more precise and convenient to check.

	\bigskip
	\textcolor{magenta}{Nick, can you check the relation between the two conditions: \eqref{E:U0finite} and
	\eqref{E:CorTZ}?}

	\textcolor{blue}{I constructed the example in the discussion document that shows that neither our condition nor their condition is more general than one another. We can find $\hat{f}$ that satisfies Dalang's condition but not Hypothesis 2.1 \cite{TZ98} and vice versa. We never completely verified this but if true then we should make a comment about this. }
\end{remark}
...
\begin{theorem}
	...
	\begin{enumerate}[(i)]
		\item  Condition \eqref{E:U0finite} (or equivalently \eqref{E:U0Eqv}) holds and for some
			$\alpha\in (0,1/2)$,
			\begin{equation}
				\label{A:fcon}
				\int_0^t \ud r \: r^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud \xi) \exp\left(-r|\xi|^2 \right) < \infty,
				\quad \text{for all $t>0$;}
			\end{equation}
		\item Suppose that for any admissible weight $\rho$, there exists another admissible weight
			$\widehat{\rho}$ with
			\begin{align}
				\label{E:rhorho}
				\int_{\R^d}\dfrac{\rho(x)}{\widehat{\rho}(x)}\ud x < \infty
			\end{align}
			such that the initial condition satisfies the following condition:
			\begin{equation}
				\label{E:InitData}
				\limsup_{t>0} \int_{\R^d} \left[\int_{\R^d} G(t,x-y)|\mu|(\ud y)\right]^2 \widehat{\rho}(x) \ud x <\infty;
			\end{equation}
\textcolor{blue}{Considering that we still need to assume the admissible weight property because if its use in Proposition 2.1 \cite{TZ98}, I dont think that this condition helps us. I think we should stick with the assumption that for all $t>0$, there exists a $K(t) \in \mathbb{N}$ such that
	\[
		\sup_{x\in \R^d} (G(t,\cdot)*|\mu|)(x) < K(t).
	\]}
		\item The Lipschitz constant $L_b$ of $b$ (see \eqref{A:lipcon}) satisfies
			\begin{equation}
				\label{E:LipLb}
			  L_b < ......,
			\end{equation}
	...
	we have that
	\begin{enumerate}[(a)]
		\item $u(t,\cdot)\in L_\rho^2(\R^d)$ for all $t>0$ with \eqref{E:BddMnt} satisfied;
		\item there exists an invariant measure for the laws of $\{u(t,\cdot)\}_{t>0}$.
	\end{enumerate}
\end{theorem}

	\textcolor{blue}{Statement (b) needs to be for $t> t_0$. A measure $\eta$ is invariant for the laws $\left\{ \mathcal{L}(u^a(t,\cdot))(A) \right\}_{t > 0}$, $A \in \mathscr{B}(L^2_\rho(\R^d)$ and $a \in L^2_\rho(\R^d)$, if 
		\[
			\eta(A) = \int_{\mathscr{B}(L^2_\rho(\R^d)} \mathcal{L}(u^a(t,\cdot))(A) \; \eta(\ud a) \quad \text{for all } t>0.
		\]
However we can only show that there exists an measure, $\eta$, that is invariant for $\left\{ \mathcal{L}(u^a(t,\cdot))(A) \right\}_{t > t_0}$ for $t_0>0$ arbitrarily small. In other words, what we were able to show was 
	\[
	\eta(A) = \int_{\mathscr{B}(L^2_\rho(\R^d)} \mathcal{L}(u^a(t,\cdot))(A) \; \eta(\ud a) \quad \text{for all } t>t_0.
	\]
}
\newpage
...
One thing to
note is that the just mentioned papers use the theory of the stochastic integral laid out by Da
Prato and Zabczyk in \cite{DZ92} while in this paper we use the theory developed by Walsh in
\cite{Walsh}. The former develops a theory of integration with respect to
...
ergodicity of dynamical systems. Da Prato and Zabczyk \cite{DZp92} established the following procedure
\begin{itemize}
...
In \cite{TZ98} they follow this procedure to show the existence of an invariant measure for the solutions to
\eqref{E:gspde} in two separate cases:
\begin{enumerate}
	\item The initial measure is a function, $\varphi \in L^2_\rho \cap L^2_{\widehat{\rho}}$ where both $\rho$ and
	$\widehat{\rho}$ satisfy the condition that $\rho \cdot (\widehat{\rho})^{-1} \in L^1(\R^d)$ and the solution is
	bounded in probability.
	\item The initial measure is a constant function while assuming certain integrability properties of the spectral
	density.
\end{enumerate}
By now using the theory of Walsh as well as subsequent results proven in \cite{CH19} we are able to mimic the
proof in \cite{TZ98}, but in the language of Walsh, and show the existence of invariant measures for a
...
example we drop hypothesis 2.1(i) as well as the assumptions from \cite[Theorem 3.3]{TZ98} for the new
...
\begin{theorem}[Theorem 1.2 \cite{CH19}]
...
\begin{theorem}[Theorem 1.7 \cite{CH19}]
	\label{T:ExUn}
	...
	\end{equation}
	where $\bar{\varsigma}=L_0/L$, $\gamma_p = 32pL^2$ and the function $H(t;\gamma_p)$ is nondecreasing in $t$ (see \cite{CH19} for
...
% \begin{assumptions}
% 			\item \begin{equation}
% 	\int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud \xi) \exp\left(-(s-r)|\xi|^2 \right) < \infty \label{A:fcon}
% 	\end{equation}
% \end{assumptions}
...
\begin{theorem}
	\label{T:Main}
	Assume that Assumption \ref{A:General} holds with the stronger assumption that $\Upsilon(0) <\infty$. Also assume \eqref{A:fcon}. Now for any admissible weight $\rho$, suppose there exists another admissible weight $\widehat{\rho}$ and an element $\hat{\varphi} \in L^2_\rho(\R^d) \cap L^2_{\widehat{\rho}}(\R^d)$ such that
	\begin{align*}
		\int_{\R^d}\dfrac{\rho(x)}{\widehat{\rho}(x)}\ud x < \infty
	\end{align*}
	and $u^{\hat\varphi}(\cdot,x)$ is bounded in probability in $L^2_{\widehat{\rho}}(\R^d)$.
...
\begin{proof}
	...
	By Lemma \ref{L:Moments} we know that for an arbitrary $t_0>0$, the solution at $t=t_0$ from the SPDE is in $L^2_\rho \cap L^2_{\widehat{\rho}}$. Theorem \ref{T:Main2} implies the result.
\end{proof}
...
\begin{align}
	\label{E:Cor}
	\E \left[ F\left(\psi\right)F(\varphi) \right]  = J(\psi,\varphi)
                                                 := \int_0^\infty \ud s \int_{\R^d} \Gamma(dx)(\psi(s,\cdot)*\tilde{\varphi}(s,\cdot))(x)
\end{align}
...
\begin{equation*}
	\mathscr L (u^\mu(t,\cdot))(A) = \mathbb{P}\left\{\omega \in \Omega : u^\mu\left(t,\cdot\right) \in A \quad A \in \mathscr B(L^2_\rho(\R^d))\right\}
\end{equation*}
and define $P_t(\mu,A) := \mathscr L (u^\mu(t,\cdot))(A)$. The following two definitions will define what it means for a measure to be invariant for an SPDE.
...
\begin{equation*}
	\forall \epsilon >0, \; \exists R>0: \; \forall t>0, \; \mathbb{P}\left[|u^\varphi(t,\cdot)|_\rho \ge R\right] <  \epsilon .
\end{equation*}
...
Following Tessitore and Zabczyk \cite{TZ98}, we assume that $\rho$ is an \textit{admissible weight}
...
\end{equation}
Several examples of admissible weights are (see Example in Section 2 of \cite{TZ98})
\begin{equation}
	\label{E:admRho}
	\begin{aligned}
...
\begin{remark}
	\label{R:admRho}
	Certainly, the smaller weight function $\rho(\cdot)$, the larger the space $L_\rho^2(\R^d)$ is.
	For example, one may choose $\rho$ to be either a nonnegative function with compact support or the
	heat kernel itself $G(1,\cdot)$. In both cases, $\rho$ is smaller than those in \eqref{E:admRho}
	(up to a constant). However, one can easily check that the admissible condition \eqref{E:aw}
	excludes these two cases. It may be much less obvious that the following function
	\begin{equation*}
		\rho(x) = \exp\left(-a |x|^b\right), \quad x\in\R^d
	\end{equation*}
	with $a>0$ and $b\in (1,2)$ is not admissible \textcolor{magenta}{(Nick, can you prove this fact
	in appendix)}. In any case, we will focus on admissible weight functions as given in \eqref{E:admRho}.
\end{remark}

% As pointed out in Remark \ref{R:admRho}, the admissible weight $\rho$ cannot be the heat kernel
% itself. But one can make it as close as possible. This fact is proved in the next lemma:
%
% \begin{lemma}
% 	\label{L:admRho}
% 	For any the following weight function	 is admissible:
% \end{lemma}
% \begin{proof}
% 	It suffices to prove that
% 	\begin{equation*}
% 		g(x):=\int_{\R^d} G(t,x-y) \exp\left(-a |y|^b + a |x|^b\right) \ud y
% 	\end{equation*}
% 	is uniformly bounded in $x$ with $t>0$ fixed. Notice that $g(x)$ is a smooth function. By the
% 	dominated convergence, one sees that
% 	\begin{equation*}
% 		\lim_{|x|\to \infty} g(x) = \int_{\R^d}\lim_{|x|\to \infty} G(t,x-y) \exp\left(-a |y|^b + a |x|^b\right)
% 		\ud y = 0.
% 	\end{equation*}
% 	Therefore, $g(x)$ is a continuous function in  $\R^d\cup \{\infty\}$, which implies that
% 	$\sup_{x\in \R^d} g(x) <\infty$.
% \end{proof}
...
We note that in order to prove \eqref{E:RhoDom}, it suffices to show that there exists some $K'_a \in \mathbb{N}$ such that
	...
	\textcolor{magenta}{No, you need to that a lower bound of the ratio goes to infinity.}
...
\begin{proof}
	...
	% 	Lemma \ref{L:phi} along with Assumption \ref{A:General} guarantee the existence and uniqueness of the  solution which is show in \cite{CH19}. We only prove the finiteness of the supremum. Below we use $C_i$ to represent a constant.
	% \begin{align*}
...
\begin{lemma}
	...
	\mathbb{P}\{u^{\hat{\varphi}}(t,\cdot) \in \mathscr K \} \ge (1-\epsilon)\mathbb{P}\{|u^{
		\hat{\varphi}}(t-t_0,\cdot)|_{\widehat{\rho}} < R\} \quad \text{for all $t>t_0$}.
	\end{equation}
\end{lemma}
...
\end{definition}
 ...
 \begin{proof}[Proof of Theorem \ref{T:Main}]
 	...
 	Now let $\epsilon \in (0,1)$. Since $u^{\hat\varphi}(t,x)$ is bounded in probability, we can choose $R>0$ such that for any $t \ge t_0$,
 		\[
 			\mathbb{P}\{|u^{\hat \varphi}(t-t_0,\cdot)|_{\widehat{\rho}} \ge R\} < \epsilon.
 		\]
 	By applying Lemma \ref{L:Main}, we can find a compact set $\mathscr K \in L^2_{\rho}$ such that
 	\begin{align*}
 	\mathbb{P}\{u^{\hat \varphi}(t,\cdot) \in \mathscr K \} \ge (1-\epsilon)\mathbb{P}\{|u^{\hat \varphi}(t-t_0,\cdot)|_{\widehat{\rho}} < R\} \quad \text{for all $t>t_0$} .
 	\end{align*}
 	In return, this implies that the family of laws $\mathscr L(u^{\hat \varphi}(t,\cdot))$ for $t \ge t_0$ is tight in $L^2_{\rho}$. Indeed, if we multiply both sides of \eqref{E:cpt} by $-1$ and then add $1$ to both sides we see that
 		\begin{align*}
 			\mathbb{P}\{u^{\hat \varphi}(t,\cdot) \not\in \mathscr K \} &\le \mathbb{P}\{|u^{\hat \varphi}(t-t_0,\cdot)|_{\widehat{\rho}} \ge R\} + \epsilon \mathbb{P}\{ |u^{\hat \varphi}(t-t_0,\cdot)|_{\widehat{\rho}} < R \}
 			\\& \le 2 \epsilon.
 		\end{align*}
...
\begin{lemma}
	...
	\label{E:vspde2}
	\begin{cases}
	\dfrac{\partial v}{\partial t}(t,x)-\frac{1}{2}\Delta v(t,x) = b(x,v(t,x)\dot W(t,x) & \text{$x\in \R^d$ , $t>0$}
	\\ v(0,x) = u(t_0,x)  &
	\end{cases}
	\end{equation}
...
We start off  with a proposition whose proof is given in Proposition 2.1 \cite{TZ98}

\begin{proposition}
	\label{P:weight}
	For any admissible weight $\rho$, the operators on $L^2_\rho(\R^d)$ defined by $\varphi \mapsto \big(G(t,\cdot)*\varphi(\cdot)\big)(x)$ can be extended to a $C_0-semigroup$ on $L^2_\rho$. Moreover, if $\widehat{\rho}$ is another admissible weight such that
	\begin{align*}
	\int_{\R^d}\dfrac{\rho(x)}{\widehat{\rho}(x)}\ud x < \infty
	\end{align*}
	then for any $t<0$, the operators defined above are compact from $L^2_{\widehat{\rho}}(\R^d)$ to $L^2_\rho(\R^d)$.
\end{proposition}
...
\end{equation}
It is shown in the proof of Theorem 3.1 \cite{TZ98} that $F_\alpha$ is compact from $L^q\big((0,t_0),L^2_{\widehat{\rho}} \big)$ to
$L^2_\rho$ where $L^q\big((0,t_0),L^2_{\widehat{\rho}} \big)$ is the space of functions that map $s \mapsto f(s,\cdot)$ and
$f(s,\cdot) \in L^2_{\widehat{\rho}}(\R^d)$ and in addition $\int_0^{t_0} |f(s,\cdot)|_{\widehat{\rho}}^q \: \ud s < \infty$.
...
\begin{proof} First, notice by applying Minkowski's Integral Inequality that
	...
	\begin{align*}
	(2\pi)^{-2d}\int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud \xi) \exp\left(-(s-r)|\xi|^2 \right).
	\end{align*}
	Now if we integrate this in $y$ over $\R^d$ with respect to $\rho(y)\ud y$ we get that
	\begin{align*}
	\int_{\R^d}I_1\cdot \rho(y)\ud y &\le (2\pi)^{-2d} \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud \xi) \exp\left(-(s-r)|\xi|^2 \right)  \int_{\R^d}\rho(y)\ud y
	\\ &\le (2\pi)^{-2d}\cdot \Norm{\rho}_{L^1(\R^d)} \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud \xi) \exp\left(-(s-r)|\xi|^2 \right) .
	\end{align*}
	...
	\\& \le (2\pi)^{-2d} \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud\xi) \exp\left( -\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)|\xi|^2\right)\int_{\R^d}\ud\sigma \: G(s,y-\sigma)|\varphi(\sigma)|.
	\end{align*}
	Now if we integrate this in $y$ over $\R^d$ with respect to $\rho(y)\ud y$ and apply the admissible weight property we get that
	\begin{align*}
	\int_{\R^d}I_2\cdot \rho(y)\ud y &\le (2\pi)^{-2d} \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud\xi) \exp\left( -\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)|\xi|^2\right)
	\\& \hspace{15em} \times \int_{\R^d}\ud y \: \rho(y)\int_{\R^d}\ud\sigma \: G(s,y-\sigma)|\varphi(\sigma)|
	\\& \le (2\pi)^{-2d} C(S)  \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud\xi) \exp\left( -\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)|\xi|^2\right)
	\\& \hspace{15em} \times \int_{\R^d}\ud\sigma \:\rho(\sigma) |\varphi(\sigma)|
	\\&= (2\pi)^{-2d} C(S) \Norm{\varphi}_{L^1_\rho(\R^d)} \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud\xi) \exp\left( -\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)|\xi|^2\right)
	\end{align*}
	...
	\begin{align*}
	I_3 \le (2\pi)^{-2d} \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud\xi) \exp\left( -\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)|\xi|^2\right)\int_{\R^d}\ud\sigma \: G(s,y-\sigma)|\varphi(\sigma)|
	\end{align*}
	...
	\begin{align*}
	\int_{\R^d}I_3\cdot \rho(y)\ud y &\le (2\pi)^{-2d} C(S) \Norm{\varphi}_{L^1_\rho(\R^d)} \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud\xi) \exp\left( -\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)|\xi|^2\right).
	\end{align*}
	\textbf{For $I_4$:}
	\begin{align*}
	...
	\\ &\le (2\pi)^{-2d} \Norm{\rho}_{L^1(\R^d)}  \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud \xi) \exp\left(-(s-r)|\xi|^2 \right)
	\\&\hspace{1em} + 2 (2\pi)^{-2d} C(S) \Norm{\varphi}_{L^1_\rho(\R^d)} \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^{d}} \widehat{f}(\ud\xi) \exp\left( -\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)|\xi|^2\right)
...
\begin{proof} We let
	...
	\begin{align*}
	L_1 &\le (2\pi)^{-2d}\int_{\R^d}\widehat{f} (\ud \xi)\: \exp(-(s-r)|\xi|^2)
	\end{align*}
	and now if we include the $\int_{\R^d}\ud y \;G(t-s,x-y)\int_0^s\ud r\; (s-r)^{-2\alpha}$ part of the integral we get that
	\begin{align*}
	\int_{\R^d}\ud y \;G(t-s,x-y)\int_0^s\ud r\; (s-r)^{-2\alpha} L_1 \le (2\pi)^{-2d} \int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f} (\ud \xi)\: \exp(-(s-r)|\xi|^2).
	\end{align*}

	\noindent \textbf{For $L_2$:} We first use the moment bounds from \cite{CH19} along with the fact that $\Norm{u(r,z_1)}_1 \le \Norm{u(r,z_1)}_2$.
	\begin{align*}
	...
	\begin{align*}
	L_2 &\le kL_1 + (2\pi)^{-2d}\int_{\R^d}\widehat{f}(\ud \xi) \: \exp\left(-\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)\right) \int_{\R^d}\ud \sigma \: G(s,y-\sigma)|\varphi(\sigma)|
	\end{align*}
	and now if we include the $\int_{\R^d}\ud y \;G(t-s,x-y)\int_0^s\ud r\; (s-r)^{-2\alpha}$ part of the integral we get that
	\begin{align*}
	& \int_{\R^d}\ud y \;G(t-s,x-y)\int_0^s\ud r\; (s-r)^{-2\alpha} L_2
	\\ & \le k\int_{\R^d}\ud y \;G(t-s,x-y)\int_0^s\ud r\; (s-r)^{-2\alpha} L_1
	\\ & \hspace{3em}+ (2\pi)^{-2d}\int_{\R^d}\ud y \;G(t-s,x-y)\int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi) \: \exp\left(-\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)\right)
	\\ & \hspace{24.5em} \times  \int_{\R^d}\ud \sigma \: G(s,y-\sigma)|\varphi(\sigma)|
	\\ & \le k (2\pi)^{-2d} \int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f} (\ud \xi)\: \exp(-(s-r)|\xi|^2)
	\\ & \hspace{3em}+ (2\pi)^{-2d} \int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi) \: \exp\left(-\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)\right)
	\\ & \hspace{18 em} \times\int_{\R^d}\ud y \;G(t-s,x-y) \int_{\R^d}\ud \sigma \: G(s,y-\sigma)|\varphi(\sigma)|
	\\ & = k  (2\pi)^{-2d} \int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f} (\ud \xi)\: \exp(-(s-r)|\xi|^2)
	\\ & \hspace{3em} +(2\pi)^{-2d}\int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi) \: \exp\left(-\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)\right)\int_{\R^d}\ud \sigma \: G(t,x-\sigma)|\varphi(\sigma)|
	\\ & \le k  (2\pi)^{-2d}\int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f} (\ud \xi)\: \exp(-(s-r)|\xi|^2)
	\\ & \hspace{3em}+ (2\pi)^{-2d}  \Norm{G(t,x-\cdot)\varphi(\cdot)}_{L^1(\R^d)}\int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi) \: \exp\left(-\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)\right).
	\end{align*}
	...
	\\ &\le k (2\pi)^{-2d}\int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f} (\ud \xi)\: \exp(-(s-r)|\xi|^2)
	\\&\hspace{2em}+ 2 k  (2\pi)^{-2d} \Norm{G(t,x-\cdot)\varphi(\cdot)}_{L^1(\R^d)}\int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi) \: \exp\left(-\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)\right)
	...
	\begin{align*}
	\eqref{E:fcond} &\le K_{x,t}\int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f} (\ud \xi)\: \exp(-(s-r)|\xi|^2)
	\\&\hspace{3em}+ K_{x,t}\int_0^s\ud r\; (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi) \: \exp\left(-\left(\dfrac{s-r}{2}+\dfrac{r(s-r)}{2s}\right)\right)
...
\begin{proof} We let
	...
	\int_{\R^d}\widehat{f}(\ud \xi)\: \exp\left(-(t-r)|\xi|^2\right)
	...
	\\ & \le k\int_0^t \ud s \: (t-s)^{\alpha-1}\int_0^s \ud r \: (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi)\: \exp\left(-(t-r)|\xi|^2\right)
	\\ & \hspace{2em}+ k\int_0^t \ud s \: (t-s)^{\alpha-1}\int_0^s \ud r \: (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi)\: \exp\left(-\left(\dfrac{t-r}{2}+\dfrac{r(t-r)}{2t}\right)|\xi|^2\right)
	\\ & \hspace{20em} \times \int_{\R^d}\ud \sigma \:G(t,x-\sigma)|\varphi(\sigma)|
	\\ & \le \big(k_{t,1}  + k J_0(t,x) k_{t,2} \big) \int_0^t \ud s \: (t-s)^{\alpha-1}
	\end{align*}
	where
	\[
	k_{t,2} =  \max_{0\le s \le t} \int_0^s \ud r \: (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi)\: \exp\left(-\left(\dfrac{t-r}{2}+\dfrac{r(t-r)}{2t}\right)|\xi|^2\right)
	\]
	...
	\\&\le k\int_0^t \ud s \: (t-s)^{\alpha-1}\int_0^s \ud r \: (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi)\: \exp\left(-(t-r)|\xi|^2\right)
	| \\& \hspace{2em} + 2k\int_0^t \ud s \: (t-s)^{\alpha-1}\int_0^s \ud r \: (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi)\: \exp\left(-\left(\dfrac{t-r}{2}+\dfrac{r(t-r)}{2t}\right) | \xi | ^2\right)
	\\& \hspace{22em} \times \int_{\R^d}\ud \sigma \:G(t , x-\sigma) | \varphi(\sigma) |
	\\& \hspace{2em}+  k\int_0^t \ud s \: (t-s)^{\alpha-1}\int_0^s \ud r \: (s-r)^{-2\alpha}\int_{\R^d}\widehat{f}(\ud \xi)\: \exp\left(-\dfrac{r(t-r)}{t}|\xi|^2\right)\left(\int_{\R^{d}}\ud \sigma \:G(t,x-\sigma)|\varphi(\sigma)\right)^2
	\\&\le k\big(k_{t,1}+ 2k_{t,2}J_0(t,x)+k_{t,3}J_0(t,x)^2 \big)\int_0^t \ud s \: (t-s)^{\alpha-1}
	\end{align*}
	where
	\[
	k_{t,3} = \max_{0\le s \le t} \int_{\R^d}\widehat{f}(\ud \xi)\: \exp\left(-\dfrac{r(t-r)}{t}|\xi|^2\right)
	\]
...
\begin{proof}[Proof of Lemma \ref{L:Main}]
	...
	\quad \text{where $|y|_{\widehat{\rho}} \le \Theta$  and  $|h|_{L^q(0,1,L^2_{\widehat{\rho}})} \le \Theta $} \}.
	\]
	Then $\mathscr K (\Theta)$ is relatively compact in $L^2_{ \rho}$ for $\Theta >0$. Moreover, by \eqref{E:yineq}, \eqref{E:1fac} and Chebychev's inequality, and if we choose $R$ and $\Theta$ so that $| \hat{\varphi}|_{\widehat{\rho}} < R < \Theta$  then we have
	\begin{align*}
	...
	\\ & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Theta^q}\E \int_{0}^{1} |Y^{\hat{\varphi}}(s,\cdot)|_{\widehat{\rho}}^q \: \ud s
	\\ & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Theta^q}\bigg( 1+ |{\hat{\varphi}}|_{\widehat{\rho} }^q \bigg)
	\end{align*}
	which implies that
	\begin{align*}
	\{ u^{\hat{\varphi}}(\delta_0,\cdot) \in \mathscr K(\Theta) \}
	& \ge 1- \dfrac{\sin^q(\alpha \pi)}{\pi^q \Theta^q}\bigg( 1+ |{\hat{\varphi}}|_{\widehat{\rho} }^q \bigg)
	...
	\epsilon)\mathbb{P}\{ |u^{\hat{\varphi}}(t-\delta_0,\cdot)|_{\widehat{\rho}} < R
	\}.
	\end{align*}
	Since we assumed that $u^{\hat \varphi}$ is bounded in probability in $L^2_{\widehat{\rho}}$ then for and $\epsilon >0$ we can fix $R>0$ such that $\mathbb{P}\{ |u^{\hat{\varphi}}(t,\cdot)|_{\widehat{\rho}} \ge R \} < \epsilon$ and then taking $\Theta > R$ such that $ \dfrac{\sin^q(\alpha \pi)}{\pi^q \Theta^q}\bigg( 1+ R^q \bigg) < \epsilon$ concludes the claim.
\end{proof}
...
\begin{small}% {{{ References
    ...
    % }}}
		\bibitem[OL$^+$10]{NIST2010}% {{{
		Olver, Frank ~W.~J., Daniel~W. Lozier, Ronald~F. Boisvert and Charles~W. Clark.
