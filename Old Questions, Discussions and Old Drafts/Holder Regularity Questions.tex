...
\begin{NickQuestion}[Lemma B.3 Questions]
	\textcolor{red}{I answered all my questions below.}
	
	\begin{enumerate}
		\item On the first line of the proof of Theorem 1.3 of \cite{BC2017} references the bounds from Lemma B.3. My question is about the first inequality right after equation (B.11). I will underline the parts of the inequality that this question pertains to. Here is the inequality:
		\begin{align*}
			| \mathcal{F} & (g_{ \textbf{t}, t+h, x }^{(n)}  - g_{ \textbf{t}, t, x }^{(n)} ) (\xi_1 , \cdots , \xi_n)|^2 
		\\& \le \lambda^{2n} \underbrace{ J_+^2(t,x) }_{(1)}   \underbrace{ \prod_{k=1}^{n-1} \exp\left( - \frac{ t_{\rho(k+1)}  - t_{\rho(k)} }{ t_{\rho(k)}t_{\rho(k+1)} } \left| \sum_{j=1}^k t_{\rho(j)}\xi_{\rho(j)} \right|^2  \right) }_{(2)}
	...
	and it says that this follows from equation (3.5) which says that 
		\begin{align}
			 ...
			 \\& \times \exp \left( -\frac{i}{t} \left( \sum_{j=1}^n t_j \xi_j \right) \cdot x \right) \int_{\R^d} \exp \left( -i \left[ \sum_{j=1}^n \left( 1- \frac{t_j}{t} \right) \xi_j  \right] \cdot x_0 \right) \underbrace{ G(t,x-x_0) }_{(1a)} u_0(\ud x_0).
		\end{align}
		
\textbf{Underline (1):} I do not understand why we can factor out $J_+(t,x)$ which is in the (1) underbrace above. If we calculate $	| \mathcal{F} (g_{ \textbf{t}, t+h, x }^{(n)}  - g_{ \textbf{t}, t, x }^{(n)} ) (\xi_1 , \cdots , \xi_n)|^2 $ by using equation (3.5), then because of the (1a) underbrace above we would have that 
	\begin{align*}
...
\textbf{Underline (2):} Why are we able to factor as was done in this step. Shouldn't we say that 
	\begin{align*}
			| \mathcal{F} & (g_{ \textbf{t}, t+h, x }^{(n)}  - g_{ \textbf{t}, t, x }^{(n)} ) (\xi_1 , \cdots , \xi_n)|^2 
			\\= \bigg| \lambda^n \prod_{k=1}^n & \exp\left( -\frac{1}{2}\frac{ t_{\rho(k+1)}  - t_{\rho(k)} }{ t_{\rho(k)}t_{\rho(k+1)} } \left| \sum_{j=1}^k t_{\rho(j)}\xi_{\rho(j)} \right|^2 \right) \cdots
			\\& - \lambda^n \prod_{k=1}^n \exp\left( -\frac{1}{2}\frac{ t_{\sigma(k+1)}  - t_{\sigma(k)} }{ t_{\sigma(k)}t_{\sigma(k+1)} } \left| \sum_{j=1}^k t_{\sigma(j)}\xi_{\sigma(j)} \right|^2 \right) \cdots \bigg|^2.	\end{align*}
In other words, the permutations are different so we can't factor like is done to get the first inequality after equation (B.11). 
	\item Similar to the last question, when calculating $ |\mathcal{F} g_{ \textbf{t}, t+h, x }^{(n)}(\xi_1, \cdots, \xi_n)|^2 $ right under equation (B.15), why is there a $J_+^2(t,x)$ term instead of a $J_+^2(t+h,x)$ term?  

	\item Right after equation (B.16), should $D_{t,h}$ be equal to 
		\[
			D_{t,h} = \cup_{\rho \in S_n} \{ (t_1, \cdots, t_n) ; 0 < t_{\rho(1)} < \cdots < t_{\rho(n)} < t+h , t_{\rho(1)} > t \}?
		\]
\end{enumerate}
\end{NickQuestion}[Lemma 3.2] Where does the $\sum_{\rho \in S_n}$ go when calculating $\mathcal{F}g_{\textbf{t},t,x}^{(n)} = n!\mathcal{F}\tilde{f_n} = \sum_{\rho \in S_n}\mathcal{F}f_n$?

\begin{NickQuestion}[Lemma 3.2] 
	\textcolor{red}{I answered my own question}
	
	Where does the $\sum_{\rho \in S_n}$ go when calculating $\mathcal{F}g_{\textbf{t},t,x}^{(n)} = n!\mathcal{F}\tilde{f_n} = \sum_{\rho \in S_n}\mathcal{F}f_n$? This is equation 3.5. 
\end{NickQuestion}
