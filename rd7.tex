...
In this equation, $\dot{W}(t,s)$ refers to a centered Gaussian noise that is white in time and homogeneously correlated in space.
The initial measure, $\mu$, is a deterministic and locally finite (regular) Borel measure. 
In particular, we investigate the conditions required to guarantee the existence of invariant measures 
for the solution to \eqref{E:gspde} for a more broad range of initial measures which include the Dirac delta
distribution $\delta_0$ as well some weighted $L^2$ functions.

The question of the existence of invariant measures in unbounded domains with dimension $d \ge 3$ has been handled in
few papers such as \cite{TessitoreZabczyk98M, AM03, MSY15, MSY16}. One thing to note is that the just mentioned papers
use the theory of the stochastic integral laid out by Da Prato and Jabczyk in \cite{DZ92} while in this paper we use the
theory developed by Walsh in \cite{Walsh}. The former develops a theory of integration with respect to
...
This paper is organized as follows: We first briefly develop some theory needed to complete the proofs of the main
results which includes defining the spatially homogeneous Gaussian noise which is white in time and colored in space and
defining the solution to \eqref{E:gspde} as well as list the necessary requirements for its exists and uniqueness.
Lastly, before we list and prove the main results, we give a brief introduction to invariant measures. The proof of one
of our main results, Theorem \ref{T:Main}, relies on our Lemma \ref{L:Main}. The proof of this lemma follows the same
blueprint laid out in \cite{TessitoreZabczyk98M} however because of our use of the theory of Walsh, it is much longer
and more technical so for this reason we devote the last section of this paper to prove it.
...
$L^p(\Omega)$ norm, $\E\big[|\cdot|^p\big]^{\frac{1}{p}}$. 
...
We now state assumptions necessary for the existence and uniqueness of the solution to \eqref{E:gspde}. The result is
found in \cite{CK15SHE,CH16Comparison}.
...
% \begin{notation}
% 	The notation $\hat{f}(\ud \xi)$ is use to denote a pairing in the sense that $\langle f, g \rangle = \langle \hat{f}, \hat{g} \rangle$. When $f$ is indeed a test function then $\hat{f}(\ud \xi)=\hat{f}(\xi)\ud \xi$. So for example, \eqref{A:dalcon} could be written as $\langle \hat{f} , (\beta + |\cdot|^2)^{-1} \rangle$. It is the value of $(\beta + |\cdot|^2)^{-1}$ under the distribution $\langle \hat{f} , \cdot \rangle$.
% \end{notation}

\begin{theorem}
Under Assumption \ref{A:General}, \eqref{E:gspde} has a unique random field solution starting from $\mu$ and the solution is $L^2(\Omega)$ continuous. 
\end{theorem}

The arguments in this paper have been made possible due to the moment bounds of the solution \eqref{E:gmsol} given in \cite{CH16Comparison} which we will now state again without proof.
...
\begin{remark} 
	Throughout this paper we will in fact be working under the assumption that $\Upsilon(0) < \infty$ and so what
	will be important to us is \eqref{E:mb0}. Also the assumption that $b(x,0)=0$ is often assumed and in addition
	it is required to prove the strict positivity \cite{CH16Comparison} of the solution to \eqref{E:gspde}. 
\end{remark}

\subsection{The weighted space $L^2_\rho(\R^d)$}
...
\bigskip
\textcolor{magenta}{Show that why the above two examples are admissible weights by giving more explicit form of the constant $C_\rho(T)$.
I would expect that for the second example, $a$ can be any positive number.
The key property for being admissible is \eqref{E:aw}, let's see if we can remove the $L^1(\R^d)$ condition in order 
to include the case $\rho\equiv 1$.} 

\textcolor{blue}{I have an explicit formula for the first admissible weight. But I don't believe a nice closed form exists for the second admissible weight. I tried using mathematica to get a nice closed form for $C_\rho(T)$ for general $a>d$. For exmaple, I tried calculating the integral 
	\[
		\int_{\R}\frac{\exp \left(-|y-x|^2/(2t)\right)}{1+|y|^a} \ud y
	\]  
for general $a>1$, $t>0$ and $x\in \R$ but mathematica could not solve this. One thing we notice is that if we assume the admissible weight property is true for this example, in other words assume that 
	\[
		\int_{\R}\frac{\exp \left(-|y-x|^2/(2t)\right)}{1+|y|^a} \ud y \le \sqrt{2 \pi t}C_\rho(T)\rho(x)
	\]
then since $\rho(0)=(1+|0|^a)^{-1}=1$ then we must have that 
	\[
		C_\rho(T) \ge  \frac{1}{\sqrt{2 \pi t}}	\int_{\R}\frac{\exp \left(-|y|^2/(2t)\right)}{1+|y|^a} \ud y
	\]
but again in this situation, mathematica was unable to calculate when $x=0$, $t>0$ and $a>1$. However, when let $t>0$, $x=0$ and $a$ be an even integer, then mathematica can calculate the integral however it can't calculate for a arbitrary $a=2n$, only when you specifically let $a=2$ or $a=4$ etc. But the resulting answer is very ugly. I will email you a screenshot of the mathematica notebook showing you what I mean. Lastly, I believe that we need $\rho$ to be integrable since $\Norm{\rho}_{L^1(\R^d)}$ appears in \eqref{T:Moment} and the proof of \eqref{L:Moments}.}
\bigskip

For the remainder of this section, we focus ourselves on \eqref{E:gspde} with initial condition $\varphi \in L^2_\rho$,
...
\begin{equation}
	u(t,x)=\int_{\R^d}G_t(x-y)\varphi(y)\ud y + \int_0^t\int_{\R^d}b(u(s,y))G(t-s,x-y)W(\ud s,\ud y) \label{E:msol}.
\end{equation}
...
\begin{proof} 
	We first claim that the fact that $\rho$ is admissible implies that for some $C_a>0$,
	\begin{align}
		\label{E:RhoDom}
		\exp\left(-a|x|^2\right)\le C_a\: \rho(x),\qquad \text{for all $x\in\R^d$.}
	\end{align}
	\textcolor{magenta}{(Please demonstrate \eqref{E:RhoDom}.)} 
	Then by the Cauchy-Schwatz inequality, 
	\begin{align*}
		\int_{\R^d} \e^{-a|x|^2} |\varphi(x)|\ud x
		&\le C'_a \left(\int_{\R^d} \e^{-a|x|^2} |\varphi(x)|^2\ud x\right)^{1/2}
		\le C'_a C_a^{1/2} \Norm{\varphi}_\rho<\infty.
	\end{align*}
\end{proof}

Lemma \ref{L:phi} and Theorem \ref{T:ExUn} implies that \eqref{E:spde} has a unique solution. 
The following theorem tells us that the solution for any $t>0$ is still in $L_\rho^2(\R^d)$.

\begin{theorem} 
	\label{T:Moment}
	Suppose Assumption \ref{A:General} holds. 
	Let $u$ be the unique solution to \eqref{E:spde} starting from  $\varphi(\cdot) \in L^2_\rho$.
	\textcolor{blue}{Then for all $T>0$ and for all $t \in [0,T]$, $u(t,\cdot)\in L^2_\rho(\R^d)$ with} 
	\begin{align}
		\label{E:uNormRho}
		\E\left(| u(t,\cdot)|_\rho^2\right) 
		\le  2 C_T \left[ \Norm{\rho}_{L^1(\R^d)} + C_\rho(T) |\varphi|_{\rho}^2 \right]<\infty.
	\end{align}
	where we use $C_T:= H(T,\gamma_2)$. Moreover, if $\Upsilon(0)<\infty$, then as a consequence of \eqref{E:mb0} \textcolor{magenta}{and provided that we can show 
	that $C_\rho(T)$ is bounded, we have that},
	\begin{align*}
		\sup_{t\ge 0}\E\left(|u(t,\cdot)|_\rho^2\right) < \infty  
	\end{align*}
\end{theorem}

\textcolor{magenta}{Update all $dx$ by $\ud x$ throughout the document.} 

\begin{proof} 
	\textcolor{blue}{Fix an arbitrary $T>0$ and denote $C_T:=H(T,\gamma_2)^{	1/2}$}. For any $t\in [0,T]$, by Fubini's Theorem and the moment bound \eqref{E:mb}, we see that
	\begin{align*}
		\E\left(| u(t,\cdot)|_\rho^2\right) 
		&= \E \left(\int_{\R^d}\vert u(t,x) \vert^2 \rho(x)\ud x\right) 
		\le C_T \int_{\R^d}\bigg(1+(|\varphi|*G(t,\cdot))(x)\bigg)^2\rho(x)\ud x.
	\end{align*}
	\textcolor{blue}{By definition $\rho\in L^1(\R^d)$, hence we see that} 
	\begin{align*}
		\E\left(| u(t,\cdot)|_\rho^2\right) 
		\le 2 C_T \left[ \Norm{\rho}_{L^1(\R^d)} + \int_{\R^d}\left(\int_{\R^d}G(t,x-y)|\varphi(y)|dy\right)^2\rho(x)\ud x\right].
	\end{align*}
...
% \end{align*}
	\bigskip
\textcolor{magenta}{The following proof is not quite right. $K$ is a constant depending on $t_0$ and it will blow up as $t_0$ goes to $\infty$.
You do need to use the condition $\Upsilon(0)<\infty$ to prove the uniform moment bounds.} 
\textcolor{blue}{There was a mistake made when we originally proved the inequality $G(t,x) \le K \cdot G(1,x)$. This in equality is actually untrue which I have shown in the other document. So the the best we can say for this Theorem is \eqref{E:uNormRho}. If $C_\rho(T)$ is bounded in $T$ then we may say $\sup_{t\ge 0}\E\left(|u(t,\cdot)|_\rho^2\right) < \infty$, but otherwise this is not true. So the following is not correct.}
...
\begin{align*}
	\E\left(| u(t,\cdot)|_\rho^2\right) 
	   & \le  C\left[1+\int_{\R^d}\left(\int_{\R^d}G(t,x-y)|\varphi(y)|dy\right)^2\rho(x)dx\right]
	\\ & \le  C\left[1+\int_{\R^d}\left(\int_{\R^d}G(t_0,x-y)|\varphi(y)|dy\right)^2\rho(x)dx\right]
	\\ & \le C\left[1+\int_{\R^d}\ud x \: \rho(x) \int_{\R^d} \ud y \: G(t_0,x-y)|\varphi(y)|^2\right]
	\\ & \le C \left[ 1+ C(t_0)|\varphi|_\rho^2  \right] < \infty 
\end{align*}
where the last inequality is due to the admissible weight property. This proves that
...
We finish off this section with a proposition on the functions $G(t,x)$ which will be used later in the proof of Lemma \ref{L:Main}. The proof of this result can be found in \cite{TessitoreZabczyk98M} \\
...
\end{align*}
Following \cite{DZ96}, we define Markovian transition functions $P_t(\varphi,A) := \mathscr L (u^\varphi(t,\cdot))(A)$ for $\varphi \in L^2_{\rho}(\R^d)$, $t \ge 0$ and $A \in \mathscr B (L^2_{\rho}(\R^d))$ where $\mathscr B (L^2_\rho(\R^d))$ are the Borel subsets of the space $L^2_\rho(\R^d)$ and $\mathscr L (u^\varphi(t,\cdot))(A)$ denotes the laws of the solution of \eqref{E:spde} 
\begin{align*}
...
\ref{T:Main} and then generalize the result for measure valued initial conditions, and in particular, the Dirac delta
measure $\delta_0$ in Theorem \ref{T:Main2}.


For the case when the initial condition is a measure, as in \eqref{E:gspde} the laws are defined for $t>0$ where $t$ needs to be strictly greater than $0$. This is indeed true by Lemma \ref{L:Moments}  below which states that under certain assumptions, the solution $u(t,\cdot)$ of \eqref{E:gspde} is almost surely in $L^2_{\rho}$ for every $t>0$. 
...
\begin{assumptions} $\;$
	\begin{enumerate}[(i')]
		\item \begin{equation} 
					\int_{\R^d}\ud x \: \rho(x)\left(\int_{\R^d}\ud y \: G(t_0,x-y)|\mu|(dy) \right)^2 < \infty \quad \text{for any $t_0 >0$} \label{A:gicon} 
				\end{equation}
...
In the proof of Theorem \ref{T:Moment}, we actually already showed that \eqref{A:gicon} is satisfied when the initial condition is a $L^2_\rho(\R^d)$ function. Also, \eqref{A:fcon} will only need to be used if $b(x,0) \not = 0$. Throughout the remainder of this paper we never assume that $b(x,0) = 0$ so we will need this assumption in order to prove  Lemma \ref{L:Main} which is crucial to complete the proof of Theorem \ref{T:Main}. 
...
\begin{theorem}  
	...
	\end{align*}
	Then for any $\delta_0 >0$ there exists an invariant measure for the laws of \eqref{E:msol}, $\{\mathscr L (u^{\hat\varphi}(t,\cdot))\}_{t \ge \delta_0}$,  in $L^2_\rho(\R^d)$.
\end{theorem}
...
\begin{lemma}
	\label{L:Main}
Under the assumptions of Theorem \ref{T:Main} and for arbitrary $\epsilon \in (0,1)$, $0< \delta_0 \le 1$ and $R>0$ there exists a compact set $\mathscr K$ in $L^2_{ \rho}$ such that
 \begin{equation}
	 \label{E:cpt}
	\mathbb{P}\{u^{\hat{\varphi}}(t,\cdot) \in \mathscr K \} \ge (1-\epsilon)\mathbb{P}\{|u^{
	\hat{\varphi}}(t-\delta_0,\cdot)|_{\hat \rho} < R\} \quad \text{for all $t>\delta_0$}.
 \end{equation}
\end{lemma}
 
 We may now complete the proof of Theorem \ref{T:Main} By Lemma \ref{L:Main}, given an $\epsilon \in (0,1)$ and $R>0$ then we could find a compact set $\mathscr K \in L^2_{\rho}$ such that  
 \begin{align*}
	 \mathbb{P}\{u^{\hat \varphi}(t,\cdot) \in \mathscr K \} \ge (1-\epsilon)\mathbb{P}\{|u^{\hat \varphi}(t-\delta_0,\cdot)|_{\hat \rho} < R\} \quad \text{for all $t>\delta_0$} .
 \end{align*}
The boundedness in probability of $u^{\hat \varphi}(\cdot,x)$ then implies that the family of laws 
$\mathscr L(u^{\hat \varphi}(t,\cdot))$ for $t \ge \delta_0$ is tight in $L^2_{\rho}$ and therefore,
the family of probability measures 
 \begin{equation}
	 \label{E:intlaw}
	 \dfrac{1}{T}\int_{\delta_0}^{T+\delta_0} \mathscr L(u^{\hat \varphi}(t,\cdot))\: \ud t, \quad T>0 
 \end{equation}
 ...
 \begin{remark}
 	The condition \begin{equation} \sup_{t \ge 0} \E |u(t,\cdot)|_\rho^2 < \infty \label{E:bipc} \end{equation} 
 	is enough to verify that $u$ is bounded in probability. 
	Indeed, let $\sup_{t\ge 0} \E |u(t,\cdot)|_\rho^2 = B$ and let $\epsilon >0$.
	Then by applying Chebyshev's inequality we may choose $R>0$ large enough so that 
 	\begin{align*}
		\mathbb{P}[|u(t,\cdot)|_\rho \ge R] \le R^{-2}\E |u(t,\cdot)|_\rho^2 \le R^{-2}B < \epsilon.
 	\end{align*}  
 	Hence $u(t,\cdot)$ is bounded in probability in $L^2_{\rho}$. 
 \end{remark}

\begin{remark} Theorem \ref{T:Moment} shows that $u^\varphi(t,x)$ is bounded in probability for any $L^2_\rho$ initial condition when Assumption \ref{A:General} with $\Upsilon(0)<\infty$ are both satisfied. 
\end{remark} 	
We now prove a result showing that \eqref{E:bipc} can be satisfied for \eqref{E:gspde} with more general initial conditions.  
 
\begin{lemma}
	\label{L:Moments}
	Consider the stochastic heat equation \eqref{E:gspde} and suppose the assumption of Theorem \ref{T:Main} remain true and \eqref{A:gicon} holds true. Then for any $t_0>0$, $u(t_0,\cdot)\in L^2_\rho$ almost surely for any admissible weight. Also the solution to the following stochastic heat equation
 \begin{equation}
	\label{E:vspde2}
	 \begin{cases}
...
\\& < \infty.
\end{align*}
The finiteness in the last inequality follows from \eqref{A:gicon}. This shows that $u(t_0,x)\in L^2_\rho$ almost surely for any admissible weight. Now it remains to show $\sup_{t\ge 0} \E |v(t,\cdot)|_\rho^2 < \infty$. 
\begin{align*}
...
\\ & < K(t_0)
\end{align*}
for some constant, $K(t_0)$, dependent on $t_0$ by \eqref{A:gicon} and $t \mapsto G(t_0+t,x)$ is bounded by $G(t_0,x)$ up to a constant. This shows $\sup_{t\ge 0} \E |v(t,\cdot)|_\rho^2 < \infty$.

\textcolor{blue}{This is incorrect. $G(t+t_0,x)$ is not bounded above up to a constant by $G(t_0,x)$(see discussion document). As of now this proof is incorrect and stronger assumptions will need to be made. Specifically we need to assume that \eqref{A:gicon} is bounded in $t_0$, not just finite for each $t_0$. However, this may not be too bad because I think that the $\delta_0$ initial condition will still satisfy this stronger assumption. 
	\begin{align*}
		\int_{\R^d}\ud x \: \rho(x)\left(\int_{\R^d}\ud y \: G(t_0,x-y)\delta_0(\ud y) \right)^2 &= \int_{\R^d}\ud x \: \rho(x)G(t_0,x)^2
		\\ & \le \int_{\R^d}\ud x \: \rho(x)
		\\ & = \Norm{\rho}_{L^1(\R^d)}
	\end{align*}
since $G(t,x)$ is bounded by $1$ and $\rho(x)$ is assumed to be integrable. Therefore \eqref{C:delta} will remain true.}
\end{proof}

\begin{theorem} 
	\label{T:Main2}
	Suppose the assumptions of Theorem \ref{T:Main} remain true and \eqref{A:gicon} holds true and suppose there existed
	another admissible weight $\hat{\rho}$ such that 
	\begin{align*}
		\int_{\R^d}\dfrac{\rho(x)}{\hat\rho(x)}\: \ud x < \infty.   	
	\end{align*}
	Recall from Lemma \ref{L:Moments} that $u^\mu(t_0,x) \in L^2_{\hat\rho}(\R^d)$ for any $t_0 >0$ and any
	admissible weight $\hat{\rho}$. Then there exists an invariant measure for the laws of the solutions to
	\eqref{E:gspde} in $L^2_\rho$ for $t>\delta>0$. In particular, for any $\delta>0$ there exists a measure that is
	invariant for $\{\mathscr L (u^\mu(t,\cdot))\}_{t>\delta}$ where $u$ is the solution to \eqref{E:gspde} with
	initial measure $\mu$.
\end{theorem}

\begin{proof} Pick $t_0>0$ and $\delta_0>0$ such that $\delta=t_0+\delta_0$ for a given $\delta >0$.  By Theorem
\ref{T:Main}, there exists an invariant measure, $\eta$, for $\{\mathscr L(v(t,\cdot))\}_{t\ge \delta_0}$ where $v$ is
the solution to \eqref{E:vspde2}. However, by a general property of the heat solution, $v(t,x)=u^\mu(t+t_0,x)$ and so
$\eta$ is invariant for $\{\mathscr L(u^\mu(t+t_0,\cdot))\}_{t\ge \delta_0} = \{\mathscr L(u^\mu(t,\cdot))\}_{t\ge
\delta}$. 
\end{proof}

\begin{corollary}
	\label{C:delta}
	Suppose the assumption of Theorem \ref{T:Main} remain true. Then for any $\delta>0$, the stochastic heat equation 
	\begin{equation}
		\label{E:dspde}
		\begin{cases}
		\dfrac{\partial u}{\partial t}(t,x)-\frac{1}{2}\Delta_x u(t,x) = b(x,u(t,x)\dot W(t,x) & \text{$x\in \R^d$ , $t>0$}
		...
		\end{cases}  
	\end{equation}
	has an invariant measure in $L^2_\rho$ for the laws of their solution for $t>\delta$ and for any $\rho$ which
	there exists $\hat{\rho}$ such that $\rho/\hat{\rho} \in L^1(\R^d)$.
\end{corollary}

\begin{proof} 
	The SPDE above satisfies \eqref{A:gicon}. 
...
\end{equation}
It is shown in \cite{TessitoreZabczyk98M} that $F_\alpha$ is compact from $L^q\big((0,t_0),L^2_{\hat\rho} \big)$ to
$L^2_\rho$ where $L^q\big((0,t_0),L^2_{\hat\rho} \big)$ is the space of functions that map $s \mapsto f(s,\cdot)$ and
$f(s,\cdot) \in L^2_{\hat\rho}$ and in addition $\int_0^{t_0} |f(s,\cdot)|_{\hat\rho}^q \: \ud s < \infty$. 

Next we define for $\varphi \in L^2_{\rho}$ the operators $Y^\varphi(s,y)$, or simply as $Y(s,y)$ when there is no
...
\Norm{Y(s,y)}_q\big|_\rho$. To do this, we start by applying the Burkholder-Davis-Gundy Inequality from
...
\begin{align*}
	...
	\\ & \le k_1\bigg(1+(1+J_0(r,z_i))\bigg) \quad \text{by \eqref{E:mb0}}
	\\ & \le k_2(1+J_0(r,z_i)).
\end{align*}
Now applying this to the inequality for $\Norm{Y(s,y)}_q^2$ above we get that 

\begin{align*}
&\Norm{Y(s,y)}_q^2\\ 
&\le k_3 \int_0^s \ud r \: (s-r)^{-2\alpha} \iint_{\R^{2d}} \ud z_1\ud z_2 G(s-r,y-z_1)  (1+J_0(r,z_1)) f(z_1-z_2)\\
&\hspace{13.5em} \times  G(s-r,y-z_2) (1+J_0(r,z_2))
\\ &=  k_3 \int_0^s \ud r \: (s-r)^{-2\alpha} \iint_{\R^{2d}} \ud z_1\ud z_2 G(s-r,y-z_1)f(z_1-z_2)G(s-r,y-z_2)
\\ &\hspace{1.5em}+  k_3 \int_0^s \ud r \: (s-r)^{-2\alpha} \iint_{\R^{2d}} \ud z_1\ud z_2 G(s-r,y-z_1)f(z_1-z_2)G(s-r,y-z_2)J_0(r,z_1)
\\ &\hspace{1.5em}+  k_3 \int_0^s \ud r \: (s-r)^{-2\alpha} \iint_{\R^{2d}} \ud z_1\ud z_2 G(s-r,y-z_1)f(z_1-z_2)G(s-r,y-z_2)J_0(r,z_2)
\\ &\hspace{1.5em}+  k_3 \int_0^s \ud r \: (s-r)^{-2\alpha} \iint_{\R^{2d}} \ud z_1\ud z_2 G(s-r,y-z_1)f(z_1-z_2)G(s-r,y-z_2)J_0(r,z_1)J_0(r,z_2)\\
   &=: I_1 +I_2+I_3+I_4.
\end{align*}
