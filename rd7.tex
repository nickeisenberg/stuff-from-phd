...
example, in \cite{TessitoreZabczyk98M}, we drop hypothesis 2.1(i) as well as the assumptions in Theorem \ref{??} for the new
assumptions listed below. 

\textcolor{magenta}{Give the correct number for the above theorem.} 
...
$L^2(\Omega,\mathcal F,P)$-valued zero mean Gaussian Processes on a complete probability space $(\Omega,\mathcal F,P)$
\begin{align*}
...
\end{align*}
where the limit is in $L^2(\Omega,\mathcal F,P)$, $\psi_n \in C_0^\infty(\R^{d+1})$ and $\psi_n \downarrow
...
\begin{assumptions} 
	...
	\begin{enumerate}[(i)]
		\item \[
			\int_{\R^d}\exp(-a|x|^2)|\mu|(\ud x) < \infty \quad \text{for any $a>0$} \label{icon} 
			\]
			\textcolor{magenta}{(This is not the correct way to cite. You can either use equation number, or 
			cite as part (i) of Assumption \ref{A:General}. Please update the documents for all these
			problems.)} 
		\item \[
				\Upsilon(\beta) := (2\pi)^{-d}\int_{\R^d}\dfrac{\hat f(\ud \xi)}{\beta + |\xi|^2} <
				\infty \quad \text{for some and therefore any $\beta>0$}  \label{dalcon} 
			\]
		\item \[  
				|b(x,u)-b(x,v)|< L |u-v| \quad \text{and} \quad |b(x,0)| \le L_0 \quad u,v \in \R, \; x \in \R^d \label{lipcon} 
			\]
	\end{enumerate}
\end{assumptions}

\begin{notation}
	The notation $\hat{f}(\ud \xi)$ is use to denote a pairing in the sense that $\langle f, g \rangle = \langle \hat{f}, \hat{g} \rangle$. When $f$ is indeed a test function then $\hat{f}(\ud \xi)=\hat{f}(\xi)\ud \xi$. So for example, \ref{dalcon} could be written as $\langle \hat{f} , (\beta + |\cdot|^2)^{-1} \rangle$. It is the value of $(\beta + |\cdot|^2)^{-1}$ under the distribution $\langle \hat{f} , \cdot \rangle$.
\end{notation}

\begin{theorem}
Suppose that Assumption \ref{A:General} are satisfied. Then \eqref{E:gspde} has a unique random field solution starting from $\mu$. The solution is $L^2(\Omega)$ continuous. 
\end{theorem}
...
\begin{theorem} 
	Under Assumption \ref{A:General} and for any $t>0$ and $x \in \R^d$, the solution to \eqref{E:gspde},$u(t,x)$,
	given by \eqref{E:gmsol} is in $L^p(\Omega)$ and for $p\ge 2$
	\begin{equation}
		\Norm{u(t,x)}_p \le \big[\bar{\varsigma}+\sqrt{2}(|\mu|*G(t,\cdot))(x)\big]H(t;\gamma_p)^{1/2} \label{E:mb}
	\end{equation}
	where $\bar{\varsigma}=L_0/L$ and $\gamma_p = 32pL^2$. The function $H$ is defined in \cite{CH16Comparison} but
	the definition is not important here. Moreover, if we assume that\begin{equation}
	 \Upsilon(0) < \infty \label{E:dalcon0}
	 \end{equation}
	 then we may make the stronger assertion that for some constant $C$
	\begin{equation}
		\Norm{u(t,x)}_p \le \text{Const.}\big[\bar{\varsigma}+\sqrt{2}(|\mu|*G(t,\cdot))(x)\big] \le
		C[1+(|\mu|*G(t,\cdot))(x)\big] \label{E:mb0}.
	\end{equation} 
	Even further, if we assume that $b(x,0)=0$ for and $x \in \R^d$ then $L_0 = 0$ and we easily see that
	\eqref{E:mb0} becomes 
	\begin{equation}
		\label{E:mb00}
		\Norm{u(t,x)}_p \le \text{Const.}\big[(|\mu|*G(t,\cdot))(x)\big].
	\end{equation}
\end{theorem}
...
a simple lemma showing that $\varphi \in L^2_\rho$ implies condition \ref{icon}.

\begin{lemma}
	\label{L:phi}
	Suppose that $\varphi \in L^2_\rho(\R^d)$. Then $\varphi$ satisfies \ref{icon}.
\end{lemma}

\begin{proof} It can be shown that for any admissible weight, there exists some constant $C>0$ such that $C \cdot \rho(x) \ge  \exp(-a|x|^2)$. This constant $C$ may depend on $a>0$. Now, since we assume $\int_{\R^d}\rho(x)|\varphi(x)|^2\ud x < \infty $ then this implies that $\int_{\R^d}\rho(x)|\varphi(x)|\ud x < \infty $ which now implies that $\int_{\R^d}\exp(-a|x|^2)|\varphi(x)|\ud x < \infty $ 
\end{proof}

\begin{theorem} 
	\label{T:Moment}
	Suppose Assumption \ref{A:General} hold. Then \eqref{E:msol} gives the unique (in the sense of versions) random field solution \{$u(t,x): t>0, \; x\in\R^d$\} of \eqref{E:spde} starting from $\varphi(x) \in L^2_\rho$. The solution is $L^2(\Omega)-continuous$. Moreover, if we assume $\Upsilon(0)<\infty$ so we may apply \eqref{E:mb0} then we may say
	\begin{align*}
...
\textcolor{magenta}{Never write sentence in equations. Instead to the following:} 

\textcolor{blue}{where the last inequality is due to the admissible weitht property. This proves....} 
...
We finish off this section with a proposition on the functions $G(t,x)$ which will be used later on in the proof of Lemma \ref{L:Main}. The proof of this result can be found in \cite{TessitoreZabczyk98M} \\

\begin{proposition} 
	\label{P:weight}
	For any admissible weight $\rho$, the operators on $L^2_\rho(\R^d)$ defined by $\varphi(x) \mapsto \big(G(t,\cdot)*\varphi(\cdot)\big)(x))$ can be extended to a $C_0-semigroup$ on $L^2_\rho$. Moreover, if $\hat\rho$ is another admissible weight such that 
	\begin{align*}
...
\begin{theorem}  
	\label{T:Main}
	Assume that the Assumption \ref{A:General}  with the stronger assumption that $\Upsilon(0) <\infty$. Also assume and \ref{fcon}. Now for any admissible weight $\rho$, suppose there exists another admissible weight $\hat\rho$ and an element $\hat{\varphi} \in L^2_\rho(\R^d) \cap L^2_{\hat\rho}(\R^d)$ such that 
	\begin{align*}
...
Under the assumptions of Theorem \ref{T:Main} and for arbitrary $\epsilon \in (0,1)$, $0< \delta_0 \le 1$ and $R>0$ there exists a compact set $\mathscr K$ in $L^2_{ \rho}$ such that
 ...
 \end{equation}
 \textcolor{magenta}{Forget period. Update the whole document for this problem.} 
\end{lemma}
...
\begin{remark} Theorem \ref{T:Moment} shows that $u^\varphi(t,x)$ is bounded in probability for any $L^2_\rho$ initial condition when Assumption \ref{A:General} with $\Upsilon(0)<\infty$ are satisfied. 
\end{remark} 	
We now prove a result showing that \eqref{E:bipc} can be satisfied for \eqref{E:gspde} with more general initial conditions.  
 
\begin{lemma}
	\label{L:Moments}
	Consider the stochastic heat equation \eqref{E:gspde} and Suppose the assumption of Theorem \ref{T:Main} remain true and \ref{gicon} holds true. Then for any $t_0>0$ $u(t_0,\cdot)\in L^2_\rho$ almost surely for any admissible weight. Also the solution to the following stochastic heat equation
 \begin{equation}
	\label{E:vspde2}
	 \begin{cases}
...
\\ & < K(t_0)
\end{align*}
for some constant, $K(t_0)$, dependent on $t_0$ by assumption \ref{gicon} and $t \mapsto G(t_0+t,x)$ is bounded by $G(t_0,x)$ up to a constant. This shows $\sup_{t\ge 0} \E |v(t,\cdot)|_\rho^2 < \infty$.
\end{proof}

\begin{theorem} 
	\label{T:Main2}
	Suppose the assumption of Theorem \ref{T:Main} remain true and \ref{gicon} holds true and suppose there existed
...
\begin{proof} 
	The SPDE above satisfies assumption \ref{gicon}. 
...
\end{align*}
and by assumption \ref{fcon} and Lemma 4.2 below,  the three integrals above are all finite any $s \ge 0$ so we may take the maximum for $0 \le s \le S$ and say there exists some constant $K_S$ dependent on $S$ such that
	\[ 
...
\begin{lemma}
Assuming \ref{fcon} then we may say the following two integrals are finite
\begin{equation}
...
which completes the proof showing assumption \ref{fcon} implies that \eqref{E:fcon2} is finite. The finiteness of \eqref{E:fcon3} follows easily by noting that 
\begin{align*}
...
\\ & \le \ref{fcon} < \infty
\end{align*}
which completes the proof showing that assumption \ref{fcon} implies \eqref{E:fcon3}. 
\end{proof}
...
\end{align*}
where the finiteness follows by assumption \ref{fcon} and Lemma 4.2. This completes the justification for the first use of Fubini's Theorem. \\
\end{proof}
...
\int_{\R^d}\hat f(\ud \xi)\: \exp\left(-(t-r)|\xi|^2\right)
\]
and in fact $k_{t,1}$ is finite for each $t>0$ by \textbf{Lemma 4.2.} 
...
\]
which is finite by \textbf{Lemma 4.2.} 
...
\]
which is again finite by \textbf{Lemma 4.2}. Now we may say that that there exists some constant $K_t$ dependent on $t$ such that \eqref{E:fcond} has the following bound which is finite since $0<\alpha <1$
\begin{align*}
