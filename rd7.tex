...
\begin{theorem} 
	...
	\textcolor{blue}{Then for all $T>0$ there exists $C_T>0$ such that for all $t \in [0,T]$, $u(t,\cdot)\in L^2_\rho(\R^d)$ with} 
	\begin{align}
		\label{E:uNormRho}
		\E\left(| u(t,\cdot)|_\rho^2\right) 
		\le  2 C_T \left[ \Norm{\rho}_{L^1(\R^d)} + C_\rho(T) |\varphi|_{\rho}^2 \right]<\infty.
	\end{align}
	Moreover, if $\Upsilon(0)<\infty$, then as a consequence of \eqref{E:mb0} \textcolor{magenta}{and provided that we can show 
