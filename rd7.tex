...
\begin{theorem} 
	...
	\end{align}
	where we use $C_T:= H(T,\gamma_2)$. Moreover, if $\Upsilon(0)<\infty$ and 
	 for each $t>0$ there exists a $K(t) \in \mathbb N$ such that $\sup\limits_{x\in \R^d}\big(|\varphi|*G(t,\cdot)\big)(x) < K(t)$, then one can show
	\begin{align*}
...
\textcolor{blue}{Now consider $t\ge t_0>0$ and further assume that $\Upsilon(0) < \infty$ and that $\sup\limits_{x\in \R^d}(|\varphi|*G(t,\cdot))(x) \le K(t)$ for any $t>0$. Now starting with \eqref{mstep}, we see that
\begin{align*}
	\E\left(| u(t,\cdot)|_\rho^2\right) 
...
\begin{remark}
	In general, the requirement on the initial condition $\mu$ 
		\[
			\sup\limits_{x\in \R^d}\big(|\mu|*G(t,\cdot)\big)(x) < K(t) \quad t>0
		\]
	is satisfied whenever $\mu=\varphi \in L^2_\rho$ is bounded and, as we will also see below, is also satisfied when $\mu = \delta_0$ where $\delta_0$ is the Dirac delta measure. 
\end{remark}
...
\begin{assumptions} $\;$
	\begin{enumerate}[(i)]
		\item \begin{equation} 
					\int_{\R^d}\ud x \: \rho(x)\big(G(t_0,\cdot)*|\mu|)(x) \big)^2 < \infty \quad \text{for any $t_0 >0$} \label{A:gicon} 
				\end{equation}
				
		\item \textcolor{blue}{\begin{equation} 
...
In the proof of Theorem \ref{T:Moment}, we actually already showed that \eqref{A:newgicon} is satisfied when the initial condition is a bounded $L^2_\rho(\R^d)$ function. Also, \eqref{A:fcon} will only need to be used if $b(x,0) \not = 0$. Throughout the remainder of this paper we never assume that $b(x,0) = 0$ so we will need this assumption in order to prove  Lemma \ref{L:Main} which is crucial to complete the proof of Theorem \ref{T:Main}. 
...
\begin{theorem}  
	...
	\end{align*}
	Then for any $t_0 >0$ there exists an invariant measure for the laws of \eqref{E:msol}, $\{\mathscr L (u^{\hat\varphi}(t,\cdot))\}_{t \ge t_0}$,  in $L^2_\rho(\R^d)$.
\end{theorem}
...
$\mathscr L(u^{\hat \varphi}(t,\cdot))$ for $t \ge t_0$ is tight in $L^2_{\rho}$ and therefore,
...
\begin{remark} Under the assumptions of Theorem \ref{T:Moment}, we have that $u^\varphi(t,x)$ is bounded in probability for any bounded $L^2_\rho$ initial condition.
\end{remark} 	
...
\begin{proof} \textcolor{blue}{We first show that $u(t_0,x)\in L^2_\rho$ for any admissible weight.
\begin{align*}
...
\begin{align*}
\E |v(t,\cdot)|_\rho^2 & \le C \int_{\R^d} \bigg( 1+\bigg( \big(\big[1+(\mu*G(t_0,\circ))\big](\cdot) * G(t,\cdot)\big)(x) \bigg) \bigg)^2\rho(x) \: \ud x
\\& \le C \int_{\R^d} \bigg( 1+\bigg( 1+\big[(\mu*G(t_0,\circ))(\cdot) * G(t,\cdot)\big](x) \bigg) \bigg)^2\rho(x) \: \ud x
\\& = C \int_{\R^d} \bigg( 1+\bigg( 1+\big[(\mu*G(t_0+t,\cdot)) \big](x) \bigg) \bigg)^2\rho(x) \: \ud x.
\end{align*}
...
\\&= C\bigg( 1+\bigg( 2+K(t_0) \bigg) \bigg)^2 \Norm{\rho}_{L^1{\R^d}}
\end{align*}
...
\begin{theorem} 
	\label{T:Main2}
	Suppose the assumptions of Theorem \ref{T:Main} remain true and \eqref{A:newgicon} holds true and suppose there existed
	...
	\eqref{E:gspde} in $L^2_\rho$ for $t>l>0$. In particular, for any $\delta>0$ there exists a measure that is
	invariant for $\{\mathscr L (u^\mu(t,\cdot))\}_{t>l}$ where $u$ is the solution to \eqref{E:gspde} with
	initial measure $\mu$.
\end{theorem}

\begin{proof} Pick $t_0>0$ and $l_0>0$ such that $l=t_0+l_0$ for a given $l >0$.  By Theorem
