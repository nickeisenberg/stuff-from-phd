...
\begin{theorem} 
	...
	Then for all $t>0$, $u(t,\cdot)\in L^2_\rho(\R^d)$ with 
	\begin{align}
		\label{E:uNormRho}
...
\begin{proof} 
	Fix an arbitrary $T>0$. For any $t\in [0,t]$, by the Fubini theorem and the moment bound \eqref{E:mb}, we see that
	\begin{align*}
		\E\left(| u(t,\cdot)|_\rho^2\right) 
		&= \E \left(\int_{\R^d}\vert u(t,x) \vert^2 \rho(x)\ud x\right) 
		\le C_T \int_{\R^d}\bigg(1+(|\varphi|*G(t,\cdot))(x)\bigg)^2\rho(x)\ud x.
	\end{align*}
	Since the fact that $\rho$ is admissible implies that $\rho\in L^1(\R^d)$ \textcolor{magenta}{(Let's assume for the moment that $\rho\in
	L^1(\R^d)$),} 
	we see that 
	\begin{align*}
		\E\left(| u(t,\cdot)|_\rho^2\right) 
