...
\textcolor{blue}{Theorem 3.3 above was in the Tessitore paper, I fixed the citation for it}
...
\begin{assumptions} $\;$
	...
	\begin{enumerate}[(i)]
		\item \begin{equation}
			\int_{\R^d}\exp(-a|x|^2)|\mu|(\ud x) < \infty \quad \text{for any $a>0$} \label{A:icon} 
				\end{equation}
		...
		\textcolor{blue}{I added equation numbers}
...
\begin{theorem}
Suppose that Assumption \ref{A:General} is satisfied. Then \eqref{E:gspde} has a unique random field solution starting from $\mu$. The solution is $L^2(\Omega)$ continuous. 
\end{theorem}
...
\begin{theorem} 
	\label{T:Moment}
	Suppose Assumption \ref{A:General} holds. Then \eqref{E:msol} gives the unique (in the sense of versions) random field solution \{$u(t,x): t>0, \; x\in\R^d$\} of \eqref{E:spde} starting from $\varphi(x) \in L^2_\rho$. The solution is $L^2(\Omega)-continuous$. Moreover, if we assume $\Upsilon(0)<\infty$ then we may apply \eqref{E:mb0} and say
	\begin{align*}
