...
\begin{corollary} \label{C:explain}
	Let $u(t,x)$ denote the solution to \eqref{spde} and $\alpha \in (0,1)$. Then for $t_0>0$ we may say that \begin{equation} u(t_0,x)=\big( G(t_0,\cdot)*\varphi(\cdot) \big)(x)+\dfrac{\sin(\alpha \pi)}{\pi}F_\alpha^{t_0}\big(Y(s,\cdot)\big)(x) \label{1fac}\end{equation} \\
