...
\begin{theorem} 
	...
	Then for all $T>0$ there exists $C_T>0$ such that for all $t \in [0,T]$, $u(t,\cdot)\in L^2_\rho(\R^d)$ with 
	\begin{align}
		\label{E:uNormRho}
		\E\left(| u(t,\cdot)|_\rho^2\right) 
		\le  2 C_T \left[ \Norm{\rho}_{L^1(\R^d)} + C_\rho(T) \Norm{\varphi}_{\rho}^2 \right]<\infty,
	\end{align}
...
\begin{proof} 
	...
	\end{align*}
	By definition $\rho\in L^1(\R^d)$, hence
	\begin{align*}
		\E\left(| u(t,\cdot)|_\rho^2\right) 
