...
then we have 
	\begin{equation}\label{E:KThetaRecursive}
		\mathbb{P}\left\{ X^x(t) \in \mathscr{K}(\theta)  \right\} \ge (1-k_3\theta^{-q}(1+R^q)) \mathbb{P}\left\{ \left| X^x(t-1) \right|_{\hat{\rho}} < R  \right\}.
	\end{equation}
Is this true because because 
	\begin{align*}
	...
	\end{align*}
And $R$ and $\theta$ being arbitrary means we can just take $R > \left| X^x(t-1) \right|_{\hat{\rho}}$ which implies that 
	\[
...
and so 
	\begin{equation}\label{E:KThetaConditional}
			\mathbb{P}\left\{ X^x(t) \in \mathscr{K}(\theta)  \right\}  = 	\mathbb{P}\left\{ X^x(t) \in \mathscr{K}(\theta) \bigg\vert  \left| X^x(t-1) \right|_{\hat{\rho}} < R  \right\} \mathbb{P}\left\{ \left| X^x(t-1) \right|_{\hat{\rho}} < R  \right\}.
	\end{equation}
Now is it true that the SHE is time homogeneous in the sense that 
	\[
