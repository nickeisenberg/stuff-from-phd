...
\textcolor{blue}{In the above wrote $K_t$ to mean the same thing as $C_\rho(T)$. However in general there is no explicit form for this constant and also in general, $\sup_{T \ge 0}K_T \not < \infty$. For example, when we consider the weight $\rho(x)=\exp(-|x|)$ in $\R$ then we can show that $(G(t,\cdot)*\exp(-|\cdot|))(x) \le 2\exp(t/2) \exp(-|x|)$ and clearly $\sup_{t \ge 0} 2\exp(t/2) = \infty$.}
