...
however in this proof they say we must assume that $d\beta / \alpha < 1$. I think this is a typo. However, this assumption still is not stated in the statement of the theorem. Another issue is that they need to assume that 
	\[
		\gamma < \alpha
	\]
 in order to apply \cite[Lemma 2.7]{FN2016} and this asumption is not stated in the statement of the theorem. This assumption however would imply that $\gamma \beta / \alpha <1$ since in \cite4{FN2016} $\beta \in (0,1)$.
...
Under the assumptions of Theorem \ref{T:ExisAndUniq} we have that 
	...
	\end{theorem} 
	\begin{proof}
The proof follows the strategy from the proof of \cite[Theorem 1.10]{FN2016}. We start by defining $A(t) = \sup_{x \in \R^d} \mathbb{E}|u(t,x)|^2$ and look to prove an inequality of the following type:
	\begin{equation}
	\label{E:Renewal}
		A(t) \le C + C \int_0^t \frac{A(s)}{(t-s)^{...}} \; \ud s,
	\end{equation}
where the positive constants $C$ and $C$ above may not match. To do this, we note that 
	\begin{align*}
...
Note that for bounded $u_0$ and $v_0$ we have that 
	\begin{align*}
		...
		&=  \begin{cases}
				...
				\end{cases} 
	\end{align*}
The $t$ above in the bound may be an issue since then \eqref{E:Renewal} would then take the form 
	\[
		A(t) \le Ct + C \int_0^t  \frac{A(s)}{(t-s)^{...}} \; \ud s.
	\]
This problem does not occur when $v_0 = 0$. 
	\end{proof}
	\end{NickQuestion}
\textcolor{blue}{
