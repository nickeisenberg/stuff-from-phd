...
Their proof relies on a probabilistic representation of the Green's function for the solution of \eqref{E:SPDE}. Our proof takes a different approach and relies on the asymptotic behavior of the Euler-Mittag function given in \cite[Theorem 1.6]{Podlubny99}. One should note that under the assumptions \eqref{A:Y=GAssumptions} that $Y(t,x)=Z(t,x)$. However, in general, this is not the case. 
	...
	\begin{lemma}
	\label{L:intYfYbdd}
Suppose that $\lambda < 2\alpha$, then there exists a constant $C >0$ such that for any $x,y \in \R^d$ we have that 
	\begin{equation}
	\label{E:YYfBdd}
...
In addition, I believe that there is a typo in the statement of \cite[Lemma 3.3]{Walsh89}. I believe that in Lemma 3.3 {\it ibid.} we should assume that $a>-1$ and not $a >1$. This would be consistent with \cite[(3.14)]{Walsh89}.
	...
	\begin{NickQuestion} 
The following tries to give an upper bound to $\sup_{x \in \R^d} \mathbb{E}|u(t,x)|^2$. There is an issue with the upper bound of $J_0(t,x)$ depending on $t$ when the initial condition $v_0 \not = 0$. 
