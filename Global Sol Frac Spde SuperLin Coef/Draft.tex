...
\textcolor{blue}{I believe that Proposition \ref{P:N(u)Bdd} below can be proven for a more general dimension $d$.}
	...
	\label{P:N(u)Bdd}
Let $u_0$ and $v_0$ be Borel functions satisfying $\Norm{u_0}_\infty + \Norm{v_0}_\infty < \infty$. Suppose that $d < 2 \alpha$. Then there exists a universal constant $K:=K(\alpha, \beta, \gamma,d) >0$ such that for any 
	\[
...
\section{Spatially Homogeneous Noise Driven SPDE}
We first examine \eqref{E:SPDE} driven by a Gaussian noise that is white in time and colored in space with Lipschitz diffusion coefficient .
\subsection{Lipschitz Diffusion Coefficient}
We will now look at \eqref{E:SPDE} where the noise is now white in time and colored in space. Informally this means that 
	\[
...
where
	...
	\]
In addition, we assume that there exists some $L_\sigma > 0$ such that 
	\[
		|\sigma(x) - \sigma(y)| \le L_{\sigma} |x-y|.
	\]

To prove existence and uniques of the solution, we will need the following lemma. The following lemma is proven by Foondun and Nane \cite{FN2016} in the case that 		
	\begin{equation}
...
We can now prove the existence and uniqueness of the solution to \eqref{E:SPDE} driven by the noise $\dot{F}$. 
	...
	\begin{theorem}
	\label{T:ExisAndUniq}
Suppose that $\lambda \beta / \alpha - 2(\beta + \gamma -1) < 1$ and in addition, suppose that $\lambda < 2 \alpha$. Then there exists a unique solution to \eqref{E:SPDE} driven by $\dot{F}$ such that 
	\[
...
\subsection{Section 4.2 Millet and Sanz-Sol\'e \cite{SoleMillet2021}}
		...
		Let $u_0$ and $v_0$ be Borel functions satisfying $\Norm{u_0}_\infty + \Norm{v_0}_\infty < \infty$. Suppose that $d < 2 \alpha$. Then there exists a universal constant $K:=K(\alpha, \beta, \gamma,d, \lambda ) >0$ such that for any 
		\[
...
where 
	...
	\end{equation*}
and $C$ is as in Lemma \ref{L:intYfYbdd}. Before we continue, we define 
	\begin{equation}
	\label{D:theta}
		\theta : = 2\beta - 1 + 2\gamma - \beta \lambda / \alpha = \frac{2 \beta \alpha - \alpha + 2 \alpha \gamma - \beta \gamma}{\alpha}.
	\end{equation}
In return, this gives us that 
	\begin{align*}
		...
		&= \frac{8p K_1 c(\sigma)^2}{ \theta} t ^ \theta + 8p K_1 N_{a,p}(u)^2 L(\sigma)^2 \frac{\exp(2at) \left[ \Gamma(\theta) - \Gamma(\theta, 2at)\right]}{(2a)^{\theta} }  \\
