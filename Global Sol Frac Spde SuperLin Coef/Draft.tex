...
Their proof relies on a probabilistic representation of the Green's function for the solution of \eqref{E:SPDE}. Our proof takes a different approach and relies on the asymptotic behavior of the Euler-Mittag function given in \cite[Theorem 1.6]{Podlubny99}. One should note that under the assumptions \eqref{A:Y=GAssumptions} that $Y(t,x)=Z(t,x)$. However, in general, this is not the case. 
	...
	\begin{lemma}
Suppose that $\lambda < \alpha$, then there exists a constant $C >0$ such that for any $x,y \in \R^d$ we have that 
	\begin{equation}
	\label{E:YYfBdd}
...
Through a change of variables, we may write the left side of \eqref{E:YYfBdd} as the following:
	\[
		\int_{\R^{2d}} Y(t,x-z)Y(t,y-w)f(w-z) \; \ud z \ud w = \int_{\R^{2d}} Y\left(t,u-(x-y)\right)(Y(t,\cdot)*f)(u) \; \ud u,
	\]
where $*$ denotes the spatial convolution. Now an application of Pareseval's identity gives us that 
	\[
		\int_{\R^{2d}} Y\left(t,u-(x-y)\right)(Y(t,\cdot)*f)(u) \; \ud u = \frac{1}{(2\pi)^d} 	\int_{\R^{2d}} \mathcal{F}Y\left(t,\cdot-(x-y)\right)(\xi) \mathcal{F}(Y(t,\cdot)*f)(\xi) \ud \xi.
	\]
Applying the translation property of the Fourier transform and \cite[Theorem 4.1]{CHN19} gives us that 
	\begin{align*}
		\frac{1}{(2\pi)^d} 	\int_{\R^{2d}} \mathcal{F}Y\left(t,\cdot-(x-y)\right)(\xi) & \mathcal{F}(Y(t,\cdot)*f)(\xi) \ud \xi \\
		& = \frac{ K_f \; t^{2(\beta  \gamma -1)} }{ (2\pi)^d } \int_{\R^{2d}} \exp(-i(x-y)\xi) \frac{E_{\beta,\beta+\gamma}\left(-\frac{\nu t^\beta |\xi|^\alpha}{2}\right)^2}{|\xi|^{d-\lambda}} \ud \xi \\
		& \le \frac{K_f \; t^{2(\beta + \gamma -1)}  }{ (2\pi)^d } \int_{\R^{2d}}  \frac{E_{\beta,\beta+\gamma}\left(-\frac{\nu t^\beta |\xi|^\alpha}{2}\right)^2}{|\xi|^{d-\lambda}} \ud \xi
	\end{align*}
where the last inequality follows from the non-negativity of the Euler-Mittag function. Now, by applying \cite[Theorem 1.6]{Podlubny99}, we see that there exists a $C>0$ such that 
	\[
		\frac{K_f \; t^{2(\beta + \gamma -1)}  }{ (2\pi)^d } \int_{\R^{2d}}  \frac{E_{\beta,\beta+\gamma}\left(-\frac{\nu t^\beta |\xi|^\alpha}{2}\right)^2}{|\xi|^{d-\lambda}} \ud \xi \le 
		\frac{C\; K_f \; t^{2(\beta + \gamma -1)}  }{ (2\pi)^d } \int_{\R^{2d}}  \frac{ 1 }{\left(1+ \frac{\nu}{2} t^\beta |\xi|^\alpha\right) |\xi|^{d-\lambda}} \ud \xi.
	\]
Another change of variables implies that 
	\[
		\frac{C\; K_f \; t^{2(\beta + \gamma -1)}  }{ (2\pi)^d } \int_{\R^{2d}}  \frac{ 1 }{\left(1+ \frac{\nu}{2} t^\beta |\xi|^\alpha\right) |\xi|^{d-\lambda}} \ud \xi \le 
		\frac{C\; K_f }{ (2\pi)^d } \left(\frac{\nu}{2}\right)^{-\lambda/\alpha}  t^{2(\beta + \gamma -1)-\beta \lambda/\alpha} \int_{\R^{2d}}  \frac{ 1 }{\left(1+ |\xi|^\alpha\right) |\xi|^{d-\lambda}} \ud \xi.
	\]
We see that the above integral is finite if $\lambda < \alpha$ and when this is the case, we have \eqref{E:YYfBdd}.
	\end{proof}

We can now prove the existence and uniqueness of the solution to \eqref{E:SPDE} driven by the noise $\dot{F}$. 

	\begin{theorem}
Suppose that $\lambda \beta / \alpha - 2(\beta + \gamma -1) < 1$ and in addition, suppose that $\lambda < \alpha$. Then there exists a unique solution to \eqref{E:SPDE} driven by $\dot{F}$ such that 
	\[
