...
\textcolor{blue}{The following is for space-time white noise. For colored noise we need $\theta(\lambda) >0$ and $2\alpha > d$.}
...
\theta(d) > 0 \quad \text{and} \quad d < 2\alpha,
\end{equation}
where 
\begin{equation}
\label{D:theta}
\theta(x) := 2\beta + 2\gamma -1 -\beta x / \alpha.
\end{equation} 
...
and
	\[
		\int_{\R^d} Y^2(t,x) \ud x = C_{\#} t^{2(\beta+\gamma-1)-d\beta/\alpha} = C_{\#} t^{\theta -1} ,
	\]
for all $t > 0$, where $\theta = \theta(d)$ and 
	\[
...
In the proof of the next proposition, we will use the following two facts:
	...
	\label{P:N(u)Bdd}
Let $u_0$ and $v_0$ be Borel functions satisfying $\Norm{u_0}_\infty + \Norm{v_0}_\infty < \infty$. Suppose that $d < 2 \alpha$ and consider $\theta = \theta(d)$ which is defined in \eqref{D:theta}. Then there exists a universal constant $K:=K(\alpha, \beta, \gamma,d) >0$ such that for any 
	\[
		a > \left( 8 L^2(\sigma)K^2\right)^{1/\theta}.
	\]
and 
	\[
		p \in \left[ 2, \frac{a^{\theta}}{4L^2(\sigma)K^2} \right].
	\]
we have that 
	\begin{equation}
	\label{E:Nbdd}
...
Now by applying Lemma \ref{L:1and2normYZ} and the Lipschitz property of $\sigma$ we get that 
	\begin{align*}
		\Norm{I(t,x)}_p^2 &\le 8pc^2(\sigma)C_{\#}\int_0^t (t-s)^{\theta -1} \ud s + 8pL^2(\sigma)N^2_{a,p}(u)\int_0^t \ud s \int_{\R^d} \ud y \; e^{2as} Y^2(t-s,x-y).
	\end{align*} 
Now under Dalang's condition \eqref{E:DalangEquiv2}, the first integral above can be calculated as 
	\[
		\int_0^t (t-s)^{\theta -1} \ud s = \frac{ t^{\theta} }{\theta}.
	\]
By applying Lemma \ref{L:1and2normYZ}, we can calculate the second integral as 
	\[
		\int_0^t \ud s \int_{\R} \ud y \; e^{2as} Y^2(t-s,x-y) = C_{\#}\int_0^t \ud s \; e^{2as}(t-s)^{\theta - 1}.
	\]
Putting this together gives us that 
	\begin{align*}
	\Norm{I(t,x)}_p^2 &\le 8pc^2(\sigma)C_{\#} \frac{ t^{\theta} }{\theta} + 8pL^2(\sigma)N^2_{a,p}(u) C_{\#}\int_0^t \ud s \; e^{2as}(t-s)^{\theta - 1}.
	\end{align*}
Now by multiplying both sides by $e^{-2at}$, we see that
	\begin{align*}
	\Norm{I(t,x)}_p^2e^{-2at} &\le \frac{8pc^2(\sigma)C_{\#} }{\theta} \; t^{\theta} e^{-2at} 
	\\&\qquad\qquad + 8pL^2(\sigma)N^2_{a,p}(u) C_{\#} \int_0^t \ud s \; e^{-2a(t-s)}(t-s)^{\theta - 1}.
	\end{align*}
We now do a change of variables and again recall the definition of the Gamma function to see that 
	\begin{align*}
	\Norm{I(t,x)}_p^2e^{-2at}  &\le \frac{8pc^2(\sigma)C_{\#} }{\theta} \; t^{\theta} e^{-2at} + 8pL^2(\sigma)N^2_{a,p}(u) C_{\#} \int_0^\infty \ud u \; e^{-2au}(u)^{\theta -1} \\
	& = \frac{8pc^2(\sigma)C_{\#} }{\theta} \; t^{\theta} e^{-2at} + 8pL^2(\sigma)N^2_{a,p}(u) \frac{C_{\#}\Gamma\left(\theta \right)}{ (2a)^{\theta}}
	\end{align*}
where the last line is true if Dalang's condition \cite{CHN19} holds. If we now apply \eqref{E:SUPidentities} and also recall that $\sqrt{a+b} \le \sqrt{a} + \sqrt{b}$, then we see that 
	\begin{align}
	N_{a,p}(I) &\le  c(\sigma)\sqrt{\frac{8pC_{\#}  }{\theta}} \left(\frac{\theta}{2ea}\right)^{\theta/2} +  L(\sigma)N_{a,p}(u) \sqrt{\frac{8pC_{\#} \Gamma\left( \theta \right)}{(2a)^\theta} }. \label{E:SoleMil3.9}
	\end{align}
To simplify things, we now let 
	\begin{align*}
		K&:= \max\Bigg\{\sqrt{\frac{8C_{\#}  }{\theta}} \left(\frac{\theta}{2e}\right)^{\theta/2}, \sqrt{\frac{8C_{\#} \Gamma\left( \theta \right) }{2^{\theta}}}  \Bigg\}
	\end{align*}
then,
	\begin{align}
	\label{E:Ibdd}
			N_{a,p}(I) &\le\frac{K \sqrt{p}}{a^{\theta/2}}\left( c(\sigma) + L(\sigma)N_{a,p}(u) \right).
	\end{align}
Now by combining \eqref{E:Jbdd} and \eqref{E:Ibdd}, we see that 
	...
	\\ (\lceil \beta \rceil - 1)^{\lceil \beta \rceil -1}(ea)^{1-\lceil \beta \rceil} \Norm{v_0}_\infty + \Norm{u_0}_\infty & \beta \in (1,2). 
	\end{cases} 
	\\& \quad \quad  + K\frac{\sqrt{p}c(\sigma)}{a^{\theta/2} }  + K  \frac{\sqrt{p}L(\sigma)}{a^{\theta/2}} N_{a,p}(u). \label{E:Nbddgen}
	\end{align}
...
This implies that the following interval is nonempty:
	\[
		\left[ 2, \frac{a^{\theta}}{4L^2(\sigma)K^2} \right].
	\]
In addition, for any $p$ in this interval, we have that 
	\[
		\frac{\sqrt{p}L(\sigma)K}{a^{\theta/2}} \le \frac{1}{2},
	\]
and we also have 
	\[
		\frac{\sqrt{p}}{a^{\theta/2}}  \le \frac{1}{2KL(\sigma)}.
	\]
...
We will need to recall the following result proved in \cite{CHN19} in order to prove Proposition \ref{P:NormInc} below. 
	...
	\label{P:YInc}
Suppose that $\alpha \in (0,2]$, $\beta \in (0,2)$, $\gamma>0$, and \eqref{E:DalangEquiv} holds. Consider $\theta = \theta(d)$ which is defined above in \eqref{D:theta}. Then $Y(t,x) = Y_{\alpha, \beta, \gamma, d}(t,x)$ satisfies the following:
	\begin{enumerate}
		\item \label{P:YIncCase1}  Suppose that $0< \rho < (\Theta -1) \wedge 2$ where $\Theta$ is given in \eqref{E:DalangEquiv}, $0< t,r \le T$ for some $T>0$, $\beta \le 1$ and $\gamma \le \lceil \beta \rceil - \beta$. Then there exists some $C := C(\alpha, \beta, \gamma, \nu, d, \rho, T)$ such that
	\[
		\int_0^T \ud s \int_{\R^d} \ud y |Y(t-s,x-y) - Y(r-s,z-y)|^2 \le C \left(|t-r|^\theta + |x-z|^{\rho}\right).
	\]
		\item \label{P:YIncCase2} If we only assume that $\alpha \in (0,2]$, $\beta \in (0,2)$, $\gamma>0$, $0< t,r \le T$ for some $T>0$ and \eqref{E:DalangEquiv} holds then for $0< \rho < (\Theta -1) \wedge 2$ there exists some $C := C(\alpha, \beta, \gamma, \nu, d, \rho, T)$ such that
			\[
				\int_0^T \ud s \int_{\R^d} \ud y |Y(t-s,x-y) - Y(r-s,z-y)|^2 \le C \left(|t-r|^q + |x-z|^{\rho}\right),
			\]
		where 
			\[
				0 < q < \theta.
	...
	\begin{remark}
In Proposition \ref{P:YInc} above, we may often ignore the dependence of $C$ on $\alpha, \beta, \gamma, \nu, d$ and just write $C(\rho, T)$. 
	\end{remark}

	\begin{proposition}
	\label{P:NormInc}
Suppose that $u_0=1$ and that $v_0 =0$ so that $u(t,x) = 1 + I(t,x)$. Let $a>0$, $\theta = 2(\beta + \gamma) -1 - \beta d/\alpha$ and suppose that $0< \rho < (\Theta -1) \wedge 2$. Consider $\theta = \theta(d)$. Then for any $x,z \in \R$, $p\ge 2$ and for any $T>0$ with $t\in [0,T]$,
	\begin{equation}
	\label{E:NormInc_N(u)}
		\frac{\Norm{u(t,x)-u(r,z)}_p}{\left(|t-r|^{q} + |x-z|^{\rho}\right)^{1/2}} \le 	C(p,\rho, T)\left[ \mathcal{M}_1 + \mathcal{M}_2 e^{aT} N_{a,p}(u)\right] ,
	\end{equation}
where 
\[
...
and
	\begin{enumerate}
		\item $0< q \le \theta$ under Case \ref{P:YIncCase1} of Proposition \ref{P:YInc},
		\item $0 < q < \theta$ under Case \ref{P:YIncCase2} of Proposition \ref{P:YInc}.
	\end{enumerate}
Moreover, for any 
\[
...
we have that 
	\begin{equation}
	\label{E:NormInc_cL}
		\frac{\Norm{u(t,x)-u(r,z)}_p}{\left(|t-r|^{q} + |x-z|^{\rho}\right)^{1/2}} \le C(p,\rho, T)\left[ \mathcal{M}_1 + \mathcal{M}_2 e^{aT} \left( 2 +  \frac{c(\sigma)}{L(\sigma)} \right)\right].
	\end{equation}
	\end{proposition}
...
We now apply Proposition \ref{P:YInc} to see that 
	\begin{align*}
	\Norm{I(t,x) - I(r,z)}_p^2 &\le 8p \bigg[ c^2(\sigma)C(\rho, T)\left(|t-r|^{q} + |x-z|^{\rho}\right)\\ 
	& \qquad +  L^2(\sigma) \exp\left(2aT \right)N_{a,p}^2(u) C(\rho, T)\left(|t-r|^{q} + |x-z|^{\rho}\right) \bigg],
	\end{align*}
where $C(\rho, T)$ is as in Proposition \ref{P:YInc}. Now by taking the square root of both sides and applying the identity $\sqrt{x+y} \le \sqrt{x} + \sqrt{y}$ yields
	\begin{align*}
	\Norm{I(t,x) - I(r,z)}_p &\le 2\sqrt{2C(\rho, T)p} \Big[ c(\sigma) + L(\sigma) \exp\left(aT \right)N_{a,p}(u) \Big]\left(|t-r|^{q} + |x-z|^{\rho}\right)^{1/2}. 
	\end{align*}
...
\textcolor{blue}{The holder continuity in the next proposition needs to be checked}
	...
	\begin{proposition}
	\label{P:Holder}
Suppose that $u_0$ and $v_0$ are as on Proposition \ref{P:NormInc} above. Then $u$ has a version, still denoted by $u$, that is $\eta_1$-H\"older continuous in time and $\eta_2$-H\"older continuous in space with $0<\eta_1 < q/2$ and $0<\eta_2 < \rho/2$ where $0< \rho < (\Theta -1) \wedge 2$ and
		\begin{enumerate}
		\item $0< q \le \theta$ under Case \ref{P:YIncCase1} of Proposition \ref{P:YInc},
		\item $0 < q < \theta$ under Case \ref{P:YIncCase2} of Proposition \ref{P:YInc}.
	\end{enumerate}
Moreover, if we consider
	\[
		a > \left( 8 L^2(\sigma)K^2\right)^{1/\theta} \quad \text{and} \quad 	p \in \left[ 2, \frac{a^{\theta}}{4L^2(\sigma)K^2} \right],
	\]
where $K$ is the constant from Proposition \ref{P:N(u)Bdd}, then we have that 
	\[
		\mathbb{E}\left[ \sup_{t\in[0,T],\; x\in[-R,R]} |u(t,x)|^p \right] \le 2^{p-1} + C(p, \rho, T,R) \left[ \mathcal{M}_1^p + \mathcal{M}_2^p e^{apT} \left(2 +  \frac{c(\sigma)}{L(\sigma)}\right)^p\right].
	\]
	\end{proposition}
...
Proposition \ref{P:NormInc} implies that 
		\begin{equation}
			\Norm{u(t,x) - u(r,z)}_p \le C(p,\rho, T)\left[ \mathcal{M}_1 + \mathcal{M}_2e^{aT}N_{a,p}(u) \right]\left(|t-r|^{q} + |x-z|^{\rho}\right)^{1/2}.
		\end{equation}
By Kolmogorov's continuity theorem, there exists a version of $u$, which we denote by $u$, that is $\eta_1$-H\"older continuous in time and $\eta_2$-H\"older continuous in space with with $0<\eta_1 < q/2$ and $0<\eta_2 < \rho/2$.
...
Now by taking the expectation of both sides and applying Proposition \ref{P:NormInc} gives us 
	\begin{align*}
			\mathbb{E}\left[ \sup_{t\in[0,T],\; x\in[-R,R]} |u(t,x)|^p \right] &\le  2^{p-1} + 2^{p-1}C(p, \rho, T)^p\left((2T)^{q/2} + (2R)^{\rho /2}\right)^p\left[ \mathcal{M}_1 + \mathcal{M}_2 e^{aT} N_{a,p}(u)\right]^p \\
			&= 2^{p-1} + C(p, \rho, T,R) \left[ \mathcal{M}_1^p + \mathcal{M}_2^p e^{apT} N_{a,p}(u)^p\right].
	\end{align*}
Now if we consider
	\[
		a > \left( 8 L^2(\sigma)K^2\right)^{1/\theta} \quad \text{and} \quad 	p \in \left[ 2, \frac{a^{\theta}}{4L^2(\sigma)K^2} \right],
...
We will now apply Proposition \ref{P:Holder} to find an upper bound on the $p$-norm of $u_N$. For this, consider 
	\[
		a > \left( 8 L^2(\sigma_N) K^2 \right)^{1/\theta} \quad \text{and} \quad p \in \left[ 2, \frac{a^{ \theta}}{4L^2(\sigma_N)K^2} \right].
	\] 
Then Proposition \ref{P:Holder} implies that 
	\begin{equation}
	\label{E:u_NMomBdd}
		\mathbb{E}\left(\sup_{t\in[0,T],\; x\in[-R,R]} |u_N(t,x)|^p\right) \le 2^{p-1} + C(p, \rho, T,R)\left[\mathcal{M}_1^p + \mathcal{M}_2^p(N) \exp(apT)\left(2+ \frac{\sigma_1}{L(\sigma_N)}\right)\right]
	\end{equation}
...
An application of \eqref{E:u_NMomBdd} gives 
	\begin{align*}
		N^{-p}\mathbb{E}\left( \sup_{t\in[0,T],\; x\in[-R,R]} |u_N(t,x)|^p \right) &\le N^{-p}\left(  2^{p-1} + C(p, \rho, T,R) \left[ \mathcal{M}_1^p + \mathcal{M}_2(N)^p \exp\{apT\} \left(2 +  \frac{\sigma_1}{L(\sigma_N)}\right)^p\right]\right) \\
		&= \frac{2^{p-1} + C(p, \rho, T,R)\mathcal{M}_1^p }{N^p} + \frac{C(p,T,R) \mathcal{M}_2(N)^p \exp\{apT\} \left(2 +  \frac{\sigma_1}{L(\sigma_N)}\right)^p }{ N^p }.
	\end{align*}
...
which is equivalent to
	\begin{equation}
		...
		\theta(\lambda) > 0 \quad \text{and} \quad \lambda < 2\alpha,
	\end{equation}
...
We follow the proof from \cite[Proposition 2.5]{FT2014}. For notation, let $g(t) = K(1+t^2)$ and define 
	\[
		(\mathcal{A}\psi)(t) := K\int_0^t (t-s)^{k-1} \psi(s) \ud s
	\]
for any locally integrable function $\psi$. Then \eqref{E:fRenew} can be written as 
	\[
		f(t) \le g(t) + \mathcal{A}\psi(t).
	\]
...
It is stated in the proof of \cite[Proposition 2.5]{FT2014} that 
	\[
		(\mathcal{A}^n f)(t) = \frac{\left(K\Gamma(\theta)\right)^2}{\Gamma(n\theta)} \int_0^t (t-s)^{n\theta -1} f(s) \ud s.
	\]
Using this, we can calculate $(\mathcal{A}^i g)(t)$ as follows:
	\begin{align*}
		(\mathcal{A}^i g)(t) &=  \frac{\left(K\Gamma(\theta)\right)^i}{\Gamma(i\theta)} \int_0^t (t-s)^{i\theta -1} g(s) \ud s \\
		&= K\frac{\left(K\Gamma(\theta)\right)^i}{\Gamma(i\theta)} \int_0^t (t-s)^{i\theta -1} (1+s^2)  \ud s \\
		&= K\frac{\left(K\Gamma(\theta)\right)^i}{\Gamma(i\theta)} \cdot \frac{t^{\theta i}(1+\theta i + t)}{\theta i (1 + \theta i)}.
	\end{align*}
In addition, it is stated in the proof of \cite[Proposition 2.5]{FT2014} that $(\mathcal{A}^n f)(t) \to 0$ as $n \to \infty$. Thus, we can say 
	\begin{align*}
		f(t) &\le \sum_{i=0}^\infty K\frac{\left(K\Gamma(\theta)\right)^i}{\Gamma(i\theta)} \cdot \frac{t^{\theta i}(1+\theta i + t)}{\theta i (1 + \theta i)} \\
		&= \sum_{i=0}^\infty K\frac{\left(Kt^{\theta }\Gamma(\theta)\right)^i}{\Gamma(i\theta)} \cdot \frac{1+\theta i + t}{\theta i (1 + \theta i)} .
	\end{align*}
In the proof of \cite[Proposition 2.5]{FT2014}, it is stated that 
	\[
		\sum_{i=0}^\infty \frac{\left(Kt^{\theta }\Gamma(\theta)\right)^i}{\Gamma(i\theta)} = E_{\theta}\left( K\Gamma(\theta) t^\theta \right),
	\]
where $E_\theta$ is the one-parameter Euler-Mittag function. The $\frac{1+\theta i + t}{\theta i (1 + \theta i)}$ term now makes this untrue. 
	\end{proof}
...
\subsection{Section 4.2 Millet and Sanz-Sol\'e \cite{SoleMillet2021}}
		...
		Let $u_0$ and $v_0$ be Borel functions satisfying $\Norm{u_0}_\infty + \Norm{v_0}_\infty < \infty$. Suppose that $d < 2 \alpha$ and for this proof consider $\theta = \theta(\lambda)$ which is defined in \eqref{D:theta}. Then there exists a universal constant $K:=K(\alpha, \beta, \gamma,d, \lambda ) >0$ such that for any 
		\[
		a^\theta > \left( 8 L^2(\sigma)K^2\right).
		\]
		and 
		\[
		p \in \left[ 2, \frac{a^{\theta}}{4L^2(\sigma)K^2} \right].
...
and $C$ is as in Lemma \ref{L:intYfYbdd}. In return, this gives us that 
	\begin{align*}
		...
		&= 8p K_1 c(\sigma)^2\frac{ t ^ \theta}{\theta} + 8p K_1 N_{a,p}(u)^2 L(\sigma)^2 \frac{\exp(2at) \left[ \Gamma(\theta) - \Gamma(\theta, 2at)\right]}{(2a)^{\theta} }  \\
...
We start with bounding $\sup_{t \ge 0} \exp(-2at)\triangle_1(t)$. Recall the identities in \eqref{E:SUPidentities}. With this we see that 
	\[
		\sup_{t \ge 0} \exp(-2at) t^{\theta} = (\theta)^{\theta} (2ea)^{-\theta}.
	\]
Hence, 
	\begin{align*}
		\sup_{t \ge 0} \exp(-2at)\triangle_1(t) & \le \frac{8p K_1 c(\sigma)^2}{\theta} \left(\frac{\theta}{2ea}\right)^{\theta}.
	\end{align*}
Next we bound $\sup_{t \ge 0} \exp(-2at)\triangle_2(t)$. Note that for any $t\ge 0$,
	\begin{align*}
		\exp(-2at)\triangle_2(t) &= 8p K_1 N_{a,p}(u)^2 L(\sigma)^2 \frac{\left[ \Gamma(\theta) - \Gamma(\theta, 2at) \right]}{(2a)^\theta} \\
		& \le  8p K_1 N_{a,p}(u)^2 L(\sigma)^2 \frac{\Gamma(\theta)}{(2a)^{\theta}}.
	\end{align*}
Putting this all together gives us that 
	\begin{align*}
		\mathcal{N}_{a,p}(I)^2 &\le \frac{8p K_1 c(\sigma)^2}{\theta} \left(\frac{\theta}{2ea}\right)^{\theta}  + \frac{8p K_1 \Gamma(\theta) L(\sigma)^2}{ (2a)^{\theta}}N_{a,p}(u)^2,
	\end{align*}
and thus,
	\begin{equation*}
		\mathcal{N}_{a,p}(I)\le	2c(\sigma)\sqrt{\frac{2p K_1}{\theta}} \left(\frac{\theta}{2ea}\right)^{\theta/2}  + 2L(\sigma)\frac{\sqrt{2p K_1 \Gamma(\theta) }}{ (2a)^{\theta/2}}N_{a,p}(u).
	\end{equation*}
Similar to the proof of Proposition \ref{P:N(u)Bdd} above, we define 
	\[
		K = K(\alpha,\beta,\gamma,d, \lambda):= \max \left\{2\sqrt{\frac{2 K_1}{\theta}} \left(\frac{\theta}{2e}\right)^{\theta/2},   2\frac{\sqrt{2 K_1 \Gamma(\theta) }}{ (2)^{\theta/2}} \right\}.
	\] 
We now see that 
	\begin{equation}
		\mathcal{N}_{a,p}(I)\le K c(\sigma)\frac{\sqrt{p}}{ a^{\theta /2}} + K L(\sigma) \frac{\sqrt{p}}{ a^{\theta/2}} \mathcal{N}_{a,p}(u)
	\end{equation}
This and the bound for $\mathcal{N}_{a,p}(J)$ found above implies that 
	\begin{align}
		...
		(ea)^{-1}\Norm{v_0}_{\infty} + \Norm{u_0}_{\infty} & \beta \in (1,2)
		\end{cases} \\
		& \qquad + K c(\sigma)\frac{\sqrt{p}}{ a^{\theta /2}} + K L(\sigma) \frac{\sqrt{p}}{ a^{\theta/2}} \mathcal{N}_{a,p}(u) \label{E:N(u)bddColored}.
	\end{align}
Now consider 
	\[
		a^\theta > 8K^2L(\sigma)^2.
	\]
This implies that the interval 
	\[
		\left[2, \frac{a^\theta}{4K^2L(\sigma)^2}  \right]
	\]
is nonempty. Hence, for any $p$ in this interval we have that 
	\[
			\frac{KL(\sigma)\sqrt{p}}{a^{\theta/2}} \le \frac{1}{2}.
	\]
