...
\begin{question}[Norm increments in time and space.]
	...
	\[
		\int_{\R}|G(s,\bar{x}-y) - G(t,x-y)| \ud y \le C(T)(|s-t|+|\bar{x}-x|) \quad \text{for } t \in [0,T] \text{ and } x \in \R.
	\]
	To simplify this at first we can look at both 
	\begin{align}
		\label{E:HGspace} & \int_{\R}|G(t,\bar{x}-y) - G(t,x-y)| \ud y \\
	...
	\]
	Starting with \eqref{E:HGspace}, we first assume that $x < \bar{x}$ and use the Mean Value Theorem to get 
	\begin{align*}
		\int_{\R}|G(t,\bar{x}-y) - G(t,x-y)| \ud y & \le  |\bar{x} - x| 	\int_{\R}\left|\frac{\partial G}{\partial x}(t,\xi(x,\bar{x}) -y)\right|\ud y 
	\end{align*}
	where $\xi(x, \bar{x}) \in (x,\bar{x})$ is a constant. Using Mathematica,
	\begin{verbatim}
	...
	and hence 
	\begin{align*}
		\int_{\R}|G(t,\bar{x}-y) - G(t,x-y)| \ud y & \le \sqrt{\frac{2}{\pi}} \frac{1}{\sqrt{t}}|\bar{x} - x|.
	\end{align*}
	...
	Now we can see that 
	\begin{align*}
	\int_{\R}|G(s,\bar{x}-y) - G(t,x-y)| \ud y &\le  \int_{\R}|G(t,\bar{x}-y) - G(t,x-y)| \ud y +  \int_{\R}|G(s,\bar{x}-y) - G(t,\bar{x}-y)| \ud y  \\
	& \le \sqrt{\frac{2}{\pi}} \frac{1}{\sqrt{t}}|\bar{x} - x| + \sqrt{\frac{2}{e \pi}} \frac{1}{\eta(t,s)}  |s - t|.
	\end{align*}
