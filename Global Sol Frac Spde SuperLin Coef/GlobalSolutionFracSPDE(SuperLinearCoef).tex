...
\begin{proposition}
\label{P:NormInc}
	Suppose that $u_0=1$ and that $v_0 =0$ so that $u(t,x) = 1 + I(t,x)$. Let $a>0$, $\rho = 2(\beta + \gamma) -1 - \beta/\alpha$ and suppose that $0< \theta < (\Theta -1) \wedge 2$. Then for any $x,z \in \R$ and for any $T>0$ with $t\in [0,T]$,
	\begin{equation}
	\label{E:NormInc_N(u)}
