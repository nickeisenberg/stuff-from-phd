...
where
	...
	\begin{lemma}[Theorem 4.1 and Lemma 5.5 \cite{CHN19}]
	\label{L:1and2normYZ}
...
In the proof of the next proposition, we will use the following two facts:
	...
	\end{equation}
	\begin{proposition}[Proposition 3.2 \cite{SoleMillet2021}] 
...
where the last line is true if Dalang's condition \cite{CHN19} holds. If we now apply \eqref{E:SUPidentities} and also recall that $\sqrt{a+b} \le \sqrt{a} + \sqrt{b}$, then we see that 
	...
	\\&\quad\quad\quad\quad+ L(\sigma)N_{a,p}(u) \sqrt{8pC_{\#} (2a)^{1+(1/\alpha - 2)\beta - 2\gamma}\Gamma\left[ (2-1/\alpha)\beta-1+2\gamma \right]}.
	\end{align*}
If we now let 
	\begin{align*}
...
then,
	...
	\end{align}
Now by combining \eqref{E:Jbdd} and \eqref{E:Jbdd}, we see that 
	\begin{align}
