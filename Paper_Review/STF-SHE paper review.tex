...
\begin{enumerate}
	\item 
Replace the third sentence, "From now on, we will state... " with the following sentence: "Since $\underline{\mathcal{L}}(p,x)/p"$ is decreasing in $p$, then it suffices to prove the first inequality in (23). 

	\item 
In the third paragraph it says that "for any $t > \max\{t_0,T_0\}$...". What is this $T_0$ referring to? 
\end{enumerate}

\textbf{Corollary 1:}
\begin{enumerate}
	\item 
Unsure how taking the log of (25) gives us (24).

	\item 
In the second part of this proof it talks about a $T_0$ defined in Theorem 1 but there is no $T_0$ in Theorem 1. Maybe you mean theorem 2. 
\end{enumerate}
...
\textbf{Lemma 6:}
	\begin{enumerate}
		\item
I believe that the $\gamma$ in $(k_1\Gamma(\gamma))^{-1}$ needs to be $\gamma_2$. In other words, it should say $(k_1\Gamma(\gamma_2))^{-1}$.
\end{enumerate}

\textbf{Lemma 8:}
\begin{enumerate}
	\item 
How is the first inequality shown in this Lemma? I have proved this lemma differently.
	\begin{align*}
	\int_{\R} 1_{\left\{ |x-y|^{\alpha+1} \le t^{\beta(1+1/\alpha)} \right\} } (y) \rho(\ud y) &= \rho\left( B(x,t^{\beta/\alpha}) \right)
	\\& \le C t^{\beta\overline{\delta}/\alpha}
	\\& \le C \max\left\{ t^{\beta/\alpha}, t^{\beta\overline{\delta}/\alpha} \right\}.
	\end{align*}

	\item
In the first equality in the second set of centered equations, I would let the reader know that you are using the Layer Cake representation, which is theorem 1.13 \cite{LiebLoss01}. 
\end{enumerate}

\textbf{Lemma 9:}
\begin{enumerate}
	\item I think we $\beta \le \min\{1, \alpha/(2-\bar{\delta})\}$. This will ensure that $(\beta/\alpha) -1 <0$ which is needed in the proof of Theorem 1 part 1. 
\end{enumerate}
