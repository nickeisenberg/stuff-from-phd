...
It is conjectured that we may be able to apply the following results to more general noises with arbitrary $\gamma$ and $\mu$ such that 
		\begin{enumerate}
...
The issue with appling the results in this paper to the more general noises listed in \ref{spectralconjecture} is proving $\rho$, defined below is finite. We mention that an example of a noise that satisfies \ref{spectralconjecture} but differs from the Riesz kernal is the noise with $\gamma(x) = \prod_{i=1}^d|x_i|^{-\alpha_i}$ for some $\alpha_i \in (0,1)$. In this example, $\mu(\ud \xi) = \prod_{i=1}^d (C_{2,\alpha_i} |\xi_i|^{-(1-\alpha_i)}\ud \xi_i )$ and $ K(x) = \prod_{i=1}^d (\beta_{\alpha_1,1} |x_i|^{-(1+\alpha_i)/2} ) $. This function gamma satisfies the conditions in \ref{spectralconjecture} with $\alpha = \sum_{i=1}^d \alpha_i$. If we consider the parameterization $\alpha_i = 2-2H_i$ with $H_i \in (1/2,1)$, then the corresponding noise is the fractional Brownian sheet with indicies $H_1,\cdots,H_n$. The heat equation with this noise is studied in Hu \cite{HuFracBrown}
