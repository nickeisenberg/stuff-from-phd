...
\end{align*}
with the convention that $I^0_t = \text{Id}$ (the identity operator). The fundamental solutions to
\eqref{E:SPDE} consist of a triplet:
\begin{align*}
...
much more technically complicated than the Green's function for either the heat or wave equation.
The actual meanings of these are not to important in this setting so for that reason we direct the
reader to \cite{CHN19} for the definitions. Since we are interested in the constant one initial
condition (and 0 initial velocity when $b>1$), by Theorem 4.1 \cite{CHN19} the corresponding
solution to the homogeneous equation (i.e. the solution when there is no driving source) is equal to
the constant one. Hence, by setting $G(t,x) = Y_{a,b,r,\nu,d}(t,x)$, \eqref{E:SPDE} can be written
as the following Skorohod integral equation:
...
where $\alpha_i \in (0, d_i)$ and we define $\alpha = \sum_{i=1}^k \alpha_i$. This correlation function corresponds with the spectral density given by $\mu (\ud \xi) = \varphi(\xi) \ud \xi$ with
	...
	\end{equation}
The Constant $C$ can be calculated through the relation $\mathcal{F}\gamma = \varphi$. An important fact stated in \cite{BCC21} is that there exists a nonnegative function $K$ on $\R^d$ such that $\gamma = K*K$ where '$*$' denotes the spatial convolution.
...
Next, we need to make some assumption on the parameters $a,b,r$ of \eqref{E:SPDE} as well as the dimension $d$. Under theses assumptions, the Greens function $G$ will be nonnegative and smooth which was shown in Theorem 4.6 \cite{CHN19} and equation (4.5) \cite{CHN19} respectively.
...
We now introduce the operator
	...
	\end{align*}
and define $\mathcal{M}_a := \mathcal{M}_a(|\cdot|^{-\alpha},1)$. See Subsection \ref{largedevresults} of this paper for more details. Our goal is to develop exact moment asymptotics to the solution $u(t,x)$. We follow the techniques laid out in \cite{BCC21} where in that paper they focus exclusively on the stochastic wave equation. This paper acts as a generalization of that paper to the fraction stochastic equation. Here is the main result of this paper.

	\begin{theorem}\label{mainresult}
Under Assumption \ref{paramassumptions}, equation \eqref{E:SPDE} driven by the noise with spectral density given in equation \ref{specfunction} has a unique solution $u(t,x)$ in $L^p(\Omega)$ for any $p \ge 2$, $t>0$ and $x \in \R^d$ if
		\begin{equation}
...
\begin{thebibliography}{999}
	...
	\newblock {\em  Ann. Inst. Henri Poincaré Probab. Stat. }, to appear.
	% }}}
	\bibitem{BS17} % {{{ 2.
	...
	\newblock {\em  Ann. Inst. Henri Poincaré Probab. Stat. } \textbf{45}, 626--666.
	% }}}
	\bibitem{CHHH17}% {{{ 4.
	...
	\newblock {\em  Ann. Inst. Henri Poincaré Probab. Stat.} \textbf{55}, 941--976.
	% }}}
