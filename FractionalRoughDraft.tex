...
and in addition, X. Chen \cite{chenlarge} also shows the equality 
 ...
 where
	\begin{align}
		\Lambda_{\alpha} &= \sup_{g \in \mathcal{F}_a} \left\{ \left( \int_{\R^{2d}} \frac{g^2(x_1)g^2(x_2)}{|x_1+x_2|^a} \ud \vec{x} \right)^{1/2} - \mathcal{E}_a(g,g) \right\}
		\\&= \sup_{g \in \mathcal{F}_a} \left\{ \langle g^2 * g^2, |\cdot|^{-a} \rangle_{L^2(\R^d)} - \mathcal{E}_a(g,g) \right\} \nonumber.
	\end{align}
...
\section{Upper Bound.} In the section we use the result obtained in Theorem \ref{p=2} and the hypercontractivity property given by Theorem \textcolor{red}{appendix theorem} to prove the following theorem.
