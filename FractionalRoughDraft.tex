...
\begin{proof}[Proof of Theorem \ref{T:solvable}]
	...
	\end{equation}
	one may check the proof of Lemma 3.4 of \cite{BCC21} for more details.
	By the scaling property for $\mathcal{F}\widetilde{f}_n$:
	\begin{equation}
		\label{E:Ffnscale}
	...
	{\bigskip\bf\noindent Step 4.~}
	% First recall \eqref{E:Ffnscale}, which says \begin{align*}
	% \mathcal{F}\widetilde{f_n}(\cdot,0,ct)(\xi_1,..., \xi_n) =
	% c^{n(b+r)}\mathcal{F}\widetilde{f_n}(\cdot,0,t)(c^{b/a}\xi_1,...,c^{b/a}\xi_n).  \end{align*} In
	% order to prove the theorem we need to show that $\sum_{n \ge 0} \theta^n n! \Norm{
	% \widetilde{f_n}(\cdot,0,t) }_{ \mathcal{H}^{\otimes n} }^2 < \infty$.
	...
	\end{align*}
	where the constant $C$ depends only on the value of $2(b+r)-b\alpha/a$.  Therefore,
	\begin{align*}
		\sum_{n \ge 0} \theta^n n! \Norm{ \widetilde{f_n}(\cdot,0,t) }_{ \mathcal{H}^{\otimes n} }^2
		\le  C \sum_{n \ge 0} \theta^n   \dfrac{ \left(2^{ \left[ 2(b+r)- \frac{b \alpha}{a} \right]}C_\mu(\nu, a) \right)^n }{ (n!)^{ 2(b+r) - 1 - b\alpha/a }},
	\end{align*}
