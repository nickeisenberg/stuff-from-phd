...
\begin{proof}
  ...
  \end{align*}
  where $\widetilde{\rho}(\cdot)$ is defined as,
  ...
  % where the supremum is taken over all nonnegative and nonnegative definite functions $h$ such that.
  ...
  \noindent (5) $\widetilde{\rho}(g)$ is well defined (i.e., finite) because for any $h\in
  ...
  \begin{gather} \label{E:doubleSup}
    \mathop{\sup_{f \in \mathcal{H}_+}}_{\Norm{f}_{\mathcal{H}}=1} \widetilde{\rho}
    \left(\mathcal{F}f\right) = \sup_{\Norm{h}_{L^2(\R^d)}=1} \mathop{\sup_{f \in
    \mathcal{H}_+}}_{\Norm{f}_{\mathcal{H}}=1} I(f,h), \qquad \text{with} \\ \notag
    I(f,h) := \int_{\R^d}\mathcal{F} f(\xi) \left[ \int_{\R^d} \frac{h(\xi +
    \eta)h(\eta)}{\sqrt{1+\frac{\nu}{2}|\xi+\eta|^a}\sqrt{1+\frac{\nu}{2}|\eta|^a}}\ud\eta \right]
    \mu(\ud \xi),
  \end{gather}
  where one can switch the order of the two suprema in \eqref{E:doubleSup} whenever necessary. Let
  $h$ be an arbitrary (real-valued) function in $L^2(\R^d)$. It is clear that the function
  $h(\cdot)/\sqrt{1+\frac{\nu}{2}|\cdot|^a}$ belongs to $L^2(\R^d)$ so that $g_h(x) := (2\pi)^{d/2}
  \mathcal{F}^{-1}\left( \frac{h(\cdot)}{\sqrt{1+\frac{\nu}{2}|\cdot|^a}} \right)(x)$ is well
  defined complex-valued function. Notice that
  \begin{align*}
	\int_{\R^d} \frac{h(\xi + \eta)h(\eta)}{\sqrt{1+\frac{\nu}{2}|\xi+\eta|^a}\sqrt{1+\frac{\nu}{2}|\eta|^a}}\ud\eta
  ...
  \begin{align*} 
    I(f,h) = \int_{\R^d}\mathcal{F}f(\xi)\mathcal{F}\left[|g_h|^2\right](\xi) \mu(\ud \xi).
  \end{align*}

  Let us take the supremum of $h$ first in \eqref{E:doubleSup} with $f\in \mathcal{H}_+$ fixed.
