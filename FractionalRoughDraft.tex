...
dimension $d$. Under theses assumptions, the Greens function $G$ will be nonnegative and smooth
which was shown in Theorem 4.6 \cite{CHN19} and equation (4.5) \cite{CHN19} respectively.
...
\begin{align*}
	...
	\rangle^{1/2} - \frac{\theta}{2}\mathcal{E}_a(g,g) \right\}.
\end{align*}
Set $\mathcal{M}_a := \mathcal{M}_a(|\cdot|^{-\alpha},1)$. See Subsection \ref{largedevresults} of
this paper for more details. Our goal is to develop exact moment asymptotics to the solution
$u(t,x)$. We follow the techniques laid out in \cite{BCC21} where the stochastic wave equation,
namely, $a=b=\nu=2$ and $r=0$, has been exclusively studied. This paper acts as a generalization of
that paper to the fraction stochastic equation. Here is the main result of this paper.
...
\bigskip In the following, we first give some preliminaries in Section \ref{S:Pre} including the
Malliavin calculus, the notation of the solution, and some variational constants.  The existence and
uniqueness of a $L^p(\Omega)$ solution will be proven in Section \ref{solvability}. The upper bound
and lower bound for \eqref{E:momentasym} will be proven in Sections \ref{upperbound} and
\ref{lowerbound}, respectively.
...
To briefly summarize \cite{Minicourse09}, we start with a nonnegative and nonnegative definite tempered measure $\Gamma$ with density $\gamma$ in the sense that $\Gamma(\ud x) = \gamma(x) \ud x$, and
	\begin{align*}
...
where $\tilde{\varphi(x)} = \varphi(-x)$. In addition we require the existance of an $r>0$ such that
	...
	\end{align*}
According to the Bochner-Schwarz theorem, there exists a nonnegative measure $\mu$, often referred to as the spectral measure on $\R^d$ whose Fourier transform is $\Gamma$ in the sense that for any $\varphi \in \mathscr(\R^d)$
	\begin{align*}
...
Recall the notation above that $G(t,x) = Y_{a,b,r,d}(t,x)$. The following result shows an important scaling property that both $\mathcal{F}G(t,\cdot)(\xi)$ and $\Norm{\tilde{f_n}(\cdot,t,x)}_{\mathcal{H}^{\otimes n}}$ have.
	...
	\end{theorem}
\noindent\textbf{Proof:}
The Fourier transform of $G$ is given in \cite{CHN19}. With this we can directly calculate
	\begin{align*}
...
The following result gives the existence and uniqueness of the solution. Its proof follows by a standard procedure.
	\begin{theorem}
		Fix any $T \in (0,\infty]$. Suppose that $f_n(\cdot,x,t) \in \mathcal{H}^{\otimes n}$ for any $t \in (0,T)$, $x \in R^d$ and $n \ge 1$. Then equation \eqref{E:SPDE} has a unique $L^2(\Omega)$ solution on $(0,T) \times \R^d$ if and only if the series \ref{solwexp} converges in $L^2(\Omega)$ for any $(t,x) \in (0,T) \times \R^d$, which is equivalent to the convergence of the series \ref{2mom}. In this case, the solution is given by \ref{solwexp} with the second moment given by
...
First we define
	...
	\end{align*}
Recall that $\mu(\ud \xi) = C_{d,\alpha}|\xi|^{-(d-\alpha)} \ud \xi$ which corresponds to the Riesz kernel $\gamma(x) = |x|^{-\alpha}$ for some $\alpha \in (0,d)$. In X. Chen \cite{BCR09}, the case of $a \in (0,2]$ and $\nu = 2$ is discussed and there it is proven that
	...
	\end{equation}
and in addition, X. Chen \cite{BCR09} also shows the equality
...
and now we can directly calculate
	...
	\end{align*}
and because of both Theorem 1.5 \cite{BCR09} and \ref{rhorelation} above,
	\begin{align*}
		\lim_{n \to \infty} \frac{1}{n} \log \left[ \frac{1}{(n!)^2} \int_{(\R^d)^n} \left( \sum_{\sigma \in \Sigma_n} \prod_{k=1}^n \frac{1}{1+ \frac{\nu}{2}|\sum_{j=k}^n \xi_{\sigma(j)}|^a}\right)^2 \mu (\ud \vec{\xi})\right]  = \log\left[ \left( \frac{\nu}{2} \right)^{-\alpha/a} \right] + \log[\rho_{2,a}] = \log[\rho]
	\end{align*}
$\hfill \qed$

Next we show that $\rho$ is finite. We do this by showing $\rho_{2,a}$ is finite and then noting that this implies that $\rho$ is finite by \ref{rhovariationaltheorem}. We stated in the introduction that Chen, X. proved in \cite{BCR09} that $\rho$ is finite for the case of the Riesz kernal. We mimic his proof but were he uses the Hardy-Littlewood-Sobelov theorem, we make use of the weak Young's inequality instead.
...
where $\alpha = \sum_{=1}^k \alpha_i$ with $k \in \{1, \cdots ,d\}$, $\alpha_i \in (0,d_i)$ and $\varphi(x) = \prod_{i =1}^k |x_{(i)}|^{-(d_i-\alpha_i)}$ where $x_{(i)} = (x_{i_1},\cdots , x_{i_{d_i}} )$ is the $i^{th}$ group of $x=(x_1, \cdots, x_d)$.
	\end{lemma}
\noindent \textbf{Proof:} By observing the proof of Lemma 1.6 \cite{BCR09}, we notice that we only need to prove equation (1.27) which says
...
\end{align*}
If we can do this then the result follows by copying the rest of Chen's proof. To prove \ref{fracrhofinite}, we use the weak version of Young's inequality. Recall that weak Young's inequality \cite{LiebLoss01} says
\begin{align*}
...
We finish this section by making a slight generalization of $\Lambda_{\alpha}$. For this, we define
	...
	\end{equation}
One notices that by making this generalization, we can apply Theorem \ref{mainresult} to the SPDE \eqref{E:SPDE} driven by an arbitrary Gaussian noise with correlation function $f$ as long as one can show that $\rho$ is finite. By slightly modifying the proof of Lemma 2.3 \cite{BCC21}, one can show that
	\begin{equation}
...
The second relation is proven the same was as in Lemma 3.6 \cite{BCC21}. Here is the proof.
	...
	\end{align}
where the last line follows by Lemma 4.1 of Balan and Song \cite{BS17} or by Lemma 3.1 in \cite{Chen19AIHP}.
...
\section{Upper Bound.}\label{upperbound} In this section we use the result obtained in Theorem \ref{p=2} and the hypercontractivity property given by Theorem B.1 in \cite{BCC21} to prove the following theorem.
...
\noindent \textbf{Proof:} We start by letting $\tau$ be an exponential random variable with mean $1$. With this,
	...
	\end{align*}
For an $f \in \mathcal{H}_+$ and $\Norm{f}_{\mathcal{H}}= 1$ we have that $\mathcal{F}f$ is nonnegative and nonnegative definite which further implies that $\mathcal{F}f$ is even. In addition, Lemma \ref{LWn} gives us that
	\begin{align*}
