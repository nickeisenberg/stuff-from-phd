...
We need to introduce the following variational constants (see Section \ref{SS:Variation} for more details):
\begin{align*}
	\mathcal{M}_a\left(f, \theta\right) := \sup_{g \in \mathcal{F}_a} \left\{ \left\langle g^2 * g^2, f \right\rangle^{1/2}_{L^2(\R^d)} - \frac{\theta}{2}\: \mathcal{E}_a(g,g) \right\} \quad \text{and} \quad
	\mathcal{M}_a(f)                              := \mathcal{M}_a\left(f,1\right).
\end{align*}
For $\gamma$ defined in \eqref{E:corfunction}, we use the convention that $\mathcal{M}_a:=
\mathcal{M}_a(\gamma,1)$. \bigskip
...
\begin{gather}
	\label{E:rho}
	\rho_{\nu,a}(\gamma) = \sup_{\Norm{f}_2 =1} \int_{\R^d} \left[ \int_{\R^d} \frac{f(x+y)f(y)}{\sqrt{1+\frac{\nu}{2}|x+y|^a } \sqrt{1+\frac{\nu}{2}|y|^a} } \ud y \right]^2 \mu(\ud x)
\end{gather}
and
\begin{gather}
	\label{E:Lambda}
...
\begin{gather}
	...
	\label{E:Fa}
\end{gather}
We use the convention that $\mathcal{M}_a(\gamma):=\mathcal{M}_a(\gamma,1)$.  By slightly modifying
the proof of \cite[Lemma 2.3]{BCC21}, one can show that
\begin{equation}
	\label{E:ScaleM}
...
case (see Remark \ref{R:NoiseEg}), Bass, Chen and Rosen \cite{BCR09} established that when $a \in
(0,2]$ and $\nu = 2$,
\begin{equation}
	\label{E:oldlim}
...
\begin{tabular}{|c|c|c|c|c|c|c|c|}
	...
	Current paper                 & $a$                          & $\nu$                      & $\alpha$ & $\gamma$             & $\varphi$            & $2$ & $\mathcal{M}_a\left(|\cdot|^{-\alpha},2\right)$ \\ \hline
\end{tabular}
	\end{tabular}
\end{table}
	...
	\begin{align*}
		% \label{E:(7.1)}
		\InPrd{g^2*g^2,\gamma}_{L^2(\R^d)}	\le C \Norm{g^2}_{L^{2d/(2d-\alpha)}(\R^d)}^2,
	\end{align*}
	which gives (7.1) ({\it ibid.}) in our setting. The rest part of the proof of Lemma 7.1 ({\it
	ibid.}) remains unchanged.

	It remains to update the proof of Theorem 1.5 on Section 7 ({\it ibid.}). For this, one needs only
	to update the three appearances of $1/(|\cdot|^\sigma)$ in both (7.22) and (7.23) ({\it ibid.}) to
...
In order to establish this theorem, by checking the corresponding proof in Bass {\it et al}
\cite{BCR09}, one needs to prove the corresponding Lemma 1.6 on p. 629 ({\it ibid.}) for more
general noise. Indeed, the proof of Lemma 1.6 ({\it ibid.}) relies on inequality (1.27) on p.  630
({\it ibid.}), which was a consequence of Sobolev's inequality. For the more general noises studied
in this paper, we can no longer apply this inequality. Instead, we prove the following lemma using
the weak Young's inequality as a generalization of Lemma 1.6 ({\it ibid.}).

\noindent \textcolor{red}{Remarks about the Riesz kernel and fractional kernel.}

In order to establish the above theorem, we note that it suffices to show that \eqref{E:oldlim} and \eqref{E:oldrhovarrep} remain true for the noise given in Assumption \ref{A:Noise}. This is because Lemma \ref{L:rhovar} remains the same line for line when the Riesz kernel is replaced by \eqref{E:corfunction}. However, due to the generality of the proofs of the corresponding results from Bass {\it et al} \cite{BCR09}, almost nothing needs to change in order to apply the their proofs to the correlation function \eqref{E:corfunction}.

To verify \eqref{E:oldlim}, one needs to verify that Theorem 1.5 \cite{BCR09} holds when considering the more general correlation function \eqref{E:corfunction}. However, the only key change is to the proof of Lemma 7.1 ({\it ibid.}). Its easy to see that equation (7.1) ({\it ibid.}) is precisely \eqref{E:fracrhofinite} below with $F=G$ and with $\alpha$ from \eqref{E:corfunction} replaced with $d-\alpha$. The rest follows line for line with the only changes being \eqref{E:corfunction} replaces the Riesz kernal and the spectral density given in \eqref{E:specfunction} replaces $\varphi_{d-\sigma}$. (Note that $\sigma$ in Bass {\it et al} \cite{BCR09} is $\alpha$ in this paper.)

To verify \eqref{E:oldrhovarrep}, one needs to verify that both Lemma 2.2 and Theorem 2.3 \cite{BCR09} remain true when replacing the Riesz kernal with the correlation function \eqref{E:corfunction}. This lemma and theorem is proven in sections 3, 4 and 5 ({\it ibid.}) and the proof is quite lengthy, however one may keep the same proof line for line and just replace the Riesz kernal with \eqref{E:corfunction} and $\varphi_{d-\sigma}$ with the spectral density given in \eqref{E:specfunction}. This is because the only key features that are needed are that
	\begin{enumerate}
		\item $\mathcal{F}\varphi = \gamma$
		\item There exists function $K$ such that $\varphi = K*K$
	\end{enumerate}
where $\varphi$, $\gamma$ and $K$ are from Remark \ref{R:Noise}.

We now show $\rho_{\nu,a}(\gamma) < \infty$. In order to establish this result, by checking the corresponding proof in Bass {\it et al} \cite{BCR09}, one needs to prove the corresponding Lemma 1.6 on p. 629 ({\it ibid.}) where we replace $\varphi_{d-\sigma}$ with $\varphi$ given in equation \eqref{E:specfunction}. Indeed, the proof of Lemma 1.6 ({\it ibid.}) relies on inequality (1.27) on p.  630 ({\it ibid.}), which was a consequence of Sobolev's inequality. For the more general noises studied in this paper, we can no longer apply this inequality. Instead, we prove the following lemma using the weak Young's inequality as a generalization of Lemma 1.6 ({\it ibid.}).
...
\end{lemma}
Note that we only need the case $p=2$. Nevertheless, in this lemma we prove a general case for all
$p\ge 2$.
\begin{proof}
	...
	Recall that according to Assumption \ref{A:Noise}, the $d$ coordinates have been partitioned into
	$k$ groups.  Define $A_R := A_1 \times \cdots \times A_k$ where $A_i = B_{R,d_i}(0)$ is the ball
	in $\R^{d_i}$ centered at the origin with radius $R$.  With this we have that
	\begin{equation}
		\label{E:int}
	...
	$\left|\mathbb{S}^{d_i-1}\right| = (2\pi^{d_i / 2})/(\Gamma(d_i / 2))$ is the surface area of the
