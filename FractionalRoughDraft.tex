...
\begin{lemma}
	\label{L:Ster+LegApp} For any $n \in \mathbb{N}$ and $k>0$, the following asymptotic holds as $n
	\to \infty$:
		\[
...
\end{lemma}
\begin{proof}
	We use the notation '$\sim$' to mean asymptotically equivalent. For example, $a(x) \sim b(x) \;
	\iff \; a(x)/b(x) \to 1$ as $x \to \infty$. The proof relies on two well known properties of the
	Gamma function:
		\begin{enumerate}
			\item Sterling's Formula: $\Gamma(x+1) \sim \sqrt{2 \pi x} (x/e)^x$ as $x \to \infty$,
			\item Legrende's duplication formula: $\Gamma(z)\Gamma(z + 1/2) = 2^{1-2z}\sqrt{\pi} \;
				\Gamma(2z)$ for $z \in \mathbb{C}$.
		\end{enumerate}
