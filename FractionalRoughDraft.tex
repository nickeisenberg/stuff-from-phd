...
\author{Le Chen\footnote{Emails: \url{lzc0090@auburn.edu};~\url{chenle02@gmail.com}.}~
	and
Nick Eisenberg\footnote{Corresponding author. Emails: \url{...@auburn.edu};~\url{eisenn1@unlv.nevada.edu}.}}
\date{\today}
...
\begin{remark}[Nonnegativity]
	\label{R:Nonnegative}
	Regarding the nonnegativity assumption --- Assumption \ref{A:Nonnegative}, thanks to \cite[Theorem
	4.6]{CHN19} (see also \cite[Theorem 3.1]{CHHH17} for the case when $r=0$), we have the following
	four groups of sufficient conditions, under either group of which $G(t,\cdot)$ is nonnegative (see
	Figure \ref{F:Nonnegative} for an illustration):
	\begin{enumerate}
...
\begin{remark}[Spectral density and decomposition]
	\label{R:Noise}
	The spectral density corresponding to \eqref{E:corfunction} is $\mathcal{F}\gamma$, which is given by
	\begin{equation}
...
details):
\begin{align*}
	\mathcal{M}_a\left(f, \theta\right) := \sup_{g \in \mathcal{F}_a} \left\{ \left\langle g^2 * g^2, f \right\rangle^{1/2}_{L^2(\R^d)} - \frac{\theta}{2}\: \mathcal{E}_a(g,g) \right\},
\end{align*}
and we use the following convention:
\begin{align*}
	\mathcal{M}_a(f) := \mathcal{M}_a\left(f,1\right) \quad \text{and} \quad
	\mathcal{M}_a    := \mathcal{M}_a(\gamma),
\end{align*}
where $\gamma$ is defined in \eqref{E:corfunction}.
...
\begin{corollary}
	...
	\begin{align}
		...
		\begin{aligned}
			\lim_{p \to \infty} (p-1)^{-\beta} \log \E\left(|u(t,x)|^p\right) = & t^\beta \left(\frac{1}{2}\right)\left(\frac{2a}{2a(b+r)- b\alpha} \right)^\beta \\
                                                                      & \times \left(\theta\nu^{-\alpha/a} \mathcal{M}_a^{\frac{2a-\alpha}{a}}\right)^{\frac{a}{2a(b+r)-b\alpha-a}}\left(2(b+r)-\frac{b\alpha}{a}-1\right).
		\end{aligned}
	\end{align}
	\end{itemize}
...
In the following, we first give some examples and make some discussions in Section \ref{S:SolvEx}.
Then in Section \ref{S:Pre}, we present some preliminaries of the paper, including the {\it
Skorohod} integral, definition of the mild solution, and some asymptotics with corresponding
variational constants. The existence and uniqueness of an $L^p(\Omega)$-solution --- Theorem
\ref{T:solvable} --- is proven in Section \ref{S:solvable}. The upper bound and lower bounds for
\eqref{E:momentasym} are established in Sections \ref{S:upperbd} and \ref{S:lowerbd}, respectively.
Finally, in the appendix --- Section \ref{S:Appendix}, we list a few proofs of results that are used
in the paper.

\section{Examples on solvability and asymptotics}
\label{S:SolvEx}

In this section, we will give various examples to our main results. We will need the following
relation:
\begin{align}
	\label{E:Mdelta1d}
	\mathcal{M}_2(\delta_0)= (3/4)(1/6)^{1/3} \qquad \text{when $d=1$,}
\end{align}
which is a consequence of Lemma 7.2 of Chen and Li \cite{ChenLi04} with $p=2$.
...
\begin{example}[SHE]% {{{
	...
	\begin{enumerate}[(a)]
		\item When $d=2$, condition \eqref{E:CrSHE} says that the $2$-dimensional SHE driven by white
			...
			\begin{align}
				\label{E:Tp-SHE2}
				T_p = \dfrac{\nu}{2\theta (p-1) \mathcal{M}_2(\delta_0)}, \qquad p\ge 2.
			\end{align}
			...
			\begin{align*}
				2\le T_2 = \frac{1}{2 \mathcal{M}_2(\delta_0)} \le 2\pi, \qquad \text{where $d=2$.}
			\end{align*}
...
\begin{example}[SWE]% {{{
	...
	\begin{align}
		\label{E:TpCrSWE}
		T_p=	\frac{\nu^{3 / 2}}{2\theta (p-1) \sqrt{\mathcal{M}_2(\delta_0)}},  \quad p\ge 2,
	\end{align}
...
=======
	\caption{Solvability for the stochastic wave equation (i.e., $a=b=2$ and $r=0$). See Figure \ref{F:SHE} for more legends.}
>>>>>>> 20836416bf4c602ef36016175e413eba7d090fcf
	\label{F:SWE}
\end{figure}% }}}
...
\begin{example}[Fractional SPDEs with $r=\Ceil{b}-b$ and $a=2$]% {{{
	...
	\begin{align}
		\label{E:TpFracSHE}
		T_p = \dfrac{\nu^{\alpha/2}}{2 \theta (p-1) \mathcal{M}_2^{2-\alpha/2}}.
	\end{align}
\end{example}% }}}
...
\begin{example}[SHE with fractional Laplace]% {{{
	...
	\begin{align*}
		\alpha = a \le d
	\end{align*}
	The solvability for this case is illustrated in Figure \ref{F:Stable}. In particular, when $a=d=1$,
	there is only a local solution under white noise. In this case, using \eqref{E:Tp} and
	\eqref{E:Mdelta1d}, we see that
	\begin{align}
	...
	\begin{enumerate}
		...
		\begin{align}
			\label{E:AsymSWE-1d}
				...
				\quad \text{and} \quad
				\lim_{p \to \infty} \frac{\log \E\left[|u(t,x)|^p\right]}{(p-1)^{3/2}} & =\frac{t^{3/2}\sqrt{\theta}}{3 (2\nu)^{1/4}},
		\end{align}
		...
		\begin{align}
			\label{E:AsymSWE-2d}
				% =p(p-1)^{1/2}\frac{2^{3/2}\sqrt{\theta} \mathcal{M}_2^{3/4}(\delta_0)}{ 2^{3/2}\nu^{1/4}}
				\lim_{t \to \infty} \frac{\log \E\left[|u(t,x)|^p\right]}{t^2} & = \frac{p(p-1)\theta\mathcal{M}_2(\delta_0)}{2\nu}
				\quad \text{and} \quad
				\lim_{p \to \infty} \frac{\log \E\left[|u(t,x)|^p\right]}{(p-1)^2} & = \frac{t^2\theta\mathcal{M}_2(\delta_0)}{2\nu}.
		\end{align}
	\end{enumerate}
\end{example}% }}}
...
\begin{example}[Asymptotics for SHE]% {{{
	...
	setting $\alpha_0=0$ and using \cite[Lemma A.2]{ChenHuSongXing15AIHP} to rewrite the constant
...
\begin{example}[Asymptotics for SHE with fractional Laplace and white noise]% {{{
	...
	\begin{align}
		...
		p (p-1)^{\frac{a}{a-1}}
		2^{\frac{7-11 a}{3 (a-1)}}(a-1) a^{\frac{a}{a-1}}\left[3^{-2/3} (2a-1)\right]^{\frac{2a-1}{a-1}}
		\nu^{\frac{1}{1-a}}\theta^{\frac{a}{a-1}},
	\end{align}
	and
	\begin{align}
...
\begin{gather}
	\label{E:Lambda}
	\begin{aligned}
		\mathcal{M}_{\alpha}(\gamma,\theta) & := \sup_{g \in \mathcal{F}_a} \left\{ \left( \iint_{\R^{2d}} g^2(x)g^2(y) \gamma(x+y)\ud x\ud y \right)^{1/2} - \frac{\theta}{2}\: \mathcal{E}_a(g,g) \right\} \\
                                        & = \sup_{g \in \mathcal{F}_a} \left\{ \langle g^2 * g^2, \gamma \rangle_{L^2(\R^d)} - \frac{\theta}{2}\: \mathcal{E}_a(g,g) \right\},                           \\
	\end{aligned}
\end{gather}
where
\begin{gather}
	...
	\label{E:Fa}
\end{gather}
We use the convention that $\mathcal{M}_a(\gamma):=\mathcal{M}_a(\gamma,1)$.  By a similar argument
as the proof of \cite[Lemma 2.3]{BCC21}, one can show that
\begin{equation}
	\label{E:ScaleM}
...
\begin{proof}[Sketch of the proof of Theorem \ref{T:Variation}]
	...
	\begin{align*}
		% \label{E:(7.1)}
	\int_{(\R^d)^2} g^2(x)g^2(-y) \gamma(x-y) \ud x \ud y 	\le C \Norm{\tilde{g}^2}_{L^{2d/(2d-\alpha)}(\R^d)} \Norm{g^2}_{L^{2d/(2d-\alpha)}(\R^d)} =  C \Norm{g}_{L^{4d/(2d-\alpha)}(\R^d)}^4,
	\end{align*}
...
\begin{thebibliography}{999}
	...
	% }}}
	\bibitem{ChenHuSongXing15AIHP} % {{{ 8.
