...
We need to introduce the following variational constants:

\textcolor{red}{The following $\mathcal{M}_a$ is for the Reisz kernal and needs to be changed for the fractional kernal. We need to define $\mathcal{M}_a := \mathcal{M}_a(\gamma,1)$ where $\gamma$ is the fractional kernal defined in \eqref{E:corfunction}.}

\begin{align*}
	\mathcal{M}_a\left(\Theta f, \theta\right) := \sup_{g \in \mathcal{F}_a} \left\{ \left\langle g^2 * g^2, f \right\rangle^{1/2}_{L^2(\R^d)} - \frac{\theta}{2}\: \mathcal{E}_a(g,g) \right\} \quad \text{and} \quad
	\mathcal{M}_a                              := \mathcal{M}_a\left(|\cdot|^{-\alpha},1\right);
\end{align*}
see Section \ref{SS:Variation} for more notation and details.
