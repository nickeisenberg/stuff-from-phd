...
\begin{abstract}
	...
	We derive sharp conditions, under which a unique global $L^p(\Omega)$-solution exists for all $p\ge
	2$. In this case, we derive exact moment asymptotics following the same strategy presented in a recent work
	by Balan {\it et al} \cite{BCC21}. In the case when there exists only a local solution, we
	determine the precise deterministic time, $T_2$, before which a unique $L^2(\Omega)$-solution
	exists, but after which the series corresponding to the $L^2(\Omega)$ moment of the solution blows
...
\end{align*}
with the convention that when $r=0$, $I^0_t = \text{Id}$ reduces to the identity operator. The fundamental solution to \eqref{E:SPDE} is expressed explicitly in terms of the {\it Fox H-function} which is
much more complicated than the fundamental function for either the heat or wave equation. We denote the fundamental solution as 
\begin{equation}
\label{E:GreenFunction}
G(t,x):= G_{a,b,r,\nu,d}(t,x)
\end{equation}
We direct the reader to
Theorem 4.1\footnote{$G(t,x)$ corresponds to $Y_{a,b,r,\nu,d}(t,x)$ from \cite{CHN19}.} of \cite{CHN19} for more details. Since we are interested in the constant one initial condition (and zero
...
In this section, we will give various examples to illustrate our main results. The cases with $b=1$ and $r=0$
...
\begin{example}[SHE]% {{{
	\label{Eg:CriSHE}
	By setting $a=2$, $b=1$ and $r=0$ in \eqref{E:critical}, we obtain the following condition for the SHE
	under which there only exists a local solution:
	\begin{align}
		\label{E:CrSHE}
		\alpha = 2 \le d .
	\end{align}
	...
	\begin{enumerate}[(a)]
		...
		\item Recall that a white noise driven SHE corresponds to when $\alpha=d$. Therefore by examining \eqref{E:alpha} and \eqref{E:critical}, we see that when $d\ge 3$, the SHE driven by white noise no longer has any $L^2(\Omega)$-solution and that local
		solutions exist only when $\alpha=2$ and the noise is not white. This is illustrated in Figure \ref{F:SHE} below. In addition, the critical time $T_p$ takes
...
\begin{example}[SWE]% {{{
	...
	\end{align}
We recall that results in Balan {\it et al} \cite{BCC21} require $d\le 3$, and likewise,
...
\begin{example}[SHE with fractional Laplacian]% {{{
	\label{Eg:StableSHE}
	...
	$\alpha=a=d=2$, there is only a local solution under white noise. 
\end{example}% }}}
...
\begin{example}[Asymptotics for SHE with fractional Laplace]% {{{
	...
	\begin{align*}
		\textbf{M}(a,\alpha_0,d,\gamma) = \sup_{g \in \mathcal{A}_a} \bigg\{ \frac{1}{2} \int_0^1\int_0^1 \int_{\R^{2d}} & \frac{\gamma(x-y)}{|r-s|^{\alpha_0}}g^2(s,x)g^2(r,y)  \ud x \ud y \ud r \ud s
...
\begin{example}[Asymptotics for SPDEs with $r=0$ and white noise]% {{{
	...
	From \eqref{E:alpha}, we see that when $a=2$, $r=0$, $d=\alpha=1$ (white noise), the
...
$\E\left(\delta(u)^2\right)=\E(\Norm{u}_{\mathcal{H}}^2)$. For a complete treatment of the Skorohod integral, see Nualart {\it et al} \cite{Nualart18Book}.
