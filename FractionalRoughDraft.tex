...
\left\{ W(\phi) : \phi \in \mathcal{D}(\R^d)  \right\}$ is a centered and time-independent
...
nonnegative definite tempered measure on $\R^d$. Let $\gamma$ be the Fourier transform of $\mu$ (see Subsection \ref{SS:Malliavin}),
which is also a nonnegative and nonnegative measure on $\R^d$ thanks to Bochner's theorem.
Throughout the paper, we use $\mathcal{F}\phi(\xi) = \int_{\R^d} \exp(-ix \xi) \phi(x) \ud
x$ to denote the Fourier transform of a test function $\phi$.
...
\begin{remark}[Spectral density and decomposition]
	\label{R:Noise}
	Recall that the {\it spectral density} of $\mu$ from \eqref{E:corfunction}, which by definition is
...
\begin{example}[SHE]% {{{
	...
	\begin{enumerate}[(a)]
		\item When $d=2$, condition \eqref{E:CrSHE} says that the $2$-dimensional SHE driven by white
			noise has only a local $L^p(\Omega)$ solution. By applying \eqref{E:Tp} to this case, the critical time $T_p$ becomes
			\begin{align}
				\label{E:Tp-SHE2}
...
\begin{example}[Asymptotics for SPDEs with $r=0$ and white noise]% {{{
	...
	global solution exists when  $b>2/3$. In this case, 
	\begin{align*}
...
W(\phi) : \phi \in \mathcal{D}(\R^d) \right\}$, with the covariance functional of $W$
given by \eqref{E:Cov}. In other words,
\begin{align*}
	\mathbb{E}(W(\phi)W(\psi)) = \int_{\R} \mathcal{F}\phi(\xi) \overline{ \mathcal{F}\psi(\xi) } \mu(\ud \xi) =: \langle \phi, \psi \rangle_{\mathcal{H}}.
\end{align*}
...
\begin{proof}[Sketch of the proof of Theorem \ref{T:Variation}]
	...
	changes in Section 5 ({\it ibid.}), where as the arguments in Section 6 ({\it ibid.}) follow unchanged.
	...
	({\it ibid.}) without any difficulty. Then the rest of Section 5 ({\it ibid.}) follows
...
\begin{proof}[Proof of Theorem \ref{T:solvable}]
	...
	{\bigskip\bf\noindent Step 3.~} Now we can apply Fubini's theorem and Lemma \ref{L:doubleExp} to
...
In this section, we will give the proof of part 2 of Theorem \ref{T:solvable} and establish the upper bound of \eqref{E:momentasym} (under Assumptions
...
As for the upper bound, we will first establish the corresponding result for $p=2$ in Lemma \ref{L:p=2} and then apply the
...
\end{lemma}
\begin{proof}
	The proof follows the same arguments as those of \cite[Lemma 4.3]{BCC21}.
...
\begin{proof}[Proof of 2-i and 2-ii Theorem \ref{T:solvable} ] The critical case happens when the exponent of $n!$ in \eqref{E:pmomBd}
	vanishes, namely,
	\begin{align*}
	...
	We start by proving \textit{2-i}. Let $u_{\lambda}$ for $\lambda >0$ be the solution of
	...
	To show \textit{2-ii}, we use the Cauchy-Hadamard theorem and the same techniques above to see that the radius of convergence of the series
	\begin{align*}
...
\begin{proof}
	...
	\begin{align*}
		\sup_{ \Norm{h}_{L^2}=1 }\left\langle |g_h|^2, |g_h|^2 \right\rangle_{\mathcal{H}}
    =\sup_{ \Norm{h}_{L^2}=1 } \int_{\R^d} \left|\mathcal{F}( |g_h|^2 )(\xi)\right|^2 \mu(\ud \xi)
		= \rho_{\nu,a}
	\end{align*}
...
\begin{proof}[Proof of \eqref{E:fHn}]
	...
	Starting from \eqref{E:fweker}, by the change of variables $t_i' = t_i/c$ and the scaling property in
