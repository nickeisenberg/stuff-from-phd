...
\begin{theorem}[Solvability]
	...
	\begin{enumerate}
		\item[(2-i)] For any $p\ge 2$,\eqref{E:SPDE} has a unique solution $u(t,x)$ in $L^p(\Omega)$ for
			...
			\begin{align}
				\label{E:Tp}
				T_p :=\frac{1}{2\: C_\mu\left(\nu,a\right)} \left[\theta\: (p-1)\right]^{ \frac{b\alpha}{a} - 2(b+r)}
				\quad \text{with} \quad
				C_\mu(\nu,a) := \int_{(\R^d)} \frac{\mu(\ud \xi)}{\left(1+\frac{\nu}{2}|\xi|^a\right)^2}\:.
			\end{align}
...
\begin{proof}[Proof of Theorem \ref{T:solvable}]
	...
	{\bigskip\bf\noindent Step 5.~} In this step, we will prove part (2-i) of Theorem
	...
	\end{align*}
	Putting these conditions together gives the conditions stated in \eqref{E:critical}. Under
	condition \eqref{E:critical}, write the right-hand side of \eqref{E:pmomBd} as $\sum_{n\ge
	0}R^{n/2}$ with
	\begin{align*}
		R = \theta (p-1) \left[ 2tC_\mu(\nu,a) \right]^{[2(b + r ) -b \alpha /a]}.
	\end{align*}
	Therefore, by solving
	\begin{align*}
		0<R<1 \quad \Longleftrightarrow \quad
		t< T_p,
	\end{align*}
	one finds the expression for $T_p$ as in \eqref{E:Tp}. Finally, under the above condition, the
	$L^p(\Omega)$-solution $u(t,x)$ exists for all $x\in\R^d$. This proves part (2-i) of Theorem
	\ref{T:solvable}.

	{\bigskip\bf\noindent Step 6.~} Finally, the content of part (2-ii) of Theorem \ref{T:solvable} is
	proved in part (ii) of Lemma \ref{L:2ineq} below. With this, we complete the whole proof of
	Theorem \ref{T:solvable}.
\end{proof}
...
\begin{proof}
	...
	{\bigskip\noindent\it (ii)~} By applying $\alpha = \frac{a}{b}\left[ 2(b+r) - 1 \right]$ and the scaling property \eqref{E:normscale} to the series $\sum_{n \ge 0} \theta^n n! \Norm{\tilde{f}_n(\cdot,0;t)}_{ \mathcal{H}^{\otimes n }}^2$, we see that
	
	\begin{align*}
		\sum_{n \ge 0} \theta^n n! \Norm{\tilde{f}_n(\cdot,0;t)}_{ \mathcal{H}^{\otimes n }}^2 &= \sum_{n \ge 0} \theta^n n! t^{n } \Norm{\tilde{f}_n(\cdot,0;1)}_{ \mathcal{H}^{\otimes n }}^2 =: \sum_{n \ge 0} (\theta t)^n R_n
	\end{align*}
	where
		\[
