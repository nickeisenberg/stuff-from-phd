...
\begin{abstract}
	...
	sharp conditions, under which a unique global $L^p(\Omega)$-solution exits for all $p\ge 2$. In
	this case, exact moment asymptotics are obtained following the same strategy in a recent work by
	Balan {\it et al} \cite{BCC21}. In the case when there exits only a local solution, we determine
	the precise deterministic time, $T_2$, before which a unique $L^2(\Omega)$-solution exits, but
	after which the series corresponding to the $L^2(\Omega)$ moment of the solution blows up. By
	properly choosing the parameters, results in this paper interpolate those for both stochastic heat
	and wave (\cite{BCC21}) equations.
\end{abstract}
