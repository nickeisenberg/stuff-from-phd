...
which is also a nonnegative and nonnegative definite measure on $\R^d$ thanks to Bochner's theorem.
...
\end{align*}
with the convention that when $r=0$, $I^0_t = \text{Id}$ reduces to the identity operator. The fundamental solution to \eqref{E:SPDE} is expressed explicitly in terms of the {\it Fox H-function}, $H^{m,n}_{p,q}(z)$, which is
...
constant one. Hence through superposition, \eqref{E:SPDE} can be written as
...
\begin{example}[Asymptotics for SHE with fractional Laplacian]% {{{
	...
	\begin{align*}
		\textbf{M}(a,\alpha_0,d,\gamma) = \sup_{g \in \mathcal{A}_{a,d}} \bigg\{ \frac{1}{2} \int_0^1\int_0^1 \int_{\R^{2d}} & \frac{\gamma(x-y)}{|r-s|^{\alpha_0}}g^2(s,x)g^2(r,y)  \ud x \ud y \ud r \ud s
	...
	\[
		\mathcal{A}_{a,d} := \left\{ g(s,x) : \int_{\R^d} g^2(s,x) \ud x = 1, \forall s \in [0,1] \text{ and } (2\pi)^{-d} \int_0^1 \int_{\R^d} |x|^a |\mathcal{F}g(s,\xi)|^2 \ud \xi \ud s < \infty \right\}.
	\]
...
\end{align*}
We direct the reader to \cite[Section 5]{BS2017} for a complete treatment of this processes. For ease of notation, throughout this article, we may write the above integrals as
\begin{align*}
...
Recall the notation above in \eqref{E:GreenFunction} that $G(t,x) = G_{a,b,r,d}(t,x)$. The following scaling properties for both $\mathcal{F}G(t,\cdot)(\xi)$ and
...
\begin{proof}[Proof of Theorem \ref{T:solvable}]
	...
	where
	\[
		\mu(\ud \vec{\xi}) = \prod_{j=1}^n \mu(\ud \xi_j)= \prod_{j=1}^n \varphi(\xi) \ud \xi.
	\]
...
\begin{lemma}
	\label{L:2ineq}
	Assume Assumptions \ref{A:Nonnegative} and \ref{A:Noise} hold and in addition that $\alpha < \min\{2a,d\}$. Recall $\rho$ which is defined in \eqref{E:rho}. Then the following identities hold true:\\
	\begin{align}
		\label{E:2ineq-a}
...
\begin{proof}
	...
	\end{align}
	Now, an application of Stirling's formula to the quantity 
		\[
...
\begin{proof}
	...
	\end{align*}
	Notice that the right hand side of the above equation takes the form as \cite[Equation (3.3)]{BCR09}. By the same arguments that follow in the proof of Theorem 2.3 {\it ibid}  with the replacement \eqref{E:Changes}, we have that
	\begin{align*}
	...
	\end{align*}
	where in addition, $\mathcal{F}^{-1}g \in \mathcal{H}_+$. Moreover, we are interested in the quantity 
	\[
		\rho'_+(g) := \sup_{h \ge 0\;\Norm{h}_{L^2(\R^d)}=1} \int_{\R^d}g(\xi)\left[ \int_{\R^d} \frac{h(\xi + \eta)h(\eta)}{\sqrt{1+\frac{\nu}{2}|\xi+\eta|^a}\sqrt{1+\frac{\nu}{2}|\eta|^a}}  \ud \eta\right] \mu(\ud \xi)
	\]
	which clearly satisfies 
	\[
		\rho'(g) \ge \rho'_+(g).
	\]
	 With this being said, we see that for any $f \in \mathcal{H}_+$ with
	$\Norm{f}_\mathcal{H} =1$,
	\begin{align*}
		\liminf_{n \to \infty} \frac{1}{n}\log \mathbb{E}(U_n(\tau)) \ge \log \rho'_+(\mathcal{F}f).
	\end{align*}
	We now need to calculate
	\begin{align*}
		\sup_{f \in \mathcal{H}_+, \: \Norm{f}_{\mathcal{H}}=1} \rho'_+\left(\mathcal{F}f\right).
	\end{align*}
	...
	\begin{align*}
		\rho'_+(\mathcal{F}f) & = \sup_{\Norm{h}_{L^2(\R^d)}=1}(2\pi)^{-d} \int_{\R^d}\mathcal{F}f(\xi)(\mathcal{F}g_h*\widetilde{\mathcal{F}g_h})(\xi) \mu(\ud \xi) \\
	...
	\begin{align}
		\label{E:supf}
		\sup_{ f\in \mathcal{H}, \Norm{f}_{\mathcal{H}=1} } \rho'_+(\mathcal{F}f) = 	\sup_{ f\in \mathcal{H}, \Norm{f}_{\mathcal{H}=1} }\sup_{\Norm{h}_{L^2}=1} \langle f, |g_h|^2 \rangle _{\mathcal{H}} = \sup_{\Norm{h}_{L^2}=1} \langle |g_h|^2, |g_h|^2 \rangle^{1/2} _{\mathcal{H}}
	\end{align}
	...
	\begin{align*}
		\sup_{ f \in \mathcal{H},\: \Norm{f}_{\mathcal{H}}=1 } \rho'_+(\mathcal{F}f) =
		\sup_{ f \in \mathcal{H}_+,\: \Norm{f}_{\mathcal{H}}=1 } \rho'_+(\mathcal{F}f) =
...
\begin{thebibliography}{999}
	...
	\newblock Hyperbolic Anderson Model with space-time homogeneous Gaussian noise.
	\newblock {\em ALEA, Lat. Am. J. Probab. Math. Stat.} \textbf{14}, 799 -- 849
