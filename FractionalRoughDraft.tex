...
\end{align*}
see Section \ref{SS:LD} for more notation and details. Our goal is to develop exact
moment asymptotics to the solution $u(t,x)$. We follow the techniques laid out in \cite{BCC21} where
the stochastic wave equation, namely, $a=b=\nu=2$ and $r=0$, has been exclusively studied. This
paper acts as a generalization of that paper to the fraction stochastic equation. Here is the main
...
\begin{theorem}
	...
	\end{equation}
	Moreover, we have the following exact moment asymptotic:
	\begin{equation}
		\label{E:momentasym}
		\begin{aligned}
...
calculus, the notation of the solution, and some variational constants. The existence and uniqueness
of a $L^p(\Omega)$ solution is proven in Section \ref{S:solvable}. The exact asymptotic for the case
$p=2$ is obtained in Section \ref{S:p=2}. The upper bound and lower bound for \eqref{E:momentasym}
for general $p\ge 2$ are established in Sections \ref{S:upperbound} and \ref{S:lowerbound},
respectively.
...
\begin{proof}
	...
	d/(d-\alpha)$ which completes the proof of Lemma \ref{L:rhofinite}.
\end{proof}
...
\end{equation}
One notices that by making this generalization, we can apply Theorem \ref{T:main} to the SPDE
...
We need to build up some theory before we prove the above theorem. We first introduce some notation that will be used throughout the remainder of his paper. We start by defining $L_n(\vec{y})$ to be the Laplace transform of $\widetilde{f_n}(\vec{y},0,\cdot)$ evaluated at $1$:
	\begin{align*}
...
\begin{lemma}
	\label{L:Hneq1}
	Under the assumptions of Theorem \ref{T:main}, we have that
	\begin{equation}
...
The first integral is shown to be finite in \cite{BCC21}. The second integral is finite if
	\begin{align}\label{E:alpha2}
		2a - \alpha +1 > 1
	\end{align}
which is already satisfied under the assumptions of Theorem \ref{T:main}. Hence the first relation in \eqref{E:muassumption} is proven.
...
\begin{lemma}
	\label{L:LNfnineq}
	Under the assumptions of Theorem \ref{T:main} and for any $t>0$.
	\begin{align*}
		\int_0^\infty e^{-t} \Norm{\widetilde{f_n}(\cdot,0;t)}_{ \mathcal{H}^{ \otimes n } }^n \ud t \le \dfrac{2^{n\left[2(b+r)-\frac{b\alpha}{a}\right]}}{(n!)^2} T_n \le 2^{n\left[2(b+r)-\frac{b\alpha}{a}\right]} (C_\mu')^n
	\end{align*}
\end{lemma}
...
\begin{proof}
	...
	\end{align*}
	and now we apply Lemmas \ref{L:inteq1} and \ref{L:intineq1} as well as Equation \eqref{E:FLfn} to get
	\begin{align*}
		\frac{2^{n(2(b+r) - b\alpha/a)}}{(n!)^2}  \int_0^\infty 2e^{-2t} \int_{\R^{nd}} H_n(t,\vec{x})^2 \ud \vec{x}   \ud t
...
In order to prove the theorem we need to show that $\sum_{n \ge 0} \theta^n n! \Norm{ \widetilde{f_n}(\cdot,0,t) }_{ \mathcal{H}^{\otimes n} }^2 < \infty.$ From the scaling property \eqref{E:normscale} we see that
	\begin{align*}
		\int_0^\infty e^{-t} \Norm{ \widetilde{f_n}(\cdot,0,t) }_{ \mathcal{H}^{\otimes n} }^2 \ud t & = \Norm{ \widetilde{f_n}(\cdot,0,1) }_{ \mathcal{H}^{\otimes n} }^2 \int_0^\infty e^{-t} t^{ \left[ 2(b+r)-\frac{b \alpha}{a} \right] n } \ud t
...
Therefore,
	\begin{align*}
		\sum_{n \ge 0} \theta^n n! \Norm{ \widetilde{f_n}(\cdot,0,t) }_{ \mathcal{H}^{\otimes n} }^2 & \le \sum_{n \ge 0} \theta^n n!  k_{\alpha} \dfrac{ \left(2^{ \left[ 2(b+r)- \frac{b \alpha}{a} \right]}C_\mu' \right)^n }{ (n!)^{ 2(b+r) - b\alpha/a } }
...
\begin{lemma}
	\begin{align}
		\lim_{n \to \infty}  \frac{1}{n} \log \left( \Norm{ \widetilde{f_n}(\cdot,0,1) }_{ \mathcal{H}^{\otimes n} }^2 \Gamma\left( \left[ 2(b+r)- \frac{b \alpha}{a} \right] n +1 \right) \right) &= \log\left(2^{2(b+r)-\frac{b\alpha}{a}}\right) + \log\left[\rho\right] \label{E:2ineq-a}
	\end{align}

	\begin{align}
		\lim_{n \to \infty}  \frac{1}{n} \log \left( (n!)^{2(b+r)- \frac{b \alpha}{a} }\Norm{ \widetilde{f_n}(\cdot,0,1) }_{ \mathcal{H}^{\otimes n} }^2  \right) &=  \log\left[\left(\frac{2}{2(b+r)-\frac{b\alpha}{a}}\right)^{2(b+r)-\frac{b\alpha}{a}}\right] + \log\left[\rho\right] \label{E:2ineq-b}
	\end{align}
\end{lemma}
\begin{proof}
	...
	\end{align*}
	which proves the upper bound for \eqref{E:2ineq-a}.
	...
	\begin{align}
		\frac{T_n}{(n!)^2} = \frac{\left( \frac{\nu}{2} \right)^{-n\alpha/a} T_n'}{(n!)^2}\le	\Gamma\left( \frac{[2(b + r ) -b \alpha / a]n}{2} \right)^2 	\Norm{ \widetilde{f_n}(\cdot,0;1) }_{ \mathcal{H}^{\otimes n} }^2 \label{E:2lowbd}
	\end{align}
...
With the above lemma, we can now prove Theorem \ref{T:p=2}.
\begin{proof}[Proof of Theorem \ref{T:p=2}]
	...
	where
	\begin{align*}
		R_n = (n!)^{2(b+r)-b\alpha/a}\Norm{\widetilde{f}_n(\cdot,0,1)}_{\mathcal{H}^{\otimes n}}^2 \quad \text{and} \quad x_n=\frac{\theta^n}{(n!)^{2(b+r)-(b\alpha/a)-1}}
	\end{align*}
	\eqref{E:2ineq-b} above says that
	\begin{align*}
	...
	\begin{align*}
		\lim_{t\to \infty} \left[\frac{1}{t^{2(b+r)-b\alpha/a}}\right]^{\frac{1}{2(b+r)-(b\alpha/a)-1}}
		& \log \sum_{n \ge 0} \frac{\left[ (\theta R) t^{2(b+r)-b\alpha/a}\right]^n}{(n!)^{2(b+r)-(b\alpha/a)-1}} \\
...
In this section we use the result obtained in Theorem \ref{T:p=2} and the hypercontractivity
property given by Theorem B.1 in \cite{BCC21} to prove the following theorem.

\begin{theorem}
	Under the assumptions of Theorem \ref{T:main}
	\begin{equation}
...
\end{theorem}
\begin{proof}
	We denote $u_{\lambda}$ for $\lambda >0$ to be the the solution of the fractional equation with $\theta$ replaced with $\lambda$ and $u=u_\theta$. Using theorem B1 from the wave paper we can say that for any $p \ge 2$ that
	\begin{align*}
	...
	\end{align*}
	and an application of Theorem \ref{T:p=2} proves the result.
\end{proof}
...
\begin{align*}
	 \langle \widetilde{f}_n(\cdot,0;t), f^{\otimes n} \rangle_{\mathcal{H}^{\otimes n} } & = \int_{(\R^d)^n} \prod_{k=1}^n \mathcal{F}f(\xi_k) \overline{ \mathcal{F}\widetilde{f}_n (\cdot, 0 ; t)(\vec{\xi} ) } \prod_{k=1}^n \mu(\ud \xi_k)                                                                                        \\
                                                                                        ...
                                                                                        & = W_n(t,\mathcal{F}f)
\end{align*}
where $t_{n+1} = t$ and the last step is due to the change of variables $s_1 = t - t_n, \cdots, s_n
= t - t_1$ and $\xi_1' = \xi_n, \cdots, \xi_n' = \xi_1$. The conclusion follows from the above
...
\begin{proof}
	...
	\begin{enumerate}
		\item \begin{align*}(q-1)\Norm{f_{\tau}}_{\mathcal{H}}^2 = (p-1)^{-1}\Norm{f_{\tau}}_{\mathcal{H}}^2 = t_p^\beta \Norm{f}_{\mathcal{H}}^2\end{align*}
		\item  \begin{align*}W_n(t, \mathcal{F}f_\tau) = W_n(t_p^\beta, \mathcal{F}f)\end{align*}
	\end{enumerate}
...
\end{lemma}
\begin{proof}
	Let $\tau$ be an exponential random variable with mean $1$. By lemma 6.7,
	\begin{align*}
...
\end{lemma}
\begin{proof}
Let $\epsilon > 0$. Lemma 6.8 guarantees the existence of an $N_\epsilon >0$ so for all $n>N_\epsilon$
	\begin{align*}
...
We now can prove Theorem \ref{T:lbound}.
...
\begin{proof}[Proof of Theorem \ref{T:lbound}]
	...
	\end{align*}
	By maximizing $h$ we see that $h$ is maximized at the point $k^* = \left( \frac{2ab -\alpha b +
	2ar}{2aB} \right)^{ \frac{2ab-\alpha b +2ar}{2a-2[2ab-\alpha b + 2ar]} }$ where
	$B=(\sqrt{\theta}\rho^{1/2})^{ \frac{2a}{2ab-\alpha b + 2ar} }$. Inserting $k*$ into $h$ gives us
	that
	\begin{align*}
