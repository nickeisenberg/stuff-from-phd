...
\begin{abstract}
	...
	sharp conditions, under which global $L^p(\Omega)$-solutions exit. In this case, the exact moment
	asymptotics are obtained following the strategy in a recent work by Balan {\it et al}
	\cite{BCC21}. In the case when there exits only a local solution, we give the precise
	...
	corresponding results for the stochastic wave equation were obtained in \cite{BCC21}. Moreover, by
...
results interpolate the asymptotics for both SWE and SHE; see Example \ref{Eg:Inter} below.
...
assumptions and results. More discussions on the solvability --- Theorem \ref{T:solvable} --- will
be presented in Section \ref{S:SolvEx}.
...
\begin{example}[Asymptotics for SHE and SWE]
	\label{Eg:Inter}
	The asymptotics in \eqref{E:momentasym} interpolates the corresponding results for the stochastic
	wave and heat equations:

	\noindent (1 - SWE) Even though our results requires $b$ to be strictly less than $2$, but by
	formally setting
	\begin{align*}
	...
	\noindent (2 - SHE) As for the stochastic heat equation case, we are not aware of the exact asymptotics
	results for SHE with time-independent noise. However, as explained in Remark 1.9 of \cite{BCC21},
	one may apply Chen's asymptotic results in \cite{Chen17AIHP} for SHE with time-dependent noise, by
	formally setting $H_0=1$, to obtain the corresponding time-independent results.  In particular, by
	setting
	\begin{align*}
	...
	conjectured results for SHE, namely, (1.16) and (1.17) of \cite{BCC21}, respectively. Note that
	when $d=1$ and the noise is white, namely,  $\alpha=1$, the upper and lower bounds for the second
	Lyapunov exponent ($p=2$) have been obtained by Hu \cite[part 1) of Theorem 4.1]{Hu02Chaos}. Due
	to the importance of the white noise, we list the asymptotics below:
	\begin{align*}
		\beta=3 \quad \text{and} \quad t_\beta= (p-1)^{2/3} t,
	\end{align*}
	and
	\begin{align}
		\label{E:momentasymSHE}
			\lim_{t_p \to \infty} t_p^{-3} \log \Norm{u(t,x)}_p =
			& \frac{16\theta^2}{27\nu} \mathcal{M}_2^3(\delta_0) = \frac{\theta^2}{24\nu},
	\end{align}
	where we have applied Lemma 7.2 of Chen and Li \cite{ChenLi04} with $p=2$ to have
	$\mathcal{M}_2(\delta_0)= (3/4)(1/6)^{1/3}$. In particular, for $p\ge 2$ and $t$ fixed, we have
	\begin{align}
		\label{E:momentasymSHE-t}
...
In the following, we first give some examples regarding the solvability stated in Theorem
\ref{T:solvable} in Section \ref{S:SolvEx}. Then in Section \ref{S:Pre}, we present some
...
upper bound and lower bounds for \eqref{E:momentasym} are established in Sections \ref{S:upperbd} and
\ref{S:lowerbd}, respectively.  Finally, in the appendix --- Section \ref{S:Appendix}, we list a few
proofs of results that are used in the paper.

\section{Examples on solvability}
\label{S:SolvEx}
...
\begin{example}[SHE]
	...
	\begin{enumerate}[(a)]
		\item When $d=2$, condition \eqref{E:CrSHE} says that the $2$-dimensional SHE driven by white
			...
			\end{align}
		\item When $d\ge 3$, SHE driven by white noise no longer has any $L^2(\Omega)$
			solution. Local solutions exist only when $\alpha=2$ and the noise is not white. The critical
			time $T_p$ takes the same expression as \eqref{E:Tp-SHE2} but one needs to replace $\delta_0$
			by $\gamma$.
		% \item Similarly, the cases for other nonzero values of $r$ are illustrated in Figure
			% \ref{F:SHE}.
	\end{enumerate}
\end{example}
...
More examples can be studied, which are left to the interested readers.

\section{Some preliminaries}
\label{S:Pre}

\subsection{Skorohod integral and mild solution}
\label{SS:Malliavin}
