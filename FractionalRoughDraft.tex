...
We may actually apply the techniques in this paper to any such noise who's correlation function and spectral density, $\gamma$ and $\mu$, satisfy
		\begin{enumerate}
...
It holds that 
		\begin{equation} \label{momentasym}
			\lim_{t_p \to \infty} t_p^{-\beta} \log \Norm{u(t,x)}_p = \left(\frac{1}{2}\right)\left( \frac{2a}{2a(b+r)- b\alpha} \right)^\beta (\theta\rho)^{\frac{a}{2a(b+r)-b\alpha-a}}\left(2(b+r)-\frac{b\alpha}{a}-1\right).
		\end{equation}	
	\end{theorem}

\section{Some preliminaries.}
\subsection{Malliavin Calculus}
\textcolor{red}{Add Intro to Malliavin Calc Here}

In general, we start with a non-negative and non-negative definite tempered measure $\Gamma$ with density $\gamma$ in the sense that $\Gamma(\ud x) = \gamma(x) \ud x$, and 
	\[
...
is non-negative define and thus we may associate it as the covariance functional of a zero-mean Gaussian processes, $W:= \{ W(\varphi) : \varphi \in \mathcal{D}(\R^d) \}$. In other words, 
	...
	\]
Let $\mathcal{H}$ be the completion of $\mathcal{D}(\R^d)$ with respect to $\langle \cdot, \cdot \rangle_{\mathcal{H}}$ and thus we see $\varphi \to W(\varphi)$ is an isometry from the normed space $(\mathcal{D}(\R^d), \Norm{\cdot}_{\mathcal{H}})$ to $L^2(\Omega)$ where $\Norm{f}_{\mathcal{H}} = \langle f, f \rangle_{\mathcal{H}}$.
...
\end{align}
For ease of notation, we write the above integrals as
	\begin{align*}
...
We first  define
	\begin{equation}\label{rhofrac}
		\rho = \rho_{\nu,a} := \sup_{\Norm{h}_{L^2(\R^d)}=1} \int_{\R^d} \left[ \int_{\R^d} \frac{h(\xi + \eta)h(\eta)}{\sqrt{1+\frac{\nu}{2}|\xi+\eta|^a} \sqrt{1+\frac{\nu}{2}|\eta|^a}} \ud \eta \right]^2 \mu(\ud \xi).
	\end{equation}
where $\nu$ and $a$ are from \ref{spdee}. In addition, $\mu(\ud \xi) = C_{d,\alpha}|\xi|^{-(d-\alpha)}$ which corresponds to the Riesz kernel $\gamma(x) = |x|^{-\alpha}$ for some $\alpha \in (0,d)$. In X. Chen \cite{chenlarge}, the case of $a \in (0,2]$ and $\nu = 2$ is discussed and there it is proven that
	...
	\end{equation}
and in addition, X. Chen \cite{chenlarge} also shows the equality $\rho_{2,a} = \Lambda_{\alpha}^{2-(\alpha/a)}$ where
	\begin{align}
		\Lambda_{\alpha} &= \sup_{g \in \mathcal{F}_a} \left\{ \left( \int_{\R^d}^2 \frac{g^2(x_1)g^2(x_2)}{|x_1+x_2|^a} \ud \vec{x} \right)^{1/2} - \mathcal{E}_a(g,g) \right\}
		\\&= \sup_{g \in \mathcal{F}_a} \left\{ \langle g^2 * g^2, |\cdot|^{-a} \rangle - \mathcal{E}_a(g,g) \right\} \nonumber.
	\end{align}
...
so we can say that
	\begin{align*}
		...
		\\&\left( \frac{\nu}{2} \right)^{-\alpha/a}  \rho_{2,a}
	\end{align*}
and this proves \ref{rhorelation}.
...
\textcolor{red}{I never mentioned $\mathcal{M}$. Since it is essentially the same thing as $\Lambda$ I may leave it out}
...
with the convention that. $s_0 = y_{\sigma(0)} = 0$. Next we define,
	\begin{align*}
		H_n(t, \vec{x}) &= n! \int_{(\R^d)^n} \prod_{k=1}^n K(x_k - y_k) \tilde{f_n}(\vec{y};0,t) \ud \textbf{y}
		\\&= \sum_{\sigma \in \Sigma_n} \int_{[0,t]^n <} \int_{(\R^d)^n} \prod_{k=1}^n K(x_k-y_k) 	\prod_{k=1}^n G(s_k - s_{k-1}, y_{\sigma(k)} - y_{\sigma(k-1)}) \ud \vec{y} \ud s
	\end{align*}
...
and using this and \ref{LandFL}, we can calculate $\mathcal{F}L_n(\vec{\xi})$:
	...
	\begin{lemma}\label{Hneq1}
		\textcolor{red}{Insert text about assumptions}
					...
					\end{equation}
	\end{lemma}
\noindent\textbf{Proof:} \textcolor{red}{Same proof from wave paper. Copy proof there}
	...
	\begin{lemma}\label{inteq1}
		\textcolor{red}{Insert text about assumptions}
					...
					\end{equation}
	\end{lemma}
\noindent\textbf{Proof:} \textcolor{red}{Same proof from wave paper. Copy proof later.}
...
By applying the substitution $\xi_k' = \left(\frac{\nu}{2}\right)^{1/a}\xi_k$ and the scaling property for $\varphi$, we may rewrite $T_n$ as
	...
	\begin{lemma}
		Under the assumption of Theorem \textcolor{red}{main theorem on the moment asymptotics that I need to put into the introduction}, we have that
			\begin{equation}
...
\section{Upper Bound.} \textcolor{red}{insert intro to section}
...
\section{Lower Bound.} The goal of this section is to prove the following theorem.
...
\noindent \textbf{Proof:} \textcolor{red}{same as wave paper. copy later.}

	\begin{proposition}\label{Wnprop1} For $f\in \mathcal{H}$, $t>0$, and $p,q>1$ with $p^{-1} + q^{-1}=1$, it holds that
		\[
...
For an $f \in \mathcal{H}_+$ and $\Norm{f}_{\mathcal{H}}= 1$ we have that $\mathcal{F}(f)$ is even and in addition, Lemma 6.2 gives us that
	...
	\]
Modifying the ideas in the Xia Chen paper, we can say that
	\begin{align*}
