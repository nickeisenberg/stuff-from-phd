...
By using the same techniques used to derive \eqref{E:ScaleM}, one can show that for any $\Theta >0$ and $\theta>0$ that
	\begin{equation}\label{E:ScaleE}
			E_{a,d}(\Theta\gamma,\theta) =\Theta^{\frac{a}{a-\alpha}} \theta^{-\frac{\alpha}{a-\alpha}}  E_{a,d}(\gamma,1).
	\end{equation}
The relation between $\mathcal{E}_{a,d}(\gamma,\theta)$ and $\mathcal{M}_{a,d}(\gamma,\theta)$ can
be established in a similar way as Lemma A.2 of \cite{CHSX15AIHP}.
\begin{lemma}
	\label{L:EvsM}
	Under Assumption \ref{A:Noise}, the following three expressions hold:
		\begin{equation}\label{E:Erelation}
			E_{a,d}(\Theta\gamma,\theta) =\Theta^{\frac{a}{a-\alpha}} \theta^{-\frac{\alpha}{a-\alpha}}\left(\frac{2\alpha}{a}\right)^{\alpha/(a-\alpha)}\frac{a-\alpha}{a}\sigma(d,\alpha)^{a/(a-\alpha)},
		\end{equation}
		\begin{equation}\label{E:Mrelation}
			\mathcal{M}_{a,d}(\Theta\gamma,\theta) =	\Theta^{\frac{a}{2a-\alpha}}\theta^{-\frac{\alpha}{2a-\alpha}} \left( \frac{\alpha}{a}\right)^{\alpha/(2a-\alpha)} \frac{2a-\alpha}{2a}\sigma(d,\alpha)^{a/(2a-\alpha)},
		\end{equation}
		\begin{equation}	\label{E:EvsM}
				E_{a,d}(\Theta\gamma,\theta) = \left( \frac{a-\alpha}{a}\right)(2)^{\alpha/(a-\alpha)}\left( \frac{2a-\alpha}{2a} \right)^{-(2a-\alpha)/(a-2)} \mathcal{M}_{a,d}(\Theta\gamma,\theta)^{\frac{2a-\alpha}{a-\alpha}}.
		\end{equation}
...
We need to prove two Lemmas in order to prove Lemma \ref{L:EvsM}.
	...
	\begin{lemma}
For any $f \in L^2(\R^d)$, when $f$ satisfies $\mathcal{E}_a(f,f) < \infty$ then
\begin{equation}\label{E:SobIneq}
\int_{\R^d} f^2(x)\gamma(x) \ud x \le \sigma(d,\alpha) \Norm{f}_2^{2-(2\alpha/a)} \mathcal{E}_a(f,f)^{\alpha/a}
\end{equation}
where $\sigma(d,\alpha)$ is a constant that will be defined below.
	\end{lemma}

	\begin{proof}
The proof of this result follows the scheme laid out in the proof of Lemma A.3 \cite{ChenAOP12}. By the same techniques presented in Lemma \ref{T:Variation}, one can show the following quantity is finite:
	\begin{align*}
		\Lambda &:= \sup_{f\in\mathcal{F}_a}  \left\{  \int_{\R^d} g^2(x)\gamma(x) \ud x - \frac{(2\pi)^{-d}}{2} \int_{\R^d} |x|^a|\mathcal{F} g(x)|^2 \ud x  \right\}
		\\& =    \sup_{f\in\mathcal{F}_a}  \left\{  \int_{\R^d} g^2(x)\gamma(x) \ud x - \frac{1}{2} \mathcal{E}_a(f,f)\right\} < \infty.
	\end{align*}
Now consider any $f \in \mathcal{F}_a$. We have $\Norm{f}_2 = 1$ and $\mathcal{E}_a < \infty$. Next we choose $C_f$ such that
	\[
...
Its easy to see that $\Norm{g}_2 = 1$. In addition, through a direct calculation one can show that
	\[
		\mathcal{E}_a(g,g) = t^a\mathcal{E}_a(f,f) \quad \text{and} \quad \int_{\R^d} g^2(x)\gamma(x) \ud x = t^\alpha \int_{\R^d} f^2(x)\gamma(x)\ud x.
	\]
