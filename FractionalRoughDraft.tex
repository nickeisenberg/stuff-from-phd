...
To prove \ref{E:LogRho}, we first notice that by applying the substitution $\xi_{\sigma(j)}' =
...
with the convention that. $s_0 = y_{\sigma(0)} = 0$. Next we define,
	...
	\end{align*}
where in the last line we used relation \ref{E:fperm0} for expressing  $\widetilde{f_n}(\cdot;0,t)$. Next, similarly to $L_n$, we define $L(x)$ and can calculate $\mathcal{F}L(\xi)$ to be
...
and through a change of variables, one sees that the Laplace transform of convolution in the time variable become a product:
	...
	\end{equation}
and using this and \ref{E:LandFL}, we can calculate $\mathcal{F}L_n(\vec{\xi})$:
...
\begin{align*}
	\Norm{\widetilde{f_n}(\cdot,0;t)}_{\mathcal{H}^{\otimes n}}^2 & = \int_{(\R^d)^n} \int_{(\R^d)^n} \prod_{k=1}^n \gamma(y_k-y_k') \widetilde{f}_n(\vec{y},0;t)\widetilde{f}_n(\vec{y'},0;t) \ud \vec{y} \ud \vec{y'} \\
...
\begin{proof}
	...
	1. Then $\tau \wedge \tau'$ has an exponential distribution with density $g(t)=2e^{-2t}$ and
	\begin{align*}
...
The first integral is shown to be finite in \cite{BCC21}. The second integral is finite if
	\begin{align}\label{E:alpha2}
		2a - \alpha +1 > 1
	\end{align}
which is already satisfied under the assumptions of Theorem \ref{T:main}. Hence the first relation in \ref{E:muassumption} is proven.
...
\begin{proof}
	...
	\end{align*}
	and now we apply Lemmas \ref{L:inteq1} and \ref{L:intineq1} as well as Equation \ref{E:FLfn} to get
	\begin{align*}
		\frac{2^{n(2(b+r) - b\alpha/a)}}{(n!)^2}  \int_0^\infty 2e^{-2t} \int_{\R^{nd}} H_n(t,\vec{x})^2 \ud \vec{x}   \ud t
...
First recall \ref{E:Ffnscale}, which says
	\begin{align*}
...
\end{lemma}
\begin{proof}
	Recall the definition of $T_n$ from above as well as \ref{E:LogRho} which says that
	\begin{align*}
	...
	\end{align*}
	Combining this and \ref{E:LogRho} we see that
	\begin{align*}
	...
	\end{align*}
	which proves the upper bound for \ref{E:2ineq-a}.
	...
	\end{align*}
	where $C_n = 2^{-1}[2(b + r ) -b \alpha / a]^{1/2}(2 \pi n )^{1/2}$. We now infer from \ref{E:2lowbd} that
	\begin{align*}
	...
	\end{align*}
	Thus \ref{E:2ineq-a} is proven. Lastly, \ref{E:2ineq-b} follows from \ref{E:2ineq-a} and the fact that
	\begin{align*}
...
\begin{lemma}
	\label{L:LWn}
	If the measure $\mu$ satisfies the relation in \ref{E:muassumption}, then $W_n(t,\phi)$ is well
...
\end{lemma}
\begin{proof}
	By Fubini's theorem, the left hand side of \ref{E:LWn} is equal to
	\begin{align*}
	...
	arguments that lead to \ref{E:lem62step}, we see the above quantity is finite.
\end{proof}
...
which finally leads us to the equality
	\begin{align*}
		\liminf_{n\to \infty} \frac{1}{n} \log \mathbb{E} (U_n(\tau) ) \ge \log(\rho^{1/2}).
	\end{align*}
Hence, we finish the proof by proving \ref{E:GH+}. First note that from the above we see that
	\begin{align*}
...
\begin{proof}
	...
	+\frac{a}{b}r \right)} $ on the inside of the log in \ref{E:lb1}
\end{proof}
...
We can easily find the maximal $c$ by solving $h'(c)=0$ and doing so we get $c=(k \sqrt{\theta}R)^{\frac{a}{b\left( a - \frac{\alpha}{2} + \frac{a}{b}r \right)}}$. After plugging in this value for $c$ and replacing $R$ we arrive at the following inequality
	...
	\end{align*}
and simplifying exponents gives us \ref{E:69ineq}.
\end{proof}
