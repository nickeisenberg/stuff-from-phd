...
\begin{definition}[Mild solution]
	\label{D:Sol}
	For $T\in (0,\infty]$, a random field $u=\left\{u(t,x) : t \in (0,T),\: x\in\R^d\right\}$ is
	...
	\begin{equation}
		...
		u(t,x) = 1 + \sqrt{\theta}\int_0^t \left( \int_{\R^d} G(t-s,x-y)u(s,y) W(\delta y) \right)\ud s.
	\end{equation}
	Moreover, for $p\ge 1$, we call it an {\it $L^p(\Omega)$-solution} if $\Norm{u(t,x)}_p < \infty$
	for all $t \in (0,T)$ and $x \in \R^d$.
\end{definition}
...
$\Norm{\widetilde{f_n}(\cdot,t,x)}_{\mathcal{H}^{\otimes n}}$ are very important.
