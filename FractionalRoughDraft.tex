...
heavily on the Feynman-Kac representation of the moments of the solution. However, whenever $b\ne
...
\begin{theorem}[Solvability]
	...
	\begin{enumerate}
		\item[(2-i)] For any $p\ge 2$,\eqref{E:SPDE} has a unique solution $u(t,x)$ in $L^p(\Omega)$ for
			...
			\begin{align}
				\label{E:Tp}
				T_p := ... \text{\textcolor{magenta}{update here}} ....
			\end{align}
...
\begin{remark}
	...
	\textcolor{red}{Is there a factor of 1/2 missing from th exponent of $K$? See how the Reisz case and less general fractional noise all have 1/2 in the exponent of $K$.}
...
\begin{proof}[Proof of Theorem \ref{T:solvable}]
	...
	\end{equation}
	% \begin{remark} By applying the substitution $\xi_k' = \left(\frac{\nu}{2}\right)^{1/a}\xi_k$ and
	% the scaling property for $\varphi$, we may rewrite $T_n$ as \begin{equation} T_n(\nu,a) = \left(
	% \frac{\nu}{2} \right)^{-n\alpha/a} T_n(2,a).  \end{equation} \end{remark}
	...
	{\bigskip\bf\noindent Step 5.~} In this step, we will prove part (2) of Theorem
	\ref{T:solvable}. The critical case happens when the exponent of $n!$ in \eqref{E:pmomBd}
	vanishes, namely,
	\begin{align*}
	...
	Now assume condition \eqref{E:critical} holds. Notice that when $p=2$, both parts (2-i) and (2-ii)
	are proved in part (ii) of Lemma \ref{L:2ineq} below. Hence, it remains to show part (2-i) for the case
	when $p>2$. Indeed, by the hypercontractivity property, the upper bound for the case of $p$ norm
	with $p>2$ can be reduced to that of the second norm. More precisely, \eqref{E:pMom2} and part
	(ii) of Lemma \ref{L:2ineq} below together imply that
	\begin{align*}
		t_p = (p-1)^{\frac{1}{2(b+r)-b\alpha/a}} t < T_2 \quad \Longrightarrow \quad \Norm{u(t,x)}_p<\infty,
	\end{align*}
	from which one can identify the critical time
	\begin{align*}
		T_p:= (p-1)^{-\frac{1}{2(b+r)-b\alpha/a}}T_2 = (p-1)^{-\frac{1}{2(b+r)-b\alpha/a}} (2\rho\theta)^{-1} = ... \\
		\text{\textcolor{magenta}{... write in terms of $\mathcal{M}$ instead of $\rho$ as in \cite{BCC21}}}
	\end{align*}
	With this, we complete the whole proof of Theorem \ref{T:solvable}.
\end{proof}
...
\begin{lemma}
	...
	\end{align}
	(ii) Under condition \eqref{E:critical}, let $T_2$ be defined as in \eqref{E:Tp}. For all
	$x\in\R^d$ and $\theta>0$, it holds that
	\begin{align*}
...
\end{lemma}
\begin{proof}
	{\it (i)~} Recall the definition of $T_n$ from above as well as \eqref{E:LogRho} which says that
	\begin{align*}
	...
	\end{align*}
	and recall \eqref{E:inteq1} which says
	\begin{align*}
	...
	\end{align*}
	Let $\tau$ and $\widetilde{\tau}$ be independent exponential random variables with mean 1. By \eqref{E:Hneq1},
	\begin{align*}
	...
	\end{align*}
	where the last line follows from \eqref{E:inteq1}. Thus so far we have shown
	\begin{align}
		\label{E:2lowbd}
	...
	\end{align*}
	This proves part (i) of Lemma \ref{L:2ineq}.


	{\bigskip\noindent\it (ii)~} By applying $\alpha = \frac{a}{b}\left[ 2(b+r) - 1 \right]$ and the
	scaling property \eqref{E:normscale} to the series in \eqref{E:2mom}, we see that
	\begin{align}
		\label{E:SerieCr-2}
		\sum_{n \ge 0} \theta^n n! \Norm{\tilde{f}_n(\cdot,0;t)}_{ \mathcal{H}^{\otimes n }}^2
    &= \sum_{n \ge 0} \theta^n n! t^{n } \Norm{\tilde{f}_n(\cdot,0;1)}_{ \mathcal{H}^{\otimes n }}^2
     = : \sum_{n \ge 0} (\theta t)^n R_n,
	\end{align}
	where $R_n = n! \Norm{\tilde{f}_n(\cdot,0;1)}_{ \mathcal{H}^{\otimes n }}^2$.  The limit in
	\eqref{E:2ineq-b} implies that
	\begin{align*}
		\lim_{n \to \infty} |R_n|^{-1/n} = (2\rho)^{-1}.
	\end{align*}
<<<<<<< HEAD
	where
		\[
			R_n = n! \Norm{\tilde{f}_n(\cdot,0;1)}_{ \mathcal{H}^{\otimes n }}^2.
		\]
	By the Cauchy-Hadamard theorem, this series diverges for 
		\[
			\theta t > \limsup_{n \to \infty} |R_n|^{-1/n} =: R.
		\]
	However, by equation \eqref{E:2ineq-b}, we see that 
		\[
			\lim_{n \to \infty} \frac{1}{n} \log(|R_n|) = \log\left( 2 \rho \right).
		\]
	Hence 
		\[
			\limsup_{n \to \infty} |R_n|^{-1/n} = (2\rho)^{-1}
		\]
	and so the series diverges for $t > \dfrac{1}{2\rho\theta} = T_2$ and in fact the Cauchy-Hadamard theorem says that $T_2$ is the precise cut off time for divergence of the series $\sum_{n \ge 0} \theta^n n! \Norm{\tilde{f}_n(\cdot,0;t)}_{ \mathcal{H}^{\otimes n }}^2$. 
	
	\textcolor{red}{$T_p$ in terms of $M$}
	
	By the hyperactivity property we have that 
		\[
			\Norm{u(t,0)}_p \le \Norm{u_{(p-1)\theta}(t,x)}_2.
		\]
	Now by recalling Theorem \ref{T:2momseries} and by applying $\alpha = \frac{a}{b}\left[ 2(b+r) - 1 \right]$ and the scaling property \eqref{E:normscale}  we see that
		\begin{align*}
	...
	where above we define 
		\[
			 R_n = n! \Norm{\tilde{f}_n(\cdot,0;1)}_{ \mathcal{H}^{\otimes n }}^2.
		\]
	By the Cauchy-Hadamard theorem, this series converges for 
		\[
			\theta t (p-1) > \limsup_{n \to \infty} |R_n|^{-1/n} =: R.
		\]
	However, by equation \eqref{E:2ineq-b} and Lemma \ref{L:rhovar}, we see that 
		\[
			\lim_{n \to \infty} \frac{1}{n} \log(|R_n|) = \log\left( 2 \rho \right) =  \log\left( 2 \nu^{-\alpha/a} \mathcal{M}_a^{(2a-\alpha)/a} \right).
		\]
	Hence 
		\[
			\limsup_{n \to \infty} |R_n|^{-1/n} = (2 \nu^{-\alpha/a} \mathcal{M}_a^{(2a-\alpha)/a} )^{-1}
		\]
	and so $\Norm{u(t,x)}_p$ converges for $t  < \dfrac{1}{2 \theta (p-1) \nu^{-\alpha/a} \mathcal{M}_a^{(2a-\alpha)/a} } =: T_p$.
...
\begin{proof}
	...
	\end{align}
	As a direct consequence of Stirling's formula, we have that
	\begin{align*}
		\lim_{n \to \infty} \frac{1}{n}\log\left( \frac{\Gamma(an+1)}{(n!)^a} \right) = a \log(a), \quad
		\text{for all $a>0$,}
	\end{align*}
	with which one immediately sees that
	\begin{align*}
