...
\cite{CHN19}.} of \cite{CHN19} for more details. Since we are interested in the constant one initial
...
\begin{example}[SHE]% {{{
  \label{Eg:CriSHE} By setting $a=2$, $b=1$ and $r=0$ in \eqref{E:critical}, we obtain the following
  condition for the SHE under which there only exists a local solution:
  \begin{align}
    \label{E:CrSHE}
    \alpha = 2 \le d .
  \end{align}
  ...
  \begin{enumerate}[(a)]
    \item When $d=2$, condition \eqref{E:CrSHE} says that the $2$-dimensional SHE driven by white
      ...
      \cite[Theorem 3.4 and Remark 3.13]{CDOT20}.
...
\begin{example}[SWE]% {{{
  \label{Eg:CriSWE} By setting $a=2$ and formally setting $b=2$ in \eqref{E:critical}, we obtain the
  following condition for the stochastic wave equation under which there only exists a local
  solution:
  \begin{align}
    \label{E:CrSWE}
...
\begin{example}[Fractional SPDEs with $r=0$ and $a=2$]% {{{
  \label{Eg:FracSHE0} In this example, we study the special case of the fractional SPDEs when $r=0$.
  The choice of $r=0$ has been used in, e.g., \cite{CHHH17}.  We will only consider the case $a=2$
  for simplicity. Now by setting $r=0$ and $a=2$ and restricting $b\le 1$, conditions in
...
\begin{example}[SHE with fractional Laplacian]% {{{
  \label{Eg:StableSHE} The stochastic heat equation with fractional Laplacian (i.e., the case when
  $b=1$, $r=0$ and $a\in (0,2]$) has been widely studied in the literature, but possibly with
  different noises. In this case, the fundamental solutions are transition densities for the
  alpha-stable jump processes, which are necessarily to be nonnegative. This is also consistent with
  the sufficient conditions for nonnegativity in Remark \ref{R:Nonnegative}. By setting $b=1$ and
  $r=0$ in \eqref{E:critical}, we have the following condition:
  \begin{align*}
    \alpha = a \le d
  \end{align*}
...
\begin{example}[Asymptotics for SWE]% {{{
  \label{Eg:AsymSWE} Even though our results requires $b$ to be strictly less than $2$, but by
  formally setting
  \begin{align*}
...
\begin{example}[Asymptotics for SHE]% {{{
  ...
  setting $\alpha_0=0$ and using \cite[Lemma A.2]{15AIHP} to rewrite the constant $\mathcal{E}$ in
  \cite{Chen17AIHP} in terms of $\mathcal{M}_a$. Due to the importance of the white noise case, we
...
\begin{example}[Asymptotics for SHE with fractional Laplacian]% {{{
  \label{Eg:AsymStable} In this example we restrict ourselves to the case when $b=1$, $a\in (0,2]$,
...
\begin{example}[Asymptotics for SPDEs with $r=\Ceil{b}-b$ and white noise]% {{{
  \label{Eg:AsymFracSHE} In this example, we consider the case when $a=2$, $d=\alpha=1$ (white
  noise), and $r=\Ceil{b}-b$. As seen in Example \ref{Eg:FracSHE}, there exists a global solution.
  In this case,
  \begin{align*}
...
integral, see Nualart {\it et al} \cite{Nualart18Book}.
...
{\it Wiener chaos expansion} \footnote{We direct the reader to \cite[Section 5]{BS2017} for a
complete treatment of this processes.}:
\begin{equation}
  \label{E:solwexp}
...
\begin{theorem}% {{{
  \label{T:Variation} Suppose that the correlation function $\gamma$ satisfies Assumption
  \ref{A:Noise} and is such that $\alpha < \min\{2a,d\}$. Then both \eqref{E:rhoLam} and
  \eqref{E:LogRhoRiez} hold with $|\cdot|^{-\alpha}$ and $\mu$ replaced by $\gamma$ and $\mu$ as in
...
\begin{remark}% {{{
  \label{R:Constant} It is often very difficult to obtain the exact value for the variational
  constant $\mathcal{M}_{a,d}(\gamma)$. To the best of our knowledge, only in case of $a=2$ and
  $\alpha=d=1$ (white noise), one can compute explicitly that
  \begin{align}
    \label{E:Mdelta1d}
...
In order to compare our results with known results, some which were presented in Example
\ref{Eg:AsymStable}, let us introduce another commonly used variational constant
\begin{align}
  \label{E:VarConstE}
  \begin{aligned}
...
established in a similar way as \cite[Lemma A.2]{15AIHP}, which is stated in the following lemma:
...
\begin{lemma}% {{{
  \label{L:gammaf2bdd} Under Assumption \ref{A:Noise}, for any $f \in L^2(\R^d)$ with
...
\begin{lemma}% {{{
  \label{L:gammaf2f2bdd} For any $f \in \mathcal{F}_a$ and for $\alpha < \min\{a,d\}$ we have
...
\end{proof}% }}}
  ...
  equations (A.2), (A.3) and (A.4) of \cite{15AIHP} respectively when $a=2$.
  \end{proof}
% }}}}}}
\section{Existence and uniqueness of the solution}% {{{
\label{S:solvable}

In this section, we will prove the first part of Theorem \ref{T:solvable}. The proof will need the
following lemma:
...
The proof of Theorem \ref{T:solvable} follows the same strategy as \cite[Section 3]{BCC21} with
...
\ref{L:p=2} and then apply the hypercontractivity property given by Theorem B.1 in \cite{BCC21} to
...
\begin{lemma}% {{{
  \label{L:2ineq} Assume Assumptions \ref{A:Nonnegative} and \ref{A:Noise} hold and in addition that
...
\begin{lemma}% {{{
  \label{L:p=2} Assume Assumptions \ref{A:Nonnegative} and \ref{A:Noise} hold. Under condition
  \eqref{E:alpha}, we have that
  \begin{equation}
...
\begin{lemma}% {{{
  \label{L:LWn} If the measure $\mu$ satisfies the relation in \eqref{E:muassumption}, then
  $W_n(t,\phi)$ is well defined and for any $d \ge 1$, $t>0$ and $\phi \in L^2_{\mathbb{C}}(\mu)$.
  Moreover,
  \begin{equation}
    \label{E:LWn}
...
\begin{proposition}% {{{
  \label{P:Wnprop1} For $f\in \mathcal{H}$, $t>0$, and $p,q>1$ with $p^{-1} + q^{-1}=1$, it holds
  that
  \begin{align}
    \label{E:Wnprop1}
...
\begin{proposition}% {{{
  \label{P:Wnprop2} For $p,q > 1$, $p^{-1}+q^{-1}=1$ and for any $f \in \mathcal{H}$,
  \begin{align}
    \label{E:Wnprop2}
...
\begin{proof}% {{{
  ...
  \begin{gather}
    \label{E:VW-1} V-W\left( (d-\alpha)+ \frac{a}{b}(b+r) \right) = 0, \\
...
\begin{proposition}% {{{
  \label{P:LogU} If $\tau$ is an exponential random variable with mean one, then
  \begin{align*}
...
\begin{proof}% {{{
  ...
  \[
    \rho'(g) \ge \rho'_+(g).
  \]
   With this being said, we see that for any $f \in \mathcal{H}_+$ with $\Norm{f}_\mathcal{H} =1$,
  \begin{align*}
...
\begin{lemma}% {{{
  \label{L:logUnlb} For any $k,\theta >0$, there exists a constant $c_1 = c_1(\alpha,\mathcal{M}, k,
  \theta) >0$ such that, by setting $n_t = [c_1 t]$, it holds that
...
{\bf Acknowledgement.}% {{{
We would like to thank Raluca Balan for some useful comments on our paper and two anonymous referees
for their careful reading of the paper with many good suggestions.
% }}}
% }}}
...
\begin{thebibliography}{999}
  ...
  equation with time-independent noise. \newblock {\em Ann. Inst. Henri Poincar\'e Probab. Stat. },
  to appear.
  % }}}
  \bibitem{BCR09}% {{{ 2.
