...
\begin{proof}
	...
	% \end{align*}
	% Now recall that $\int_0^\infty e^{-t} \mathcal{F}G(t,\cdot)(\xi) \; \ud t =
...
\end{proposition}
\begin{proof}
	The above inequality motivates us to find a function $f_{\tau}(x)$ and some $\tau$ such that
	\begin{align*}
		(q-1)\Norm{f_{\tau}}_{\mathcal{H}}^2 = (p-1)^{-1}\Norm{f_{\tau}}_{\mathcal{H}}^2 = t_p^\beta
		\quad \text{and} \quad
		W_n(t, \mathcal{F}f_\tau) = W_n(t_p^\beta, \mathcal{F}f)
	\Norm{f}_{\mathcal{H}}^2,
	\end{align*}
	since then we could just apply Proposition \ref{P:Wnprop1} to the function $f_\tau$. Note above
	that we have not yet defined $\beta$ or $t_p$. We guess $f_\tau$ takes the form $f_\tau(x) =
	\tau^Vf(\tau^Wx)$. We now mention, through direct calculation one can show that
	$\Norm{f_\tau}_\mathcal{H}^2$ and $W_n(t,\mathcal{F}f_\tau)$ can be calculated as follows:
	\begin{equation}
	...
	\begin{align*}
		...
		\end{cases}.
	\end{align*}
	We need to now choose an appropriate $\tau$ and we choose $\tau = (p-1)t$ and so the above system
	...
	\begin{align*}
		...
		\end{cases}.
	\end{align*}
	Equating the exponents of either the $(p-1)$ term or the $t$ term both lead to the following
	equation
	\begin{equation}
	...
	\end{equation}
	then Proposition \ref{P:Wnprop2} is proven by applying Proposition \ref{P:Wnprop1} to the function
	\begin{align*}
