...
We mention that this setup has been studied in \cite{CHSS18} for the case of a time-dependent noise where the covariance function is given by 
\[
...
\end{cases}
\]
In addition, the variational constant is given by 
	\begin{align*}
...
with
	...
	\]
and there they proved the following:
	\begin{theorem}
Let $\alpha < d$ and let $p \ge 2$. Then
		\[
			\lim_{t\to\infty} t^{-\frac{2a - \alpha - \alpha\alpha_0}{a-\alpha}}\log\mathbb{E}[|u(t,x)|^p] = p(p-1)^{\frac{a}{a-\alpha}}\textbf{M}(a,\alpha_0,d,\gamma).
		\]
	\end{theorem}

We mention that setting $\alpha_0=0$ and letting $g(s,x)=g(x) \in \mathcal{F}_a$ be independent in $s$, which is the time-independent setup, then Equation \eqref{E:AsymStable-t} and Lemma \ref{L:EvsM} together recover the above theorem. 
\end{example}% }}}
