...
\begin{remark}
	...
	$\R^d$ (i.e., $a=2$, $b=1$, $\alpha=d$ and $r=0$), Hairer and Labb\'e constructed the pathwise
	solution using the regularity structure for both cases $d=2,3$ in \cite{HairerLabbe15SHE2} and
...
\begin{figure}[htb]% {{{ F:SWE
	...
	\ref{F:SHE} for more legends.}
	\label{F:SWE}
\end{figure}% }}}

\begin{example}[Fractional SHE with $r=1-b$]
	\label{Eg:FracSHE}
	For the fractional stochastic heat equation, i.e., the case when $b<1$ and $a\le 2$, many known
	works focus on the case when $r=1-b$; see, e,g., \cite{Chen17Tran,MijenaNane15SPA}. To facilitate
	the discussions here, we will only focus on the case when $a=2$. In particular, by setting $r=1-b$
	and $a=2$, conditions in \eqref{E:critical} become
	\begin{align}
		\label{E:CrFrSHE}
		\alpha =\frac{2}{b} \le d \quad  \text{and} \quad b\in [1/2,1].
	\end{align}
	When $b=1$, we have $r=0$ and the fundamental solutions are the standard heat kernel. Hence,
	Assumption \ref{A:Nonnegative} is satisfied for all $d\ge 1$. On the other hand, when $b<1$,
	sufficient conditions in Remark \ref{R:Nonnegative} guarantees Assumption \ref{A:Nonnegative} for
	all $d\ge 1$. The solvability for this case is illustrated in Figure \ref{F:FracSHE} and the
	critical time $T_p$ is equal to
	\begin{align}
		\label{E:TpFracSHE}
...
\begin{figure}[htpb]% {{{ F:FracSHE
	...
	\caption{Solvability for the fractional stochastic heat equation in case of $a=2$, $b\le 1$ and
	$r=1-b$. See Figure \ref{F:SHE} for the legends.}
	\label{F:FracSHE}
\end{figure}% }}}

\begin{example}[Fractional SHE with $r=0$]
	\label{Eg:FracSHE0}
	In this final example, we study the special case of the fractional stochastic heat equation when
	$b<1$, $a\le 2$ and $r=0$. The choice of $r=0$ has been used in, e.g., \cite{CHHH17}.  As in
	Example \ref{Eg:FracSHE}, we will only consider the case $a=2$. Now by setting $r=0$ and $a=2$ and
	restricting $b\le 1$, conditions in \eqref{E:critical} become
	\begin{align}
		\label{E:CrFrSHE}
		\alpha =4- \frac{2}{b} \le d \quad  \text{and} \quad b\in (0,1].
	\end{align}
	As in Example \ref{Eg:FracSHE}, Assumption \ref{A:Nonnegative} is satisfied for all $d\ge 1$.  The
	solvability for this case is illustrated in Figure \ref{F:FracSHE0} with $T_p$ given in
	\eqref{E:TpFracSHE}. In particular, for the example in the second figure in Figure
	\ref{F:FracSHE0}, namely, when $b=2/3$ and $\alpha=d=1$, the white noise driven SHE has a local
	solution with
	\begin{align}
...
\begin{figure}[htpb]% {{{ F:FracSHE
	...
	\end{tikzpicture}
	\caption{Solvability for the fractional stochastic heat equation in case of $a=2$, $b\le 1$ and
	$r=0$. See Figure \ref{F:SHE} for the legends.}
	\label{F:FracSHE0}
\end{figure}% }}}
