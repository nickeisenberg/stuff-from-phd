...
We need to introduce the following variational constant (see Section \ref{SS:Variation} for more details):
\begin{align*}
...
\end{align*}
where $\gamma$ is defined in \eqref{E:corfunction}. \bigskip
...
Before the end of this Introduction, let us make a few remarks and give some examples:
...
\begin{remark}[Local $L^p(\Omega)$ solutions]
	\label{R:CritcalSHE}
	(1 - SWE) By setting $a=2$ and formally setting $b=2$ in \eqref{E:critical}, we obtain
	the following condition for SWE under which there only exits a local solution:
	\begin{align}
		\label{E:CrSWE}
	...
	\end{align*}
	which says that
	at dimension $d=3$, when the noise $\W$ is white noise, there exits only a local $L^p(\Omega)$
	solution for all $p\ge 2$. Moreover, one can check easily that the expression for the critical
	time $T_p$ in \eqref{E:Tp} in this case reduces to
	\begin{align*}
		T_p=	\frac{\nu^{3 / 2}}{2\theta (p-1) \sqrt{\mathcal{M}_2(\delta_0)}},  \quad p\ge 2,
	\end{align*}
	which is identical to (1.12) ({\it ibid.}) when setting $\nu=2$. \bigskip

	\noindent (2 - SHE)	Now let us see what happens for SHE. By setting $a=2$ and $b=1$ in
	\eqref{E:critical}, we obtain the following condition for SHE under which there only exits a local
	solution:
	\begin{align}
		\label{E:CrSHE}
		\alpha = 2 + 4r \le d \quad \text{and} \quad r\in [0, 1/2].
	\end{align}
	Since at $b\le 1$, the sufficient conditions in Remark \ref{R:Nonnegative} guarantee the
	nonnegativity of the fundamental solution for all $d\ge 1$. Hence, the picture is more complicated
	in this case since we need to check all possible dimensions $d\ge 1$. We illustrate possible cases
	in Figure \ref{F:SHE}. In particular, let us explain a few cases:
	\begin{enumerate}[(a)]
		\item when $r=0$, conditions in \eqref{E:CrSHE} become $\alpha=2\le d$. Moreover, when $d=2$, this says that
			the $2$-dimensional SHE driven by white noise has only an local $L^p(\Omega)$ solution. In
			this case, the critical times $T_p$ become
			\begin{align}
				\label{E:Tp-SHE2}
				T_p := \dfrac{\nu}{2\theta (p-1) \mathcal{M}_2(\delta_0)}, \qquad p\ge 2.
			\end{align}
		\item When $r=0$ and $d\ge 3$, SHE driven by white noise no longer has any $L^2(\Omega)$
			solution. Local solutions exist only when $\alpha=2$.
		\item Similarly, the cases for other nonzero values of $r$ are illustrated in Figure
			\ref{F:SHE}.
	\end{enumerate}
	In both cases (b) and (c), the critical time $T_p$ takes the same expression as \eqref{E:Tp-SHE2}
	but one needs to replace $\delta_0$ by $\gamma$.
\end{remark}
...
\begin{figure}[htpb]
	...
	\qquad \qquad
	\begin{tikzpicture}[scale=0.9]
		\draw [thick,dashed] (2.5,0) -- (2.5,5.5) node [above] {$r=1/8$};
		\draw [->] (-0.3,0) -- (5.5,0) node [right] {$\alpha$};
		\draw [->] (0,-0.3) -- (0,5.5) node [above] {$d$};
		\foreach \x in {1,...,5}{
			\draw (\x,0.1)--++(0,-0.2) node [below] {$\x$};
	...
	\begin{tikzpicture}[scale=0.9]
		\draw [thick,dashed] (3,0) -- (3,5.5) node [above] {$r=1/4$};
		\draw [->] (-0.3,0) -- (5.5,0) node [right] {$\alpha$};
		\draw [->] (0,-0.3) -- (0,5.5) node [above] {$d$};
		\foreach \x in {1,...,5}{
			\draw (\x,0.1)--++(0,-0.2) node [below] {$\x$};
	...
	% Legendary below
	\begin{tikzpicture}[scale=0.9]
		\draw (-0.4,-1.5) rectangle +(6,3);
		\filldraw (0, 1) + (0.1, -0.05) rectangle +(1,0.05) node [right] {Global $L^p(\Omega)$-solution};
		\draw (0.5,0) circle (0.1); \node at (3,0) {Local $L^p(\Omega)$-solution};
		\draw (0, -1) + (0.1, -0.05) rectangle +(1,0.05) node [right] {No $L^2(\Omega)$-solution};
		\draw (6.5, 0) + (-0.15,-0.15) rectangle +(0.15,0.15); \node at (8,0) {White noise};
	\end{tikzpicture}
	\caption{Stochastic heat equation ($a=2$ and $b=1$) with $r=0,1/8,1/4, 1/2$ and $p\ge 2$.}
	\label{F:SHE}
\end{figure}
...
\begin{remark}[Nonnegativity]
	...
	Fig.  \ref{F:Nonnegative} for an illustration):
	\begin{enumerate}
...
In the following, we first give some preliminaries in Section \ref{S:Pre} including the {\it
...
\ref{T:solvable} --- is proven in Section \ref{S:solvable}. The upper bound and lower bound for
\eqref{E:momentasym} are established in Sections \ref{S:upperbd} and \ref{S:lowerbd}, respectively.
Finally, in the appendix --- Section \ref{S:Appendix}, we list a few proofs of the results in the
paper.
