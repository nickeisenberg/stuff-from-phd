...
In the following, the fundamental solution will exclusively refer to $G(t,x)$, which is indeed a
smooth function for $x\ne 0$ and sometimes nonnegative (see Remark \ref{R:Nonnegative} below).
\bigskip
...
Now let us state our main results of the paper. The first main result is about the well-posedness of
the equation \eqref{E:SPDE} as stated in the following theorem.

\begin{theorem}
	\label{T:solvable}
	Under Assumption \ref{A:Noise}, if
	\begin{equation}
		\label{E:alpha}
		\alpha < \min\left( \frac{a}{b}\left[2(b+r)-1\right], 2a, d \right),
	\end{equation}
	then \eqref{E:SPDE} has a unique solution $u(t,x)$ in $L^p(\Omega)$ for all $p \ge 2$, $t>0$, and $x
	\in \R^d$.
\end{theorem}
...
\end{align*}
see Section \ref{SS:LD} for more notation and details. For the lower bound of the asymptotics, due
to some technical reasons (see Section \ref{S:lowerbd}), we need to introduce one additional
assumption on the fundamental solution:

\begin{assumption}[Nonnegativity]
	\label{A:Nonnegative}
	Assume that the fundamental solution $G(t,\cdot)$ is nonnegative.
	% The parameters $a,b,r$ and the dimension $d$ must satisfy one of the following conditions:
\end{assumption}

Now we are ready to state the second main result as follows:

\begin{theorem}
	\label{T:asym}
	Under Assumption \ref{A:Noise}, if condition \eqref{E:alpha} holds, then we have that
	\begin{equation}
	...
	\end{align}
	Moreover, if in addition Assumption \ref{A:Nonnegative} holds, then
	\begin{equation}
...
\end{theorem}
\begin{proof}
	The matching upper and lower bounds in \eqref{E:momentasym-upper} and \eqref{E:momentasym-lower}
	are proved at the end of Sections \ref{S:upperbd} and \ref{S:lowerbd} below, respectively.
	\eqref{E:momentasym} is a direct consequence of  \eqref{E:momentasym-upper} and
	\eqref{E:momentasym-lower}.
\end{proof}
...
\begin{corollary}
	\label{C:Lim}
	Under both Assumptions \ref{A:Noise} and \ref{A:Nonnegative}, if condition \eqref{E:alpha} holds,
	then for any $p\ge 2$ and $t>0$ fixed, we have the following large time asymptotics
	\begin{align}
		...
		\end{aligned}
	\end{align}
	and the following large moment asymptotics:
	\begin{align}
...
\section{Upper bound of the asymptotics --- the proof of \eqref{E:momentasym-upper}}
\label{S:upperbd}

In this section, we will prove \eqref{E:momentasym-upper} by first establishing the corresponding
result for $p=2$ in Lemma \ref{L:p=2} and then applying the hypercontractivity property given by
Theorem B.1 in \cite{BCC21} to obtain the general case for $p\ge 2$.
...
\begin{proof}[Proof of \eqref{E:momentasym-upper}]
	...
	\end{align*}
	and an application of Lemma \ref{L:p=2} proves \eqref{E:momentasym-upper}.
\end{proof}
...
In this section, we will prove \eqref{E:momentasym-lower}. We start by defining the function
$W_n(t,\phi)$ on $(0,\infty) \times L_{\mathbb{C}}^2(\mu)$ by
\begin{equation}
...
\begin{proof}
	...
	\end{align*}
	see the proof of Theorem 4.1 of \cite{CHN19} for more details. 
