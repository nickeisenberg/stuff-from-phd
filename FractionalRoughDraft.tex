...
\begin{align}
	\label{E:VarConstE}
	\begin{aligned}
		\mathcal{E}_{a,d}(\gamma,\theta) & := \sup_{g \in \mathcal{F}_a} \left\{ \iint_{\R^{2d}} g^2(x)g^2(y) \gamma(x+y)\ud x\ud y - \frac{\theta}{2}\: \mathcal{E}_a(g,g) \right\} \\
...
\begin{lemma}
	\label{L:EvsM}
	Under Assumption \ref{A:Noise}, it holds that
	\begin{align}
		\label{E:EvsM}
		\mathcal{E}_{a,d}(\gamma,\theta) = ... \mathcal{M}_{a,d}(\gamma,\theta).
	\end{align}
\end{lemma}
\begin{proof}
	We first claim that for $g\in \mathcal{F}_a$,
	\begin{align}
		\label{E:FracSoblev}
	  \iint_{\R^{2d}} g^2(x)g^2(y) \gamma(x+y)\ud x\ud y \le C \mathcal{E}_a^2(g,g).
	\end{align}
	Follow the proof of Lemma 7.1 of \cite{BCR09} and use our estimate \eqref{E:(7.1)} to show this.

	Let $\kappa$ be the best constant in the inequality \eqref{E:FracSoblev}. Follow the arguments in
	the proof of Lemma A.2 of \cite{CHSX15AIHP} to write both $\mathcal{M}_{a,d}(\gamma,\theta)$ and
	$\mathcal{E}_{a,d}(\gamma,\theta)$ in terms of $\kappa$, from which one deduces the relation
	\eqref{E:EvsM}. Could you please feel in the details?
\end{proof}
