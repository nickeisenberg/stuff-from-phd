...
\begin{example}[Asymptotics for SHE with fractional Laplace and white noise]% {{{
	\label{Eg:AsymStable}
	In this example, we restrict us to the case when $b=1$, $a\in (1,2]$, $\alpha=d=1$, and $r=0$.
	This is the 1-dimensional SHE with fractional Laplace driven by white noise. In this case,
	\begin{align*}
		\beta = \frac{2a-1}{a-1} \qquad \text{and} \qquad t_p = (p-1)^{\frac{a}{2a-1}} t,
	\end{align*}
	and by \eqref{E:Mdelta1d} and Corollary \ref{C:Lim},
	\begin{align}
		\label{E:AsymStable-t}
		\lim_{t \to \infty} \frac{\log \E\left(|u(t,x)|^p\right)}{t^{\frac{2a-1}{a-1} }} =
		p (p-1)^{\frac{a}{a-1}}\left( \frac{1}{2} \right)\left( \frac{2a}{2a-1} \right)^{\frac{2a-1}{a-1}} \left[ \theta \nu^{-1/a} \mathcal{M}_{a,1}(\delta_0)^{\frac{2a-1}{a}} \right]^{ \frac{a}{a-1} } \frac{a-1}{a}
	\end{align}
	and
	\begin{align}
		\label{E:AsymStable-p}
		\lim_{p \to \infty} \frac{\log \E\left(|u(t,x)|^p\right)}{p^{\frac{2a-1}{a-1} }} =
		t^{\frac{2a-1}{a-1}}
		\left( \frac{1}{2} \right)\left( \frac{2a}{2a-1} \right)^{\frac{2a-1}{a-1}} \left[ \theta \nu^{-1/a} \mathcal{M}_{a,1}(\delta_0)^{\frac{2a-1}{a}} \right]^{ \frac{a}{a-1} } \frac{a-1}{a}.
	\end{align}
\end{example}% }}}
...
\begin{align}
	...
	\end{aligned}
\end{align}
By using the same techniques used to derive \eqref{E:ScaleM}, one can show that for any $\theta>0$ that 
	\begin{equation}\label{E:ScaleE}
			E_{a,d}(\gamma,\theta) =\left(\frac{1}{\theta} \right)^{1/(a-\alpha)} E_{a,d}(\gamma,1).
	\end{equation}
...
\begin{lemma}
	\label{L:EvsM}
	Under Assumption \ref{A:Noise}, the following three expressions holds:
		\begin{equation}\label{E:Erelation}
			E_{a,d}(\gamma,\theta) =\theta^{1/(a-\alpha)}\left(\frac{2\alpha}{a}\right)^{\alpha/(a-\alpha)}\frac{a-\alpha}{a}\sigma(d,\alpha)^{a/(a-2)}. 
		\end{equation}
		\begin{equation}\label{E:Mrelation}
			\mathcal{M}_{a,d}(\gamma,\theta) = \theta^{\alpha/(2a-\alpha)}\left( \frac{\alpha}{a}\right)^{\alpha/(2a-\alpha)} \frac{2a-\alpha}{2a}\sigma(d,\alpha)^{a/(2a-\alpha)}
		\end{equation}
		\begin{equation}	\label{E:EvsM}
				E_{a,d}(\gamma,\theta) = \left( \frac{a-\alpha}{a}\right)(2)^{\alpha/(a-\alpha)}\left( \frac{2a-\alpha}{2a} \right)^{-(2a-\alpha)/(a-2)} \theta^{(\alpha-1)/(a-\alpha)}\mathcal{M}_{a,d}(\gamma,\theta).
		\end{equation}
...
The proof of this result follows the scheme laid out in the proof of Lemma A.3 \cite{ChenAOP12}. By the same techniques presented in Lemma \ref{T:Variation}, one can show the following quantity is finite:
	...
	\end{align*}
First consider any $f \in \mathcal{F}_a$. We have $\Norm{f}_2 = 1$ and $\mathcal{E}_a < \infty$. Next we choose $C_f$ such that 
	\[
...
We only prove the case of $\theta=1$, the general case can be proven by applying the scaling properties \eqref{E:ScaleM} and \eqref{E:ScaleE}. 
