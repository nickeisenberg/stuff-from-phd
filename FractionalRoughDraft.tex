...
\begin{remark}[Critical dimensions]
	\label{R:CritcalSHE}
	(1 - SWE) By formally setting $a=b=2$ and $r=0$ in \eqref{E:critical}, we obtain that $\alpha=3\le
	d$, which recovers the critical dimension --- $d=3$ --- and the critical condition: $\alpha =3
	=d$, which implies the noise $\W$ is the spatial white noise. This recovers corresponding results
	in \cite{BCC21}. Moreover, one can check easily that the expression for the critical time $T_p$ in
	\eqref{E:Tp} in this case becomes
	\begin{align*}
		T_p:=	\frac{\nu^{3 / 2}}{2\theta (p-1) \sqrt{\mathcal{M}_2(\delta_0)}},
	\end{align*}
	which recovers that for SWE when $\nu=2$; see (1.12) ({\it ibid.}). \bigskip

	\noindent (2 - SHE)	By setting
	\begin{align*}
		a=2 \quad b=1 \quad \text{and} \quad r=0 \quad \text{in \eqref{E:critical}},
	\end{align*}
	we see that
	\begin{align*}
		\alpha = 4 \le d.
	\end{align*}
	Hence, the critical dimension for SHE is $d=4$ and the critical condition is $\alpha=d=4$, which
	implies, as in the case of SWE, the corresponding noise $\W$ the 4-dimensional spatial white
	noise. The critical times $T_p$ in this case become
	\begin{align*}
		T_p := \dfrac{\nu^{2}}{2 \theta (p-1)}, \qquad p\ge 2,
	\end{align*}
	which, rather surprisingly, do not depend on the variational constant $\mathcal{M}_2(\delta_0)$.
\end{remark}
...
\begin{definition}
	...
	with $0<s\le t <T$, $y\to G(t-s,x-y)u(s,y)$ is Skorohod integrable and the following integral
	equation holds
	\begin{equation}
		\label{E:IntEq}
		u(t,x) = 1 + \sqrt{\theta}\int_0^t \left( \int_{\R^d} G(t-s,x-y)u(s,y) W(\delta y) \right)\ud s.
	...
	\end{align}
	Otherwise, if \eqref{E:Lp} holds only for $t\in(0,T)$ with some finite $T>0$, we call $u(t,x)$
	a {\it local $L^p(\Omega)$-solution}.
