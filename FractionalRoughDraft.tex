...
\begin{proof}% {{{
  ...
  \end{align*}
  For any $f \in \mathcal{H}_+$ with $\Norm{f}_{\mathcal{H}}= 1$, by Bochner's theorem, $\mathcal{F}f$ is nonnegative
  and nonnegative definite, which further implies that $\mathcal{F}f$ is even. In addition, Lemma \ref{L:LWn} gives us
  that
  \begin{align*}
  ...
  \end{align*}
  Notice that the right hand side of the above equation takes the form as \cite[Equation (3.3)]{BCR09}. By the same
  arguments that follow in the proof of Theorem 2.3 {\it ibid} with the replacement \eqref{E:Changes}, we have that
  \begin{align*}
    \liminf_{n\to \infty}\frac{1}{n} \log \int_{\R^{dn}} \prod_{k=1}^n(\mathcal{F}f)(\xi_k) \prod_{k=1}^n\frac{1}{1+\frac{\nu}{2}|\xi_k+\cdots+\xi_n|^a} \mu(\ud \xi_1) \cdots \mu(\ud \xi_n) \ge  \log\widetilde{\rho}(\mathcal{F}f),
  \end{align*}
  where $\widetilde{\rho}(\cdot)$ is defined on the space of nonnegative, nonnegative definite, and
  $L^2\left(\mu(\ud\xi);\R^d\right)$--integrable functions is given by
  \begin{align} \label{E:rho'g}
    \widetilde{\rho}(g) &:= \sup_{\Norm{h}_{L^2(\R^d)}=1} \int_{\R^d}g(\xi)
    \left[
      \int_{\R^d} \frac{h(\xi + \eta)h(\eta)}{\sqrt{1+\frac{\nu}{2}|\xi+\eta|^a}\sqrt{1+\frac{\nu}{2}|\eta|^a}} \ud \eta
    \right]
    \mu(\ud \xi).
  \end{align}
  Before we proceed, we first make a few comments: \medskip

  \noindent (1) $g\in L^2\left(\mu\left(\ud \xi\right); \R^d\right)$ if and only if
  $\mathcal{F}^{-1}g \in \mathcal{H}$. Moreover,
  $\Norm{g}_{L^2(\mu)}=\Norm{\mathcal{F}^{-1}g}_{\mathcal{H}}$.

  \noindent (2) For $g\in L^2\left(\mu\left(\ud \xi\right); \R^d\right)$, $g$ is nonnegative
  definite if and only if $\mathcal{F}^{-1}g \in \mathcal{H}_+$.

  \noindent (3) For $g\in L^2\left(\mu\left(\ud \xi\right); \R^d\right)$, $g$ is nonnegative if and
  only if $\mathcal{F}^{-1}g$ is nonnegative definite.

  \noindent (4) Finding the best nonnegative and nonnegative-definite $g$ with $g$ with
  $\Norm{g}_{L^2(\mu)}=1$ to maximize $\widetilde{\rho}(g)$ is equivalent to finding the best
  nonnegative and nonnegative-definite $f\in \mathcal{H}_+$ with $\Norm{f}_{\mathcal{H}}=1$ to
  maximize $\widetilde{\rho}\left(\mathcal{F}f\right)$.

  \noindent (5) $\widetilde{\rho}(g)$ is well defined, namely, finite, because for any $h\in L^2(\R^d)$ with
  $\Norm{h}_{L^2(\R^d)}=1$,
  \begin{align*}
    & \left|\int_{\R^d}g(\xi)
    \left[
  ...
  Now we restrict $h$ to be nonnegative functions and define
  % We are interested in the quantity
  \[
    \widetilde{\rho}_+(g) := \mathop{\sup_{h \ge 0}}_{\Norm{h}_{L^2(\R^d)}=1} \int_{\R^d}g(\xi)
    \left[
      \int_{\R^d} \frac{h(\xi + \eta)h(\eta)}{\sqrt{1+\frac{\nu}{2}|\xi+\eta|^a}\sqrt{1+\frac{\nu}{2}|\eta|^a}}\ud\eta
    \right]
    \mu(\ud \xi).
  \]
  It is clearly that $\widetilde{\rho}(g) \ge \widetilde{\rho}_+(g)$. With this being said, we see
  that for any $f \in \mathcal{H}_+$ with $\Norm{f}_\mathcal{H} =1$,
  \begin{align*}
    \liminf_{n \to \infty} \frac{1}{n}\log \mathbb{E}(U_n(\tau)) \ge \log \widetilde{\rho}_+(\mathcal{F}f).
  \end{align*}
  We now need to calculate
  \begin{align*}
    \mathop{\sup_{f \in \mathcal{H}_+}}_{\Norm{f}_{\mathcal{H}}=1}
    \widetilde{\rho}_+\left(\mathcal{F}f\right)
    =
    \mathop{\sup_{h \ge 0}}_{\Norm{h}_{L^2(\R^d)}=1}
    \mathop{\sup_{f \in \mathcal{H}_+}}_{\Norm{f}_{\mathcal{H}}=1}
    \int_{\R^d}\mathcal{F} f(\xi)
    \left[
      \int_{\R^d} \frac{h(\xi + \eta)h(\eta)}{\sqrt{1+\frac{\nu}{2}|\xi+\eta|^a}\sqrt{1+\frac{\nu}{2}|\eta|^a}}\ud\eta
    \right]
    \mu(\ud \xi).
  \end{align*}
  Let $h$ be an arbitrary nonnegative function in $L^2(\R^d)$. The function
  $h(\cdot)/\sqrt{1+\frac{\nu}{2}|\cdot|^a}$ belongs to $L^2(\R^d)$ so that $g_h(x) := (2\pi)^{d/2}
  ...
  \begin{align} \notag
    ...
    \right]
    \mu(\ud \xi)
    = & (2\pi)^{-d} \int_{\R^d}\mathcal{F}f(\xi)(\mathcal{F}g_h*\widetilde{\mathcal{F}g_h})(\xi) \mu(\ud \xi).% \\ \label{E_:LBfg-a}
    % = & \int_{\R^d}\mathcal{F}f(\xi)\: \mathcal{F}[|g_h|^2](\xi) \mu(\ud \xi)                                 \\ \label{E_:LBfg-b}
    % = & \langle f, |g_h|^2 \rangle _{\mathcal{H}}.
  \end{align}
  Because $\mathcal{F}f (\cdot)\ge 0$ and $\mu$ is a nonnegative measure, the best $h$ should be the
  one that makes $\mathcal{F}g_h(\cdot)$ nonnegative, or equivalently, the best $h$ should be the
  nonnegative $L^2(\R^d)$ functions. Hence, $\widetilde{\rho}_{+}(\mathcal{F}f)=
  \widetilde{\rho}(\mathcal{F}f)$. Moreover, notice that
  \begin{align*}
        \widetilde{\rho}(\mathcal{F}f)
  ...
  \begin{align}
    \label{E:supf}
      \sup_{ f\in \mathcal{H}, \Norm{f}_{\mathcal{H}=1} } \widetilde{\rho}_+(\mathcal{F}f)
  ...
  \begin{align*}
    \sup_{f \in \mathcal{H},\: \Norm{f}_{\mathcal{H}}=1 } \widetilde{\rho}_+(\mathcal{F}f) =
    \sup_{f \in \mathcal{H}_+,\: \Norm{f}_{\mathcal{H}}=1 } \widetilde{\rho}_+(\mathcal{F}f) =
