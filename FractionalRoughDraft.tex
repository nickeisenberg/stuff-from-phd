...
\begin{example}[Asymptotics for SHE]% {{{
	...
	setting $\alpha_0=0$ and using \cite[Lemma A.2]{15AIHP} to rewrite the constant
...
\begin{example}[Asymptotics for SHE with fractional Laplacian]% {{{
	\label{Eg:AsymStable}
...
\begin{theorem}
	\label{T:Variation}
	For the correlation function $\gamma$, with $\alpha < 2a$, that also satisfies Assumption \ref{A:Noise}, both
...
\begin{proof}[Sketch of the proof of Theorem \ref{T:Variation}]
	...
	\begin{align}
		...
		\end{aligned}
	\end{align}
	where $\tilde{g}^2(x) = g(-x)$. Note that we must have $\alpha < 2a$ to ensure that the right hand side of the above is finite. This is seen by applying $h=\gamma$ in Lemma \ref{L:rhofinite}. Thus equation (7.1)  ({\it ibid.}) can be applied in our setting . The rest of the proof of Lemma 7.1 ({\it
...
In order to compare our results with known results, some which were presented in Example \ref{Eg:AsymStable}, let us introduce another commonly used
...
be established in a similar way as \cite[Lemma A.2]{15AIHP}, which is stated in the following
lemma:
...
\begin{lemma}
\label{L:gammaf2bdd}
	Under Assumption \ref{A:Noise}, for any $f \in L^2(\R^d)$ with $\mathcal{E}_a(f,f) < \infty$, it holds that
	\begin{equation}
		\label{E:SobIneq}
		\int_{\R^d} f^2(x)\gamma(x) \ud x \le C \Norm{f}_2^{2-(2\alpha/a)} \mathcal{E}_a(f,f)^{\alpha/a},
	\end{equation}
	where the constant $C$ only depends on $a, d$ and $\alpha$ with $\alpha < \min\{a,d\}$. Denote the best constant in
	\eqref{E:SobIneq} by $\sigma(a,d,\alpha)$.
\end{lemma}

\begin{proof}
	The proof of this result follows the scheme laid out in the proof of  \cite[Lemma A.3]{ChenAOP12}. By the same techniques presented above in Lemma \ref{T:Variation}, one can show the following quantity is finite:
	\begin{align*}
	...
	\end{equation}
	For arbitrary $f\in L^2(\R^2)$ with $\mathcal{E}_a(f,f) < \infty$ we apply \eqref{E:SobIneqNorm1} to $f/\Norm{f}_2$ and see that
	\begin{align*}
...
Then by applying a change of variables and recalling \eqref{E:SobIneq} we see that
	\[
		\int_{\R^d} \frac{f^2(x)}{|x+y|^\alpha} \ud x \le \sigma(a,d,\alpha) \Norm{f}_2^{2-(2\alpha/a)}\mathcal{E}_a(f,f)^{\alpha/a}
	\]
and in return,
	\[
		\sup_{y\in \R^d} \int_{\R^d} \frac{f^2(x)}{|x+y|^\alpha} \le \sigma(a,d,\alpha) 		\Norm{f}_2^{2-(2\alpha/a)}\mathcal{E}_a(f,f)^{\alpha/a}.
	\]
Next, notice that
	\begin{align*}
...
and when $f \in \mathcal{F}_a$, and thus $\Norm{f}_2 = 1$, we see that this reduces to
	\[
		\int_{\R^{2d}} \gamma(x+y)f^2(x)f^2(y) \ud x \ud y \le \sigma(a,d,\alpha) \mathcal{E}_a(f,f)^{\alpha/a}.
	\]
We note that the sharpness of $\sigma(a,d,\alpha)$ follows immediately from Lemma \ref{L:gammaf2bdd}. In addition, this reduces to equation (A.1) \cite{15AIHP} for the time independent case when $a=2$.
	\end{proof}
...
We have that
	\begin{align}
		E_{a,d}(\gamma,1) &\le \sup_{g\in \mathcal{F}_a} \left\{ \sigma(d,\alpha) \mathcal{E}_a(g,g)^{\alpha/a} - \frac{1}{2}\: (\mathcal{E}_a(g,g)^{1/a})^a \right\} \nonumber
...
and similarly
	...
	\end{align}
Recall Lemma \ref{L:gammaf2f2bdd} above and choose $0 < \epsilon < \sigma(a,d,\alpha)$ and let $f\in \mathcal{F}_a$ be such that
	\[
...
In this section, we will prove the first part of Theorem \ref{T:solvable}. The proof will need the following lemma:
...
\begin{proof}[Proof of Theorem \ref{T:solvable}]
	...
	\begin{align}
		H_n(t, \vec{x}) & = n! \int_{(\R^d)^n} \prod_{k=1}^n K(x_k - y_k) \widetilde{f_n}(\vec{y};0,t) \ud \vec{y} \nonumber\\
										& = \sum_{\sigma \in \Sigma_n} \int_{[0,t]^n <} \int_{(\R^d)^n} \prod_{k=1}^n K(x_k-y_k) 	\prod_{k=1}^n G(s_k - s_{k-1}, y_{\sigma(k)} - y_{\sigma(k-1)}) \ud \vec{y} \ud \vec{s} \label{E:H}
	\end{align}
	where $K$ is defined in \eqref{E:K}. Under the nonnegativity assumption  --- Assumption \ref{A:Nonnegative}, we see that for any $\vec{x}
	...
	where
	\[
		\mu(\ud \vec{\xi}) = \prod_{j=1}^n \mu(\ud \xi_j).
	\]
	...
	\begin{equation}
		...
		T_n(\nu, a) \le (n!)^2 C_\mu^n(\nu, a)
		\qquad \text{with} \quad
		C_\mu(\nu,a) := \int_{(\R^d)} \left(\frac{1}{1+\frac{\nu}{2}|\xi|^a}\right)^2 \mu(\ud \xi) =  C \int_0^\infty \frac{\rho^{\alpha-1}}{(1+\rho^{a})^2} \ud \rho<\infty.
	\end{equation}
	Therefore, the finiteness of $C_\mu(\nu,a)$ will require $\alpha < 2a$ which is included in \eqref{E:alpha}:
	\begin{align}
		\label{E:alpha2}
	...
	\end{align*}
	where the constant $C$ depends only on the value of $2(b+r)-b\alpha/a$ and is due to Stirling's formula (see \eqref{E:Stirling} below and also the proof of Theorem 3.1 \cite{BCC21}).  
	
	Because of the constant $1$ initial condition, $\Norm{u(t,x)}_2 =\Norm{u(t,0)}_2$ for $x \in \R^d$. Therefore, by
	...
	B.1]{BCC21} or \cite{Le16} for the case of the SHE) shows that for all $p\ge 2$,
	\begin{align}
		\nonumber
		\Norm{u(t,x)}_p & \le \sum_{n\ge 0} \theta^{n/2} (p-1)^{n/2} \sqrt{n!} \Norm{\widetilde{f_n}(\cdot,0,t) }_{ \mathcal{H}^{\otimes n} } \\
                    & \le   \sum_{n \ge 0} \theta^{n/2} C^{n/2} (p-1)^{n/2}  t^{[2(b + r ) -b \alpha /a]n/2} \dfrac{ \left(2^{ \left[ 2(b+r)- \frac{b \alpha}{a} \right]}C_\mu(\nu, a) \right)^{n/2} }{ (n!)^{\frac{1}{2}\left[ 2(b+r) - 1 - b\alpha/a \right]}} < \infty.
		\label{E:pmomBd}
	\end{align}
...
\begin{lemma}
	\label{L:2ineq}
	Assume Assumptions \ref{A:Nonnegative} and \ref{A:Noise} hold. Recall $\rho$ which is defined in \eqref{E:rho}.\\
...
\begin{proof}
	...
	\end{align*}
	and $H$ is defined in \eqref{E:H}. Notice that using the above notation, \eqref{E:inteq1} can be rewritten as
	\begin{align*}
	...
	\begin{align}
		\label{E:2lowbd}
		\frac{T_n(\nu,a)}{(n!)^2} \le \Gamma\left( \frac{[2(b + r ) -b \alpha / a]n}{2} +1 \right)^2 	\Norm{ \widetilde{f_n}(\cdot,0;1) }_{ \mathcal{H}^{\otimes n} }^2 .
	\end{align}
	Now, an application of both Stirling's formula for the Gamma function and the Legrande duplication formula (see Lemma \ref{L:Ster+LegApp} below) shows that, as $n\to \infty$,
	\begin{align}
		\label{E:GammaSq}
		\Gamma\left( \frac{[2(b + r ) -b \alpha / a]n}{2} +1 \right)^2  \sim \Gamma\left( [2(b + r ) -b \alpha / a]n +1 \right)2^{- [2(b + r ) -b \alpha / a]n}C_n,
	\end{align}
	...
	\end{align*}
	The reason for having a strict inequality above is our need to apply \eqref{E:2ineq-b} and Theorem \ref{T:Variation} later on in this proof. Putting these conditions together gives the conditions stated in \eqref{E:critical}.
	...
	hypercontractivity property (see \cite[Lemma B.1]{BCC21}), we have that
	\begin{align}
		\label{E:HyContr}
...
proof as that of Proposition 6.3 of \cite{BCC21}. We will not repeat it here.
\begin{proposition}
	\label{P:Wnprop1}
...
\begin{proof}
	...
	\begin{align*}
		\mathbb{E}(U_n(\tau)) = \int_0^\infty e^{-t} U_n(t) \ud t
...
\begin{proof}[Proof of \eqref{E:momentasym-lower}]
	...
	\begin{align*}
		\liminf_{t_p \to \infty} t_p^{-\beta} \log \Norm{u(t,x)}_p & \ge 	\liminf_{t_p \to \infty} \left( \frac{-\frac{1}{2}t_p^\beta k^2}{t_p^\beta} + \frac{1}{t_p^\beta}\log \left[ \sup_{f\in \mathcal{H}_+ \; \Norm{f}_{\mathcal{H}}=1} k^{n_t} \theta^{{n_t}/2} U_{n_t}(t_p^\beta, f) \right]  \right) \\
...
where in the last line we applied \eqref{E:greenscale}. Now,
	...
	\begin{align*}
     ...
     \end{align*}
    where the last line follows from a change of variables and recalling that $\mu(\ud \vec{\xi}) = \prod_{j=1}^n \varphi(\xi) \ud \xi$. Lastly,
     \begin{align*}
     	\int_0^\infty e^{-t} \Norm{ \widetilde{f}_n(\cdot,0,t) }_{\mathcal{H}^{\otimes n}}^2 \ud t 
     & = 2^{2n(b+r)}\int_0^\infty 2e^{-2t} \int_{\R^{nd}} \left| \mathcal{F}\widetilde{f_n}(\cdot,0;t)(\xi_1,\cdots,\xi_n) \right|^2 2^{\frac{-nbd}{a}} 2^{\frac{nb(d-\alpha)}{a}} \mu(\ud \vec{\xi})   \ud t \\
     ...
     & = \frac{2^{n(2(b+r) - b\alpha/a)}}{(n!)^2}  \int_0^\infty 2e^{-2t} \int_{\R^{nd}} H_n(t,\vec{x})^2 \ud \vec{x}   \ud t,
	\end{align*}
	This proves \eqref{E:fHn}.
\end{proof}

\begin{lemma}
	\label{L:Ster+LegApp}
	For any $n \in \mathbb{N}$ and $k>0$, the following asymptotic holds as $n \to \infty$:
		\[
			\Gamma\left( \frac{kn}{2} + 1 \right)^2 \sim 2^{-1}\sqrt{k} \; 2^{-kn}\sqrt{2\pi n} \; \Gamma(kn+1).
		\]
\end{lemma}
\begin{proof}
	We use the notation '$\sim$' to mean asymptotically equivalent. For example, $a(x) \sim b(x) \; \iff \; a(x)/b(x) \to 1$ as $x \to \infty$. The proof relies on two well know properties of the Gamma function:
		\begin{enumerate}
		\label{E:SterApp}
			\item Sterling's Formula: $\Gamma(x+1) \sim \sqrt{2 \pi x} (x/e)^x$ as $x \to \infty$,
			\item Legrende's duplication formula: $\Gamma(z)\Gamma(z + 1/2) = 2^{1-2z}\sqrt{\pi} \; \Gamma(2z)$ for $z \in \mathbb{C}$.
		\end{enumerate}
	First, by Sterling's formula we see that 
		\begin{equation}
		\label{E:LegApp}
			\Gamma\left( \frac{kn}{2} +1 \right) \sim \sqrt{ \pi k n}\left(\frac{kn}{2e}\right)^{kn/2} .
		\end{equation}
	Now by writing 
		\[
			\Gamma\left( \frac{kn}{2} +1 \right) = \Gamma\left( \frac{kn+1}{2} +\frac{1}{2} \right)
		\]
	and applying Legrende's duplication formula, we see that 
		\begin{equation}
			\Gamma\left( \frac{kn}{2} +1 \right)  = \frac{2^{-kn}\sqrt{\pi}  \;\Gamma(kn+1) }{\Gamma\left(\frac{kn+1}{2}\right)}.
		\end{equation}
	Lastly, by combining \eqref{E:SterApp} and \eqref{E:LegApp}, get that 	
		\begin{align}
				\Gamma\left( \frac{kn}{2} +1 \right)^2 & \sim \frac{ \sqrt{\pi kn} \left(\frac{kn}{2e}\right)^{kn/2} 2^{-kn} \sqrt{\pi} \Gamma(kn +1) }{ \sqrt{\pi kn - \pi} \left(\frac{kn-1}{2e}\right)^{(kn-1)/2} }
				\\& = \sqrt{\frac{\pi kn}{\pi kn -1}} \left(\frac{kn}{kn-1}\right)^{kn/2} \left(\frac{kn-1}{2e}\right)^{1/2} 2^{-kn} \sqrt{\pi} \; \Gamma(kn+1)
				\\& \sim \sqrt{e} \left(\frac{kn-1}{2e}\right)^{1/2} 2^{-kn} \sqrt{\pi} \; \Gamma(kn+1)
				\\& = 2^{-1} (kn-1)^{1/2} \; 2^{-kn} \sqrt{2\pi} \; \Gamma(kn+1)
				\\& \sim 2^{-1} \sqrt{k} \; 2^{-kn} \sqrt{2 \pi n} \; \Gamma(kn+1)
		\end{align}
