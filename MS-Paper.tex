...
We need this new set $\mthscr K^* (\Theta)$ to be relatively compact in $L^2_\rho$ but then we would need the operator $\bigg(G(\circ,\cdot)\star f\big(\cdot,q(\circ,\cdot)\big) \bigg)(\delta_0,x)$ to be compact so that $\mathscr K^*(\Theta)$ remains relatively compact. So we need to see what space this function $q$ needs to be in. Note that we want $ f\big(x,u(t,x)\big)$ to be a specific example of $f\big(x,q(t,x)\big)$. Maybe we want the following 
\begin{enumerate}
