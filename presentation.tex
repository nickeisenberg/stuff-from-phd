...
{Stochastic Seminar, November 2, 2021}
...
\begin{frame}[t]
\frametitle{The noise, $W$.}
Recalling the theorem mentioned above, we have a zero mean Gaussian processes,$W=\{ W(\phi) : \phi \in C_0^{\infty} \}$, with covariance given by $C(\cdot,\cdot)$. In other words,
	\begin{align*}
...
By replacing $\nu=2$ in the following, we recover (1.12) Balan, Chen and Chen:	
	...
	\begin{column}{0.5\textwidth}
		\begin{itemize}
			...
			\item It can be shown that $2 \le T_2 = \frac{1}{2\mathcal{M}_{2,2}(\delta_0)} \le 2\pi$
		\end{itemize}
\end{column}
\end{columns}
	...
	\]
	where $X_i(t)$ are all i.i.d. $d$-dimensional symmetric stable processes of index $ \beta = 2$, 
	\[
...
\end{theorem}
	\end{frame}
		...
		\frametitle{$\rho$ and the SWE}
	\begin{theorem}[X. Chen]
		Suppose $\tau_i$ for $1\le i \le n$ are exponential random variables with mean 1. Then,
		\[
...
It can be shown that 
	\begin{align*}
		\mathbb{E}[ \zeta ( [0,\tau_1]  &\times [0,\tau_2] )^n ] 
		\\ &= (n!)^2\int_{(\R^d)^2} \mathcal{F}\mathcal{L}\left[\tilde{f}_n(\circ_1,\cdots\circ_n,0;\cdot)\right]^2(\xi_1, \cdots,\xi_n,1) \mu(\ud \vec{\xi})
	\end{align*}
