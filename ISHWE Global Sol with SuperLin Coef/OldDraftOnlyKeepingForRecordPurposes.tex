...
where 
	\begin{equation}
	\label{D:rho}
		\rho := 2(\beta + \gamma) -1 - \beta/\alpha 
	\end{equation}
 and $0< \theta < (\Theta -1) \wedge 2$ and $\Theta$ is given in \eqref{E:DalangEquiv}.
	\end{proposition}

	\begin{proposition}
	\label{P:NormInc}
Suppose that $u_0=1$ and that $v_0 =0$ so that $u(t,x) = 1 + I(t,x)$. Let $a>0$, $\rho = 2(\beta + \gamma) -1 - \beta/\alpha$ and suppose that $0< \theta < (\Theta -1) \wedge 2$. Then for any $x,z \in \R$, $p\ge 2$ and for any $T>0$ with $t\in [0,T]$,
	\begin{equation}
	\label{E:NormInc_N(u)}
...
\textcolor{blue}{The holder continuity in the next proposition needs to be checked}
	...
	\begin{proposition}
	\label{P:Holder}
Suppose that $u_0$ and $v_0$ are as on Proposition \ref{P:NormInc} above. Then $u$ has a version, still denoted by $u$, that is $\eta_1$-H\"older continuous in time and $\eta_2$-H\"older continuous in space with $0<\eta_1 < \rho/2$ and $0<\eta_2 < \theta/2$. Moreover, if we consider
\[
a > \left( 8 L^2(\sigma)K^2\right)^{\alpha/(\beta(2\alpha-1)+\alpha(2\gamma -1))} \quad \text{and} \quad 	p \in \left[ 2, \frac{a^{(\beta(2\alpha-1)+\alpha(2\gamma-1))/\alpha}}{4L^2(\sigma)K^2} \right],
\]
where $K$ is the constant from Proposition \ref{P:N(u)Bdd}, then we have that 
\[
\mathbb{E}\left[ \sup_{t\in[0,T],\; x\in[-R,R]} |u(t,x)|^p \right] \le 2^{p-1} + C(p,T,R) \left[ \mathcal{M}_1^p + \mathcal{M}_2^p e^{apT} \left(2 +  \frac{c(\sigma)}{L(\sigma)}\right)^p\right].
\]
	\end{proposition}
...
Proposition \ref{P:NormInc} implies that 
		\begin{equation}
			\Norm{u(t,x) - u(r,z)}_p \le C(p,T)\left[ \mathcal{M}_1 + \mathcal{M}_2e^{aT}N_{a,p}(u) \right]\left(|t-r|^{\rho} + |x-z|^{\theta}\right)^{1/2}.
		\end{equation}
By Kolmogorov's continuity theorem, there exists a version of $u$, which we denote by $u$, that is $\eta_1$-H\"older continuous in time and $\eta_2$-H\"older continuous in space with with $0<\eta_1 < \rho/2$ and $0<\eta_2 < \theta/2$.
...
Now by taking the expectation of both sides and applying Proposition \ref{P:NormInc} gives us 
	\begin{align*}
			\mathbb{E}\left[ \sup_{t\in[0,T],\; x\in[-R,R]} |u(t,x)|^p \right] &\le  2^{p-1} + 2^{p-1}C(p,T)^p\left((2T)^{\rho/2} + (2R)^{\theta /2}\right)^p\left[ \mathcal{M}_1 + \mathcal{M}_2 e^{aT} N_{a,p}(u)\right]^p \\
			&= 2^{p-1} + C(p,T,R) \left[ \mathcal{M}_1^p + \mathcal{M}_2^p e^{apT} N_{a,p}(u)^p\right].
	\end{align*}
Now if we consider
	\[
		a > \left( 8 L^2(\sigma)K^2\right)^{\alpha/(\beta(2\alpha-1)+\alpha(2\gamma -1))} \quad \text{and} \quad 	p \in \left[ 2, \frac{a^{(\beta(2\alpha-1)+\alpha(2\gamma-1))/\alpha}}{4L^2(\sigma)K^2} \right],
	\]
where $K$ is the constant from Proposition \ref{P:N(u)Bdd}, then by Proposition \ref{P:N(u)Bdd}, we have that 
	\[
		\mathbb{E}\left[ \sup_{t\in[0,T],\; x\in[-R,R]} |u(t,x)|^p \right] \le 2^{p-1} + C(p,T,R) \left[ \mathcal{M}_1^p + \mathcal{M}_2^p e^{apT} \left(2 +  \frac{c(\sigma)}{L(\sigma)}\right)^p\right].
	\]
\end{proof}
...
Note that since $\sigma$ satisfies \eqref{C:SigSupLin}, then $\sigma_N$ is Lipschitz continuous with Lipschitz constant $L(\sigma_N) := \sigma_2 \ln(2N)^{\delta}$. In other words, 
	...
	\]
Hence we can apply Theorem \ref{T:SolveabilityGenLip} and see that there exists an unique solution of the truncated version of \eqref{E:SPDE}, which we denote by $u_N := \left\{ u_N(t,x) : (t,x) \in [0,T] \times \R \right\}$. In addition, Proposition \ref{P:Holder} implies that the solution has a version, still denoted by $u_N$, which is $\eta_1$-H\"older continuous in time and $\eta_2$-H\"older continuous in space  with $0<\eta_1 < \rho/2$ and $0<\eta_2 < \theta/2$.

We will now apply Proposition \ref{P:Holder} to find an upper bound on the $p$-norm of $u_N$. For this, consider 
	\[
		a > \left( 8 L^2(\sigma_N) K^2 \right)^{\alpha/(\beta(2\alpha-1)+\alpha(2\gamma-1))} \quad \text{and} \quad p \in \left[ 2, \frac{a^{ (\beta(2\alpha-1)+\alpha(2\gamma-1))/\alpha}}{4L^2(\sigma_N)K^2} \right].
	\] 
Then Proposition \ref{P:Holder} implies that 
	\begin{equation}
	\label{E:u_NMomBdd}
		\mathbb{E}\left(\sup_{t\in[0,T],\; x\in[-R,R]} |u_N(t,x)|^p\right) \le 2^{p-1} + C(P,T,R)\left[\mathcal{M}_1^p + \mathcal{M}_2^p(N) \exp(apT)\left(2+ \frac{\sigma_1}{L(\sigma_N)}\right)\right]
	\end{equation}
...
An application of \eqref{E:u_NMomBdd} gives 
	\begin{align*}
		N^{-p}\mathbb{E}\left( \sup_{t\in[0,T],\; x\in[-R,R]} |u_N(t,x)|^p \right) &\le N^{-p}\left(  2^{p-1} + C(p,T,R) \left[ \mathcal{M}_1^p + \mathcal{M}_2(N)^p \exp\{apT\} \left(2 +  \frac{\sigma_1}{L(\sigma_N)}\right)^p\right]\right) \\
		&= \frac{2^{p-1} + C(p,T,R)\mathcal{M}_1^p }{N^p} + \frac{C(p,T,R) \mathcal{M}_2(N)^p \exp\{apT\} \left(2 +  \frac{\sigma_1}{L(\sigma_N)}\right)^p }{ N^p }.
	\end{align*}
