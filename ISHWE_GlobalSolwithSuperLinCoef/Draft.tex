...
\textit{Upper Bound for $\Norm{Q_2^k}_{p/2}$.} We apply Lemma \ref{L:Y_phiParseval} with $\phi(y_2) = \Sigma_{x}^k(s,y_2)$ and $\psi(y_1) = \Sigma_{x,z}^k(s,y_1)$. Thus we have that up to a constant of $(2\pi)^{-d}$,
	\[
		Q_2^k = \int_0^t \ud s \int_{\R^d} \mu(\ud \zeta) \overline{ \mathcal{F}(\Sigma_{x}^k(s,\cdot)Y(t-s,\cdot))(\xi) }\mathcal{F}(\Sigma_{x,z} Y(t-s,\cdot))(\xi)[e^{-i\xi \cdot \zeta}-1],
	\]
where we recall that $\zeta = x - z$. 
\end{proof}
