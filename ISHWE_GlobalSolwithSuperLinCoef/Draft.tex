...
Now, by applying \cite[Theorem 1.6]{Podlubny99}, we see that there exists a $C>0$ depending only on $\beta$ and $\gamma$ such that 
 ...
 Another change of variables implies that 
  	...
  	\]
  \textcolor{blue}{typo? Recall that $\varphi(c \zeta) = c^{d-\lambda}\varphi(\zeta)$ and so when doing the substitution $\eta = \left(\frac{\nu}{2}t^{\beta} \right)^{1/\alpha} \zeta$ then we should get 
  		\begin{align*}
  			...
  			&=\left(\frac{\nu}{2}\right)^{\frac{\lambda - 2d}{\alpha}}\frac{C  t^{2(\beta + \gamma -1)-\frac{2d\beta - \lambda \beta}{\alpha}}}{ (2\pi)^d } \int_{\R^{d}}  \frac{ \left(\frac{\nu}{2}t^{\beta} \right)^{(-d+\lambda)/\alpha} \varphi\left( \eta\right)\ud \zeta }{\left(1+  \left|\eta \right|^\alpha \right)^2 \left(\frac{\nu}{2}t^{\beta} \right)^{d/\alpha} } 
  		\end{align*}
  	check this later
   }
...
\textit{Upper Bound for $\Norm{Q_2^k}_{p/2}$.} We apply Lemma \ref{L:Y_phiParseval} with $\phi(y_2) = \Sigma_{x}^k(s,y_2)$ and $\psi(y_1) = \Sigma_{x,z}^k(s,y_1)$. Thus we have that up to a constant of $(2\pi)^{-d}$,
	...
	\]
where we recall that $\xi = x - z$. We now apply the the following to bounds: $|e^{-i\xi \cdot \zeta}-1| \le C |\xi|^{\gamma} |\zeta|^{\gamma}$ for any $\gamma \in [0,1]$ and some $C>0$ and $\sqrt{ab} \le \frac{1}{2}(a+b)$ for and $a,b > 0$. Indeed, for some $\gamma \in [0,1]$,
	\begin{align*}
		|Q_2^k| & \le C \int_0^t \ud s \int_{\R^d} \mu(\ud \zeta) \left| \mathcal{F}\left(\Sigma_{x}^k(s,\cdot)Y(t-s,\cdot)\right)(\zeta)\right| \left|\mathcal{F}\left(\Sigma^k_{x,z} Y(t-s,\cdot)\right)(\zeta)\right| |\xi|^{\gamma} |\zeta|^{\gamma} \\
		&\le \int_0^t \ud s |\xi|^{\gamma} \left(\int_{\R^d} \mu(\ud \zeta) \left| \mathcal{F}\left(\Sigma_{x}^k(s,\cdot)Y(t-s,\cdot)\right)(\zeta)\right|^2 |\zeta|^{2\gamma} \right)^{1/2} \\
		& \qquad \qquad \times \left( \int_{\R^d} \mu(\ud \zeta)\left|\mathcal{F}\left(\Sigma^k_{x,z} Y(t-s,\cdot)\right)(\zeta)\right|^2 \right)^{1/2} \\
		& \le \frac{1}{2} \int_0^t \ud s |\xi|^{2\gamma} \int_{\R^d} \mu(\ud \zeta) \: |\zeta|^{2\gamma} \left| \mathcal{F}\left(\Sigma_{x}^k(s,\cdot)Y(t-s,\cdot)\right)(\zeta)\right|^2  \\
		& \qquad \qquad \times \int_{\R^d} \mu(\ud \zeta)\left|\mathcal{F}\left(\Sigma^k_{x,z}(s,\cdot) Y(t-s,\cdot)\right)(\zeta)\right|^2  \\
		& =: Q^k_{2,1} + Q^k_{2,2}.
	\end{align*}
As for $Q^k_{2,1}$,
	\begin{align*}
		\Norm{Q^k_{2,1}}_{p/2} &\le C |\xi|^{2\gamma} \sup_{(s,y) \in [0,T]\times \R^d} \Norm{\Sigma^k_{x,z}(s,y)}^2_{p} \int_0^t \ud s \int_{\R^d} \mu(\ud \zeta) \: |\zeta|^{2\gamma} \left| \mathcal{F}\left(Y(t-s,\cdot)\right)(\zeta)\right|^2 \\
		&\le C |\xi|^{2\gamma} \left[c(\sigma) + L(\sigma) \sup_{(s,y) \in [0,T]\times \R^d} \Norm{u(s,y)}_{p}\right]^2 \int_0^t \ud s \int_{\R^d} \mu(\ud \zeta) \: |\zeta|^{2\gamma} \left| \mathcal{F}\left(Y(t-s,\cdot)\right)(\zeta)\right|^2 
	\end{align*}
which follows from the global Lipschitz assumption on $\sigma$.
\end{proof}
