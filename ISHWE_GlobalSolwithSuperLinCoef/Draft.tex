...
Parseval's identity implies that 
	\begin{align*}
		\int_{\R^{d}} \ud x \; Y_{\phi}(t,x) (Y_{\psi}(s,\cdot)*f(\cdot + z)) (x) & = (2 \pi)^{-d}\int_{\R^{d}} \ud \xi \; \mathcal{F}(Y_{\phi}(t,\cdot))(\xi) \overline{\mathcal{F}((Y_{\psi}(s,\cdot))(\xi) \mathcal{F}(f(\cdot + x))} \\
		&= (2 \pi)^{-d}\int_{\R^{d}} \mu(\ud \xi) \; \mathcal{F}(Y_{\phi}(t,\cdot))(\xi) \overline{\mathcal{F}((Y_{\psi}(s,\cdot))(\xi)} e^{-ix\cdot\xi},
	\end{align*}
...
where
	\begin{equation}
	Q_i = \int_0^t \iint_{\R^{2d}} \ud y_1 \ud y_2 Y(t-s,y_1)Y(t-s,y_2) f(y_1-y_2) h_i(s, y_1, y_2).
	\end{equation}
Thus 
	\[
...
\textit{Upper bound for $Q_1$:} Using the Lipschitz property of $\sigma$, Minkowski's inequality and the triangle inequality gives is 
	\begin{align*}
	...
	\end{align*} 
where the second inequality above follows from Lemma \ref{L:intYfYbdd}.
\end{proof}
