...
We prove that $\{t < \tau_N\} \uparrow \Omega$ and then show that the solution to \eqref{E:SupLinSPDE} exists pathwise for $\omega \in \Omega$. We remind the reader that in the case of a globally Lipschitz diffusion term, one usually proves that the solution exists as a limit in $L^2(\Omega)$. Because of the stopping-time argument, existence in $L^2(\Omega)$ is no longer guaranteed. Moreover, because $L^2(\Omega)$ existence is no longer guaranteed, then the solution may no longer satisfy the standard Ito isometry. 
	...
	\section{Preliminary results for a globally Lipschitz $\sigma$}
...
\section{Some comments and current progress with a spatially colored noise}
Consider equation \ref{E:SupLinSPDE} driven by a noise that is white in time and colored space. In other words, $W$ is a $0$-mean Gaussian process indexed on the space of Schwartz functions with the following covariance structure:
	...
	\end{equation}
As per usual, the tempered measure $\Gamma$ is non-negative and nonnegative definite and satisfies the following three conditions:
...
\begin{remark}
	To the best of our knowledge, the only correlation function and spectral density that satisfies the conditions of Assumption \ref{A:NoiseAssum1} is the Riesz kernal with parameter $0<\Lambda<d $,
	\[
		f(x) = \frac{1}{|x|^{\Lambda}},
	\]
		and its fractional generalization,
	\[
		f(x) = \prod_{i=1}^k \frac{1}{|x_{(i)}|^{\Lambda_i}},
	\]
		for $0< \Lambda_i < d_i$ where $\sum_{i=1}^k d_i = d$ and $\sum_{i=1}^k \Lambda_i =: \Lambda \in (0,d)$ and $x_{(i)} = (x^i_1,\cdots,x^i_{d_i})$ where $x_{(i)}$ essentially partitions the coordinate $x=(x_1,\cdots, x_d)$. 
\end{remark}

\begin{assumption} \label{A:RieszKernel}
	Because of the above remark, we will assume that the correlation function is given by the Riesz kernel. In other words, for $\lambda \in (0,d)$:
	\[
	...
	\]
	Taking into consideration the first point from Assumption \ref{A:NoiseAssum1}, we must take 
	\[
		0 < d- \Lambda < d \quad \text{and} \quad d-\Lambda + 2 \alpha > d,
	\]
	and since $\Lambda \in (0,d)$, this implies that
	\[
		0 < \Lambda < d \quad \text{and} \quad \Lambda < 2\alpha,
	\]
	or in other words,
	\[
...
\noindent Under Assumption \ref{A:NoiseAssum1}, we may rewrite \eqref{E:Colored} as 
\[
...
or if we take $c(\sigma) = \sigma(0)$,
	...
	\begin{theorem}
	\label{T:ExisAndUniq}
	Suppose that $\lambda \beta / \alpha - 2(\beta + \gamma -1) < 1$ and in addition, suppose that $\lambda < 2 \alpha$. Then there exists a unique $L^2(\Omega)$-solution to \eqref{E:SupLinSPDE} driven by $\dot{F}$ such that 
	\[
...
\begin{remark}
We must strengthen Assumptions \ref{A:NoiseAssum1} and \ref{A:RieszKernel} because of the need to require $\lambda \beta / \alpha - 2(\beta + \gamma -1) < 1$ in Theorem \ref{T:ExisAndUniq}. More precisely, we require that 
	\[
		\lambda < \frac{2 \alpha}{\beta}(\beta + \gamma -1) + \frac{\alpha}{\beta} = \frac{\alpha}{\beta} \left[ 2(\beta + \gamma) - 1 \right].
	\]
Therefore, Assumption \ref{A:RieszKernel} to require that 
...
\subsection{Some moment bounds for a globally Lipschitz $\sigma$}
