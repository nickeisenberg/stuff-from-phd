...
We will now focus on proving some moment bounds that parallel those proved above in the space-time white noise case. 
	...
	\begin{proposition}{\cite[Proposition 4.4]{millet.sanz-sole:21:global}}
...
\begin{remark}
	Note that the additional assumption \eqref{A:RieszParamStrong} excludes the SHE, $\alpha = 2$, $\beta = 1$ and $\gamma=1$, as well as the popular case where $\gamma = 1 - \beta$.
\end{remark}

\begin{proposition}{ \cite[Proposition 4.8]{millet.sanz-sole:21:global}}
...
\begin{proof}
	...
	\end{align*} 
	where the second inequality above follows from Lemma \ref{L:YfYbdd} and the last line requires $2(\beta + \gamma -1) - \beta \lambda / \alpha > 0$. 
	...
	& =: Q^k_{2,1} + Q^k_{2,2}.
	\end{align*}
	As for $Q^k_{2,1}$\footnote{There is a comment made in \cite[Proposition 4.8]{millet.sanz-sole:21:global} and \cite[Theorem 3.2]{hu.huang.ea:14:on} that minkowsi inequality is being used for the first inequality but it seems incorrect how they apply minkowski's and also unnecessary.},
	\begin{align*}
