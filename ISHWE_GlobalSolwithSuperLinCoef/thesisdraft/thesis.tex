...
Here is the main result of this chapter. The proof will be postponed until Section \ref{S:TMainSupLin} below.
	...
	Suppose that $\sigma$ satisfies \eqref{C:SigSupLin} and the initial data are Borel measurable functions such that for any $T>0$, $\widehat{C}_T < \infty$. Then for any $R>0$ and $(t,x) \in [0,T] \times \bar{B}_R(0)$, there exists a random field solution to \eqref{E:SupLinSPDE} denoted as $(u(t,x): (t,x) \in [0,T] \times \bar{B}_R(0))$. This solution is unique and satisfies 
	\begin{equation}
...
We prove that $\{t < \tau_N\} \uparrow \Omega$ and then show that the solution to \eqref{E:SupLinSPDE} exists pathwise for $\omega \in \Omega$. We remind the reader that in the case of a globally Lipschitz diffusion term, one usually proves that the solution exists as a limit in $L^2(\Omega)$. Because of the stopping-time argument, existence in $L^2(\Omega)$ is no longer guaranteed. Moreover, because $L^2(\Omega)$ existence is no longer guaranteed, then the solution may no longer satisfy the standard Ito isometry. 
	...
	\section{Globally Lipschitz Coefficient}
	...
	\] 
	where $c(\sigma) = \sigma(0)$ and $L(\sigma)$ is the Lipschitz constant of $\sigma$. We assume that $L(\sigma)>0$. To establish the existence and uniqueness of random field solutions in the to \eqref{E:SupLinSPDE}, we will need to assume Dalang's condition:
	\begin{equation}
	\label{C:Dalang}
...
We now state an existence and uniqueness result proven in \cite{chen.hu.ea:19:nonlinear}.
...
In this section, we will proves some moment bounds and a continuity result for the solution with a globally Lipschitz diffusion term. In the proof of the next proposition, we will use the following two facts:
	\begin{equation}
