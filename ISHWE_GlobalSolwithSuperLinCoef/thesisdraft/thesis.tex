...
The stochastic integral above is a {\em Walsh integral} (e.g. see \cite{walsh:86:introduction}) and the fundamental solution is a triple, $\{Z, Z^*, Y\}$, and each member of the triple is defined through the Fox-H function, however, one can more compactly define them through their Fourier transforms \cite[Theorem 4.1]{chen.hu.ea:19:nonlinear}: 
	...
	\end{align*}
where we choose $\mathcal{F}f(\xi) = \int_{\R^d} \exp(-i x \cdot \xi) f(x) \ud x$ as the definition of the Fourier transform, $E_{a, b}(z)$ denotes the {\em two-parameter Mittag-Leffler function} \cite{podlubny:99:fractional}:
	\[
...
\quad \text{and} \quad
	...
	\begin{proof}
		...
		\end{align*}
		where above we use the symbol $\sim$ to mean asymptotically equivalent as $N \to \infty$. This completes the proof of the Theorem \ref{T:MainSupLin}. 
	\end{proof}
