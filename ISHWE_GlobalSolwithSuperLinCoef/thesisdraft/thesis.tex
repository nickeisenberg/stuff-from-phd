...
\tableofcontents
	...
	\end{cases}
	\end{equation}
	where the fractional differential operators $\partial^\beta_t$, $(-\Delta)^{\alpha/2}$ and $I^\gamma_t$ respectively denote the {\em Caputo derivative}, the {\em fractional Laplacian} and the {\em Riemann-Liouville fractional integral operator}. The noise, $\dot{W}$, is a centered Gaussian noise that is taken to be white in time and colored in space. In other words, $W$ is a $0$-mean Gaussian processes on the space of Schwartz functions, $\mathscr{S}(\R^{d+1})$, with the following covariance functional:
		\[
		...
		\]
	where $\delta_0$ denotes the {\em Dirac delta distribution}. In addition, some comments will be made in Section \ref{S:colorednoise} below about the current progress with a colored noise. 
	
	The initial data, $u_0$ and $v_0$, are assumed to be Borel measurable functions which are Bounded on $\R^d$. The nonlinearity, $\sigma$, is {\em locally Lipschitz} in the sense that the following asymptotic relation holds as $|x|,|z| \to \infty$:
	\begin{equation}
	\label{C:SigSupLin}
...
The stochastic integral above is a {\em Walsh integral} (e.g. see \cite{walsh:86:introduction}) and the fundamental solution consists of a triple, $\{Z, Z^*, Y\}$, and each member of the triple is defined through {\em Fox-H functions}, however, one can more compactly define them through their Fourier transforms \cite[Theorem 4.1]{chen.hu.ea:19:nonlinear}: 
	\begin{align*}
	...
	\end{align*}
where we use $\mathcal{F}f(\xi) = \int_{\R^d} \exp(-i x \cdot \xi) f(x) \ud x$ to denote the Fourier transform, $E_{a, b}(z)$ denotes the {\em two-parameter Mittag-Leffler function} \cite{podlubny:99:fractional}:
	\[
...
Here is the main result of this chapter. The proof will be postponed until Section \ref{S:TMainSupLin} below.
	...
	Suppose that $\sigma$ satisfies \eqref{C:SigSupLin} and the initial data, $u_0$ and $v_0$, are Borel measurable functions and satisfy for all $T>0$,
		\[
			\sup_{(t, x) \in [0,T] \times \R^d} |J_0(t,x)| < \infty,
		\]
	 which is true if $u_0$ and $v_0$ are bounded. Then for any $R>0$ and $(t,x) \in [0,T] \times \bar{B}_R(0)$, where $\bar{B}_R(0)$ denotes the closed $d$-dimensional ball centered around the origin, there exists a random field solution to \eqref{E:SupLinSPDE} denoted as $(u(t,x): (t,x) \in [0,T] \times \bar{B}_R(0))$. This solution is unique and satisfies 
	\begin{equation}
	\sup_{t\in[0,T],\; |x| \le R} |u(t,x)| < \infty, \quad \text{almost surely.}
	\end{equation}
\end{theorem}

The proof of Theorem \ref{T:MainSupLin} uses a standard stopping time argument along with a series of truncations of the super-linear term, $\sigma$, in order to construct a solution of \eqref{E:SupLinSPDE}. Indeed, we define for $ N >0$,
	\[
...
Note that when $d = 1$, $\sigma_N$ reduces to the following:
	...
	\]
Then we associate for each $N>0$, the corresponding global solution to \eqref{E:SupLinSPDE} with $\sigma$ replaced my $\sigma_N$ as $u_N(t,x)$, whose existence follows due to Theorem \ref{T:SolveabilityGenLip} below. We then define the stopping time $\tau_N$ as follows:
\[
\tau_N := \inf \left\{ t>0 : \sup_{|x| \le R} |u_N(t,x)| \ge N \right\} \wedge T,
\]
and prove that $\{t < \tau_N\} \uparrow \Omega$ and then show that the solution to \eqref{E:SupLinSPDE} exists pathwise for $\omega \in \Omega$. We remind the reader that in the case of a globally Lipschitz diffusion term, one usually proves that the solution exists as a limit in $L^2(\Omega)$. However, the stopping-time argument applied here will no longer guarantee the $L^2(\Omega)$ existence of the solution. Moreover, because $L^2(\Omega)$ existence is no longer guaranteed, the solution may no longer satisfy the standard \Ito isometry. 
...
\quad \text{and} \quad
	...
	\begin{proof}
		We first solve a {\it truncated} version of \eqref{E:SupLinSPDE}. By this, we mean $\eqref{E:SupLinSPDE}$ with $\sigma$ replaced by $\sigma_N$ where for any $N >0$,
		\[
			\sigma_N(x) = \sigma\left( \left(1 \wedge \frac{N}{|x|}\right) x \right).
		\]
		...
		where $\sigma_1$ is chosen so that 
			...
			\]
		Hence we can apply Theorem \ref{T:SolveabilityGenLip} and see that there exists an unique solution of the truncated version of \eqref{E:SupLinSPDE}, which we denote by $u_N := \left\{ u_N(t,x) : (t,x) \in [0,T] \times \R^d \right\}$. In addition, Proposition \ref{P:Holder} implies that the solution has a version, still denoted by $u_N$, which is $\eta_1$-H\"older continuous in time and $\eta_2$-H\"older continuous in space  with $0<\eta_1 < q/2$ and $0<\eta_2 < \theta/2$.
		...
		We now momentarily assume that $\sup_N \tau_N = T$, which would imply that $\{t < \tau_N \} \uparrow \Omega$. On each $\{ t < \tau_N \}$, define $(u(t,x) : (t,x) \in [0,T) \times \R^d)$ by $u(t,x) = u_N(t,x)$ and hence $u(t,x) = u_{N+k}(t,x)$ for any $k\in \mathbb{N}$. This implies that on $\{ t < \tau_N \}$ 
		\[
		u(t,x) = J_0(t,x) + \int_0^t \ud s \int_{\R^d} \ud y \; Y(t-s,x-y) \sigma_N(u(s,y)) W(\ud s, \ud y).
		\]
		...
		\end{equation}
		See the proof of \cite[Theorem 3.5]{millet.sanz-sole:21:global} for a comment pertaining to the issue that the solution no longer exists in $L^2(\Omega)$.  
		
		We now focus on proving that $\sup_N \tau_N = T$, or equivalently that $ \mathbb{P}\left(\tau_N <T\right) \to 0$ as $N \to \infty$. To do this, note that by Chebychev's inequality that
		\begin{align*}
		...
		\end{align*}
		where above we use the symbol $\sim$ to mean asymptotically equivalent as $N \to \infty$. This completes the proof of Theorem \ref{T:MainSupLin}. 
	\end{proof}

\section{Some comments and current progress on a spatially colored noise} \label{S:colorednoise}
Consider Equation \eqref{E:SupLinSPDE} driven by a noise that is white in time and colored space. In other words, $W$ is a centered Gaussian process indexed by the space of Schwartz functions with the following covariance structure:
...
We now impose some assumptions on $\Gamma$:
	...
	\begin{enumerate}
		\item $\Gamma$ is absolutely continuous with respect to the Lebesgue measure and therefore has a density, $\Gamma(\ud x) = f(x) \ud x$. Furthermore, the spectral measure, $\mu$, has density $\varphi$, namely $\mu(\ud \zeta) = \varphi(\zeta) \ud \zeta$ where we have the relation $\varphi = \mathcal{F}f$.
...
We now prove a lemma which is a generalization of equation \eqref{C:Dalang} which accounts for the colored noise.
	...
	\begin{lemma}
	\label{L:YfYbdd}
	Suppose Assumption \ref{A:RieszKernel} holds. Then there exists a constant $C(\beta,\gamma) =: C >0$ such that for any $t>0$ and $x,y \in \R^d$,
	\begin{equation}
	\label{E:YYfBdd}
...
or if we take $c(\sigma) = \sigma(0)$,
	...
	\begin{theorem}
	\label{T:ExisAndUniq}
	 Suppose that Assumption \ref{A:RieszKernel} holds and that $\sigma$ is globally Lipschitz. In addition, suppose the initial conditions are such that 
	 \[
	 	\sup_{t\ge 0, \: x \in \R^d} |J_0(t,x)| < \infty.
	 \] 
	 Then there exists a unique $L^2(\Omega)$-solution to \eqref{E:SupLinSPDE} driven by $\dot{W}$ as in \eqref{E:Colored} such that 
	\[
...
\begin{proof}
	We note that $\mathcal{N}_{a,p}(J_0)$ has the same upper bound as in Proposition \ref{P:N(u)Bdd} above. In other words,
	\[
	...
	We now work to find an upper bound for $\mathcal{N}_{a,p}(I)$. The Burkholder-Davies-Gundy inequality and H\"older's inequality imply that 
	\begin{align*}
