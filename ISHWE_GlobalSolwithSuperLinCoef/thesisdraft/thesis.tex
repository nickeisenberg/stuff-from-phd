...
or if we take $c(\sigma) = \sigma(0)$,
	...
	\begin{theorem}
	\label{T:ExisAndUniq}
	 Suppose that Assumption \ref{A:RieszKernel} holds, $\sigma$ is globally Lipschitz and that $\lambda \beta / \alpha - 2(\beta + \gamma -1) < 1$. Then there exists a unique $L^2(\Omega)$-solution to \eqref{E:SupLinSPDE} driven by $\dot{F}$ such that 
	\[
...
\begin{remark}
We must strengthen Assumption \ref{A:RieszKernel} because of the need to require $\lambda \beta / \alpha - 2(\beta + \gamma -1) < 1$ in Theorem \ref{T:ExisAndUniq}. More precisely, we require that 
	\[
		\lambda < \frac{2 \alpha}{\beta}(\beta + \gamma -1) + \frac{\alpha}{\beta} = \frac{\alpha}{\beta} \left[ 2(\beta + \gamma) - 1 \right].
	\]
Therefore, this and Assumption \ref{A:RieszKernel} requires us to assume that 
	\begin{equation} \label{A:RieszParam}
		\Lambda < \min\left\{d, 2\alpha, \frac{\alpha}{\beta} \left[2(\beta + \gamma) -1\right] \right\},
	\end{equation}
which is the same condition that was assumed in \cite[Theorem 1.6]{chen.eisenberg:22:interpolating} where they studied \eqref{E:SupLinSPDE} but with a time-independent noise.
\end{remark}
