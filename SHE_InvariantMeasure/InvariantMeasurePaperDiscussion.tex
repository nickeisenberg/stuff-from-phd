...
and 
	...
	v(t,x) =
	\begin{cases}
	\dfrac{\partial v}{\partial t}(t,x)-\dfrac{1}{2}\Delta v(t,x) = b(x,v(t,x))\dot W(t,x) & \text{$x\in \R^d$ , $t>0$}, \\
	u(0,\cdot) = u(t_0,\cdot)       .                                                      &
	\end{cases}
	\end{equation}
We want it to be the case that $v(t,x) = u(t+t_0,x)$. Is the following true?
	\[
		v(t,x) = \int_{\R^d} G(t,x-y) u(t_0,y) \ud y + \int_0^t \int_{\R^d} G(t-s,x-y) b(y,v(s,y)) W(\ud s, \ud y)?
	\]
If so then we would like to have 
	\begin{align}
		v(t,x) &= \int_{\R^d} G(t,x-y) v(0,y) \ud y + \int_0^t \int_{\R^d} G(t-s,x-y) b(y,v(s,y)) W(\ud s, \ud y) \nonumber \\ 
		&= \int_{\R^d} G(t,x-y) u(t_0,y) \ud y + \int_0^t \int_{\R^d} G(t-s,x-y) b(y,u(s+t_0,y)) W(\ud s, \ud y) \label{E:vsol}
	\end{align}
and also we would like to have 
	\begin{equation}
	\label{E:vsolrestart}
		v(t,x) = \int_{\R^d} G(t+t_0,x-y) \mu(\ud y) + \int_0^{t+t_0} \int_{\R^d} G(t+t_0-s,x-y) b(y,u(s,y)) W(\ud s, \ud y).
	\end{equation}
The first integral in \eqref{E:vsol} can be shown using Fubini's theorem to be written as
	\[
		\int_{\R^d} G(t,x-y) u(t_0,y) \ud y = J(t+t_0,x) + \int_0^{t_0}\int_{\R^d} G(t+t_0-s, x-y)b(y,u(s,y)) W(\ud s, \ud y).
	\]
Then \eqref{E:vsol} and \eqref{E:vsolrestart} would need to imply that
	\begin{align}
		\int_0^{t_0}\int_{\R^d} G(t+t_0-s, x-y) & b(y,u(s,y)) W(\ud s, \ud y) + \int_0^t \int_{\R^d} G(t-s,x-y) b(y,u(s+t_0,y)) W(\ud s, \ud y) \\
		&  = \int_0^{t+t_0}\int_{\R^d} G(t+t_0-s, x-y)b(y,u(s,y)) W(\ud s, \ud y) \label{E:ChgOfVar}.
	\end{align}

I have been unable to show in what sense the above equality could be true. 
