...
Unfortunately, we may not consider the Riesz kernel in our setting due to the assumption that $\Upsilon(0) < \infty$. Indeed, for $0<s<d$, the Riesz kernel is defined to be $f(x) = |x|^{s-d}$ and in order for 
	\[
		...
		\textcolor{magenta}{\int_{\R^d} \frac{1}{|x|^{2d-s-2}} < 0},
	\]
...
where, 
	\[
	  \mathcal{F}G_s(\xi) = \frac{1}{(1+ \textcolor{magenta}{4\pi^2} |\xi|^2)^{s/2}}.
	\]
	Moreover, it can be shown (see \textcolor{magenta}{\cite[Proposition
	6.1.5]{Gra14} -- I cannot find this reference}) that
	\begin{enumerate}
