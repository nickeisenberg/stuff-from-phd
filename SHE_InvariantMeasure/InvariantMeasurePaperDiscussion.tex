...
and this holds when 
	\begin{equation}
		4\alpha + d -2 < s.
	\end{equation}
Recall that we are only considering $\alpha \in (0, 1/2)$. Because of this, the following interval is nonempty:
	\begin{equation}
		(4\alpha + d -2 , d).
	\end{equation}
Thus we may choose 
...
First, for the $\delta_0$ initial condition we have that,	
	\begin{align*}
		\mathcal{G}_{\widehat{\rho}} (t) &= \int_{\R^d} \ud x \; \left[\int_{\R^d} G(t,x-y) \delta_0(\ud y)  \right]^2 \hat{\rho}(x) \\
		&= \int_{\R^d} \ud x \; G(t,x)^2 \hat{\rho}(x) \\
		&= \frac{1}{(2 \pi t)^d} \int_{\mathbb R^d} \exp\left(- |x|^2/t\right) \hat\rho(x) \ud x.
	\end{align*}
We now apply the admissible weight property,
	\begin{align*}
		\frac{1}{(2 \pi t)^d} \int_{\mathbb R^d} \exp\left(- |x|^2/t\right) \hat\rho(x) \ud x &= \frac{(\pi t)^{d/2} }{(2\pi t)^d} (G(t/2, \cdot) * \hat{\rho})(0) \\
		&\le C(T) t^{-d/2} \hat{\rho}(0), \quad \forall \; 0 \le t \le T.
	\end{align*}
\textcolor{blue}{
Hence,	
	\begin{equation} \label{E:Gorigin}
			\mathcal{G}_{\widehat{\rho}} (t) = O\left( t^{-d/2} \right) , \quad t \to 0.
	\end{equation}
This is the mistake.}
...
Recall that because of \eqref{E:BesParamCond}, we have $2 - 4\alpha + s -d > 0$. Putting \eqref{E:Gorigin} and \eqref{E:Horigin} together gives us 
	\begin{equation}
				\mathcal{G}_{\widehat{\rho}} (t)\mathcal{H}_\alpha(t) = O\left( t^{(2-4\alpha-d)/2} + t^{(2-4\alpha+s-2d)/2}\right), \quad t \to 0.
	\end{equation}
Since we assume $\alpha \in (0,1/2)$, we have that $2 - 4\alpha >0$, and again recall that $2 - 4\alpha + s -d >0$. Then
	\[
		\frac{2-4\alpha-d}{2} \ge -d, \quad \text{and} \quad \frac{2-4\alpha+s-2d}{2} \ge -d,
	\]
which implies that 
	\[
		\int_0^T \mathcal{G}_{\widehat{\rho}} (t)\mathcal{H}_\alpha(t) \ud t < \infty, \quad \forall T>0.
	\]
\textcolor{blue}{I made a mistake above in blue... When applying the admissible weight property, we get that $\mathcal{G}_{\rho} \le O(t^{-d/2})$. This does not imply that $\int_0^T \mathcal{G}_{\rho}(t) \ud t < \infty$. I didn't realize that this was a $1-d$ integral... 
}
\subsection{Summary}
Suppose $d \ge 3$. If we consider the $\delta_0$ initial and suppose that the correlation function of the noise is defined to be the Bessel kernel, $G_s(x)$, where $s \in (4\alpha + d -2 , d)$, then we have the following:
	\begin{enumerate}
		\item $\mathcal{G}_\rho(t) := \int_{\R^d} \left(\int_{\R^d} G(t, x-y) \delta_0(\ud y)\right)^2 \rho(x) \ud x < \infty, \quad \forall t>0$,
		\item $\limsup_{t \to \infty} \mathcal{G}_\rho(t) < \infty$,
		\item for any $\alpha \in (0,1/2)$ we have that $\mathcal{H}_{\alpha}(t)< \infty$ for any $t>0$,
		\item for any $q>2$ and $\alpha \in (q^{-1}, 2^{-1})$ we have that
			\[
				\int_0^T \mathcal{G}_{\widehat{\rho}} (t)\mathcal{H}_\alpha(t) \ud t < \infty, \quad \forall T>0.
			\]
	\end{enumerate}
Hence the main results in the paper will apply.
