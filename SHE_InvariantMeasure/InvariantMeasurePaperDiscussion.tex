...
\subsubsection{Finiteness of $\int_0^T \mathcal{G}_{\hat{\rho}}(t)\mathcal{H}_{\alpha}(t) \ud t$ for the $\delta_0(\cdot)$ initial condition}
...
I don't think there is any room. Consider the first condition in \eqref{E:OriginProblem}. This says that we must have 
	...
	\]
But we must require that $d \ge 3$. Thus $	\int_0^T \left(\mathcal{G}_{\rho}(t) \mathcal{H}_{\alpha}(t)\right)^{q/2}$ will not converge if we consider the Bessel kernel, $G_s$ with $s < d$ and the $\delta_0$ initial condition.
