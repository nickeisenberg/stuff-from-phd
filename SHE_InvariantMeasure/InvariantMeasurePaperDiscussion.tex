...
Hence, we just need to show the finiteness of \eqref{E:FacBdd}. We start with looking at $\mathcal{G}_{\rho, q}$. 
	\begin{align*}
			...
			&\le 2 \Norm{\rho}_{L^1(\R^d)} + \int_{\R^d} \left(\int_{\R^d} G(s,x-y) \Norm{v(0,y)}_q\ud y\right)^2 \rho(x) \ud x.
	\end{align*}
Recall that $u(t_0,x) \stackrel{\mathcal{L}}{=} v(0,x)$. Then by the moment bounds we have 
	\begin{align*}
		...
		&\le 2C\Norm{\rho}_{L^1(\R^d)} +  C\int_{\R^d} (G(t_0+s,\cdot)*\delta_0)(x)^2 \rho(x) \ud x \\
		&= 2C\Norm{\rho}_{L^1(\R^d)} +  C\int_{\R^d} (G(t_0+s, x))^2 \rho(x) \ud x \\
		&\le 2C\Norm{\rho}_{L^1(\R^d)} + \frac{C}{(t_0 + s)^{d/2}} \int_{\R^d} G\left(\frac{t_0 + s}{2}, x\right) \rho(x) \ud x.
	\end{align*}
Putting this all together gives us 
	\begin{align*}
		\mathcal{G}_{\rho, q}(s) \le 4\Norm{\rho}_{L^1(\R^d)} + \frac{C}{(t_0 + s)^{d/2}} \int_{\R^d} G\left(\frac{t_0 + s}{2}, x\right) \rho(x) \ud x =: \textbf{G}_{\rho, q}(s),
	\end{align*}
