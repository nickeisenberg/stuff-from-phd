...
Unfortunately, we may not consider the Riesz kernel in our setting due to the assumption that $\Upsilon(0) < \infty$. Indeed, for $0<s<d$, the Riesz kernel is defined to be $f(x) = |x|^{s-d}$ and in order for 
	\[
		...
		\int_{\R^d} \frac{1}{|x|^{2d-s-2}} < \textcolor{magenta}{0},
	\]
...
where, 
	...
	1.2.5]{Gra14}) that
	\begin{enumerate}
		\item $G_s(x) = \frac{(2 \sqrt{\pi})^{-d}}{\Gamma(s/2)}\int_0^\infty e^{-t}e^{-|x|^2/(4t)} t^{(s-d)/2} \frac{\ud t}{t}$,
...
that both $\mathcal{H}(t) \textcolor{magenta}{< 0}$ for any $t>0$ and $\Upsilon(0) < \infty$. 

\bigskip
\textcolor{blue}{I removed the $4 \pi$ which is all that was needed when considering out definition of the Fourier transform. Why did you write $\Upsilon < 0$ and $\mathcal{H}(t) < 0$ which I left in magenta? }
\textcolor{magenta}{Please translate results in \cite{Gra14} to our setting
using our definition of Fourier transform without $2\pi$.}
