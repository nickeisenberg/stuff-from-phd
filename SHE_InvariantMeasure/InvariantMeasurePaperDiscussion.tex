...
Then \eqref{E:vsol} and \eqref{E:vsolrestart} would need to imply that
	...
	\end{align}
My intuition is that \eqref{E:ChgOfVar} can not be true almost surely. The first two integrals on the left hand side are integrating over $[0,t_0] \times \R^d$ and $[0,t] \times \R^d$ while the second integral is integrating over $[0,t_0+t] \times\R^d$ and so the left hand side integrals are not containing the information of $\dot{W}$ on $[t,t+t_0] \times \R^d$ which is contained in the integral on the right hand side.

What may be true is $\mathbb{E}(|LHS|^2) = \mathbb{E}(|RHS|^2)$ but I have been unable to verify this.
