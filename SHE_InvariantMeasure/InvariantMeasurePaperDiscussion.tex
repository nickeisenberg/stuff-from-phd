...
\subsection{Verification of restart}
\textcolor{blue}{There is circular reasoning at \eqref{E:t0Restart}. I will fix later}
...
If we assume that \eqref{E:t0Restart} is true, then
	...
	\end{align}
and because of \eqref{E:t0Restart}, we would also like to have 
	\begin{equation}
	\label{E:vsolrestart}
...
The first integral in \eqref{E:vsol} can be shown using Fubini's theorem to be written as
	...
	\end{equation}
Thus to prove \eqref{E:vsolrestart}, it suffices to prove the following:
	\begin{align}
...
and lastly we would like for it to be the case that 
	...
	\end{align}
Then \eqref{E:ChgOfVar2} and \eqref{E:SumOfInt} would imply that \eqref{E:ChgOfVar} which in return would prove \eqref{E:vsolrestart}. Hence it suffices to prove \eqref{E:ChgOfVar2} and \eqref{E:SumOfInt}. However, both 
...
Recall that 
	...
	\]
So $|u(t_0,\cdot)|$ is a processes and the absolute values add some difficulty. The comparison principle show that we may remove the absolute value signs for the $\delta_0$ initial condition though. Thus we would just need to show
	\begin{align*}
		\sup_{x \in \R^d} (G(t,\cdot) * u(t_0,\cdot))(x) < \infty \quad \text{for any } t\ge 0.
	\end{align*}
We have that 
...
\textcolor{blue}{I answered this concern as I finished typing it. As long as we assume \eqref{A:initialbdd}, then we will be able to show 
	\[
