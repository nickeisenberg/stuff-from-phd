...
Unfortunately, we may not consider the Riesz kernel in our setting due to the assumption that $\Upsilon(0) < \infty$. Indeed, for $0<s<d$, the Riesz kernel is defined to be $f(x) = |x|^{s-d}$ and in order for 
	\[
		\Upsilon(0) = \int_{\R^d} \frac{f(x)}{|x|^{d-2}} \ud x = \int_{\R^d} \frac{1}{|x|^{2d-s-2}} < 0,
	\]
...
where, 
	\[
		\mathcal{F}G_s(\xi) = \frac{1}{(1+ 4\pi^2 |\xi|^2)^{s/2}}.
	\]
Moreover, it can be shown (see \cite[Proposition 6.1.5]{grafakos}) that
	\begin{enumerate}
	...
	\end{enumerate}
Hence, $G_s(x)$ exhibits exponential decay as $x \to \infty$, and so if we consider $d \ge 3$, $d-2 < s < d$, and $\alpha < 1/2$ then it is easy to see that both $\mathcal{H}(t) < 0$ for any $t>0$ and $\Upsilon(0) < \infty$. 
