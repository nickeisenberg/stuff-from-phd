...
\noindent \textbf{\eqref{E:IntProb} is tight if the $p_t$ are tight:} Now lets further assume that the family of probability measures $\{p_t\}_{t \in [a,T]}$ is tight. Let $\epsilon>0$ be given. Then there exists a a compact set $K \in \mathscr{E}$ such that for each $t \in [a,b]$, $p_t(K^c) < \epsilon$ where $K^c = E \setminus K$. Then, 
	\begin{align*}
		P_T(K) &= \frac{1}{b-a} \int_a^b p_t(K) \; \ud t \\
		&= \lim_{t_n \in P_n} \frac{1}{b-a}\sum_{t_k \in P_n} p_{t_k}(K) \Delta_n \\
		& \le \epsilon  \lim_{t_n \in P_n} \frac{1}{b-a}\sum_{t_k \in P_n} \Delta_n \\
		&= \epsilon,
	\end{align*}
