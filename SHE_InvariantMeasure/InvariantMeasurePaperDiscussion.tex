...
\section{Bassel kernal}
It doesn't seem like there is any issue to apply the Bassel kernal. The Bassel kernel according to wikipedia is given by 
	\[
		G_s(x) = \frac{1}{(4 \pi)^{s/2}\Gamma(s/2)}\int_0^\infty \frac{e^{-\frac{\pi |x|^2}{y} - \frac{y}{4 \pi}}}{y^{1+\frac{d-s}{2}}} \ud y
	\]
and that 
	\[
...
So,
	\[
		\mathcal{H}(t) = \int_0^t \ud r \; r^{-2\alpha} \int_{\R^d} \ud \xi \; \frac{\exp(-r|\xi|^2)}{(1+ 4 \pi^2 |\xi|^2)^{s/2}},
	\]
and we see that $\mathcal{H}(t)$ is finite when $0<s<d$ and when $\alpha < 1/2$. It is known that if $F(t) = \int_0^t f(x) \ud x$ exists then $F$ is continuous. Thus, $\mathcal{H}(t)$ is continuous in $t$ and this means that 
	\[
		\int_0^t \mathcal{H}(s)^{q/2} \ud s < \infty, \quad \text{for any } t \ge 0.
	\]
To satisfy $\Upsilon(0) < \infty$, which is
	\[
		\int_{\R^d}, \frac{\hat{f}(\ud \xi)}{|\xi|^2} = \int_{\R^d}, \frac{\ud \xi}{|\xi|^2(1+ 4 \pi^2 |\xi|^2)^{s/2}} < \infty,
	\]
we will need $s > d - 2$ and $ d > 2$. Hence, we will need to require that 
	\begin{enumerate}
		\item $d \ge 3$,
		\item $d-2 < s < d$,
		\item $\alpha < 1/2$.
