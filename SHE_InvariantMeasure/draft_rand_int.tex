...
nonnegative and nonnegative definite tempered measure, which is called {\it spectral measure} and is denoted by $\widehat{f}(\ud \xi)$. Moreover, in the case where $\Gamma$ has density $f$, then $\widehat{f}(\ud \xi) = \widehat{f}(\xi) \ud \xi$. 

 The aim of the paper is to investigate the conditions
required to guarantee the existence of invariant measures for the solution to \eqref{E:SHE}, which is a crucial step towards the study of the ergodicity of the system. Following the
...
<\infty.
\end{align}
In the setting of equation \eqref{E:SHE}, Tessitore and Zabczyk \cite{tessitore.zabczyk:98:invariant} verified \eqref{E:BddMnt} when $\mu(\ud x) = \ud x$. We build from their method and show the existence of an invariant measure when $\mu(\cdot)$ satisfies \eqref{E:InitData} and \eqref{E:GH-int} below, as well as one of the following:
\begin{enumerate}
	...
	\item $\mu(\ud x) = \varphi(x) \ud x$ where $\varphi$ is a processes in $L^2(\Omega, L^2_{\rho}(\R^d))$, namely  $\mathbb{E} \left( \Norm{\varphi(\cdot)}_{ L^2_{\rho}(\R^d)}\right) < \infty$.
\end{enumerate}  
Some applicable deterministic intitial measures include: 
	\[
...
$L_\rho^2(\R^d)$ as usual; see Section \ref{S:Prelim} below for more details. 

We first note that the solution to \eqref{E:SHE} is interpreted as the {\it mild solution},
...
\begin{align} \label{E:J}
  \begin{gathered}
    J_0(t,x) := (G(t,
    \cdot) * \mu)(x) =\int_{\R^d}G(t,x-y)\mu(\ud y) ,   \quad
    J_+(t,x) := (G(t,
    \cdot) * |\mu|)(x) , \\
...
For convenience, we will from now on write $\Norm{\cdot}_\rho$ instead of $\Norm{\cdot}_{L^2_\rho(\R^d)}$ and $\Norm{X}_p = \left(\mathbb{E}(|X|^p)\right)^{1/p}$ where $p\ge 1$. 

We now state Assumption \ref{A:General}, under which one can find a unique mild solution to \eqref{E:SHE} with sharp moment estimates; see
\cite{chen.kim:19:nonlinear,chen.huang:19:comparison,chen.dalang:15:moments}. 

\begin{assumptions} \label{A:General} $\;$
  \begin{enumerate}[(i)]
    \item The initial measure $\mu$ can either random or deterministic. When $\mu$ is random, it must satisfy the following integrability condition: 
    \begin{equation} \label{A:icon}
    \int_{\R^d}\exp\left(-a|x|^2\right)\Norm{\mu(\ud x)}_2 :=  \int_{\R^d}\exp\left(-a|x|^2\right)\Norm{\varphi(x)}_2 \ud x < \infty \quad \text{for all $a>0$},
    \end{equation} 
    where $\varphi \in L^2\left(\Omega, L^2_{\rho}(\R^d)\right)$ for some weight $\rho$, namely
    \[
  	 	\mathbb{E} \left( \Norm{\varphi(\cdot)}_{ L^2_{\rho}(\R^d)}\right) < \infty.
    \]
     When $\mu$ is deterministic, it satisfies the 	following integrability condition:
      \begin{equation} \label{A:iconDet}
...
\end{assumptions}
\begin{notation}
	For ease of notation, we will always write $\Norm{\mu(\ud x)}_q$ whether $\mu$ is deterministic or random. This shall not cause any confusion since if $\mu$ is deterministic, then we can informally think of $\mu$ as a constant in that $\Norm{\mu(\ud x)}_q = |\mu|(\ud x)\Norm{1}_q = |\mu|(\ud x)$.
\end{notation}
...
\begin{remark} \label{R:Cor}
  We should make several comments the condition \eqref{E:U0finite}. The assumption on the correlation function in \cite{tessitore.zabczyk:98:invariant} is given
  ...
  \end{equation}
  see Hypothesis 2.1 ({\it ibid.}) and recall that $d\ge 3$. One can construct an example that satisfies \eqref{E:CorTZ} but fails to satisfy \eqref{E:U0finite}. However as was illustrated in \cite{tessitore.zabczyk:98:invariant}, \eqref{E:CorTZ} was not enough of an assumption to give a concrete example of the existence of an invariant measure and additional assumptions were needed. See Theorem 3.3 {\it ibid} where it is further assumed that $d \ge 3$ and the Lipschitz constant of $b$ satisfies $l^{-2} > \textbf{l}$, where
  	\begin{equation}
  	...
  	\end{equation}
  where $\mathbf{\Gamma}$ is the Gamma function. These additional assumptions parallel our assumptions \eqref{E:U0finite} and \eqref{E:LipLb}, however one can see that \eqref{E:U0finite} is more compact and easier to check.
\end{remark}

\begin{notation}
We need to introduce some more notation. Denote
\begin{align} \label{E:qJ*}
	\mu_q^*:= 1+ \Norm{\mu}_q \quad \text{and} \quad J_{q}^*(r,z) = \left(G(r,\cdot)*\mu_q^*\right)(x) .
\end{align}
We remind the reader that for deterministic initial measures
\begin{align} \label{E:J*}
	\mu_q^* = \mu^* = 1+ |\mu| \quad \text{and} \quad  J_{q}^*(r,z) = J^*(r,z) = \left(G(r,\cdot)* \mu^*\right)(x).
\end{align}
...
\begin{theorem} \label{T:Lrho}
  ...
  \begin{enumerate}[(i)]
    ...
    \item the correlation measure $\Gamma$ satisfies \eqref{E:U0finite} (or equivalently
      \eqref{E:U0Eqv});
    \item the Lipschitz constant $L_b$ of $b$ (see Theorem \ref{T:H(t)Bdd} below) satisfies 
      \begin{equation} \label{E:LipLb}
        2L_b^2 < \frac{1}{128 \Upsilon(0)}.
      \end{equation}
      \textcolor{blue}{In Lemma \ref{L:yineq}, we will need to assume that $qL_b^2 < \frac{1}{128 \Upsilon(0)}$ for some $q > 2$. This $q$ can be fixed throughout the paper so we do not run into the problem that $qL_b^2 < \frac{1}{128 \Upsilon(0)}$ needs to hold for every $q>2$.}
  \end{enumerate}
...
\begin{remark}
  ...
  \textcolor{blue}{Need to make a comment that when $u(0, \cdot) = \delta_0(\cdot)$ then $u(t_\cdot) \in L^2_\rho)$ for any weight $\rho$. This is important for \eqref{E:Restartbdd}.}
\end{remark}
...
\begin{theorem}[Theorem 1.2 \cite{chen.huang:19:comparison}] \label{T:H(t)Bdd}
