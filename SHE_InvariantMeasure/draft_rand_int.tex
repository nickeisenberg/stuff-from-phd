...
to the \textit{parabolic Anderson model} \cite{carmona.molchanov:94:parabolic}.
 ...
 \textcolor{blue}{
 This is the only part left of the paper that I want to confirm. I think $E$ needs to be the bounded space of continuous functions. This is how it is defined in \cite{da-prato.g-atarek.ea:92:invariant} and \cite{assing.manthey:03:invariant}. How do we define the laws for random initial conditions, $\varphi$, that are almost surely in $L^2_{\rho}(\R^d)$? See \eqref{E:RandInit}. I believe there may not be an issue because of the $\varphi \in \mathcal{F}_0$ assumption but I want to make sure. It seems that other authors ignore this issue. \vspace{.2in} \\
 After reading \cite{assing.manthey:03:invariant} a little more carefully, I think what we need as our definition is 
 	\[
		 \eta(A) = \int_{E} \mathscr{L}(u(t,\cdot, \zeta))(A) \; \eta(\ud \zeta), \quad \forall t \ge t_0,
	 \]
where $\zeta$ is any measure satisfying \eqref{E:DetInit}. I believe that $\eta$ would still be a measure on the space $L^2_{\rho}(\R^d)$. Thus we have expanded the domain of 
	\[
		\zeta \mapsto \mathscr{L}(u(t,\cdot, \zeta))(A), \quad t \ge t_0,
	\]
from $\zeta \in C_b(\R^d)$ to $\zeta \in M$ where $M$ is the space of deterministic measures as in \eqref{E:DetInit}.
...
Let's examine the $\int_0^\infty \ud z$ integral and apply the change of variables $y = z^2$. This yields,
	...
	\end{align*}
where $U\left(\alpha,\beta,t\right)$ is the confluent hypergeometric U function (see \cite[Chapter 13]{oliver.lozier.boisvert.clark:2010:nist}) and the last equality can be found in \cite[13.4.4]{oliver.lozier.boisvert.clark:2010:nist}. Thus, to show the finiteness of $\mathcal{H}_{\alpha}(s)$ for each $s >0$, it suffices to show that the following integral is finite:
	\begin{equation} \label{E:BesselHG}
		\int_0^s r^{-2\alpha} \; U\left(\frac{d}{2},\frac{d+2-s}{2},r\right) \; \ud r.
	\end{equation}
The behavior of $U(\alpha, \beta, t)$ around the origin is given in \cite[Section 13.2 (iii)]{oliver.lozier.boisvert.clark:2010:nist} and is given in cases on $\beta$, which in our case is $(d+2-s)/2$. Recall above that we are taking $d-2 < s < d$. For this reason, the only case in \cite[Section 13.2 (iii)]{oliver.lozier.boisvert.clark:2010:nist} that is applicable is $1 < (d+2-s)/2 < 2$ and in this case 
	\begin{equation}
