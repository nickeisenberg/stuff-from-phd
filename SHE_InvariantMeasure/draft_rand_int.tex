...
\begin{proof}
  ...
  \end{align*}
  Without loss of generality, we may assume below that $r\gg 1$. Denote $y=(y_1,\cdots, y_d) =
  ...
  \end{align*}
  which implies that
  \begin{align} \label{E_:IVT}
    r^{b-1} - (r-y)^{b-1} \ge (b-1) \frac{y}{(r-y)^{2-b}}.
  \end{align}
  Plugging \eqref{E_:IVT} and \eqref{E_:superadd} back to \eqref{E_:K'} shows that
  \begin{align*}
    K'(r) & \ge  e^{r^b-r^2} + b(b-1) \int_0^r e^{ - y^2 + y^b } \frac{y}{(r-y)^{2-b}} \ud y         \\
          & \ge  e^{r^b-r^2} + b(b-1) \int_{r/2}^r e^{ - y^2 + y^b } \frac{y}{(r-y)^{2-b}} \ud y     \\
          & \ge  e^{r^b-r^2} + b(b-1) \int_{r/2}^r e^{ - y^2 + y^b } \frac{r /2}{(r /2)^{2-b}} \ud y \\
          & =   \underbrace{e^{r^b-r^2}}_{\to 0}  + \underbrace{2^{b-2}b(b-1)r^{2-b}\int_{r/2}^r e^{ - y^2 + y^b } \ud y}_{\to 0}.
  \end{align*}
  \textcolor{blue}{
  I think both underlines terms go to $0$ as $r \to \infty$. Since $1 < b < 2$ then the first term will go to $0$. As for the second term, we have that 
  	\[
  		\int_{r/2}^r e^{ - y^2 + y^b } \ud y \le(r/2) e^{ - (r/2)^2 + (r/2)^b } \to 0.
  	\]
}
  
  Hence, $K'(r)$ is positive and unbounded as $r\to\infty$. Therefore, this implies that $K(r)$
  blows up as  $r\to\infty$,  which completes the case  $b\in(1,2]$.

Finally suppose that $b > 2$. Note that 
	\begin{align*}
		\int_{\R} e^{-y^2 +r^b - |y-r|^b} \ud y
		 &\ge e^{r^b} \int_{|y-r|<\frac{r}{2}} e^{-y^2- |y-r|^b} \\
		 &\ge e^{r^b} \int_{|y-r|<\frac{r}{2}} e^{-y^2} e^{-\frac{r^b}{2^b}} \\
		 &= e^{(2^b -1)r^b / 2^b} \int_{r / 2}^{3r/2} e^{-y^2} \ud y \\
		 & \ge e^{(2^b -1)r^b / 2^b} e^{-9r^2 / 4}.
	\end{align*}
Since $b>2$, then as $r \to \infty$, we see that
	\[
		\frac{(2^b -1)r^b}{2^b} - \frac{9r^2}{4} \to \infty.
	\]
Putting this all together gives us that
	\[
		I(1/2,x_r) \ge \int_{\R} e^{-y^2 +r^b - |y-r|^b} \ud y \ge e^{(2^b -1)r^b / 2^b} e^{-9r^2 / 4},
	\]
