...
Following \cite[Section 11.1]{da-prato.zabczyk:14:stochastic}, a probability measure $\eta$ on the
Borel $\sigma$-field $\mathscr{B}(H)$ is said to be {\it invariant} for \eqref{E:SHE} if
...
For notation, let us denote $P_t(\zeta, A) := \mathscr{L}(u(t,\cdot;\zeta))(A)$ where $\zeta \in H$ and $A \in \mathscr{B}(H)$. Then we can define the Markovian transition semi-group corresponding to $P_t$ as 
	...
	\]
 where $C_b(H)$ denotes the space of bounded continuous function on $H$. Moreover, if we define $\mathscr{M}(H)$ as the space of probability measures on $H$, then for each $\nu \in \mathscr{M}(H)$, we define the measure $ P_t^*\nu \in \mathscr{M}(H)$ as follows:
 	\[
...
\begin{align} \label{E:BddMnt}
  ...
  <\infty,
\end{align} 
and this will be the content of out Theorem \ref{T:Main} below.
...
For example, to cancel the growth of the moments, the authors in \cite{assing.manthey:03:invariant, cerrai:01:second, cerrai:06:asymptotic} require a drift coefficient, $f(u):\R \to \R$, to be added to the right hand side of equation \eqref{E:SHE} and that this term must satisfy the following dissipativity condition for some $m,\: k_i, \: c_i>0$ as $|u| \to \infty$:
	\begin{equation} \label{E:dis}
		...
		\end{cases} 
	\end{equation}
Furthermore, Cerrai \cite{cerrai:01:second, cerrai:06:asymptotic} considered the case of a bounded spatial domain while Assing and Manthey \cite{assing.manthey:03:invariant} considered the whole space $\R^d$. In addition, the just mentioned papers only allow for bounded initial conditions, however, an unbounded Lipschitz continuous diffusion term as in \eqref{E:lipcon} is allowed to be considered. It is also worth noting that Eckmann and Hairer \cite{eckmann.hairer:01:invariant} also require the above dissipativity condition \eqref{E:dis} along with a bounded initial condition, however they consider the case of an additive noise. 

Aside from this current article and \cite{tessitore.zabczyk:98:invariant}, there are few works which do not require an added drift term with dissipativity as in \eqref{E:dis}. In lack of this condition, Misiats {\em et al} \cite[Theorem 1.2]{misiats.stanzhytskyi.ea:20:invariant} provide a result guaranteeing the existence of an invariant measure for the stochastic heat equation on the whole space $\R^d$. More precisely, they allow for a drift term, $f(x,u):\R^d \times \R \to \R$, such that for all $x \in \R^d$ and $u_1,u_2 \in \R$, 
	\[
...
\end{align}
\end{subequations}
and moreover, these two conditions will guarantee the existence of the following non-empty open interval:
...
$\Upsilon_\alpha(\beta)$, i.e., $\Upsilon(\beta)=\Upsilon_0(\beta)$. 
...
We now state and prove a quick Corollary which shows that the solution to \eqref{E:SHE} starting from an $L^2_{\rho}(\R^d)$ initial condition with almost surely be in $L^2_{\rho}(\R^d)$ for all $t >0$.
...
\end{corollary}
\begin{proof}
	Let $\zeta \in L^2_{\rho}(\R^d)$. Then, by (ii) of Theorem \ref{T:Lrho}, it suffices to show that the finiteness of $\mathcal{G}_{\rho}(t,;|\zeta|)$ for all $t>0$. Indeed,
		\begin{align*}
		...
		\begin{align*}
			\int_{\R^d} \int_{\R^d} G(t,x-y) |\zeta(y)|^2 \ud y \rho(x) \ud (x) &\le C_{\rho}(T) \Norm{\zeta}_{\rho} < \infty.
		\end{align*}
...
and provide some examples. In particular, in Section \ref{SS:Weight} we will extend the discussions
...
\end{equation}
which can be easily verified. Recall the spherical coordinate integration formula:
\begin{align*}
...
measure. Moreover, the measure takes the form of \eqref{E:InvMeasForm} above with $\varphi = 1$.
...
difficulties caused by the square root that may arise. Before that, let us first introduce a
