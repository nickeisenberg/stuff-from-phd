...
\begin{theorem}[Theorem 1.7 \cite{chen.huang:19:comparison}] \label{T:ExUn}
  ...
  the expression of the function $H$). Moreover \footnote{This statement is from Lemma 2.5 of
    \cite{chen.kim:19:nonlinear}. In the current paper, we do not need the lower bound of the
  moments, hence, there is no need to make the cone conditions as in {\it ibid.}; only the Lipschitz
condition on $b$ will be sufficient to guarantee the upper bounds of the $p$-th moments.}, if the
...
\begin{lemma} \label{L:yineq}
  ...
  \begin{enumerate}
    ...
    \item Under conditions in part 2, if $\rho$ is a nonnegative $L^1(\R^d)$ function, then there
