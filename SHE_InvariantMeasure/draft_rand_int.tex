...
\begin{remark} \label{R:admRho}
  ...
  condition \eqref{E:aw} excludes these two cases. However, the following example is less obvious.
%   Lastly, we should mention that numerical results
...
\begin{proof}
  ...
  Next we assume that $b\in(1,2]$. We need to show that $\rho_b$ is not admissible. Without loss of
  ...
  (y_1,y_*)$ where $y_*\in\R^{d-1}$. Because $b /2\in (0,1]$, we see that
  \begin{align} \label{E_:y}
    | y | ^b =  \left(y_1^2+y_2^2+\cdots+y_d^2\right)^{b/2}
            \le |y_1|^b + \left(y_2^2+\cdots+y_d^2\right)^{b/2}
            = |y_1|^b + |y_*|^b.
  \end{align}
  Hence,
  \begin{align*}
    ...
    & \ge C \int_{\R^{d-1}}\ud y_* \int_\R \ud y_1\: e^{- |y_*|^2 -|y_*|^b - |y_1-r|^2 + (r^b - |y_1|^b)}         \\
    & =   C \int_{\R^{d-1}}\ud y_* \: e^{- |y_*|^2 -|y_*|^b } \int_\R \ud y_1\:  e^{ - |y_1-r|^2 + r^b - |y_1|^b} \\
    & =  C \int_\R e^{ - y^2 + r^b - |y-r|^b} \ud y \\
    & \ge C \int_0^r  e^{ - y^2 + r^b - (r-y)^b} \ud y.
  \end{align*}

  Now denote $K(r):=\int_0^r  e^{ - y^2 + r^b - (r-y)^b} \ud y$. Then,
  \begin{align} \label{E_:K'}
    K'(r) = e^{r^b-r^2} + b \int_0^r e^{ - y^2 + r^b - (r-y)^b} \left(r^{b-1}-(r-y)^{b-1}\right) \ud y
  \end{align}
  % Since $b\in (1,2]$, we see that
  % \begin{align} \label{E_:superadd}
  %   r^b -(r-y)^b = (r-y +y)^b  - (r-y)^b \ge y^b.
  % \end{align}
By the intermediate value theorem, we see that
  ...
  \end{align*}
  By the intermediate value theorem, $r^b-(r-1)^b = b \xi^{b-1}$ with $\xi\in [r-1,r]$. Hence,
  $r^b-(r-1)^b \ge b (r-1)^{b-1}$ and then
  \begin{align*}
    K'(r) & \ge C r^{b-2} \exp\left(b (r-1)^{b-1}\right).
  \end{align*}
  Hence, for $r \gg 2$, $K'(r)$ is positive and unbounded as $r\to\infty$. Therefore, this implies that $K(r)$ blows up
  as  $r\to\infty$,  which completes the case  $b\in(1,2]$.

\textcolor{blue}{
Finally, suppose that $b > 2$. The proof of this case follows from the proof of the case when $1<b<2$, but with some minor changes?? For example, \eqref{E_:y} becomes
\begin{align*}
|y|^b \le 2^{b/2-1}\left(|y_1|^b + |y_*|^b\right).
\end{align*}
Thus by setting $c := 2^{b/2 -1}>1$, $K(r)$ and $K'(r)$ become the following:
\begin{align*}
	K(r) = \int_0^r  e^{ - y^2 + r^b - c(r-y)^b} \ud y,
\end{align*}
and
\begin{align*}
K'(r) = e^{ - y^2 + r^b} + b \int_0^r  e^{ - y^2 + r^b - c(r-y)^b}\left(r^{b-1} -c(r-y)^{b-1} \right)\ud y.
\end{align*}
The $c$ inside $\left(r^{b-1} -c(r-y)^{b-1} \right)$ makes it so we can no longer use the intermediate value theorem. 
}

  Finally, suppose that $b > 2$. Note that in this case, \eqref{E_:y} becomes
  \begin{align*}
    |y|^b \le 2^{b/2-1}\left(|y_1|^b + |y_*|^b\right).
  \end{align*}
By setting $c := 2^{b/2 -1}>1$, we see that
	\begin{align*}
    I(1/2, x_r) &\ge \int_{\R} e^{-y^2 +r^b - c |y-r|^b} \ud y \\
		 &\ge e^{r^b} \int_{|y-r|<\frac{r}{2}} e^{-y^2- c|y-r|^b} \\
		 &\ge e^{r^b} \int_{|y-r|<\frac{r}{2}} e^{-y^2} e^{-c\frac{r^b}{2^{b}}} \\
		 &= e^{(2^{b} -c)r^b / 2^b} \int_{r / 2}^{3r/2} e^{-y^2} \ud y \\
		 & \ge e^{(2^{b} - c)r^b / 2^b} e^{-9r^2 / 4}.
	\end{align*}
  Since $b>2$, we see that
	\begin{align*}
		2^{b} - c = 2^b - 2^{(b-2)/2} > 0,
	\end{align*}
  and thus as $r \to \infty$,
	\begin{align*}
		\frac{(2^{b} -c)r^b}{2^b} - \frac{9r^2}{4} \to \infty.
	\end{align*}
  Putting this all together gives us that
	\begin{align*}
		I(1/2,x_r) \ge \int_{\R} e^{-y^2 +r^b - c|y-r|^b} \ud y \ge e^{(2^{b} -c )r^b / 2^b} e^{-9r^2 / 4},
	\end{align*}
  and so $I(1/2,x_r) \to \infty$ as $r \to \infty$. This completes the proof.
\end{proof}
...
However, in the following we show that if $\widehat{f}(\xi) = \max\{2-|\xi|, 0\}$, then Tessitore's condition is to hard to calculate because the absolute value sign can't be removed. But this particular $\widehat{f}$ can be easily calculated with our condition \eqref{E:Dalang-00}. Indeed, up to a constant, we have that
	\begin{align*}
		\Upsilon(0) = \int_{\R^d} \frac{\max\{2-|\xi|,0\}}{|\xi|^2(1-\alpha)} \ud \xi & = \int_0^{\infty} \max\{ 2-r,0 \}r^{-2(1-\alpha)} r^{d-1} \ud r \\
