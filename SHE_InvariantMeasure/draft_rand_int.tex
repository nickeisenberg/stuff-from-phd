...
\begin{proposition} \label{P:Bessel}
  ...
  \begin{align} \label{E:HaB}
    \mathcal{H}_\alpha(t) <\infty \quad \Longleftrightarrow \quad 0<\alpha<\frac{1}{2} - \frac{(d-s)_+}{4}\quad \text{and} \quad s>d-2>0,
  \end{align}
  where $a_+ := \max(a,0)$. Moreover, for $\alpha\in(0,1/2)$, we have the following series
  expansions $t\to 0$: \renewcommand{\arraystretch}{2}
  \begin{align*}
    \mathcal{H}_\alpha(t) = \left\{
      \begin{array}{lc}
        \begin{aligned}
          \dfrac{\pi^{d/2} \Gamma\left((d-s)/2\right)}{\left((s-d)/2+1-2\alpha\right)\Gamma(d/2)}  t^{(s-d)/2+1-2\alpha}                  \\
          \quad - \dfrac{\pi^{d/2}\Gamma\left((s-d)/2\right)}{(1-2\alpha)\Gamma(s/2)}              t^{1-2\alpha}                          \\
          +                                                                                        O\left(t^{(s-d)/2+2(1-\alpha)}\right), \\[1em]
        \end{aligned}
        &
        \begin{gathered}
         d-2<s<d \quad \text{and} \\
         \alpha < \frac{1}{2} -\frac{1}{4}(d-s),
        \end{gathered} \\
        \begin{aligned}
           -\frac{\pi ^{d/2}}{(1-2 \alpha) \Gamma \left(d/2\right)}t^{1-2\alpha}\log(t)                                                                \\
           +\frac{\pi ^{d/2} \left(4 \alpha  \gamma -2 \gamma +(2 \alpha -1) \psi(d/2)+1\right)}{(1-2 \alpha)^2 \Gamma \left(d/2\right)}t^{1-2 \alpha} \\
           +O\left(t^2\log(t)\right),                                                                                                                  \\[1em]
        \end{aligned}                                                                                                           & s=d,     \\
        \dfrac{\pi^{d/2}\Gamma\left((s-d)/2\right)}{(1-2\alpha)\Gamma(s/2)}t^{1-2\alpha} + O\left(t^{(s-d)/2+1-2\alpha}\right), & d<s<d+2, \\
        \dfrac{\pi^{d/2}}{(1-2\alpha)\Gamma\left(d/2+1\right)}t^{1-2\alpha}  + O\left(t^{2(1-\alpha)}\log(t)\right),            & s=d+2,   \\
        \dfrac{\pi^{d/2}}{(1-2\alpha)\Gamma\left(d/2+1\right)}t^{1-2\alpha}  + O\left(t^{2(1-\alpha)}\right),                   & s>d+2,   \\
      \end{array}
      \right.
  \end{align*}
  where $\psi(x)=\frac{\ud}{\ud x}\Gamma(x)$ refers to the {\em digamma function} and $\gamma\approx
  0.57721$ to {\em Euler's constant}; see 5.2.2 and 5.2.3 on p. 136 of \cite{askey.roy:10:gamma}.
\end{proposition}
\begin{proof}
  ...
  \begin{align*}
    ...
    & = \frac{C_d}{2} \int_0^t \ud r \; r^{-2\alpha} \int_0^\infty \ud u \; \exp(-r u)(1+u)^{-s/2} u^{\frac{d}{2}-1} \\
  ...
  \end{align*}
  By \cite[13.4.4 on p.326]{olde-daalhuis:10:confluent}, $I(r)$ is the {\it confluent hypergeometric
  function}:
  \begin{align*}
    I(r) =U\left(\frac{d}{2},\frac{2+d-s}{2},r\right).
  \end{align*}
  By 18.2.18 -- 13.2.22 on p. 323 {\it ibid.}, we see that
  ...
  over $[0,t]$ proves Proposition \ref{P:Bessel}.
\end{proof}
