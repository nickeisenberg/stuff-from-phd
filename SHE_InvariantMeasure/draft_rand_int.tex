...
space $\R^d$ has been much less studied. Except Tessitore and Zabczyk
\cite{tessitore.zabczyk:98:invariant}, with which we will follow closely in this paper, there are a
few other papers \cite{assing.manthey:03:invariant, misiats.stanzhytskyi.ea:20:invariant,
misiats.stanzhytskyi.ea:16:existence}. Since the space $\R^d$ is not compact, one needs to work on a
...
Let $\rho$ be an admissible weight function, which introduces the Hilbert space $H:=L_\rho^2(\R^d)$
with $\langle f,g \rangle_\rho := \int_{\R^d}f(x)g(x)\rho(x) \: \ud x$ and $\Norm{f}_\rho^2
:=\int_{\R^d}f(x)^2\rho(x) \: \ud x$. More discussions about the weight function can be found in
Section \ref{SS:Weight}. Let $\mathcal{B}(H)$ be the space of all bounded Borel functions on $H$.
Following \cite{da-prato.g-atarek.ea:92:invariant}, a probability measure $\eta$ on the Borel
$\sigma$-field $\mathcal{B}(H)$ is said to be {\it invariant} for \eqref{E:SHE} if
...
initial condition $u(0,\cdot)=\varphi$. Note that in \eqref{E:InvMeas} we require $t$ to be strictly
bigger than zero, unlike Eq. (3) of \cite{da-prato.g-atarek.ea:92:invariant} where one imposes $t\ge
0$.

Due the {\it Krylov-Bogoliubov theorem} (see, e.g., \cite[Theorem
...
\end{equation}
for any sequence $\{T_n\}_{n\ge 1}$ with $T_n\uparrow \infty$.  A critical step in obtaining the
tightness result is to show that the moment is uniformly bounded in time:
...
to cancel the otherwise exponential growing moments. The absence of such a drift term in this paper
...
$b$) in \eqref{E:lipcon} have to satisfy the following conditions:
...
\eqref{E:Dalang} in a different way as follows
\begin{align*}
  \Upsilon_\alpha(\beta) := \left(2\pi\right)^{-d} \int_{\R^d} \frac{\widehat{f}(\ud\xi)}{\left(\beta+|\xi|^2\right)^{1-\alpha}}<\infty
  \quad \text{for some $\alpha\in (0,1]$ and for some (hence all) $\beta>0$;}
\end{align*}
see Theorem 1.8 of \cite{chen.huang:19:comparison} or \cite{sanz-sole.sarra:02:holder}. Similarly,
one may strengthen \eqref{E:U0finite} as follows
\begin{equation} \label{Ea:U0finite}
  \Upsilon_\alpha(0) := \left(2\pi\right)^{-d} \int_{\R^d} \frac{\hat{f}(\ud \xi)}{|\xi|^{2(1-2\alpha)}} <\infty \quad \text{and} \quad
  64 L^2_{b}   <  \frac{\alpha}{2\Upsilon(0)},
  \quad  \text{for some $\alpha\in (0,1/2]$.}
\end{equation}
  ...
  \begin{enumerate}[(i)]
    ...
    \item the initial condition $u_0$ satisfies
      \begin{equation} \label{E:G}
        ...
        := \int_{\R^d} \E\left(J_0^2\left(t,x;1+|u_0|\right)\right) \rho(x)\: \ud x< \infty
...
state-of-art moment estimates. The improvements over \cite{tessitore.zabczyk:98:invariant} consist
of the following aspects:
\begin{enumerate}
  \item Results in \cite{tessitore.zabczyk:98:invariant} allows essentially all bounded functions as
...
This paper is organized as follows: In Section \ref{S:Example}, we provide some more discussions and
examples for our main results. In particular, we make a few discussions on the weight function in
Section \ref{SS:Weight}; we show that our results could include a wider class of initial conditions
in \ref{SS:Init};

Finally, two main Theorems \ref{T:Lrho} and \ref{T:Main} will be proved in Sections \ref{S:Lrho}
and \ref{S:mainproof}, respectively.
\bigskip

Throughout the paper, we will use $\Norm{X}_p $ to denote the $L^p(\Omega)$ norm, namely,
$\left(\mathbb{E}(|X|^p)\right)^{1/p}$. We will use several times the following factorization
...
\end{equation}
which can be easily verified. The convention of Fourier transform is given in Remark \ref{R:Fourier}
below.
...
\begin{remark} \label{R:admRho}
  ...
  condition \eqref{E:aw} excludes these two cases. It is much less obvious that the following weight
...
\subsection{Comparing Tessitore and Zabczyk's condition} \label{SS:TessZabz}
...
% For the remainder of the paper, $\mu$ will always denote the initial condition to \eqref{E:SHE}
% and
% $u(t,\cdot) = u(t,\cdot, \mu)$ will always denote the solution of \eqref{E:SHE} starting from
% $\mu$.
...
\begin{remark} \label{R:Fourier}
  ...
  \end{align}
  The authors did not explicitly mention in \cite{tessitore.zabczyk:98:invariant} about the
  ...
  \end{align*}
  Hence, the Plancherel theorem takes the form of $\int_{\R^d} \psi(x) \phi(x)\ud x = \int_{\R^d}
  ...
  corresponds to $\left(2\pi\right)^{-d/2}\widehat{f}$ in this paper. This accounts the slightly
...
\begin{remark}
  ...
  equation), one can avoid this issue (of the potential singularity of
  $\mathcal{G}_{\widehat{\rho}}$ at $s=0$).
\end{remark}
\begin{proof}
  In the proof, we use $C$ to denote a generic constant that may change its value at each
  appearance. \bigskip
...
Finally, we characterize conditions \eqref{Ea:U0finite} and \eqref{E:H} in the following lemma:
...
\begin{lemma} \label{L:HUA}
  ...
  \item $\left(2\pi\right)^{-d}\mathcal{H}_\alpha(t)\le
    \Gamma\left(1-2\alpha\right)\Upsilon_\alpha(0)$ for all $t>0$ and hence
    \begin{align} \label{E:HUA}
      \Upsilon_\alpha(0)<\infty \quad \Longrightarrow \quad \mathcal{H}_\alpha(t)<\infty \quad \text{for
      all $t>0$};
    \end{align}
...
\begin{proof}
  ...
  \begin{align} \label{E:Fubini}
    ...
    & = \int_0^\infty \ud r \: r^{-2\alpha} \int_{\R^d} \widehat{f}(\ud\xi)\exp\left(-r|\xi|^2\right)
      = \Gamma(1-2\alpha) \int_{\R^d} \frac{\widehat{f}(\ud\xi)}{|\xi|^{2\left(1-2\alpha\right)}}
      = C \Upsilon_\alpha(0),
  \end{align}
  with $C:=\Gamma(1-2\alpha)(2\pi)^d$. As for (3), from \eqref{E:Fubini}, for any $t>0$, by
  splitting the $\ud r$ integral into two parts we see that
  \begin{align*}
    C \Upsilon_\alpha(0) &= \int_0^\infty \ud r \: r^{-2\alpha} \int_{\R^d}
    \widehat{f}(\ud\xi)\exp\left(-r|\xi|^2\right) = \mathcal{H}_\alpha(t) + I_\alpha(t), \quad \text{with} \\
    I_\alpha(t) &= \int_t^\infty \ud r \: r^{-2\alpha} \int_{\R^d}
    \widehat{f}(\ud\xi)\exp\left(-r|\xi|^2\right).
    % & = + \int_t^\infty \ud r \: r^{-2\alpha} \int_{\R^d}
    % \widehat{f}(\ud\xi)\exp\left(-r|\xi|^2\right) \\
    % & \le \mathcal{H}_\alpha(t) + t^{-2\alpha} \int_t^\infty \ud r \: \int_{\R^d}
    % \widehat{f}(\ud\xi)\exp\left(-r|\xi|^2\right) \\
    % & \le \mathcal{H}_\alpha(t) + t^{-2\alpha} \Upsilon(0).
  \end{align*}
  Notice that
  \begin{align*}
  ...
  Therefore,
  \begin{align*}
    \Upsilon_\alpha(0) \le \frac{\mathcal{H}_\alpha(t)}{(2\pi)^d\Gamma(1-2\alpha)} +
...
The proof of part (b) based on (a) is routine.
...
\begin{proof}[Proof of Theorem \ref{T:Main} (b)]
  ...
  $L_\rho^2(\R^d)$. Then one can apply for the Krylov-Bogoliubov existence theorem (see, e.g.,
  \cite[Theorem 11.7]{da-prato.zabczyk:14:stochastic}) to conclude that the measure $\eta$ is the
  invariant measure for $\mathscr{L}(u(t,\cdot;\mu))$, $t\ge \tau$. Finally, since $\tau$ can be
