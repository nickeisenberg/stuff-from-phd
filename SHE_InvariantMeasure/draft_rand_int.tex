...
\begin{proof}[Proof of Theorem \ref{T:Main} (a)]
  In this proof, $u(t,x)$ refers to $u(t,x;\mu)$. Fix $\tau>0$ and $t_0=\tau/2$. We consider the
  case $t\ge \tau$. Let $v$ and  $v'$ be the solution to \eqref{E:restart} restarted from $ t_0$ and
  $t- t_0$, respectively. Then
  \begin{align*}
    v(t_0,\cdot)  \stackrel{\mathcal{L}}{=} u(\tau,\cdot) \quad \text{and} \quad
    v'(t_0,\cdot) \stackrel{\mathcal{L}}{=} u(t,\cdot) \quad \text{for $t>\tau$}.
  \end{align*}

  We first consider $v(t_0,\cdot)$. Recall that Lemma \ref{L:Factorize} implies that
  \begin{align*}
    v(t_0,x) = \Big(G(t_0,\cdot) * u(t_0,\cdot)\Big)(x) +\dfrac{\sin(\alpha \pi)}{\pi} \left[F_\alpha Y_v\right](t_0,x).
  \end{align*}
  ...
  \end{align*}
  Then by Propositions \ref{P:weight} and \ref{P:F-Comp}, $\mathscr{K} (\Lambda)$ is relatively
  compact in $L^2_{ \rho}(\R^d)$ for $\Lambda>0$. Moreover, by Chebyshev's inequality,
  \eqref{E:uInLrho} and \eqref{E:yineq}, we see that
  \begin{align*}
    \mathbb{P} \left[ v(t_0,\cdot) \not \in \mathscr{K}(\Lambda) \right ]
    & \le \mathbb{P}\left[ \left(\int_{0}^{t_0} \Norm{Y_v(s,\cdot)}_{\widehat{\rho}}^q \: \ud s\right)^{1/q} > \dfrac{\pi \Lambda}{\sin(\alpha \pi )} \right] + \mathbb{P}\left[ \Norm{u(t_0,\cdot)}_{\widehat{\rho}} > \Lambda \right] \nonumber       \\
    & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q}\E \int_{0}^{t_0} \Norm{Y_v(s,\cdot)}_{\widehat{\rho}}^q \: \ud s + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t_0,\cdot)}_{\widehat{\rho}}^2\right) \nonumber                                    \\
    & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \Theta\int_0^{t_0} \left(\mathcal{G}_{\widehat{\rho}}(s + t_0;\mu) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t_0,\cdot)}_{\widehat{\rho}}^2\right) \\
    & < \infty,
  \end{align*}
  where the upper bound does not depend on $t$. Hence, for any $\epsilon>0$,
  % \begin{align*} \mathbb{P}\{ v(t_0,\cdot) \in \mathscr{K}(\Lambda) \} & \ge 1-
  % \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \left(\Theta\int_0^{t_0}
  % \left(\mathcal{G}_{\widehat{\rho}}(s + t_0;\mu) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud
  % s\right) - \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t_0,\cdot)}_{\rho}^2\right), \end{align*}
  by choosing $\Lambda > 0$ big enough such that
  \begin{align*}
    \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \left(\Theta\int_0^{t_0} \left(\mathcal{G}_{\widehat{\rho}}(s + t_0;\mu) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s\right) + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t_0,\cdot)}_{\rho}^2\right) < \epsilon,
  \end{align*}
  we can ensure that
  \begin{align*}
  ...
  Now we study $v'(t_0,\cdot)$. The argument is similar to the previous case except that some
  appearances of  $t_0$ should be replaced by $t-t_0$. In particular,
  \begin{align*}
    \mathbb{P} \left[ v'(t_0,\cdot) \not \in \mathscr{K}(\Lambda) \right ]
    ...
    + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t-t_0,\cdot)}_{\widehat{\rho}}^2\right)\\
    ...
    + \frac{1}{\Lambda^2} \sup_{t\ge t_0} \mathbb{E}\left(\Norm{u(t,\cdot)}_{\widehat{\rho}}^2\right)\\
    & < \infty,
  \end{align*}
  where the upper bound does not depend on $t$.
  ...
  \begin{equation}\label{E:vtight}
    \mathbb{P}\{ v(t,\cdot) \in \mathscr{K}(\Lambda) \} \ge (1- \epsilon)\mathbb{P}\{ \Norm{v(t-t_0,\cdot)}_{\widehat{\rho}} < \Lambda \}.
  \end{equation}
  We now apply Chebyshev's inequality and \eqref{E:InitData} to see that for all $t > t_0$ that
  there exists a $C>0$ such that
  \begin{align*}
    \mathbb{P}\{ \Norm{v(t-t_0,\cdot)}_{\widehat{\rho}} \ge \Lambda
    \} \le \frac{\mathbb{E}\left( \Norm{v(t-t_0,\cdot)}_{\widehat{\rho}}^2 \right)}{\Lambda^2} = \frac{\mathbb{E}\left( \Norm{u(t,\cdot, \mu)}_{\widehat{\rho}}^2 \right)}{\Lambda^2} \le C\frac{\mathcal{G}_{\widehat{\rho}}\left( t;\mu \right) }{\Lambda^2} \le \frac{C}{\Lambda^2}.
  \end{align*}
  Thus, by enlarging $\Lambda$ if necessary,
  \begin{align*}
    \mathbb{P}\{ \Norm{v(t-t_0,\cdot)}_{\widehat{\rho}} \ge \Lambda\} = \mathbb{P}\{ \Norm{u(t,\cdot)}_{\widehat{\rho}} \ge \Lambda \}  \le \epsilon, \quad \forall \; t \ge t_0.
  \end{align*}
  Combining this with \eqref{E:vtight} yields
  \begin{align*}
    \mathbb{P}\left\{ u(t, \cdot) \in \mathscr{K}(\Lambda) \right\} \ge (1-\epsilon), \quad t \ge t'_0,
  \end{align*}
  where $t'_0 = 2 t_0$. Since $t_0 >0$ is arbitrary, this completes the proof of \eqref{E:cpt}.
\end{proof}
...
% \quad \text{where $\Norm{y}_{\widehat{\rho}} \le \Lambda$  and  $\Norm{h}_{L^q(0,1,L^2_{\widehat{\rho}})} \le \Lambda $} \}.
% \]
...
% & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q}\E \int_{0}^{t_0} \Norm{Y(s,\cdot)}_{\widehat{\rho}}^q \: \ud s + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(0,\cdot}_{\widehat{\rho}}^2\right) \nonumber \\
% & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \Theta\int_0^{t_0} \left(\mathcal{G}_{\widehat{\rho}, q}(s) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(0,\cdot}_{\widehat{\rho}}^2\right) < \infty \label{E:Restartbdd}
% \end{align}
% With all this said, \eqref{E:Restartbdd} implies that
% \begin{align*}
% \mathbb{P}\{ u(t_0,\cdot) \in \mathscr{K}(\Lambda) \}
% & \ge 1- \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \left(\Theta\int_0^{t_0} \left(\mathcal{G}_{\widehat{\rho}, q}(s) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s\right) - \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(0,\cdot)}_{\rho}^2\right)
% \end{align*}
% Therefore, we can choose $\Lambda > 0$ big enough such that
% \[
% \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \left(\Theta\int_0^{t_0} \left(\mathcal{G}_{\widehat{\rho}, q}(s) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s\right) + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(0,\cdot)}_{\rho}^2\right) < \epsilon,
% \]
% thus we get that
% \begin{align*}
...
% \epsilon)\mathbb{P}\{ \Norm{u(t-t_0,\cdot)}_{\widehat{\rho}} < \Lambda
% \}.
% \end{align*}
% We now apply Chebyshev's inequality and \eqref{E:InitData} to see that for all $t > t_0$ that there exists a $C>0$ such that
% \[
% \mathbb{P}\{ \Norm{u(t-t_0,\cdot)}_{\widehat{\rho}} \ge \Lambda
% \} \le \frac{\mathbb{E}\left( \Norm{u(t-t_0,\cdot)}_{\widehat{\rho}}^2 \right)}{\Lambda^2} \le C\frac{\mathcal{G}_{\hat{\rho}, q}\left( t - t_0 \right) }{\Lambda^2} \le \frac{C}{\Lambda^2}.
% \]
% Thus, by enlarging $\Lambda$ if necessary,
% \[
% \mathbb{P}\{ \Norm{u(t-t_0,\cdot)}_{\widehat{\rho}} \ge \Lambda
...
% % Since we assumed that $u$ is bounded in probability in $L^2_{\widehat{\rho}}$ then for and $\epsilon >0$ we can fix $R>0$ such that $\mathbb{P}\{ |u(t,\cdot)|_{\widehat{\rho}} \ge R \} < \epsilon$ and then taking $\Lambda > R$ such that $ \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q}\bigg( 1+ R^q \bigg) < \epsilon$ concludes the claim.
%
% %\newpage
...
% % \end{align*}
% %We will use the notation $Y_v$ and $\mathcal{G}_{\widehat{\rho}, q}^v$ where
% %  \[
% %    Y_v(s,y):= \int_0^s \int_{\R^d} (s-r)^{-\alpha}G(s-r,y-z)b(z,v(r,z))W_{t_0/2}(\ud r,\ud z),
% %  \]
% % and
% % \begin{align*}
% %  \mathcal{G}_{\widehat{\rho}, q}^v(s) &= \int_{\R^d} \left(\int_{\R^d} G(s,x-y)\Norm{v(0,y)}_q\ud y\right)^2 \widehat{\rho}(x) \ud x \\
% %  &= \int_{\R^d} \left(\int_{\R^d} G(s,x-y)\Norm{u(t_0/2,y)}_q\ud y\right)^2 \widehat{\rho}(x) \ud x.
% % \end{align*}
% % We define $\mathcal{G}_{\widehat{\rho}, q}^u(s)$ and $Y_u$ in an equivalent manner. We wish to be able to apply the factorization result \eqref{E:1fac} and also the bound in \eqref{E:yineq} for $Y_v$. This requires us to show that $v(0,\cdot) = u(t_0/2, \cdot)$ satisfies \eqref{E:G} and \eqref{E:GH-int'} with $\hat{\rho}$.  Indeed, H\"older's inequality and the admissible weight property imply that
% % \begin{align*}
% %  \mathcal{G}_{\widehat{\rho}, q}^v(s)  &= \int_{\R^d} \left(\int_{\R^d} G(s,x-y)\Norm{u(t_0/2,y)}_q\ud y\right)^2 \widehat{\rho}(x) \ud x \\
% %  &\le \int_{\R^d} \int_{\R^d} G(s,x-y)\Norm{u(t_0/2,y)}_q^2\ud y  \; \widehat{\rho}(x) \ud x \\
% %  & \le C(t_0) \int_{\R^d} \Norm{u(t_0/2,y)}_q^2 \widehat{\rho}(y) \ud y.
% % \end{align*}
% %Now by \eqref{E:mb0},
% % \begin{align*}
% %  C(t_0) \int_{\R^d} \Norm{u(t_0/2,y)}_q^2 \widehat{\rho}(y) \ud y &\le C(t_0) \int_{\R^d} \Big(G(t_0/2,\cdot) * (1+\Norm{\mu}_q)\Big)^2(y) \widehat{\rho}(y) \ud y \\
% %  &= C(t_0) \mathcal{G}_{\widehat{\rho}, q}^u(t_0/2) < \infty,
% % \end{align*}
...
% % \[
% %  \int_0^{t_0/2} \left(\mathcal{G}_{\widehat{\rho}, q}^v(s) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s \le \mathcal{G}_{\widehat{\rho}, q}^u(t_0/2)^{q/2} \int_0^{t_0/2} \left( \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s < \infty.
% % \]
...
% % & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q}\E \int_{0}^{t_0/2} \Norm{Y_v(s,\cdot)}_{\widehat{\rho}}^q \: \ud s + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t_0/2,\cdot}_{\widehat{\rho}}^2\right) \nonumber \\
% % & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \Theta\int_0^{t_0/2} \left(\mathcal{G}_{\widehat{\rho}, q}^v(s) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t_0/2,\cdot}_{\widehat{\rho}}^2\right) < \infty \label{E:Restartbdd}
% % \end{align}
% %With all this said, \eqref{E:Restartbdd} implies that
% % \begin{align*}
% % \mathbb{P}\{ u(t_0,\cdot) \in \mathscr{K}(\Lambda) \}
% % & \ge 1- \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \left(\Theta\int_0^{t_0/2} \left(\mathcal{G}_{\widehat{\rho}, q}^v(s) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s\right) - \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t_0/2,\cdot)}_{\rho}^2\right)
% % \end{align*}
% % Therefore, we can choose $\Lambda > 0$ big enough such that
% %  \[
% %   \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q} \left(\Theta\int_0^{t_0/2} \left(\mathcal{G}_{\widehat{\rho}, q}^v(s) \mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s\right) + \frac{1}{\Lambda^2} \mathbb{E}\left(\Norm{u(t_0/2,\cdot)}_{\rho}^2\right) < \epsilon,
% %  \]
% % then we get that
% % \begin{align*}
% % \mathbb{P}\{ u(t_0,\cdot) \in \mathscr{K}(\Lambda) \} & \ge 1- \epsilon.
...
% % \begin{align*}
% % \mathbb{P}\{ u(t,\cdot) \in \mathscr{K}(\Lambda) \} & \ge (1-
% % \epsilon)\mathbb{P}\{ \Norm{u(t-t_0/2,\cdot)}_{\widehat{\rho}} < \Lambda
% % \}.
% % \end{align*}
% % We now apply Chebyshev's inequality and \eqref{E:InitData} to see that for all $t > t_0/2$ that there exists a $C>0$ such that
% %  \[
% %   \mathbb{P}\{ \Norm{u(t-t_0/2,\cdot)}_{\widehat{\rho}} \ge \Lambda
% %   \} \le \frac{\mathbb{E}\left( \Norm{u(t-t_0/2,\cdot)}_{\widehat{\rho}}^2 \right)}{\Lambda^2} \le C\frac{\mathcal{G}_{\hat{\rho}, q}\left( t - \frac{t_0}{2} \right) }{\Lambda^2} \le \frac{C}{\Lambda^2}.
% %  \]
% % Thus, by enlarging $\Lambda$ if necessary,
% %  \[
% %   \mathbb{P}\{ \Norm{u(t-t_0/2,\cdot)}_{\widehat{\rho}} \ge \Lambda
