...
\begin{equation} \label{E:SHE}
  ...
  \end{cases}
\end{equation}
which can be interpreted as the following stochastic integral equation: 
...
Any $L^2(\Omega)$-continuous processes, $u=(u(t,x): \; t \ge 0,\; x\in \R^d)$, that is measurable with respect to the natural filtration generated by $W$ that solves \eqref{E:gmsol} is referred to as a {\it mild solution}. See \cite{chen.huang:19:comparison} and for more.

The noise, $\dot{W}(t,s)$, is a centered Gaussian noise that is white in time and homogeneously correlated in
space. Its covariance structure, $J$, is defined on $C_c^\infty(\R^{d+1}) \times C_c^\infty(\R^{d+1})$ as follows:
\begin{align} \label{E:Cor}
   J(\psi,\phi) := \E \left[ \dot{W}\left(\psi\right)\dot{W}\left(\phi\right) \right]
  = \int_0^\infty \ud s \int_{\R^d} \Gamma(\ud x)(\psi(s,\cdot)*\tilde{\phi}(s,\cdot))(x),
\end{align}
where $\tilde{\phi}(x):= \phi(-x)$ and $\Gamma$ is a tempered measure on $\R^d$ that is
nonnegative and nonnegative definite and is referred to as the {\it correlation measure}. When it is absolutely
...
required to guarantee the existence of invariant measures for the solution to \eqref{E:SHE}, which is a crucial step towards the study of the ergodicity of the system. Existence in unbounded domains has been studied in few
...
In the setting of equation \eqref{E:SHE}, Tessitore and Zabczyk \cite{tessitore.zabczyk:98:invariant} verified \eqref{E:BddMnt} when $\mu(\ud x) = \ud x$ under the assumption that the spectral density $\hat{f}$ satisfies \textcolor{blue}{...reference example below...}. We expand this result and show the existence of an invariant measure when $\mu(\cdot)$ satisfies \eqref{E:InitData} and \eqref{E:GH-int} below, as well as one of the following:
\begin{enumerate}
...
\end{enumerate}  
Some applicable deterministic intitial measures include (see Example \ref{E:Gamma+f} below): 
	\[
