...
\begin{example}[More initial conditions not in $L_\rho^2(\R^d)$] \label{Ex:Riesz}
  ...
  Hence,
  \begin{align*}
    J_0(t,x) \le C_d (2\pi t)^{-d/2} \int_0^\infty e^{-\frac{r^2}{2t}} r^{-\alpha + d-1} \ud r = C_* t^{-\alpha/2},
    \quad \text{with $C_*=\frac{2^{-\alpha/2}\Gamma\left((d-\alpha)/2\right)}{\Gamma(d/2)}$},
  \end{align*}
  which implies that
  \begin{align*}
    \mathcal{G}_\rho\left(t;|\cdot|^{-\alpha}\right)
    \le \int_{\R^d} J_0^2(t,x) \rho(x) \ud x
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &  = \frac{C_d}{\left(2\pi\right)^{d/2}}\int_0^\infty \big| |\xi| - x \big|^\alpha e^{-\frac{x^2}{2}}x^{d-1}\ud x \\
...
Let us denote the right-side of the second condition in \eqref{E:TZspec} by $M(f)$, namely,
\begin{align*}
...
\end{align*}
and consider two cases: \bigskip

{\bf\noindent Case I:~} If $\widehat{f}(\zeta)$ is strictly positive for all $\zeta\in\R^d$, then
there is no ambiguity when taking square root and hence one can remove the absolute value to see
that
\begin{align*}
  M(f)
...
(2\pi)^{d/2}\Upsilon(0)$. Comparing this with the second condition in \eqref{E:U0finite}, namely,
$L_b^{-2}> 256 \Upsilon(0)$, our condition is sharper when $d> \frac{16\log(2)}{\log(2\pi)} \approx
6.03$. \bigskip

{\bf\noindent Case II:~} If $\widehat{f}(\cdot)$ is not strictly positive, but only nonnegative,
namely, $\widehat{f}(\xi)=0$ for some $\xi\in\R^d$, how to remove the absolute value in
...
\begin{example}
  ...
  \begin{enumerate}
    ...
    \item However, if one chooses the positive branches, namely,
      $\sqrt{\widehat{f}(\xi)}=\left|\xi^{-1}\sin(\xi)\right|$, it is not clear how to compute its
      ...
      $\sqrt{\widehat{f}(\xi)}$ into a distribution, then taking the absolute value of a
      distribution (unless it is a measure) may be problematic. Or even one may find some choice
      such that $\sqrt{\widehat{f}(\xi)}$ is a well-defined function, it may well be possible to be
      signed and after taking absolute values, the integral in \eqref{E:TZspec} may blow up.
  \end{enumerate}
\end{example}
...
%   Moreover \footnote{This statement is from Lemma 2.5 of
...
\begin{corollary}
  ...
  \begin{equation} \label{E:mb0}
    ...
    \quad \text{for all $p$ s.t. } 1/p \in \left(128 L_b^2\Upsilon(0),1/2\right),
  \end{equation}
  where $C_p=\left(\sqrt{2}\vee \overline{\varsigma}\right)\sup_{t\ge 0} H(t;\gamma_p)^{1/2}<\infty$.
\end{corollary}
\begin{proof}
  ...
  \begin{equation}\label{E:pLip}
     32 p L_b^2 < \frac{1}{4\Upsilon(0)},
  \end{equation}
  we have all $p$-th moments ($p\ge 2$) bounded as given in \eqref{E:mb0}.
\end{proof}
