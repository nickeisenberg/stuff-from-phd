...
\end{align*}
Due to the {\it Krylov-Bogoliubov theorem} (see, e.g., \cite[Theorem
11.7]{da-prato.zabczyk:14:stochastic}), once the tightness of
...
\end{equation}
As shown in Lemma \ref{L:HUA} below, condition \eqref{E:Dalang-a0} implies condition
\eqref{E:Dalang-H} and under condition \eqref{E:Dalang-00}, these two conditions become equivalent.
The relations of these conditions are illustrated in Figure \ref{F:Dalang}.
...
\begin{theorem} \label{T:Lrho}
  ...
  \begin{enumerate}[(i)]
    ...
    \item for all $t > 0$, the initial condition $\mu$ satisfies $\mathcal{G}_{\rho}(t;\mu^*) < \infty$ where $\mu^* = 1 + |\mu|$ and
...
\begin{remark}\label{R:Condition}
We would like to make a quick comment about the condition in \eqref{E:G}. First, because the heat kernel, the measure $1 + |\mu|$ and $\rho$ are all positive, then clearly we have that 
	\[
		\mathcal{G}_{\rho}(t,|\mu|) \le \mathcal{G}_{\rho}(t;\mu^*).
	\]
On the other hand, we have that 
	\begin{align*}
		\mathcal{G}_{\rho}(t;\mu^*) &= \int_{\R^d} \left( \int_{\R^d} G(t,x-y) (1+ |\mu|)(\ud y)\right)^2\rho(x) \ud x \\
		&= \int_{\R^d} \big( 1+ J_0(t,x; |\mu|)\big)^2\rho(x) \ud x \\
		&\le 2\bigg(\Norm{\rho}_{L^1(\R^d)} +\mathcal{G}_{\rho}(t;|\mu|) \bigg).
	\end{align*}
Thus for all $t>0$, 
	\[
		\mathcal{G}_{\rho}(t;|\mu|) \le 
		\mathcal{G}_{\rho}(t;\mu^*) \le 
		C \bigg(1 + \mathcal{G}_{\rho}(t;|\mu|) \bigg),
		\qquad \text{where } C = \max\{ 2\Norm{\rho}_{L^1(\R^d)}, 2 \},
	\]
and so the finiteness of $\mathcal{G}_{\rho}(t;\mu^*)$ is equivalent to the finiteness of $\mathcal{G}_{\rho}(t;|\mu|)$. However, it will be notationally convenient to consider $\mu^*$ instead of just $|\mu|$.
\end{remark}
...
Recall from Remark \ref{R:Condition} that in order to verify both conditions \eqref{E:G} and \eqref{E:InitData}, then it suffices to just consider the term $\Norm{J_0(t,\cdot;|\mu|)}^2_{\rho}$.  
