...
to the \textit{parabolic Anderson model} \cite{carmona.molchanov:94:parabolic}.
 ...
 The aim of this paper is to investigate the conditions required to guarantee the existence of invariant measures for the solution to \eqref{E:SHE}, which is a crucial step towards the study of the ergodicity of the system. Due to a theorem by Krylov and Bogoliubov \cite[Theorem 3.1.1]{da-prato.zabczyk:96:ergodicity}, we see that that existence of the measure can depend on the initial condition. Namely under suitable conditions (see Theorem \ref{T:Main} below), the following measure, $\eta$, is invariant for \eqref{E:SHE}:
 ...
 \end{equation}
 In the above, $\{T_n\}$ is an appropriately chosen sequence of positive numbers increasing to infinity, $\varphi$ is the initial condition and $A \in \mathscr{B}(L^2_{\rho}(\R^d))$ which are the Borel subsets of the space of weighted square integrable functions (see Section \ref{S:Prelim}). 
 ...
 We note that proving uniqueness of the invariant measure is a much harder question. An open question is if $\varphi_1$ and $\varphi_2$ are two such initial conditions that are suitable for \eqref{E:InvMeasForm} and if $\eta_1$ and $\eta_2$ are the two invariant measures that arise from $\varphi_1$ and $\varphi_2$ respectively then does $\eta_1 = \eta_2$? Another question one can ask in the absence of uniqueness is what initial data are applicable to \eqref{E:InvMeasForm} and can we give explicit examples? We address the second question in Examples \ref{Ex:Gamma+f} and \ref{Ex:More} and also in the discussion below.
  ...
  We will see below in Theorem \ref{T:Lrho} that for any $t_0 >0$, the solution $u(t_0,\cdot)$ to \eqref{E:SHE} starting from an intitial condition that satisfies either \eqref{E:DetInit} or \eqref{E:RandInit}, almost surely lives in the Hilbert space, $\mathcal{E}_{t_0} = (E_{t_0}, \; \Norm{\cdot}_{E_{t_0}})$ where $E_{t_0}$ is the space of measurable random fields, $\{\varphi_{t_0}(x)\}_{\{x\in \R^d\}}$, that are $\mathcal{F}_{t_0}$ measurable, such that,
 	\[
 		\Norm{\varphi_{t_0}}_{E_{t_0}}^2 := \mathbb{E} \left(\Norm{\varphi_{t_0}(\cdot)}_{L^2_{\rho}(\R^d)}^2\right) < \infty,
 	\] 
 where $\Norm{\cdot}_{L^2_{\rho}(\R^d)}$ denotes the $\rho$-weighted $L^2(\R^d)$ norm (see Section \ref{S:Prelim}). 
 
 By an invariant measure for \eqref{E:SHE}, we mean a measure, $\eta$, on $\mathscr{B}(E_{t_0})$ such that
 	\begin{equation}\label{D:InvMeas}
 		\eta(A) = \int_{E} \mathscr{L}(u(t,\cdot, \zeta))(A) \; \eta(\ud 	\zeta), \quad \forall \; t \ge t_0,
	 \end{equation}
 where $A \in \mathscr{B}(L^2_{\rho}(
 \R^d))$ and $\mathscr{L}(u(t,\cdot,\zeta))(A)$ is the law of \eqref{E:SHE} starting from $\zeta \in E_{t_0}$. DaPrato and Zabczyk \cite{da-prato.g-atarek.ea:92:invariant} established the following procedure:
 \begin{enumerate}
...
\cite[Theorem 1.3 and Lemma 2.5]{chen.kim:19:nonlinear}, where necessary and sufficient conditions are given. More precisely, one has to have $L_b$ (the Lyapunov exponent of $b$) in \eqref{E:lipcon}  small enough, the spatial dimension $d \ge 3$, and we must strengthen \eqref{A:dalcon} to the following:
	\begin{equation} \label{E:U0finite}
		\int_{\R^d} \frac{\hat{f}(\ud \xi)}{|\xi|^2} =: \Upsilon(0) <\infty.
	\end{equation}
...
We point out that by Theorem \ref{T:Lrho}, that for any $t_0 >0$ the solution $u(t_0,\cdot)$ to \eqref{E:SHE} starting from an intitial condition that satisfies either \eqref{A:dalcon0}, lives in the Hilbert space, $\mathcal{E}_{t_0} = (E_{t_0}, \; \Norm{\cdot}_{E_{t_0}})$ where $E_{t_0}$ is the space of random fields, $\{\varphi_{t_0}(x)\}_{\{x\in \R^d\}}$, that are $\mathcal{F}_{t_0} \times \mathscr{B}(\R^d)$ measurable, such that,
	\[
		\Norm{\varphi_{t_0}}_{E_{t_0}}^2 := \mathbb{E} \left(\Norm{\varphi_{t_0}(\cdot)}_{L^2_{\rho}(\R^d)}^2\right) < \infty.
	\] 
Indeed, let $u$ denote the mild solution to \eqref{E:SHE} starting from $\mu$ which satisfies the conditions of Theorem \ref{T:Main}. For any $t_0 >0$, consider the following restarted equation:
...
This is a very import observation. Recall that Tessitore et.al. \cite[Theorem 3.3]{tessitore.zabczyk:98:invariant} proved that there exists in invariant measure in $L^2_{\rho}(\R^d)$ of the form \eqref{E:InvMeasForm} for the constant initial data $\varphi = 1$. Because of the restart property of the equation, we may rewrite \eqref{E:InvMeasForm} as the following:
	...
	\end{align*}
where we recall that $A \in \mathscr{B}(L^2_{\rho}(\R^d))$ and $v$ denotes the solution to \eqref{E:restart} starting from $u(t_0/2, \cdot, 1)$ with the time-shifted noise $\dot{W}_{t_0/2}$. Hence, whether we originally start our equation \eqref{E:SHE} at the constant 1 as in \cite{tessitore.zabczyk:98:invariant} or a measure that satisfies the assumptions of Theorem \ref{T:Main} main below, for example the Dirac delta measure $\delta_0$, then either way end end up in the same function space. In other words, the solution $u(t_0,\cdot) \in \mathcal{E}_{t_0}$ for any $t_0 >0$. 
...
\begin{theorem} \label{T:Lrho}
	...
	\begin{enumerate}[(i)]
		...
		\mathcal{G}_{\rho,2}(t)
		= \mathcal{G}_{\rho,\mu^*}(t)
		:= \int_{\R^d} \left(J^*(t,x)\right) ^2 \rho(x)\: \ud x< \infty
...
\begin{theorem} \label{T:Main}
  Consider \eqref{E:SHE} starting from $\mu$ which satisfies \eqref{E:RandInit}. Assume that all assumptions of Theorem \ref{T:Lrho} hold. Let $q >2$ be arbitrary but fixed. Moreover, suppose that
  \begin{enumerate}[(i)]
...
Lastly we need to verify \eqref{E:GH-int}. But essentially all the work for this has been completed. The verification of \eqref{E:H} implies that $\mathcal{H}_\alpha(s)$ is continuous on $s \in [0, T]$ for any $T>0$ and we already saw aboves that $\mathcal{G}_{\widehat{\rho}, q}(s)$ was bounded on $[0, T]$. These two facts imply \eqref{E:GH-int}, thus allowing us to apply Theorem \eqref{T:Main} and verify the existence of an invariant measure to \eqref{E:SHE} with initial data, $\mu(\cdot) = u(t_0, \cdot)$ and noise $W$ with correlation function given to be the Bessel kernel $B_s$. Since we are only concerned with the long term behavior of the solution and because for all $t >0$ we have $u(t + t_0,x) \stackrel{\mathcal{L}}{=} v(t,x)$, what we have actually done was verify the existence of an invariant measure for \eqref{E:SHE} starting from $\delta_0$ with correlation function $B_s(x)$.
\end{example}
