...
functions on $H$. Following \cite{da-prato.g-atarek.ea:92:invariant}, a probability measure $\eta$
...
\begin{align} \label{E:Dalang-00}
  ...
  & \text{and} \quad 64 L^2_{b}   <  \frac{1}{4\Upsilon(0)}\:.\label{E:Lipb}
\end{align}
\end{subequations}
...
\end{equation}
which can be easily verified. We also will repeatedly use the following spherical coordinate formula:
	\[
		\int_{\R^d} f(|x|) \ud x = \sigma(\mathbb{S}^{d-1}) \int_0^\infty f(r) r^{d-1} \ud r,
	\]
and we may from time to time use the following $\sigma(\mathbb{S}^{d-1})  = 2(\pi)^{d/2} / \Gamma(d/2)$.
...
\begin{proof}
  ...
  \end{align*}
  In the following, let $C$, $C_\alpha$, $C_\alpha'$, $C_{\alpha, d}$ and $C'_{\alpha, d}$ be generic constants that may depend on
  ...
  \end{align*}
  Note that in the first inequality of the above chain of inequalities, we have considered two
...
\begin{proposition} \label{P:Bessel}
  ...
  \begin{gather} \label{E:UpsionB}
    \Upsilon_{\alpha}(0) = \frac{\Gamma\left(\frac{d}{2} -1 + \alpha\right) \Gamma\left( \frac{2-d}{2} + 1 - \alpha \right)}{2^d \pi^{d/2} \: \Gamma(d/2)\:\Gamma\left(s/2\right)},
    \qquad \text{for all $s>d-2$ and $d>2$, and} \\
    \label{E:HaB} \mathcal{H}_\alpha(t) <\infty \quad \forall t>0 \quad \Longleftrightarrow \quad
    0<\alpha<\frac{1}{2} - \frac{(d-s)_+}{4}\quad \text{and} \quad s>d-2>0,
  \end{gather}
  where $a_+ := \max(a,0)$ and $\mathcal{H}_\alpha(t)$ is defined in \eqref{E:H} below.
  \textcolor{magenta}{In particular, when $\alpha=0$, we have
  \begin{align*}
   \Upsilon(0) = ... \text{Nick, please include this special case}
  \end{align*}}
  Moreover, for $\alpha\in(0,1/2)$, we have the following series expansion of
...
\end{proposition}
\begin{proof}
  By the spherical coordinates and \eqref{E:FB}, for all $\alpha\in[0,1)$,
  \begin{align*}
  ...
  \end{align*}
  which is finite provided that $s>d-2(1-\alpha)>0$. This proves \eqref{E:UpsionB}.
...
\textcolor{blue}{Why is the Fourier inverse used here and what happened to the $(2\pi)^{2d}$ term in the denominator from \eqref{E:TZspec}? Pretty sure it is a type since letting
\[
	  M(f) := \frac{\Gamma(d/2 -1)2^{d/2-2}}{(2\pi)^{2d}} \int_{\R^d} \left(\left| \mathcal{F}\left(\sqrt{ \widehat{f}\: }\right) \right| * \left| \mathcal{F}\left(\sqrt{ \widehat{f}\: }\right)\right|\right) (\zeta) |\zeta|^{2-d} \ud \zeta,
\]
simplifies correctly to
	\[
	\frac{1}{(2\pi)^{3d/2}} \int_{\R^d} \widehat{f}(\zeta)  |\zeta|^{-2} \ud \zeta
	\]
}
\textcolor{magenta}{Either way does the job, Fourier or inverse Fourier. The difference is a factor
of $(2\pi)^{-d}$. Please do it again.}
...
(2\pi)^{d/2}\Upsilon(0)$. Comparing this with \eqref{E:Lipb}, namely, $L_b^{-2}> 256 \Upsilon(0)$,
our condition is sharper when $d> \frac{16\log(2)}{\log(2\pi)} \approx 6.03$. \bigskip


\textcolor{blue}{Have you seen remark 3.6 \cite{tessitore.zabczyk:98:invariant}? They seem to ignore the issue that arises when $\hat{f}$ has zeros.}

\textcolor{magenta}{Yes, Remark 3.6 doesn't specify which square root to take and only assume that
$\mathcal{F}\left(\sqrt{\widehat{f}}\right)\ge 0$. This condition is satisfied easily for case 1 in
Example \ref{Ex:sqrt} but is far from being clear for case 2. Can you tell for case 2 of the example
below, whether $\mathcal{F}\left(\sqrt{\widehat{f}}\right)\ge 0$?}
...
\textcolor{blue}{We have already denoted $\mu$ as a deterministic measure and in the following theorem we say that it is random. Also, when $\mu$ is random, we need to specify what $|\mu|$ means in Corollary \ref{C:Mom}. When $\mu$ is random then $|\mu| = \Norm{\mu}_{p}$.}

\textcolor{magenta}{Yes, this has to be made consistent. I haven't gone through this. Can you make
the corresponding updates regarding this point?}
...
\begin{corollary} \label{C:Mom}
  ...
  \begin{equation} \label{E:mb0}
    ...
    \quad \text{for all $p$ s.t. } 1/p \in \left(128 L_b^2\Upsilon(0),1\right),
  \end{equation}
...
\begin{proposition} \label{P:F-Comp}
  Let $\rho$ and $ \tilde{\rho}$ be given in condition (i) of Theorem \ref{T:Main} (see
...
\begin{lemma} \label{L:yineq}
  ...
  \textcolor{blue}{We need to define $\mu^*$ in the case that $\mu$ is random and also deterministic. In this case we have that
  \[ \mu^* =
  		\begin{cases}
	  		1 + |\mu| & \text{$\mu$ deterministc} \\
	  		1 + \Norm{\mu}_q & \text{$\mu$ random}.
  		\end{cases}
  \]
}
  \begin{enumerate}[(1)]
    ...
    \item Under the two conditions in \eqref{E:U0finite}, if the integral in \eqref{E:H} is finite for
      some $\alpha\in \left(128 L_b^2 \Upsilon(0),\: 1/2\right)$, then for any $q$ with $1/q\in
      \left(128 L_b^2 \Upsilon(0), \: \alpha\right)$, the function $H\left(t;32qL_b^2\right)$ in
      \eqref{E:NormY} is uniformly bounded in $t\ge 0$, i.e., $\sup_{t\ge
      0}H\left(t;32qL_b^2\right)<\infty$;
    \item Under the two conditions in \eqref{E:U0finite}, if the integral in \eqref{E:H} is finite for
      some $\alpha\in \left(128 L_b^2 \Upsilon(0),\: 1/2\right)$, then for any $q$ with $1/q\in
      \left(128 L_b^2 \Upsilon(0), \: \alpha\right)$ and for any nonnegative and
...
\textcolor{blue}{
I find it a little confusing when we restart at $t-t_0$. The first centered equation says that
	\[
		    v(t_0,\cdot) \stackrel{\mathcal{L}}{=} u(t,\cdot) \quad \text{for $t\ge\tau$}.
	\]
which is a little confusing since we said already that $t_0 = \tau / 2$ is fixed. So saying that $ v(t_0,\cdot) \stackrel{\mathcal{L}}{=} u(t,\cdot)$ for any $t \ge \tau$ makes is seem like $u$ is constant. But what really is going on is that $v$ is implicitly defined through $t$ in the sense that
	\[
		v(s, x) \stackrel{\mathcal{L}}{=} u(s, x, u(t-t_0,x)) \quad s \ge 0
	\]
where $t \ge 2t_0 > 0$. \bigskip \\
This seems to be avoidable if we just consider
	\[
			v(t, x) \stackrel{\mathcal{L}}{=} u(t, x, u(t_0,x)) \quad t \ge 0
	\]
where $t_0 > 0$ is arbitrary.
}

\textcolor{magenta}{Restarting at $t-t_0$ is crucial, check the original proof by Tessitore and
Zabczyk. This is also needed in the proof below for $ I_1$ to be uniform in time. Note that the
integral is from zero to $ t_0$ in the upper bound of $I_1$, if it is from $ t_0$ to $t$, then this
upper bound will depend on $t$. $u$ is not constant certainly, but you can always think that it is
started just $t_0$ period time ago, in other words, at $ t_0$ time ago, $u$ was started somewhere
and what you have now is that process having been run just for $ t_0$ period of time.\\
For suggestion of $v(t,x)$, could you carry out the rest of the proof to make sure you get bounds
uniform in time? This is exactly what we missed before. Please check the proof below and also
those by Tessitore and Zabczyk. Let me know if you agree or maybe I have missed something.}
...
\begin{proof}[Proof of Theorem \ref{T:Main} (a)]
  ...
  $t- t_0$. Then (see Figure \ref{F:RestartedSHE} for some illustrations)
  % \begin{align*}
  %   v(t_0,\cdot) \stackrel{\mathcal{L}}{=} u(t,\cdot) \quad \text{for $t\ge\tau$}.
  % \end{align*}
...
\begin{figure}[htpb]
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  \caption{Some illustrations for the restarted SHE in \eqref{E:RestaredSHE}.}
...
\end{figure}
  ...
  \end{align*}
  is relative compact in $L_\rho^2(\R^d)$.

\textcolor{blue}{I think there is some redundancy in the following paragraph. Note that we only need $\mathcal{H}_{\alpha}(t) < \infty$ for any $t \ge 0$. We already proved that $\Upsilon_{2\alpha}(0) < \infty$ will imply this. So for that reason, there is no reason to say that "Now we can apply Part (3) of Lemma \ref{L:HUA}, thanks to
	\eqref{E:Dalang-00}, to see that \eqref{E:H} holds." \bigskip \\
Lastly, $\Upsilon_{2\alpha}(0) < \infty$ seems to be sufficient but not necessary and really what we really need is only for $\mathcal{H}_{\alpha}(t) < \infty$. Maybe we want to make a remark about this. It doe not matter too much since the Bessel kernel easily verifies both.
}

\textcolor{magenta}{I agree with the first paragraph. I have updated the statement accordingly. \\
  As for the second paragraph, note that we always assume that $\Upsilon(0)<\infty$, under which
$\Upsilon_{2\alpha}(0)<\infty$ is equivalent (not only sufficient, but also necessary) to
$\mathcal{H}_\alpha(t)<\infty$ for all $t>0$. Have you missed this part -- Part (3) of Lemma
\ref{L:HUA} or do you have any concern about this lemma?}
  ...
  \eqref{E:Dalang-00}, to see that \textcolor{magenta}{$\Upsilon_{2\alpha}(0)<\infty$ if and only
  if} \eqref{E:H} holds. Therefore, both Lemmas \ref{L:Factorize} and \ref{L:yineq} (in particular,
  the part (3)) are applicable. In particular, Lemma \ref{L:Factorize} ensures that the following
...
\textcolor{blue}{
Let $t_0 > 0$ and consider $v(t,x) = u(t,x; u(t_0 / 2,\cdot)) = u(t+t_0,x; \mu)$ so that $u(t_0,x) = v(t_0 /2, x)$. When we have a measure initial condition we sort of need to run a little time, and then run a little more time. What I mean is that we start with $u(t,x; \mu)$ and then consider $u(t_0 / 2, x ; \,mu)$ and use this as our new starting point. Then we start Tessitore's argument. \bigskip \\
So we have $u(t_0,x) = v(t_0 /2, x) = (G(t_0, \cdot)*u(t_0/2, \cdot))(x) + \frac{\sin(\alpha \pi)}{\pi}[F_{\alpha}Y_v](t_0/2, x)$. We define 
	\[
		\mathscr{K}(\Lambda) := \left\{ \big(G(t_0,\cdot)*y(\cdot)\big)(x)+(F_\alpha h)(t_0/2, x): \:\Norm{y}_{\tilde{\rho}}
		\le \Lambda \quad \text{and}\quad\Norm{h}_{L^q\left((0,t_0/2);L^2_{\tilde{\rho}}\left(\R^d\right)\right)} \le \Lambda \right\}.
	\]
Then,
	\begin{align*}
		\mathbb{P}\left\{ u(t_0, \cdot) \not \in \mathscr{K}(\Lambda) \right\} &= \mathbb{P}\left\{ v(t_0/2, \cdot) \not \in \mathscr{K}(\Lambda) \right\} \\
		& \le \mathbb{P}\left\{ \left( \int_0^{t_0/2} \Norm{Y_v(s,\cdot)}^q_{\tilde \rho} \ud s\right)^{1/q} > \frac{\pi \Lambda}{\sin(\alpha \pi)} \right\} + \mathbb{P}\left\{ \Norm{u(t_0/2, \cdot)}_{\tilde \rho} > \Lambda \right\} \\
		&\le \frac{\sin^q(\alpha \pi)}{\pi^q \lambda^q} \mathbb{E} \int_0^{t_0 / 2} \Norm{Y_v(s,\cdot)}_{\tilde \rho}^q \ud s + \frac{1}{\Lambda^2} \mathbb{E}\left( \Norm{u(t_0/2,\cdot)}_{\tilde \rho}^2 \right) \\
		& \le \frac{\sin^q(\alpha \pi)}{\pi^q \lambda^q} \Theta \int_{0}^{t_0/2} \left(\mathcal{G}_{\tilde \rho}(s + t_0 /2; \mu)\mathcal{H}_{\alpha}(s)\right)^{q/2} \ud s + \frac{1}{\Lambda^2} \mathbb{E}\left( \Norm{u(t_0/2,\cdot)}_{\tilde \rho}^2 \right) < \infty.
	\end{align*}
Note that  $\mathbb{P}\left\{ \Norm{u(t_0/2, \cdot)}_{\tilde \rho} > \Lambda \right\}$ from the second line of the proof does not appear in Tessitore's paper. This is because they have a deterministic initial condition and they do not need to do this initial runnng of $t_0/2$ time and they can immediately choose their initial condition, $\varphi$, to be such that $\Norm{\varphi}_{\tilde \rho} < \Lambda$. We can't do this directly and this is why we have to start by running $t_0/2$ time and then consider $u(t_0/2,x)$ as our starting point. \bigskip \\
Continuing above, we can choose $\Lambda>0$ sufficiently big so that 
	\[
		\mathbb{P}\left\{ u(t_0, \cdot) \in \mathscr{K}(\Lambda) \right\} \ge 1 - \epsilon.
	\]
Now we can continue with your $t-t_0/2$ argument. Indeed, pick any $t > t_0$. The Markov property of the solution allows us to say that 
	\begin{align*}
		\mathbb{P}\left\{ u(t, \cdot) \in \mathscr{K}(\Lambda) \right\} &= \mathbb{P}\left\{ u(t, \cdot) \in \mathscr{K}(\Lambda) \Big\vert  \Norm{u(t-t_0/2, \cdot)}_{\tilde{\rho}} < \Lambda \right\} \mathbb{P}\left\{ \Norm{u(t-t_0/2, \cdot)}_{\tilde \rho} < \Lambda \right\} \\
		& \ge (1-\epsilon) \mathbb{P}\left\{\Norm{u(t-t_0/2, \cdot)}_{\tilde \rho} < \Lambda \right\}.
	\end{align*}
This is what Tessitore does in the centered equation above Remark 3.2. Lastly we finish by saying 
	\[
		\mathbb{P}\left\{\Norm{u(t-t_0/2, \cdot)}_{\tilde \rho} < \Lambda \right\} \le \frac{\mathbb{E}\left( \Norm{u(t-t_0/2, \cdot)}_{\tilde \rho}^2 \right) }{\Lambda^2} \le C \frac{\mathcal{G}_{\tilde \rho}(t - t_0 / 2; \mu) }{\Lambda^2}.
	\]
There we can enlarge $\Lambda$ to get $\mathbb{P}\left\{\Norm{u(t-t_0/2, \cdot)}_{\tilde \rho} < \Lambda \right\} < \epsilon$ for all $ t \ge t_0$. This completes the proof.
}
