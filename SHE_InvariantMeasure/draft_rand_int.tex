...
\begin{proof}
  ...
  \end{align*}
  \textcolor{blue}{Then by applying the change of variables $r' = t^{-1/2}r$},
  \begin{align*}
    I(t,x) \le C \int_0^\infty e^{-\frac{r^2}{2} + t^{b/2} r^b} r^{\textcolor{blue}{d}-1}\ud r
           \le C \int_0^\infty e^{-\frac{r^2}{2} + T^{b/2} r^b} r^{\textcolor{blue}{d}-1}\ud r
           < \infty.
  \end{align*}
  ...
  \begin{align*}
    | y | ^b =  \left(y_1^2+y_2^2+\cdots+y_d^2\right)^{\textcolor{blue}{b} /2}
            \le |y_1|^b + \left(y_2^2+\cdots+y_d^2\right)^{\textcolor{blue}{b}/2}
            = |y_1|^b + |y_*|^b.
  \end{align*}
  Hence,
  \begin{align*}
  ...
  Now denote $K(r):=\int_0^r  e^{ - y^2 + r^b - (r-y)^b} \ud y$.
  \begin{align*}
      \lim_{r\to\infty} \frac{K(r)}{r^{b-1}}
      = \lim_{r\to\infty} \frac{b\int_0^r \left(r^{b-1}-(r-y)^{b-1}\right) e^{ - y^2 + r^b - (r-y)^b} \ud y}{r^{b-1}}
  \end{align*}



  Now, set $g(r) = r^b - (y-r)^2$ with $y>0$ and $r>0$. By differentiation, we find that the function
  achieve its minimum at $g'(r)=0$, namely,
  \begin{align*}
    g'(r) = b r^{b-1}-2(r-y) = 0 \quad \Longleftrightarrow \quad y = r_0 - \frac{b}{2} r_0^{b-1},
  \end{align*}
  where we use $r_0$ or $r_0(y)$ to denote the point where the function $g$ achieve maximum. Hence,
  for all small $\epsilon\in(0,1)$,
  \begin{align*}
    I\left(1/2,x_r\right)
    \ge C\int_{y_0-\epsilon}^{y_0+\epsilon} e^{g(r) -y^b} \ud y
    \approx C \epsilon \exp\left(g\left(r_0(y)\right) -y_0^b\right).
  \end{align*}
  It reduces to show that the above exponential goes to infinity as $r\to\infty$. Notice that
  \begin{align*}
    g(r_0(y)) = r_0^b-(r_0(y) -y)^2
              = r_0^{b}- \frac{b^2}{4} r_0^{2(b-1)},
  \end{align*}
  and because $b\in(1,2)$,
  \begin{align*}
    y_0^b =  \left(r_0 - \frac{b}{2} r_0^{b-1}\right)^b
         \le r_0^b - \frac{b^b}{2^b} r_0^{b(b-1)}.
  \end{align*}
  Hence,
  \begin{align*}
   g(r_0) - y_0^b  \ge \frac{b^b}{2^b} r_0^{b(b-1)} - \frac{b^2}{4} r_0^{2(b-1)}.
  \end{align*}
 ...
 -----
  ...
  Hence,
  \begin{align*}
    I(1/2,x_r) \ge C\int_0^\infty \: t^{-1/2} e^{ - \frac{|y-r|^2}{2t} + (r^b - y^b)}\ud y.
  \end{align*}
\end{proof}
