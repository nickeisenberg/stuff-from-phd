...
\textcolor{magenta}{In this case, we assume that
\begin{align} \label{E:Ff>0}
  \text{both $\mathcal{F}^{-1}\big(\sqrt{\widehat{f}\:}\:\big)$ and $\widehat{f}$ are strictly positive on $\R^d$.}
\end{align}
This is not a trivial assumption (see Example \ref{Ex:Finv} below) and one can check that the Bessel
kernels satisfy this condition. Under \eqref{E:Ff>0}, there} is no ambiguity when taking the square
root and one can remove the absolute value to see that
\begin{align*}
...
\begin{example} \label{Ex:Finv-d}
  ...
  \begin{align*}
    \Upsilon_\alpha(0)
    & = (2\pi)^{-d} \int_{\R^d} \frac{\prod_{i=1}^d\max\{2-|\xi_i|,0\}}{|\xi|^{2(1-\alpha)}} \ud \xi
      = (2\pi)^{-d} \frac{2 \pi^{d /2}}{\Gamma(d/2)}\int_0^{2} \left(2-r\right) r^{-2(1-\alpha)} r^{d-1} \ud r \\
    & = \frac{4^{\alpha } \pi^{-d/2}}{(2 \alpha +d-2) (2\alpha +d-1) \Gamma\left(d/2\right)}, \quad
    \text{provided $\alpha>1-d /2$,}
  \end{align*}
