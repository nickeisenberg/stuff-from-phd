...
to the \textit{parabolic Anderson model} \cite{carmona.molchanov:94:parabolic}.
 ...
 The aim of this paper is to investigate the conditions required to guarantee the existence of invariant measures for the solution to \eqref{E:SHE}, which is a crucial step towards the study of the ergodicity of the system. We will see below in Theorem \ref{T:Lrho} that for any $t \ge t_0 >0$, the solution $u(t,\cdot)$ to \eqref{E:SHE} starting from an intitial condition that satisfies either \eqref{E:DetInit} or \eqref{E:RandInit}, almost surely lives in the Hilbert space, $\mathcal{E}_{t_0} = (E_{t_0}, \; \Norm{\cdot}_{E_{t_0}})$ where $E_{t_0}$ is the space of measurable random fields, $\{\varphi_t(x)\}_{\{ t \ge t_0, \; x\in \R^d\}}$, adapted to $\{\mathcal{F}_t\}_{t \ge t_0}$, such that for any $t\ge t_0$,
 	\[
 		\Norm{\varphi_t}_{E_{t_0}}^2 := \mathbb{E} \left(\Norm{\varphi_t(\cdot)}_{L^2_{\rho}(\R^d)}^2\right) < \infty,
 	\]
 ...
 \]
 where $A \in \mathscr{B}(E_{t_0})$ and $\mathscr{L}(u(t,\cdot,\zeta))(A)$ is the law of \eqref{E:SHE} starting from $\zeta$. DaPrato and Zabczyk \cite{da-prato.g-atarek.ea:92:invariant} established the following procedure:
 \begin{enumerate}
...
see Hypothesis 2.1 ({\it ibid.}). However as was illustrated in \cite[Theorem 3.3]{tessitore.zabczyk:98:invariant}, in order to apply this theorem, the following additional assumptions were needed: $d \ge 3$ and the Lipschitz constant of $b$ must satisfy $L(b)^{-2} > \textbf{l}$, where
	...
	\end{equation}
where $\mathbf{\Gamma}$ is the Gamma function. With these assumptions, they were able to prove that \eqref{E:SHE} starting from the constant $1$ satisfies \eqref{E:BddMnt} and thus is bounded in probability, verifying the existence of an invariant measure. In other words, the invariant measure takes the form of \eqref{E:InvMeasForm} below with $\varphi = 1$. 

Proving uniqueness of the measure is a much harder task. Tessitore and Zabczyk \cite{tessitore.zabczyk:98:invariant} apply the {\em Krylov-Bogoliubov} theorem \cite[Theorem 3.1.1]{da-prato.zabczyk:96:ergodicity} in order to show existence of the invariant measure which shows the invariant measure, $\eta$, takes the following form:
	\begin{equation} \label{E:InvMeasForm}
		\eta(A) = \lim_{n \to \infty} \frac{1}{T_n} \int_1^{T_n + 1} \mathscr{L}(u(t,\cdot, \varphi))(A) \ud t.
	\end{equation}
In the above, $\{T_n\}$ is an appropriately chosen sequence of positive numbers increasing to infinity. Thus, we see from this theorem that the invariant measure arises from the initial condition. An open question is if $\varphi_1$ and $\varphi_2$ are two such initial conditions that satisfy \eqref{E:BddMnt} and if $\eta_1$ and $\eta_2$ are the two invariant measures that arise from $\varphi_1$ and $\varphi_2$ respectively then does $\eta_1 = \eta_2$? 

The additional assumptions given by Tessitore and Zabczyk parallel our assumptions \eqref{E:U0finite} and \eqref{E:LipLb}. However, it is not at all trivial to check which spectral densities, $\hat{f}(\ud \xi)$, satisfy \eqref{E:TZspec} and no such examples were given in \cite{tessitore.zabczyk:98:invariant}. For example, it may not be obvious from \eqref{E:TZspec} that one can not take the spectral density of the noise to be the Reisz kernel, $\hat{f}(\xi) = |\xi|^{d-\alpha}$, for any $0 < \alpha < d$ but this can be easily seen from our condition \eqref{E:U0finite}. The condition that $d \ge 3$ forces us to only consider a colored noise as in \eqref{E:Cor} and so the loss of the ability to apply the Riesz kernel is unfortunate. However, as we will see in Example \ref{E:Gamma+f} below, we are able to apply the Bessel kernel, $B_s(x)$, with $s < d$, which has exponential decaying tails and power growth at the origin. So in absence of the Riesz kernel, we still may apply our results to a kernel, namely the Bessel kernel, that has origin blow up. 

Moreover, our method allows us to verify \eqref{E:BddMnt} for more general initial conditions $\mu(\cdot)$. Some applicable deterministic initial measures include (see Example \ref{E:Gamma+f} below):
	\[
		\text{Dirac delta measure }\delta_0(\ud x) \quad \text{and} \quad \varphi(x)\ud x \text{ with } \varphi \in C^{\infty}(\R^d),
	\]
but more generally, any initial data $\mu$ that satisfies \eqref{E:InitData} and \eqref{E:GH-int} below, as well as one of the following:
\begin{fleqn}
\begin{align}
...
\begin{remark}
	We point out that \eqref{E:SHE} starting from any deterministic measure, $\mu$, as in \eqref{E:DetInit} can be viewed as a solution of \eqref{E:SHE} starting from a process $\varphi \in L^2(\Omega, L^2_{\rho}(\R^d))$. Indeed, let $u$ denote the mild solution to \eqref{E:SHE} starting from $\mu$ which satisfies the conditions of Theorem \ref{T:Main}. For any $t_0 >0$, consider the following restarted equation:
	...
	where $\dot{W}_{t_0}(t,x) := \dot{W}(t + t_0,x)$ denotes the time shifted noise. Ie,
		...
		\]
	By Theorem \ref{T:Solve}, $u(t_0,\cdot)$ is $\mathcal{F}_{t_0}$ measurable and by \eqref{E:uInLrho}, we see that $u(t_0,\cdot) \in L^2(\Omega, L^2_{\rho}(\R^d))$. Moreover, the semi-group property of the heat kernel implies that for any $t\ge 0$, we have $u(t + t_0,x) \stackrel{\mathcal{L}}{=} v(t,x)$ where $\mathcal{L}$ denotes equality in law. Since we are interested in studying the long time behavior of \eqref{E:SHE}, we obtain the same information by choosing to study the restarted equation. For this reason unless we state otherwise, we will focus our attention completely on \eqref{E:SHE} starting from a $\mathcal{F}_0$ measurable processes $\varphi \in L^2(\Omega, L^2_{\rho}(\R^d))$, in others words \eqref{E:SHE} starting from data which satisfy \eqref{E:RandInit}. This simplification of \eqref{E:SHE} to the restarted equation is key in order to argue the existence of an invariant measure. See Example \ref{E:Gamma+f} below for a continue of this discussion.
...
\begin{theorem} \label{T:Lrho}
  Consider \eqref{E:SHE} starting from $\mu$ which satisfies \eqref{E:RandInit}. Assume that
  \begin{enumerate}[(i)]
