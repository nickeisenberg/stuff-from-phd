...
{\bf\noindent Situation I:~} If both $\mathcal{F}^{-1}\left[\sqrt{\widehat{f}(\cdot)}\right](\xi)$ and $\widehat{f}$ are strictly positive for all $\zeta\in\R^d$, then
...
\end{figure}
One should note that the assumption that $\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(\zeta)$ is nonnegative is quite strong and requires $\sqrt{\widehat{f}}$ to be non-negative definite, which does not follow from the non-negative definiteness of $f$. Indeed, suppose that $f$ was such that $\widehat{f}(\xi) = 2^{-2}\max\{2-|\xi|,0\}$. Then $f$ is non-negative and non-negative definite, which is shown in Example \ref{Ex:sqrt} below. Moreover, one can show that in this case that $\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(\zeta)$ takes on both positive and negative values. Indeed, since $\sqrt{\widehat{f}(\xi)} = 2^{-1}\sqrt{\max\{2-|\xi|,0\}}$ is even, the inverse Fourier transform takes the following form:
	\[
...
Where in the first equality we used the change of variables, $x' = 2 - x$ and in the second equality we used the the change of variables $x' = \sqrt{x}$. We now do an integration by parts to see that 
	...
	\end{align*}
and lastly applying the change of variables $x' = x\sqrt{2\zeta/\pi}$ gives us 
	\[
...
{\bf\noindent Situation II:~} Suppose $\widehat{f}(\cdot)$ is not strictly positive, but only nonnegative,y
