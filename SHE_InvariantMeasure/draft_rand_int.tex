...
\begin{remark}[$L^\infty(\R^d)$ initial condition] \label{R:BddInit}
  ...
  \varphi(x)\ud x$ with $\varphi\in L^\infty(\R^d)$, then many conditions in both Theorems
  \ref{T:Lrho} and \ref{T:Main} can be significantly simplified. To be more precise, conditions
  \eqref{E:G} and \eqref{E:InitData} are trivially satisfied because:
  \begin{align*}
    \mathcal{G}_\rho(t;\varphi) \le \Norm{\varphi}_{L^\infty(\R^d)}^2 \Norm{\rho}_{L^1(\R^d)}< \infty
    \quad \text{uniformly for all $t\ge 0$.}
  \end{align*}
  Accordingly, condition \eqref{E:GH-int} reduces to
  \begin{align} \label{E:GH-int-bdd}
...
\begin{proposition} \label{P:Bessel}
  Let $f_s$ be the Bessel kernel defined in Example \ref{Ex:Gamma+f}. Then we have:
...
\begin{proof}
  ...
  \end{align*}
  Notice that for $r\in (0,t]$,
  \begin{align*}
    \int_0^\infty \ud z \; \frac{\exp(-z^2)}{(t+z^2)^{s/2}} z^{d-1}
    \le
    \int_0^\infty \ud z \; \frac{\exp(-z^2)}{(r+z^2)^{s/2}} z^{d-1}
    \le
    \int_0^\infty \ud z \; \exp(-z^2) z^{d-1-s/2},
  \end{align*}
  where the lower and upper bounds are finite and do not depend on $r$. Hence,
  \begin{align*}
    \mathcal{H}_\alpha(t)<\infty \quad  \Longleftrightarrow  \quad -2\alpha + (s-d) /2 >-1.
  \end{align*}
  This proves \eqref{E:HaB}.
\end{proof}
