...
to the \textit{parabolic Anderson model} \cite{carmona.molchanov:94:parabolic}.
 ...
 The aim of this paper is to investigate the conditions required to guarantee the existence of invariant measures for the solution to \eqref{E:SHE}, which is a crucial step towards the study of the ergodicity of the system.  We will see below in Theorem \ref{T:Lrho} that in our setting, the solution to \eqref{E:SHE} almost surely belongs to a weighted space of square integrable function, $L^2_{\rho}(\R^d)$. By an invariant measure for \eqref{E:SHE}, we mean a measure, $\eta$, on the space $(L^2_{\rho}(\R^d), \mathscr{B}(L^2_{\rho}(\R^d))$ such that 
 	\[
 		\eta(A) = \int_{L^2_{\rho}(\R^d)} \mathscr{L}(u(t,\cdot, \zeta))(A) \; \eta(\ud \zeta), \quad \forall t \ge 0,
 	\]
 where $A \in \mathscr{B}(L^2_{\rho}(\R^d))$ and $\mathscr{L}(u(t,\cdot,\zeta))(A)$ is the law of \eqref{E:SHE} starting from $\zeta$. 
 \textcolor{blue}{
 This is the only part left of the paper that I want to confirm. How do we define the laws for random initial conditions, $\varphi$, that are almost surely in $L^2_{\rho}(\R^d)$? See \eqref{E:RandInit}. I believe there may not be an issue because of the $\varphi \in \mathcal{F}_0$ assumption but I want to make sure. It seems that other authors ignore this issue.
}
