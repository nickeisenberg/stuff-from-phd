...
\end{align}
for all $\psi,\phi\in C_c(\R^{1+d})$, i.e., continuous functions with compact support, where
$\widetilde{\phi}(x) := \phi(-x)$, ``$*$" refers to the convolution in spatial variable, and
...
\end{align}
Note that conditions in \eqref{E:U0finite} imply that the following open interval is not empty:
