...
From now on we will denote $H:=L^2_{\rho}(\R^d)$. In Theorem \ref{T:Lrho} below, we will see that the solution to \eqref{E:SHE} maps $H$ into $H$ in the sense that for each $\zeta \in H$, we have $x \mapsto u(t,x;\zeta)$ is almost surely $H$-valued where $u(t,x;\zeta)$ is the solution to \eqref{E:SHE} starting from $\zeta$. Moreover, we introduce a new condition which shows that the solution to $\eqref{E:SHE}$ will map any measure, $\mu$, that satisfies \eqref{E:G} below into $H$. This condition will allow us to consider more general initial conditions outside of $H$, see Section \ref{S:Example} below, while at the same time still operate in the same framework laid out by the authors in \cite{tessitore.zabczyk:98:invariant, da-prato.zabczyk:14:stochastic}.
...
\begin{lemma} \label{L:yineq}
  ...
  \begin{enumerate}[(1)]
    \item for all $q\ge 2$, $s>0$, $y\in\R^d$,
      ...
      \end{align}
      where $\mu^* = 1 + |\mu|$ and we refer to Theorem \ref{T:ExUn} for the function
