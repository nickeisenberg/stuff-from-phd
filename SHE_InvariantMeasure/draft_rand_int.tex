...
In this part, we investigate the case when the correlation function $f(x)$ is given by the {\it
Mat\'ern function}, namely,
\begin{align*}
  f(x) = f_{\rho,\nu}(x) := \sigma^2 \frac{2^{1-\nu}}{\Gamma(\nu)} \left(\sqrt{2\nu} \frac{|x|}{\rho}\right)^\nu K_\nu\left(\sqrt{2\nu} \frac{|x|}{\rho}\right),
\end{align*}
where $K_\nu(\cdot)$ is the modified Bessel function of the second kind, and $\rho,\nu>0$ are two
parameters.
\bigskip

\textcolor{magenta}{Nick, I found this correlation function is very popular now in machine learning
and statistics. Shall we try if this works in our case? I may search first from Wikipedia and find
more references and especially find the Fourier transform of $f$.}
