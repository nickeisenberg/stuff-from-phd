...
\eqref{E:Dalang} in a different way as follows, for some $\alpha\in (0,1]$,
...
\begin{figure}[htpb]
  \begin{center}
    ...
    \begin{tikzpicture}[scale=1, x=6em, y=6em]
      ...
      \def\xg{3.0}
      \def\yg{1.7}
      \draw[->] (-0.2,0) -- (\xg+1,0) node[right] {$\alpha$};
      \draw[->] (0,-0.2) -- (0,\yg+0.3) node[above] {$\beta$};
      \draw (\xo,\inc) --++ (0,-2*\inc) node[below] {$0$};
      \draw (\xg,\inc) --++ (0,-2*\inc) node[below] {$\alpha\in(0,1]$};
      \draw (\inc,\yo) --++ (-2*\inc,0) node[left] {$0$};
      \draw (\inc,\yg) --++ (-2*\inc,0) node[left] {$\beta>0$};
      \node[text width=10em, align = center] (01) at (\xo,\yg) {$\Upsilon(\beta)<\infty$\\ Dalang's condition \eqref{E:Dalang}};
      \node[text width=10em, align = center] (00) at (\xo,\yo) {$\Upsilon(0)<\infty$\\ first condition\\ in \eqref{E:U0finite}};
      \node[text width=10em, align = center] (10) at (\xg,\yo) {$\Upsilon_\alpha(0)<\infty$\\ Condition \eqref{E:Dalang-a0}};
      \node[text width=10em, align = center] (11) at (\xg,\yg) {$\Upsilon_\alpha(\beta)<\infty$\\ Condition \eqref{E:Dalang-ab}};
      % \draw[double,double distance=0.4em, ->] (00) -- (01);
  ...
  the first condition in \eqref{E:U0finite}. Check also Lemma \ref{L:HUA} for the relation between
$\Upsilon(0)<\infty$ and $\Upsilon_\alpha(0)<\infty$.}
...
\begin{theorem} \label{T:Lrho}
  ...
  $C>0$ which does not depend on $t$,
...
\begin{lemma} \label{L:yineq}
  ...
  $\widehat{f}$ satisfies Dalang's condition \eqref{E:Dalang}. Assume that for some $\alpha\in
