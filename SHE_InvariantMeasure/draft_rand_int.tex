...
\begin{lemma} \label{L:yineq}
  ...
  \begin{align} \label{E:NormY}
  \Norm{Y_v(s,y)}_q^2 \le C_q\: [J_*(s + t_0,y)]^2\: \mathcal{H}_\alpha(s).
  \end{align}
  Moreover, if
  \begin{enumerate}
...
\begin{proof}
  ...
  \begin{align} \label{E:EYqr}
    ...
    % = \Norm{\Norm{Y(s,\cdot)}_q}_\rho^q.
  \end{align}
  Now we look to find an appropriate upper bound for $\Norm{Y(s,y)}^2_q$. By
  ...
  \begin{align} \label{E:NormB}
    ...
    \le C\bigg(1+(G(r+ t_0,\cdot) * |\mu|))(z_i)\bigg) = C J_*(r + t_0,z_i), \quad i=1,2,
  \end{align}
  where $J_*$ is defined in \eqref{E:J*u}. Therefore,
  \begin{align*}
    \Norm{Y_v(s,y)}_q^2
    & \le C \int_0^s \ud r \: (s-r)^{-2\alpha} \iint_{\R^{2d}} \ud z_1\ud z_2\: f(z_1-z_2) \prod_{i=1}^2  \bigg( G(s-r,y-z_i)  J_*(r + t_0,z_i)\bigg)                        \\
    & = C \int_0^s \ud r \: (s-r)^{-2\alpha} \iint_{\R^{2d}} \mu^*(\ud \sigma_1) \mu^*(\ud \sigma_2) \iint_{\R^{2d}} \ud z_1\ud z_2                                            \\
  ...
  where we have applied \eqref{E:gsim} and Plancherel's theorem. Therefore,
  \begin{align*}
   \Norm{Y_v(s,y)}_q^2 \le C (2\pi)^{-2d} [J_*(s + t_0,y)]^2 \int_0^s \ud r \: (s-r)^{-2\alpha} \int_{\R^d} \widehat{f}(\ud \xi)\exp\left(-\frac{(r + t_0)(s-r)}{s + t_0}|\xi|^2\right).
  \end{align*}
  Notice the function
  \[
