...
\begin{proof}[Proof of Theorem \ref{T:Main} (a)]
  ...
  \begin{align*}
    v(t_0,x) = \Big(G(t-t_0,\cdot) * u(t_0,\cdot)\Big)(x) +\dfrac{\sin(\alpha \pi)}{\pi} \left[F_\alpha Y_v\right](t_0,x).
  \end{align*}
  Now define the set $\mathscr{K} (\Lambda)$ as
  \begin{align*}
    \mathscr{K} (\Lambda) = \left\{ \big(G(t-t_0,\cdot)*y(\cdot)\big)(x)+(F_\alpha h)(x):
  ...
  \end{align*}
  Then by Proposition \ref{P:weight} (since $t-t_0\ge t_0>0$) and Proposition \ref{P:F-Comp},
  $\mathscr{K} (\Lambda)$ is relatively compact in $L^2_{ \rho}(\R^d)$ for $\Lambda>0$. Moreover, by
  Chebyshev's inequality, \eqref{E:uInLrho} and \eqref{E:yineq}, we see that
  \begin{align*}
