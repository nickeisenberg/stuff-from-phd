...
\begin{theorem}[Theorem 1.7 \cite{chen.huang:19:comparison}] \label{T:ExUn}
  ...
  two conditions in \eqref{E:U0finite} hold,
...
\begin{lemma} \label{L:yineq}
  ...
  \ref{T:Main}, then
  \begin{align} \label{E:NormY}
    \Norm{Y_v(s,y)}_q^2 \le C_q\: [J_*(s + t_0,y)]^2\: \mathcal{H}_\alpha(s) <\infty
...
\begin{lemma}[Factorization lemma] \label{L:Factorize}
  Fix an arbitrary $ t_0>0$. Let $u(t, x;\mu)$ be the solution to \eqref{E:SHE}
  and $Y_v$ be as in \eqref{E:Y}. Under the conditions of Theorem \ref{T:Lrho}
  and (iii) of Theorem \ref{T:Main}, for all $\alpha \in (0,1/2)$, the
...
\begin{proof}
  ...
  \begin{align*}
    \int_{\R^d}\ud y \: G(t-s,x-y) \Norm{Y_v(s,y)}_2^2
    & \le C \int_{\R^d} \ud y \;  G(t-s,x-y)\left(J_*(s+t_0,y)\right)^2 \mathcal{H}_\alpha(s) \\
    & =C \mathcal{H}_\alpha(s) \int_{\R^d} \ud y \;  G(t-s,x-y)\left(J_*(s + t_0,y)\right)^2.
  \end{align*}
