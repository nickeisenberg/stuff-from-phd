...
our condition is sharper when $d> \frac{14\log(2)}{\log(2\pi)} \approx 5.28$. One should note that the assumption that $\mathcal{F}^{-1}\left[\sqrt{\widehat{f}(\cdot)}\right](\xi)$ is nonnegative is quite strong and requires $\sqrt{\widehat{f}}$ to be non-negative and non-negative definite, which does not follow from the non-negative definiteness of $f$. Indeed, suppose that $f$ was such that $\widehat{f}(\xi) = \max\{1-x,0\}$. Then $f$ is non-negative and non-negative definite but one can show in this case that $\mathcal{F}^{-1}\left(\widehat{f}\right)(\xi)$ takes on both positive and negative values, thus one cannot remove the absolute value signs. \bigskip

{\bf\noindent Case II:~} Suppose $\widehat{f}(\cdot)$ is not strictly positive, but only nonnegative,
