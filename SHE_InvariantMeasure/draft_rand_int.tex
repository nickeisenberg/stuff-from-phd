...
\begin{figure}\label{Fig:fresnel}
	\centering
...
One should note that the assumption that $\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x)$ is nonnegative, or equivalently that $\sqrt{\widehat{f}}$ is non-negative definite, is quite strong and may not be true even if $f$ is non-negative and non-negative definite. Indeed, suppose that $f$ was such that $\widehat{f}(\xi) = 2^{-2}\max\{2-|\xi|,0\}$. Then $f$ is non-negative and non-negative definite, which is shown in Example \ref{Ex:sqrt} below. We will now show that in this case, $\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x)$ takes on both positive and negative values. Since $\sqrt{\widehat{f}(\xi)} = 2^{-1}\sqrt{\max\{2-|\xi|,0\}}$ is even, the inverse Fourier transform takes the following form:
	\[
		\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x) = 2 \int_0^2 2^{-1}\sqrt{2-\xi} \cos(x \xi) \: \ud \xi.
	\]
For simplicity, we will only consider the case $x \ge 0$ and real, however, due to the evenness of cosine, the calculations for $x < 0$ are exactly the same. Therefore 
	\begin{align*}
			\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x) &= 2 \int_0^{\sqrt{2}} \xi^2 \cos(2x- x \xi^2) \: \ud \xi \\
			&= 2\cos(2x) \int_0^{\sqrt{2}} \xi^2 \cos(x \xi^2) \: \ud \xi  +2 \sin(2x) \int_0^{\sqrt{2}} \xi^2 \sin(x \xi^2) \: \ud \xi.
	\end{align*}
Where in the first equality we used the change of variables, $\xi' = \sqrt{2 - \xi}$. We now do an integration by parts to see that 
	\begin{align*}
		\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x) &= - \cos(2x) \int_0 ^{\sqrt{2}} \frac{\sin(x \xi^2 )}{x} \: \ud \xi + \sin(2x) \int_0 ^{\sqrt{2}} \frac{\cos(x \xi^2)}{x} \: \ud \xi,
	\end{align*}
\begin{codes}
	Integrate[(-Cos[2*x]Sin[y^2 * x]/x)+(Sin[2*x]Cos[y^2 * x] / x),{y,0,Sqrt[2]},Assumptions->x>=0]
\end{codes}
and lastly we apply the change of variables $\xi' = \sqrt{2x/\pi} \: \xi $ to conclude that 
	\[
		\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x) = \frac{ \sqrt{\frac{\pi}{2}}\left(-\cos(2|x|)\mathcal{S}\left(\frac{2\sqrt{|x|}}{\sqrt{\pi}}\right) + \sin(2|x|) \mathcal{C}\left( \frac{2 \sqrt{|x|}}{\sqrt{\pi}} \right) \right)}{ |x|^{3/2} }, \quad \text{for all } x \in \R,
	\]
where $\mathcal{S}$ and $\mathcal{C}$ are the Fresnel integrals (see \cite[7.2 (iii)]{olver.lozier.ea:10:nist}):
	...
	\]
Thus even if $f$ is both non-negative and non-negative definite, we may still have problems with removing the absolute value sign (see Figure \ref{Fig:fresnel}). 
\end{example}
\bigskip

{\bf\noindent Case II:~} Suppose $\widehat{f}$ is not strictly positive, but only nonnegative, namely, $\widehat{f}(\xi)=0$ for some $\xi\in\R^d$. Then removing the absolute value in
