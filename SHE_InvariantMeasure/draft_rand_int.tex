...
\begin{proposition} \label{P:Bessel}
  ...
  In particular, when $\alpha=0$, we have
  \begin{equation} \label{E:Upsilon_0}
   \Upsilon(0) = \frac{-\pi \csc\left(\frac{\pi d}{2}\right)}{2^d \pi^{d/2} \Gamma(d/2) \Gamma(s/2)}.
  \end{equation}
...
\begin{proof}
  ...
  \end{align*}
  which is finite provided that $s>d-2(1-\alpha)>0$. This proves \eqref{E:UpsionB}. We then easily deduce \eqref{E:Upsilon_0} by letting $\alpha = 0$ in \eqref{E:UpsionB} and applying the formula $\Gamma(z)\Gamma(1-z) = \pi \csc(\pi x)$. 
...
Recall the definition of $\mathcal{F}^{-1}$ in \eqref{E:Fourier} above. Now let us denote the right-side of the second condition in \eqref{E:TZspec} by $M(f)$, namely,
\begin{align*}
