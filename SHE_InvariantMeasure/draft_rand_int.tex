...
\begin{abstract}
  ...
  case when $\rho(u)=\lambda u$) starting from a Dirac delta measure.
\end{abstract}
\bigskip
...
satisfies the following integrability condition at infinity (see also \eqref{A:icon} below):
\begin{equation*}
...
cancel the otherwise exponential growing moments. The absence of such a drift term in this paper
...
\end{equation}
which can be easily verified. Next, the following spherical coordinate integration formula will be used often,
	\[
		\int_{\R^d} f(|x|) \ud x = \sigma(\mathbb{S}^{d-1}) \int_0^\infty f(r) r^{d-1} \ud r,
	\]
where $\sigma(\mathbb{S}^{d-1})  = 2(\pi)^{d/2} / \Gamma(d/2)$ and $\Gamma(x)$ denotes the Gamma function.
Lastly, the convention of Fourier transform is given in Remark \ref{R:Fourier}
below.
...
\subsection{A discussion on the conditions by Tessitore and Zabczyk} \label{SS:TessZabz}
...
\begin{remark} \label{R:Fourier}
  ...
  corresponds to $\left(2\pi\right)^{-d/2}\widehat{f}$ in this paper. Our equation \eqref{E:TZspec}, which is condition (3.4) ({\it ibid.}), takes into account this difference, therefore explaining the slightly
