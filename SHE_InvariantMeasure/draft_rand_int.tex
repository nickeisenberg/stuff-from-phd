...
\end{align*}
For notation, let us denote $P_t(\zeta, A) := \mathscr{L}(u(t,\cdot;\zeta))(A)$, where $\zeta \in H$. Then we can define the {\em Markovian transition semi-group} corresponding to $P_t$ as 
	\[
		P_t\varphi(\zeta) = \int_{H} \varphi(y)P_t(\zeta, \ud y),
	\]
 where $\varphi \in C_b(H)$, and $C_b(H)$ denotes the space of bounded continuous function on $H$. Moreover, if we define $\mathcal{M}(H)$ as the space of probability measures on $H$, then for each $\nu \in \mathcal{M}(H)$, we define $ P_t^*$ as follows:
 	\[
	 	P_t^*\nu(A) = \int_{H} P_t(y,A) \nu(\ud y),
 	\]
...
\cite{tessitore.zabczyk:98:invariant} are the only papers that have considered the case where the spatial domain is the whole space $\R^d$, the Lipschitz continuous diffusion term, $b(x,u)$, may be unbounded in the second variable and there is no added drift term, $f(u(t,x))$. The challenge is proving \eqref{E:BddMnt} and it is known that the solution to \eqref{E:SHE} is usually {\it intermittent}, namely, the probability moments to \eqref{E:SHE} have a certain exponential growth in $t$; see, e.g., \cite{carmona.molchanov:94:parabolic,foondun.khoshnevisan:09:intermittence}. For that reason, one typically requires added assumptions on either the initial conditions, the noise or the coefficients of \eqref{E:SHE} to combat this exponential growth in time of the moments.
...
As mentioned above, we consider the whole space $\R^d$, a Lipschitz continuous diffusion term, $b(x,u)$, which may be unbounded in the second variable and lastly we do not consider any drift term. As Tessitore and Zabczyk \cite{tessitore.zabczyk:98:invariant} is the only other paper that we know of that considers the same set up, it is a strong influence in the current work. From a purely mathematical stand point, something interesting about this work is that all prior articles cited relating to the existence of invariant measures, including \cite{tessitore.zabczyk:98:invariant}, use the theory of the stochastic integral formulated by DaPrato and Zabcyzy \cite{da-prato.zabczyk:14:stochastic}, while in this paper we formulate and solve the problem using the random field language of Walsh \cite{walsh:86:introduction} with some state-of-the-art moment estimates. Even though both theories have been shown to be more or less equivalent \cite{dalang.quer-sardanyons:11:stochastic}, they appear quite different from a technical sense. Thus, initiating a crossover from one theory to the other, as this paper does, is appealing on its own.
...
$\Upsilon_\alpha(\beta)$, i.e., $\Upsilon(\beta)=\Upsilon_0(\beta)$. We will also need the following
...
\end{corollary}
\begin{proof}
	Let $\zeta \in L^2_{\rho}(\R^d)$. Then, by Remark \ref{R:Condition}, it suffices to show that the finiteness of $\Norm{J_0(t,\cdot;|\zeta|)}^2_{\rho}$ for all $t>0$. Indeed,
		\begin{align*}
			\Norm{J_0(t,\cdot;|\zeta|)}^2_{\rho} &= \int_{\R^d} \left( \int_{\R^d} G(t,x-y) |\zeta(y)| \ud y \right)^2 \rho(x) \ud (x) \\
