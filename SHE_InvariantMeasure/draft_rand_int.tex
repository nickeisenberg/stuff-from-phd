...
\Upsilon(0) <\infty.
\end{equation}
In \cite{assing.manthey:03:invariant,misiats.stanzhytskyi.ea:20:invariant,misiats.stanzhytskyi.ea:16:existence},
...
We now give some notation before we stating Assumption \ref{A:General}, under which one can find a unique mild solution to \eqref{E:SHE} with sharp moment estimates; see
\cite{chen.kim:19:nonlinear,chen.huang:19:comparison,chen.dalang:15:moments}.
\begin{notation}
We will use $\Norm{\cdot}_\rho$ instead of $\Norm{\cdot}_{L^2_\rho(\R^d)}$. Also by defining $\Norm{X}_p := \left(\mathbb{E}(|X|^p)\right)^{1/p}$ where $p\ge 1$, 
...
We must introduce some more notation before stating the main results.
\begin{notation}
...
We may now state a preliminary result used to prove Theorem \eqref{T:Main} below.
...
\begin{example}[Correlation measures] \label{E:Gamma+f}
