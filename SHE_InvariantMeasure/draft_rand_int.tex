...
As mentioned above, we consider the whole space $\R^d$, a Lipschitz continuous diffusion term, $b(x,u)$, which may be unbounded in the second variable and lastly we do not consider any drift term. As Tessitore and Zabczyk \cite{tessitore.zabczyk:98:invariant} is the only other paper that considers the same set up, it a strong influence in the current work. We formulate and solve the problem using the random field language of Walsh \cite{walsh:86:introduction} with some state-of-the-art moment estimates. The improvements over \cite{tessitore.zabczyk:98:invariant} consist of the following aspects.
\begin{enumerate}
...
We would like to make a quick comment about the condition in \eqref{E:G}. First, because the heat kernel, the measure $1 + |\mu|$ and $\rho$ are all positive, then clearly we have that 
	\[
		\Norm{J_0(t,\cdot,|\mu|)}^2_{\rho}  = \int_{\R^d} J_0^2(t,x; |\mu|)\rho(x) \ud x \le \mathcal{G}_{\rho}(t;\mu).
	\]
On the other hand, we have that 
	\begin{align*}
		...
		&\le 2\bigg(\Norm{\rho}_{L^1(\R^d)} + \Norm{J_0(t,\cdot,|\mu|)}^2_{\rho} \bigg).
	\end{align*}
Thus for all $t>0$, 
	\[
		\Norm{J_0(t,\cdot,|\mu|)}^2_{\rho} \le 
		\mathcal{G}_{\rho}(t;\mu) \le 
		C \bigg(1 + \Norm{J_0(t,\cdot,|\mu|)}^2_{\rho} \bigg),
		\qquad \text{where } C = \max\{ 2\Norm{\rho}_{L^1(\R^d)}, 2 \},
	\]
and so the finiteness of \eqref{E:G} is equivalent to the finiteness of $\Norm{J_0(t,\cdot;|\mu|)}_{\rho}$. However, it will be notationally convenient to keep \eqref{E:G} the way it is. 
\end{remark}
