...
$\{\mathscr{L}(u(t,\cdot;\mu)\}_{t>t_0}$ for some $ t_0\ge 0$ is established, one can construct the
...
\cite{tessitore.zabczyk:98:invariant} are the only papers that have considered the case where the spatial domain is the whole space $\R^d$, the Lipschitz continuous diffusion term, $b(x,u)$, may be unbounded and there is no added drift term, $f(x,u)$. The challenge is proving \eqref{E:BddMnt} and it is known that the solution to \eqref{E:SHE} is usually {\it intermittent}, namely, the
probability moments to \eqref{E:SHE} have a certain exponential growth in $t$; see, e.g.,
\cite{carmona.molchanov:94:parabolic,foondun.khoshnevisan:09:intermittence}. For that reason, one typically requires added assumptions on either the initial conditions, the drift term $f$ or the diffusion term $b$ to combat this exponential growth in time of the moments.

For example, Assing and Manthey  \cite{assing.manthey:03:invariant} study the problem of showing the existence of an invariant measure for \eqref{E:SHE} on the whole space $\R^d$, but in order to combat the intermittency of the moments, they require an added a drift term, $f(u)$, which must satisfy the following dissipativity condition for all $u \in \R$:
	\[
		uf(u) \le C_1 - C_2|u|^2,
	\]
where $C_1, C_2 > 0$. Moreover, they allow for any bounded initial condition and their Lipschitz continuous diffusion term may be unbounded. 

For another example, Misiats {\em et al} \cite{misiats.stanzhytskyi.ea:16:existence} study the equation on the whole space and also allow for the absence of a drift term but they require that the initial condition be in $L^2(\R^d)$ and diffusion term be globally bounded in the sense that $|b(x,u)| < k < \infty$ for some $k > 0$ and all $(x,u) \in \R^d \times \R$. The same authors improve this result in \cite{misiats.stanzhytskyi.ea:20:invariant} by allowing the drift term to satisfy the following:
	\[
		|b(x,u_1) - b(x,u_2)| < \varphi(x)|u_1-u_1|,
	\]
where $\varphi$ is a bounded function in $L^2(\R^d)$. So in this case, $u \mapsto b(x,u)$ may be unbounded but it is now required that $x \mapsto b(x,u)$ must vanish as $|x| \to \infty$. Furthermore, in either case the restriction on the initial condition to be in $L^2(\R^d)$ is quite strong and it excludes the popular case $u(0,x) = 1$.  
