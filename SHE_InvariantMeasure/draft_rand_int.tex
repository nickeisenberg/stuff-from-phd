...
Recall that under the following Assumption \ref{A:General}, one can find a unique mild solution to \eqref{E:SHE} with sharp moment estimates; see
\cite{chen.kim:19:nonlinear,chen.huang:19:comparison,chen.dalang:15:moments}.
\begin{assumptions} \label{A:General}
\textcolor{blue}{Random initial measure. Need to clarify this.}
  \begin{enumerate}[(i)]
    \item The initial measure $\mu$ satisfies the following integrability condition:
      \begin{equation} \label{A:icon}
	\int_{\R^d}\exp\left(-a|x|^2\right)\mu_2^*(\ud x) < \infty \quad \text{for all $a>0$},
      \end{equation}
      where $\mu_*^2$ is defined below in \eqref{E:qJ*}.
  ...
  \end{enumerate}
\end{assumptions}
\textcolor{blue}{I think \eqref{A:icon} is good enough}
...
\begin{proof}[Proof of Theorem \ref{T:Main} (b)]
 ...
 \begin{equation} \label{E:SHErandint}
	...
	\end{cases}
 \end{equation}
where $\dot W_{t_0/2}(t,x) = \dot W(t + t_0/2,x)$ denotes the time shifted noise. We note that \eqref{E:uInLrho} implies that this random initial condition, $u(t_0/2,\cdot)$ satisfies \eqref{A:icon} and thus the solution to \eqref{E:SHErandint} exists and we denote the solution as $v$. By the Markov property of the solution to \eqref{E:SHE}, we may say that for any $t \ge 0$,  $u(t + t_0/2,x) \stackrel{\mathcal{L}}{=} v(t,x)$, where $\mathcal{L}$ means the equality is in law. In other words, for all $t \ge 0$, the following holds:
	\begin{align*}
