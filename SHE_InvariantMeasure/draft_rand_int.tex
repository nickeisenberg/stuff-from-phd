...
to the \textit{parabolic Anderson model} \cite{carmona.molchanov:94:parabolic}.
 ...
 The aim of this paper is to investigate the conditions required to guarantee the existence of invariant measures for the solution to \eqref{E:SHE}, which is a crucial step towards the study of the ergodicity of the system.  We will see below in Theorem \ref{T:Lrho} that in our setting, the solution to \eqref{E:SHE} almost surely belongs to a weighted space of square integrable function, $L^2_{\rho}(\R^d)$. By an invariant measure for \eqref{E:SHE}, we mean a measure, $\eta$, on the space $(E, \mathscr{B}(E))$ such that
 	\[
 		\eta(A) = \int_{E} \mathscr{L}(u(t,\cdot, \zeta))(A) \; \eta(\ud \zeta), \quad \forall t \ge 0,
 	\]
 where $A \in \mathscr{B}(E)$ and $\mathscr{L}(u(t,\cdot,\zeta))(A)$ is the law of \eqref{E:SHE} starting from $\zeta$.
 \textcolor{blue}{
 This is the only part left of the paper that I want to confirm. There is a lack of definitions in the papers \cite{assing.manthey:03:invariant}, \cite{tessitore.zabczyk:98:invariant}, and \cite{misiats.stanzhytskyi.ea:16:existence} about the meaning of a invariant measure. However \cite{da-prato.g-atarek.ea:92:invariant} has a definition. Following that, I believe the definition is there exists a measure $\eta$ such that 
 	\begin{equation} \label{D:invmeasureC_b}
		 \eta(A) = \int_{E} \mathscr{L}(u(t,\cdot, \zeta))(A) \; \eta(\ud \zeta), \quad \forall t \ge 0.
	 \end{equation}
What is confusing is that we are integrating over all of the possible initial conditions. Daprato showed that if we can verify that the solution is bounded in probability for just one specific initial condition $\zeta_0$, then there exists an invariant measure. So it almost seems that proving that there exists an invariant measure for the constant $1$ initial condition is just as good as proving an invariant measure for a bounded and continuous $\varphi$ since both $1, \varphi \in C_b(\R^d)$.
\vspace{.2in} \\ 
Instead of starting our equation from a $C_b(\R^d)$ function, we started it from a measure satisfying $\eqref{E:DetInit}$. Lets call the space of measures that satisfy \eqref{E:DetInit} $\mathcal{M}$. Is this a measure space? Did we prove that there exists an $\eta$ such that 
	 \begin{equation} \label{D:invmeasureMEASURE}
		\eta(A) = \int_{\mathcal{M}} \mathscr{L}(u(t,\cdot, \zeta))(A) \; \eta(\ud \zeta), \quad \forall t \ge t_0?
	\end{equation}
