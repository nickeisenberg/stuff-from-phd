...
instead of 
	\[
		64 L^2_{b}   <  \frac{1}{2\Upsilon(0)}
	\]
which is all that is required in \cite{chen.kim:19:nonlinear}.
}
...
\begin{theorem} \label{T:Main}
  ...
  \begin{enumerate}[(a)]
    \item for any $\tau>0$, the sequence of laws of $\{\mathcal{L}u(t,\cdot;\mu)\}_{t\ge \tau}$ is
      ...
      \end{equation}
    \item there exists an invariant measure for the laws $\{\mathcal{L}u(t,\cdot;\mu)\}_{t \ge \tau}$ in
      $L^2_\rho(\R^d)$.
  \end{enumerate}
\end{theorem}
...
\textcolor{blue}{
	Should Proposition \ref{P:growtail} below be changed to an example? 
}
