...
\begin{remark} \label{R:Fourier}
  ...
  \end{align*}
  Hence, Plancherel's theorem takes the form of $\int_{\R^d} \psi(x) \phi(x)\ud x = \int_{\R^d}
...
\end{align*}
We now consider the following two cases. \bigskip

{\bf\noindent Case I:~} If $\widehat{f}(\zeta)$ is strictly positive for all $\zeta\in\R^d$, then
there is no ambiguity when taking the square root and hence one can remove the absolute value to see
that

\textcolor{blue}{
Why is there no issue with removing the absolute value signs when $\widehat{f} > 0$? Is it because $\sqrt{\widehat{f}}$ is non-negative definite so the positivity of $\widehat{f}$ will imply the positivity of $\mathcal{F}^{-1}(\sqrt{\widehat{f}})$?
}
...
\begin{corollary} \label{C:Mom}
  ...
  \begin{equation} \label{E:mb0}
    ...
    \quad \text{for all $p$ such that } 1/p \in \left(128 L_b^2\Upsilon(0),1\right),
  \end{equation}
...
\begin{proof}
  ...
  \begin{equation}\label{E:pLip}
     32 p L_b^2 < \frac{1}{4\Upsilon(0)}, \quad \textcolor{blue}{\text{why not } 32p L_b^2 < 1 / (2\Upsilon(0))?}
  \end{equation}
  we have all $p$-th moments ($p\ge 2$) bounded as given in \eqref{E:mb0}.

  \textcolor{blue}{
For each $p_1 > p_2 \ge 2$,  we need to choose a smaller $L_b$ so that $32pL_b^2 < 1 / (2\Upsilon(0) ) $. We can't make a statement that all $p$ norms for $p \ge 2$ are bounded in $t$, we can only make that state for individual $p$ by appropriately choosing $L_b$ small enough. This is not an issue though as we always are fixing our $p$ values for the norm.
}
\end{proof}
...
\begin{lemma} \label{L:yineq}
 ...
 \textcolor{blue}{
	I think $(64L_b^2 \Upsilon(0), \alpha)$ being nonempty is all we need. Note that we must consider $\alpha \in (0, 1/2)$. Next, because of Prop \ref{P:F-Comp}, we need $1 / q \in (0, \alpha)$ which is going to be the first restriction. Our second restriction comes from Theorem \ref{T:ExUn}. Note that for bounded moments in time, we need $2\gamma_q \Upsilon(0) < 1$, or in other words, $64 q L_b^2 \Upsilon(0) < 1$ and this is the second restriction. Summarizing this gives us that we need both 
		\[
			64 L_b^2 \Upsilon(0) < 1 / q \quad \text{and} \quad 1 / q < \alpha < 1/2,
		\]  
	or by putting this together 
		\[
				64 L_b^2 \Upsilon(0) < 1 / q < \alpha.
		\]
}
    \item under the two conditions in \eqref{E:U0finite}, if the integral in \eqref{E:H} is finite for
      some $\alpha\in \left(128 L_b^2 \Upsilon(0),\: 1/2\right)$, then for any $q$ with $1/q\in
...
\begin{proof}
  ...
  invariant measure for $\mathscr{L}(u(t,\cdot;\mu))$, $t\ge \tau$. \textcolor{blue}{Finally, since $\tau$ can be
  arbitrarily close to zero, one can conclude part (b) of Theorem \ref{T:Main}.}
\end{proof}

\textcolor{blue}{
I think we should remove the last blue sentence from the proof. I changed (b) from Theorem \ref{T:Main} too. I think we need (b) as it is now ie $t\ge \tau$ not $t>0$.
}
