...
Let $\rho$ be an admissible weight function, then we may introduce the Hilbert space $H:=L_\rho^2(\R^d)$
...
\begin{proof}
  ...
  \end{align*}
  \textcolor{blue}{I believe for the last equality to hold, this would force $C_\alpha'$ to also depend on $\xi$. This issue is resolved if we just replace $=$ with $\le$ and then let $C_\alpha' = \max\{1, \int_{\R^d}G(1,z)|z|^\alpha \ud z \}$
}
  ...
  \end{align*}
  where in the first inequality, we considered two cases: $2\alpha \ge 1$ and $2 \alpha < 1$. When $2\alpha < 1$, we used concavity to say that $ (1 + r^{2\alpha})^{2\alpha} / 2 \le ((1+r)/2)^{2\alpha}$. When $2\alpha \ge 1$ we used the fact that for $a,b>0$ that $a^{2\alpha} + b^{2\alpha} \le (a+b)^{2\alpha}$.
  This shows that $\inf_{r>2} \frac{\psi(r)}{\sqrt{1+r^{2\alpha}}} >0$.
