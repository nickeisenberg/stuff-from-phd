...
Aside from this current article and \cite{tessitore.zabczyk:98:invariant}, there are few works which do not require an added drift term with dissipativity as in \eqref{E:dis}. In lack of this condition, Misiats {\em et al} \cite{misiats.stanzhytskyi.ea:16:existence, misiats.stanzhytskyi.ea:20:invariant} provide two results guaranteeing the existence of an invariant measure for the stochastic heat equation on the whole space $\R^d$. In \cite{misiats.stanzhytskyi.ea:16:existence}, they allow for a drift term, $f(x,u):\R^d \times \R \to \R$, such that for all $u \in \R$, $x \mapsto f(x,u)$ is bounded and decays fast enough to be in $L^1(\R^d)$, while for each $x\in\R^d$, $u \mapsto f(x,u)$ is only required to be Lipschitz continuous and thus is not required to satisfy \eqref{E:dis}. However, with the lack of dissipativity of $f$, they require the diffusion term to be globally bounded, thus excluding the popular case of the parabolic Anderson model, $b(u) = \lambda u$. In \cite{misiats.stanzhytskyi.ea:20:invariant} they again allow for the absence of a drift term, however  the diffusion term, $b(x,u)$, is allowed to grow linearly in $u$ for eaxh $x\in R^d$, but for each $\ud\in\R$, $x \mapsto b(x,u)$ must be bounded and decay fast enough to be in $L^2(\R^d)$.
