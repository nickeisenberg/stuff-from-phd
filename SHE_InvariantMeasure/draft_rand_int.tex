...
that satisfies some integrability condition at the infinity (see \eqref{A:icon} below).
...
\begin{theorem} \label{T:Main}
  ...
  Theorem \ref{T:Lrho}, with $u_0$ replaced by $\mu$, are satisfied. Moreover, we assume that
  \begin{enumerate}[(i)]
  ...
  \begin{enumerate}[(a)]
    \item for any $\tau>0$, the sequence of laws of $\{\mathcal{L}u(t,\cdot;\mu)\}_{t\ge \tau}$ is
      ...
      \end{equation}
    \item there exists an invariant measure for the laws $\{\mathcal{L}u(t,\cdot;\mu)\}_{t>0}$.
  \end{enumerate}
\end{theorem}
...
\begin{proposition}
  ...
  \end{align*}
  Then condition \eqref{E:G} is satisfied, but condition \eqref{E:InitData} fails.
\end{proposition}
\begin{proof}
  ...
  \end{align*}
  Let $C_\alpha$ and $C_\alpha'$ be generic constants that may depend on $\alpha $ and may change
  their value at each appearance. Because
  \begin{align*}
  ...
  \begin{align*}
    ...
    & = C_\alpha'\int_{\R^d}t^{\alpha+d/2} \left(1+r^{2\alpha}\right) e^{-\sqrt{t}r}r^{d-1} \ud r      \\
    & = C_\alpha'\left(t^\alpha \Gamma(d) + \Gamma(d+2\alpha)\right)                                   \\
    & = C_\alpha (1+ t^\alpha)<\infty.
  \end{align*}
  Hence, condition \eqref{E:G} is satisfied.

  To show that condition \eqref{E:InitData} fails, one needs to find the lower bound of
  $\mathcal{G}_\rho(t;\mu)$.
  % Using super-additivity when $\alpha\ge 2$ and the concavity when
  % $\alpha\in (0,2)$, we see that for some constant $C_\alpha>0$,
  % \begin{align*}
  %   |\xi|^\alpha = \left(\xi_1^2 + \cdots + \xi_d^2\right)^{\alpha /2}
  %   \ge C_\alpha \left(\xi_1^\alpha + \cdots + \xi_d^\alpha\right).
  % \end{align*}
  % Hence,
  % \begin{align*}
  %   \int_{\R^d}G\left(1,z\right) |\xi-z|^\alpha \ud z
  %   & \ge C_\alpha \sum_{k=1}^{d}\int_{\R^d}G\left(1,z\right)|\xi_k-z_k|^\alpha \ud z \\
  %   & = C_\alpha \sum_{k=1}^{d}\int_{\R}G\left(1,z\right)|\xi_k-z_k|^\alpha \ud z_k   \\
  %   & = C_\alpha d \int_{\R}G\left(1,z\right)|\xi_k-z|^\alpha \ud z
  % \end{align*}
  Indeed,
  \begin{align*}
    \int_{\R^d}G\left(1,z\right) |\xi-z|^\alpha \ud z
    ...
    % & \ge \frac{C_d}{\sqrt{2\pi}}\int_{2|\xi|}^\infty \big( x-|\xi| \big)^\alpha e^{-\frac{x^2}{2}}x^{d-1} \ud x
    % +   \frac{C_d}{\sqrt{2\pi}}\int_{0}^{|\xi|} \big( |\xi|-x \big)^\alpha e^{-\frac{x^2}{2}}x^{d-1} \ud x \\
    % & \ge \frac{C_d}{\sqrt{2\pi}} |\xi|^\alpha\int_{2|\xi|}^\infty e^{-\frac{x^2}{2}}x^{d-1} \ud x               \\
    % & = C |\xi|^\alpha.
  \end{align*}
  Now we study the function $\psi(r):=\int_0^\infty \left(1+r^\alpha\right)^{-1}|r-x|^\alpha e^{-x^2/2}x^{d-1}\ud x$. It is
  clear that $\psi(x)>0$ and $\psi(0)\in (0,\infty)$. Because
  \begin{align*}
    \left(1+r^\alpha\right)^{-1}|r-x|^\alpha e^{-x^2/2}x^{d-1}
    & \le C \frac{r^\alpha + x^\alpha+1}{1+r^\alpha}e^{-x^2/2}x^{d-1}    \\
    & \le C \left(1+\frac{x^\alpha}{1+r^\alpha}\right) e^{-x^2/2}x^{d-1} \\
    & \le C \left(1+x^\alpha\right) e^{-x^2/2}x^{d-1},
  \end{align*}
  which is integrable with respect to $\ud x$. Hence, by the dominated convergence,
  $\lim_{r\to \infty}\psi(r) = \int_0^\infty e^{-x^2/2}x^{d-1}\ud x<\infty$. Therefore, $\psi(r)$ is
  a strictly positive function on $[0,\infty]$. Hence,
  \begin{align*}
    C_1:=\sup_{r\ge 0} \psi(r)<\infty.
  \end{align*}
  On the other hand,
  \begin{align*}
    \psi(r) & \ge \int_1^2 \left(1+r^\alpha\right)^{-1}|r-x|^\alpha e^{-x^2/2}x^{d-1}\ud x
              \ge C \int_1^2 \left(1+r^\alpha\right)^{-1}|r-x|^\alpha \ud x
              =:C \left(1+r^\alpha\right)^{-1}I(r).
  \end{align*}
  By consider three cases, $r\in (0,1]$, $r\in (1,2]$, and  $r>2$, we see that
  \begin{align*}
    I(r)=\int_1^2 |r-x|^\alpha \ud x = \frac{1}{1+\alpha}\left(\text{sgn}(r-1)|r-1|^{\alpha+1}- \text{sgn}(r-2)|r-2|^{\alpha+1}\right)
  \end{align*}
  Now, by taking derivatives, it is ready to see that when $I(r)$ is nonincreasing for $r\in (0,1]$
  and hence,
  \begin{align*}
    \inf_{r\in(0,1)} \frac{I(r)}{1+r^\alpha} = \frac{1}{2} I(1).
  \end{align*}
  For $r>2$, we see that
  \begin{align*}
    \frac{(1+\alpha)I(r)}{1+r^\alpha}
    & = \frac{(r-1)^{\alpha+1}-(r-2)^{\alpha+1}}{1+r^\alpha} \\
    & \ge \frac{(r-1)^{\alpha+1}-(r-1)^{\alpha}(r-2)}{1+r^\alpha} \\
    & =  \frac{(r-1)^{\alpha}}{1+r^\alpha} \ge C_\alpha \left(\frac{r-1}{1+r}\right)^\alpha
    = C_\alpha \left(1-\frac{2}{1+r}\right)^\alpha \ge C_\alpha \left(1-\frac{2}{3}\right)^\alpha.
  \end{align*}
  For the case when $r\in(1,2]$, the global minimum is achieved at $r=3/2$. Combing these cases, we
  see that
  \begin{align*}
    \inf_{r\in(1,2)} \frac{I(r)}{1+r^\alpha} \ge \frac{I(3/2) }{1+2^\alpha}>0.
  \end{align*}
  Combining all these cases, we have proved that
  \begin{align*}
    C_2:=\inf_{r\ge 0}\psi(r) >0.
  \end{align*}
  Therefore,
  \begin{align*}
   a
  ...
  Therefore,
  \begin{align*}
    \mathcal{G}_\rho(t;\mu)\ge C \int_{\R^d} t^{\alpha + d/2} |\xi|^{2\alpha} e^{-\sqrt{t}|\xi|}\ud
    \xi \ge C.
  \end{align*}
  \textcolor{magenta}{Nick, This is not what we need. We need $t^\alpha$ term not $t^0$ term.}
\end{proof}
  ...
  \begin{enumerate}
    ...
    \item there exists a constant $c(s,d)>0$ such that
      \begin{gather*}
        ...
        H_s(x) =
        \begin{cases}
          |x|^{s-d} + 1 + O(|x|^{s-d+2})                  & \text{for $0<s<d$,}\cr
          \log\left(\frac{2}{|x|}\right) + 1 + O(|x|^{2}) & \text{for $s=d$,}\cr
      ...
      \item the Fourier transform of $f_s$ is strictly positive:
        ...
        \end{equation}
  \end{enumerate}
    Properties 1 and 4 ensure that $f_s$ is a nonnegative and nonnegative-definite tempered measure
    for all $s>0$. From Property 4, we see that
    \begin{align*}
...
\begin{proposition} \label{P:Bessel}
  ...
  \begin{align} \label{E:UpsionB}
    \Upsilon(0) = \frac{2 \Gamma\left((s-d+2)/2\right)}{\pi^{d/2} (d-2)\:\Gamma\left(s/2\right)},
    \qquad \text{for all $s>d-2$ and $d>2$,}
  \end{align}
  and for all $t>0$,
  \begin{align} \label{E:HaB}
    \mathcal{H}_\alpha(t) <\infty \quad \Longleftrightarrow \quad 0<\alpha<(s-d-2)/4.
  \end{align}
\end{proposition}
\begin{proof}
  ...
  As for \eqref{E:HaB}, from \eqref{E:H} and by the spherical coordinates, for all $t>0$,
  \begin{align*}
    \mathcal{H}_{\alpha}(t)
  ...
  \end{align*}
  Denote the $\ud z$-integral in the far right-hand side of the above equalities by $I$. Notice that
  \begin{align*}
    \int_0^\infty \ud z \; \frac{\exp(-z^2)}{(t+z^2)^{s/2}} z^{d-1}
    \le I \le
    \int_0^\infty \ud z \; \exp(-z^2) z^{d-1-s}, \quad \text{for all $r\in[0,t]$,}
  \end{align*}
  with both lower and upper bounds being finite and independent of $r$. Therefore,
  $\mathcal{H}_\alpha(t)<\infty$ if and only if $-2\alpha + (s-d) /2 >-1$. This proves
  \eqref{E:HaB}.
\end{proof}
...
%   $\delta_0$ and is driven by a Gaussian noise with correlation function $f_s(x)$. We denote
...
%   u(t_0,\cdot)$ and $f_s(x)$ satisfy the conditions \eqref{E:InitData}, \eqref{E:H} and
%   \eqref{E:GH-int}.
%
...
%
%   Now lets verify \eqref{E:H} for the Bessel kernel, $f_s$. By recalling
%   \eqref{E:FB}, we have
%    \begin{align*}
%     \mathcal{H}_{\alpha}(s)
...
%   $\dot{W}_{t_0}$ with correlation function given to be the Bessel kernel $f_s$.
...
%   $\delta_0$ with correlation function $f_s(x)$. Indeed,
%    \begin{align*}
...
\begin{lemma} \label{L:yineq}
  ...
  \eqref{E:ShiftW}) when $t_0>0$ and let $v=u$ when $ t_0=0$. Then
  \begin{equation} \label{E:Y}
  ...
  \begin{enumerate}[(1)]
    ...
    \item If $\Upsilon(0)<\infty$ and $q>\max\left(1/\alpha, 128 L_b^2 \Upsilon(0)\right)$, then the
      function $H\left(t;32qL_b^2\right)$ in \eqref{E:NormY} is uniformly bounded in $t\ge 0$, i.e.,
      $\sup_{t\ge 0}H\left(t;32qL_b^2\right)<\infty$;
    \item Under conditions in part (2), if $\rho$ is a nonnegative and $L^1(\R^d)$ function, then
...
\begin{proof}[Proof of Theorem \ref{T:Main} (a)]
  ...
  relatively compact in $L^2_{ \rho}(\R^d)$ for $\Lambda>0$. Notice that
  \begin{align*}
  ...
  \end{align*}
  where the upper bound does not depend on $t$ and the upper bound is finite due to the fact that
  $\mathcal{G}_{\widehat{\rho}}(t;\mu)$ is a continuous function for $t>0$ that is bounded at
  infinity thanks to condition \eqref{E:InitData}. Similarly, by Chebyshev's inequality and
  \eqref{E:yineq},
  \begin{align*}
    I_1 & \le \dfrac{\sin^q(\alpha \pi)}{\pi^q \Lambda^q}\E \int_{0}^{t_0} \Norm{Y_v(s,\cdot)}_{\widehat{\rho}}^q \: \ud s \\
        ...
        & \le  \dfrac{\sin^q(\alpha \pi)\Theta }{\pi^q \Lambda^q} \left( \sup_{t\ge t_0} \mathcal{G}_{\widehat{\rho}}\left(t;\mu\right)\right)^{q/2} \int_0^{t_0}\mathcal{H}_{\alpha}^{q/2}(s) \ud s.
  \end{align*}
  Condition \eqref{E:H} implies that $\mathcal{H}_\alpha(s)$ is a continuous function that vanishes
  at $s=0$. Hence, $\int_0^{t_0}\mathcal{H}_{\alpha}^{q/2}(s) \ud s<\infty$. Combining these two
  ...
  \begin{align*}
    ...
    <\infty,
  \end{align*}
  with the upper bound uniform in $t\ge \tau$. Hence, for any $\epsilon>0$, by choosing $\Lambda >
