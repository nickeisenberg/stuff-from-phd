...
\begin{remark}[$L^\infty(\R^d)$ initial condition] \label{R:BddInit}
  ...
  To be more precise, conditions \eqref{E:G}, \eqref{E:InitData} and \eqref{E:GH-int} hold by
  noticing that the function $\mathcal{H}_\alpha(t)$ in \eqref{E:H} is a continuous function on
  the closed interval $[0,t_0]$ and
  \begin{align*}
...
\begin{proposition} \label{P:Bessel}
  ...
  \begin{align} \label{E:UpsionB}
    \Upsilon(0) = \frac{2 \pi^{d/2} \Gamma\left((s-d+2)/2\right)}{(d-2)\:\Gamma\left(s/2\right)},
...
\begin{proof}
  ...
  \begin{align*}
    \Upsilon(0) = \int_{\R^d} \frac{\ud\xi}{|\xi|^2\left(1+|\xi|^2\right)^{s/2}}
    = C_d \int_0^\infty \frac{r^{d-1}}{r^2\left(1+r^2\right)^{s/2}} \ud r, \qquad
