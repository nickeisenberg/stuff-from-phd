...
\end{equation}
where $G(t,x) = (2 \pi t)^{-d/2}\exp\left( -\frac{|x|^2}{2t} \right)$ is the
heat kernel and the stochastic integral is the {\it Walsh integral} (see
...
a crucial step towards the study of the ergodicity of the system. We will see
...
\begin{equation}\label{D:InvMeas}
  \eta(A) = \int_{E} \mathscr{L}(u(t,\cdot, \varphi))(A) \; \eta(\ud  \zeta),
...
\begin{theorem} \label{T:Main}
  ...
  \begin{enumerate}[(a)]
    \item for any $ t_0>0$, the sequence of laws of
      $\{\mathcal{L}u(t,\cdot)\}_{t\ge t_0}$ is tight, i.e., for any $\epsilon
      \in (0,1)$, there exists a compact set $\mathscr{K}\subset
      L^2_{\rho}(\R^d)$ such that
      \begin{equation} \label{E:cpt}
        \mathcal{L}u(t,\cdot)(\mathcal{K})
