...
$L_\rho^2(\R^d)$ as usual; see Section \ref{S:Prelim} below for more details. Following the
...
\begin{align} \label{E:BddMnt}
  ...
  <\infty.
\end{align}
For convenience, we will from now on write $\Norm{\cdot}_\rho$ instead of $\Norm{\cdot}_{L^2_\rho(\R^d)}$ and $\Norm{X}_q = \left(\mathbb{E}(|X|^q)\right)^{1/q}$ where $q\ge 1$. 
...
\begin{assumptions} \label{A:General} $\;$
  \begin{enumerate}[(i)]
    ...
    \end{equation} 
    where $\varphi \in L^2_\rho\left(\R^d, L^2(\Omega)\right)$ for some weight $\rho$, namely
    \[
  	 \int_{\R^d} \Norm{\varphi(x)}_2^2 \rho(x) \ud x < \infty.
    \]
...
\begin{theorem} \label{T:Main}
  ...
  \begin{enumerate}[(i)]
	...
	\int_{0}^t \left[\mathcal{G}_{\widehat{\rho}, q}(s)\:\mathcal{H}_\alpha(s)\right]^{q/2} \ud s <\infty, \quad \text{for all $t>0$}.
      \end{align}
     \item for some $q>2$, the condition \eqref{E:LipLb} is strengthened to 
     \[
