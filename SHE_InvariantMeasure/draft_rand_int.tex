...
and there is no added drift term, $f(u(t,x))$. The challenge is proving \eqref{E:BddMnt} and it is
...
cerrai:01:second, cerrai:06:asymptotic} require a drift coefficient, $f(u):\R \to \R$, to be added
...
dissipativity condition for some $m,\: k_i, \: c_i>0$ as $|u| \to \infty$:
	\begin{equation} \label{E:dis}
		\begin{cases}
			f(u) \le -k_1|u|^m + k_2 & u > 0 \\
			f(u) \ge c_1|u|^m - c_2  & u < 0.
		\end{cases}
	\end{equation}
...
allow for a drift term, $f(x,u):\R^d \times \R \to \R$, such that for all $x \in \R^d$ and $u_1,u_2
\in \R$,
\begin{align*}
		|f(x,0)| \le \varphi(x) \qquad \text{and} \qquad |f(x,u_1) - f(x,u_2)| \le L\varphi(x)|u_1 - u_2|,
\end{align*}
...
\cite{tessitore.zabczyk:98:invariant}, use the theory of the stochastic integral formulated by
\textcolor{magenta}{DaPrato and Zabcyzk} \cite{da-prato.zabczyk:14:stochastic}, while in this paper we formulate and
