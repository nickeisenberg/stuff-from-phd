...
\begin{enumerate}[(1)]
  ...
  \item[A:] We thank the referee for bringing up this inequality with the effort
    to clarifying the relation of the standard Dalang's condition and the two
    strengthened version. Relation~\eqref{E:Interpolation} can be combined by
    the following separate inequalities:
    \begin{align*}
    ...
    \end{align*}
    In order to make the relation of the following four conditions clear:
    \begin{align*}
    ...
    \end{align*}
    we have expanded the discussion in Example~5.10 and included Figure 5.5. We
    also provide one example constructed in~\cite{chen.huang.ea:19:dense} that
    \begin{align*}
      \Upsilon(\beta)<\infty, \quad
      \Upsilon(0)<\infty, \quad
      \Upsilon_\alpha(\beta)=\infty, \quad
...
\begin{enumerate}
  ...
  \item Example~5.10 has been expanded in accordance with Comment 2 above.
  % \item Reference has been ordered alphabetically.
