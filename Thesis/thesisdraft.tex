...
or even a more general fractional operator of the following type:
	...
	\]
where $\partial_b$ is the {\em Caputo} time-derivative of order $b > 0$ and $(-\Delta)^{-\alpha / 2}$ is the fractional Laplacian. The fractional operator, $\mathcal{L}_{\alpha, \beta}$ will be thoroughly discussed in sections \ref{S:ISHWEmom} and \ref{S:Global} below. As for the stochastic wave equation, we direct the interested reader to \cite{dalang.khoshnevisan.ea:09:minicourse, milletsole.global2021, balan.chen.chen:03:invariant} to name a few. We will now briefly discuss the stochastic heat equation before introducing the first project of this thesis.

As for the well-possedness of Equation \eqref{E:introSHE} above, Chen and Dalang \cite{chen.dalang:15:moments} proved the following:
...
Describing the growth of the peaks is an interesting question on its own. At the moment, it is an open question to describe the growth of the sample paths in time to the stochastic heat equation, in other words, the growth of the function
	...
	\]
However, we do have some results on the growth in time of the $p^{th}$ moments of the solution for $p\ge2$. Some references that address this matter can found in \cite{chen.dalang:15:moments, chen.huang:19:comparison, chen.kim:19:nonlinear}. Namely, in many cases of interest, the $p^{th}$ moments to the solution of the stochastic heat equation grow exponentially in time. In the first project described in Section \ref{S:InvMeas} below, we will require bounded moments in time and we will see that this will require us to only consider the spatial dimension $d \ge 3$ and the noise, $\dot{W}$ to be colored in space rather than white. We will now continue this discussion more in the next section where we will rigorously define the stochastic heat equation.
...
\chapter{ \normalfont Exact Moment Asymptotics for the Interpolated Stochastic Heat and Wave Equation} \label{S:ISHWEmom}

text...


\chapter{ \normalfont Existence of Global Solutions to the ISHWE with a Super-linear Diffusion Term}\label{S:Global}
