...
\draw[black] (1.57,-.7) -- (1.57, -.7) node[yshift=\qmarkHeight/2]{?}; 
\end{axis}
\end{tikzpicture} 
\end{center}
\caption{Position of the string at a certain time before vibrations have ceased.}
...
With the weight, we define the invariant measure to be any probability measure $\eta$ on $\mathscr{B}(L^2_{\rho}(\R^d))$, the Borel subsets of $L^2_{\rho}(\R^d)$, such that:
...
\end{equation} 
where $u(t,x;\zeta)$ is the solution to the SHE starting from $\zeta \in L^2_{\rho}(\R^d)$. 

Note that this above equality says that the $\eta$-average of the laws $A \mapsto \mathscr{L}(u(t,\cdot;\zeta))(A)$ are invariant with respect to time which is key part of an ergodic dynamical system. We should mention that there an equivalent way to define an invariant measure which may 
...
\begin{enumerate}
	...
	\end{equation}
\end{enumerate}
where $\mathscr{L}$ denotes the law. Lastly, when one verifies the above steps, then by applying the Krylov–Bogoliubov existence theorem (\cite[Theorem 11.7]{da-prato.zabczyk:14:stochastic}), we can show that the invariant measure, $\eta$, takes the following form:
	...
	\end{equation}
	for some appropriately chosen sequence $\{T_n\}_{n\ge 1}$ with $T_n\uparrow \infty$ and where $\mathscr{B}(L^2_{\rho}(\R^d))$.

 It should be noted that the first bullet posses no issue to us here as the solution to the SHE is Markov (e.g. see \cite[Theorem 9.21]{da-prato.zabczyk:14:stochastic}). 

On the other hand, the second bullet point above is quite difficult to show and requires a lot of work. 

however proving uniqueness is a much harder question \cite{hairer.mattingly:06:ergodicity}
