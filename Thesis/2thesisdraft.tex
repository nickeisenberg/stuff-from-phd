...
Consider the situation where a perfectly round and infinitesimally thin piece of wire is initially heated. Also suppose that there is a source of heat that is a function of the temperature of the wire itself. Can we model the temperature of the wire as a function of position and time? In other words, does there exist a function, $u(t,x)$ such that $u(t,x)$ gives the temperature of the wire at a position $x$ and at a time $t$? The answer is yes, and this scenario described can be modeled by the well known heat equation: 
\[
\begin{cases}
\dfrac{\partial u}{\partial t}(t,x) - \dfrac{\partial^2 u}{\partial x^2}(t,x) = \rho(u(t,x)) & t \ge 0, \; x \in \R^d \\
u(0,x) = f(x) & x \in \R^d,
\end{cases}
\]
...
Following Walsh \cite{walsh:86:introduction}, we can improve the above system by introducing a Gaussian noise term, $\dot{W}$ (see ): 
