...
In the final chapter, we will turn our focus towards proving the existence of an invariant measure for the stochastic heat equation \eqref{E:introSHE}. Equations such as the SHE can be used by researchers to describe dynamical systems. Moreover, often there is a strong need to know the ergodic behavior if the dynamical system and no ergodic behavior can exists without an invariant measure, hence, proving the existence of such a measure is a crucial starting point. 

Da Prato and Zabczyk laid forth forth a schematic to prove existence in \cite{da-prato.g-atarek.ea:92:invariant}, which revolves around showing that the probability laws of the solution satisfy the following tightness condition:
	...
	\end{equation}
	where $\mathcal{K}$ is some compact set to be determined (see Theorem \ref{T:Main} below) and $
