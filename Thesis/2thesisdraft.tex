...
\end{cases}
\]
where $f(x)$ describes the initial temperature at a position $x$ and $\rho$ is a globally Lipschitz function. 

However, this above system is completely deterministic in the sense that there is no randomness involved. In other words, every single simulation of modeling the evolution of heat starting from $f(x)$ will be the same. In practice this is unrealistic and for sure there will be some randomness involved that will affect the evolution of the temperature of the wire in some random way. The need to model this randomness is the motivation behind the field of stochastic partial differential equations.
...
Following Walsh \cite{walsh:86:introduction}, we can improve the above system by introducing a Gaussian noise term, $\dot{W}$: 
	\begin{equation}\label{E:introSHE}
\begin{cases}
\dfrac{\partial u}{\partial t}(t,x) - \dfrac{\partial^2 u}{\partial x^2}(t,x  = \rho(u(t,x))\dot{W}(t,x) & t \ge 0, \; x \in \R^d \\
u(0,x) = f(x) & x \in \R^d.
\end{cases}
\end{equation}
The above equation can be legitimately interpreted as the following stochastic integral equation:
	\begin{equation} \label{E:introMsol}
u(t,x) = \int_{\R^d} G(t, x-y)f(y) \ud y + \int_0^t \int_{\R^d} G(t-s, x-y) \rho(u(s,y)) W(\ud s, \ud y),
\end{equation}
where the stochastic integral above is the {\em Walsh integral} and $G(t,x)$ is the Gaussian heat kernal
\[
...
In particular, in the following chapter we will study the following space-time fractional stochastic partial differential equation driven by the {\em time-independent noise} $\dot{W}(x)$ that interpolates both the stochastic heat and wave equation:
...
\end{equation}
where $I_k$ denotes the $k^{th}$ order Skorohod integral and the kernels $f_k$ are calculated through a standard Picard iteration procedure (see the discussion after Definition \ref{D:Sol} below). Because of an orthogonality result that is exhibited by integrals $I_k$ (e.g. \cite[Equation 4.1]{nualart.nualart:18:introduction}), one may deduce the following useful expression for the second moment of the solution (see Theorem \ref{T:2momseries} below):
\[
...
The main goals will be to prove existence and uniqueness of the solution (Theorem \ref{T:solvable}) and to calculate the following limits:
	...
	\]
where $\beta$ is a constant that is to be determined (Theorem \ref{T:asym} and Corollary \ref{C:Lim}). One should note that both the positivity and the finiteness of the above limits imply that the following functions exhibit exponential growth:
\[
t \mapsto \E\left(|u(t,x)|^p\right) 
\quad \text{and} \quad
p \mapsto \E\left(|u(t,x)|^p\right).
\]
The calculation of these limits is a very long an lengthly process so hold any further discussion on the, until Chapter \ref{Ch:ISHWEmom}.


However, as for the solvability of \eqref{E:IntroISHWE}, we prove that the solution may uniquely exist either {\em globally} or {\em locally}. We say a global solution exists if the $p^{th}$ moments, $\Norm{u(t,x)}_p$, exist for any $x \in \R^d$ and $t>0$. On the other hand, we say that a local solution exists when there exists two times $T_{1,p} < T_{2,p}$ such that for any $x \in \R^d$, the $p^{th}$ moments exist for $t<T_{1,p}$ and blow up for $t > T_{2,p}$. We prove that $T_{1,2} = T_{2,2}$ and in addition for any $p \ge 2$, we calculate $T_{1,p}$ and give precise conditions on when a solution will be global or local. It is worth mentioning that the calculation of the cutoff time, $T_{1,p}$, is only possible due to our ability to express the solution in terms of its Wiener Chaos expansion as in \eqref{E:IntroISHWEwcexp}.

\subsection{A brief overview of the second project}
In the final chapter, we will turn our focus towards proving the existence of an invariant measure for the stochastic heat equation \eqref{E:introSHE} on the whole space $\R^d$. Equations such as the SHE can be used by researchers to describe dynamical systems. Moreover, there often is a strong need to know the ergodic behavior if the dynamical system and no ergodic behavior can exists without an invariant measure, hence, proving the existence of such a measure is a crucial starting point. 

Since we work on the whole space, one needs to introduce a positive, bounded and continuous weight $\rho \in L^1(\R^d)$ and the corresponding $\rho$-weighted space of square integrable functions $L^2_{\rho}(\R^d)$ and we will see below in Theorem \ref{T:Lrho} that the solution starting from $\mu$, which we denote as $u(t,\cdot; \mu)$ is almost surely in this weighted space for a broad range on initial data which may be a measure such as the Dirac delta measure, an unbounded function such as $|x|^{-\alpha}$ with $0 < \alpha < d/2$ or simply a bounded continuous function.

A crucial property of the system that must be exhibited for an invariant measure to exist is the Markov property. Essentially this means that one must be able to restart their system whenever they choose and the system will act as if it is just "picking up where it left off". It makes sense that the SHE would satisfy this property as heat always moves from hot to cold. Thus no matter when we restart are system, the transfer of heat will just continue as if nothing happened. 

However, this restart property is not always satisfied. Consider the situation where we pull back a string that is tightly secured at both end points. When we release the the string, it will vibrate back and fourth until the string loses its momentum and ends back at rest. Well, when we stop this system we completely lose the information on which direction the string was moving in and how fast the string was moving (see Figure \ref{Fig:stringstop}). 
...
\end{tikzpicture} 
\end{center}
\caption{A still shot of two vibrating strings that happen to be at the same position but moving in opposite directions.}
\label{Fig:stringstop}
\end{figure}
When we re-release the string and restart the system, the string will always move in the direction of least resistance but this may not have been the direction the strong was moving in when stopped (see Figure \ref{Fig:stringstart}). So we can't restart this system in the sense that when we do restart, the system continues like nothing happened.
\begin{figure}[t]
\begin{center}
	...
	\end{tikzpicture} 
\end{center}
\caption{Direction of movement when re-releasing the string, regardless of the direction the string was moving when stopped.}
\label{Fig:stringstart}
\end{figure}

Because of the Markov property, the probability laws of the solution
	\[
	P_t(\zeta, A) := \mathscr{L}(u(t,\cdot;\zeta))(A) = \mathbb{P} \left[ \omega \in \Omega: u(t,\cdot; \zeta)(\omega) \in A \right], \quad A\in \mathscr{B}(L^2_{\rho}(\R^d)),
	\]
form a family of Markovian transition functions where $\mathscr{B}(L^2_{\rho}(\R^d))$ denotes the Borel subsets of $L^2_{\rho}(\R^d)$. Moreover, the transition functions form a {\em transition semi-group} defined as follows:
	\[
		P_t\varphi(x) = \int_{L^2_{\rho}(\R^d)} P_t(x, \ud y) \varphi(y), \quad \varphi \in \mathcal{B}(L^2_{\rho}(\R^d)),
	\]
 where $\mathcal{B}(L^2_{\rho}(\R^d))$ denotes the bounded Borel measurable functions on $L^2_{\rho}(\R^2)$. With this said, we define the invariant measure to be any probability measure $\eta$ on $\mathscr{B}(L^2_{\rho}(\R^d))$, such that
...
\end{equation} 
Recall that by definition, $P_0\varphi(x) = \varphi(x)$, and so \eqref{E:IntroInvMeas2} is essentially saying that the transition semi-group is {\em time-invariant} with respect to the invariant measure $\eta$ which is a key feature of ergodicity. 

We should also mention now that there is an equivalent way that one can define the invariant measure, and in fact it is the definition that we will choose to use for the remainder of this thesis. Any probability measure $\eta$
on the Borel $\sigma$-field $\mathscr{B}(H)$ is said to also be invariant for \eqref{E:SHE} if
...
Da Prato and Zabczyk laid forth forth a schematic to prove existence in \cite{da-prato.g-atarek.ea:92:invariant}, which revolves around showing the following:
\begin{enumerate}
	\item the probability laws of the solution form a {\em Markov transition function},
	\item  the existence of a compact set $\mathcal{K} \subset L^2_{\rho}(\R^d)$ such that the probability laws of the solution satisfy the following tightness condition:
	\begin{equation} \label{E:Introcpt}
	\mathscr{L}u(t,\cdot;\mu)(\mathcal{K})
	:=  \mathbb{P}\left( u(t,\cdot;\mu) \in \mathscr{K}\: \right) \ge 1-\epsilon,
	\qquad \text{for all $t\ge \tau>0$},
	\end{equation}
\end{enumerate}
where $\mathscr{L}$ denotes the law. Moreover, when one verifies the above steps, then by applying the Krylov–Bogoliubov existence theorem (\cite[Theorem 11.7]{da-prato.zabczyk:14:stochastic}) it can be shown that the invariant measure, $\eta$, takes the following form:
	...
	\end{equation}
	for some appropriately chosen sequence $\{T_n\}_{n\ge 1}$ with $T_n\uparrow \infty$ and where $\mathscr{B}(L^2_{\rho}(\R^d))$ (see Theorem \ref{T:Main} below).
