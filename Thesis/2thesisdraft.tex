...
\title{Solvability and exact moment asymptotics for the interpolated stochastic heat and wave equation and the existence of an invariant measure for a special case.}
\author{Nicholas Eisenberg} 
\date{August 6, 2022} %date of graduation
\copyrightyear{2022} %copyright year

\keywords{Stochastic partial differential equations, interpolated stochastic heat and wave equation, stochastic heat equation, invariant measure, exact moment asymptotics}
...
\professor{Xiaoying Han, Professor of Mathematics and Statistics}
...
\begin{abstract} 
This thesis will consist of two main projects, Chapters \ref{Ch:ISHWEmom} and \ref{Ch:InvMeas}, and a smaller project in Chapter \ref{Ch:SupLin}. We will be studying a general space-time fractional stochastic partial differential equation in Chapters \ref{Ch:ISHWEmom} and \ref{Ch:SupLin} and the stochastic heat equation in Chapter \ref{Ch:InvMeas}, which is a special case of the just mentioned space-time fractional equation. The aim of this thesis is to handle the following: solvability of the equations, deriving exact moment asymptotics and proving the existence of an invariant measure.
	
In Chapter \ref{Ch:ISHWEmom}, we study a class of space-time fractional stochastic partial differential equations subject to some time-independent multiplicative Gaussian noise.
...
It is hard to put into words how grateful I am to have had the opportunity to work with my advisor, Dr. Le Chen. I can not thank him enough for all the advice and encouragement he has given me along to way. I feel honored to have been his first PhD student and he will aways be considered to me as a great mentor and wonderful friend. \bigskip

I would like to thank the Graduate Program Officer here at Auburn University, Dr. Yanzhao Cao. Dr. Cao made my transfer to Auburn go very smoothly and he was always quick to respond to any question that I had for him. Dr. Erkan Nane was also a huge help and I thank him greatly for answering all my questions about the thesis and graduation process. \bigskip

I would also like to thank the members of my committee, Dr. Erkan Nane, Dr. Xiaoying Han, Dr. Jerzy Szulga and Dr. Jimmy Hilliard, for taking the time to approve this thesis and be apart of the processes with me. \bigskip

Lastly, I would like to thank both of my parents for always supporting me and being there for me throughout my academic career. I feel very fortunate to have parents who value education as much as mine do and there is no way I could be in the position I am in today without them. 
\end{acknowledgments}
...
However, the above system is completely deterministic in the sense that there is no randomness involved. In other words, every single simulation of modeling the evolution of heat starting from $\zeta(x)$ will be the same. In practice this is unrealistic and for sure there will be some randomness involved that will affect the evolution of the temperature. The need to model this randomness is the motivation behind the field of stochastic partial differential equations.

We will use a centerd, or $0$-mean, Gaussian noise, which we denote as $\dot{W}$, to introduce randomness into our system. The noise will always be uniquely defined by associating it with an appropriate positive-definite covariance functional of the following form:
	...
	\end{align}
where $\psi$ and $\phi$ are Schwartz functions, $\widetilde{\phi}(s,\cdot))(x) = \phi(s,-x)$ and $'*'$ denotes the spatial convolution. We will always assume that $\Gamma(\ud x) = f(x) \ud x$ is a non-negative and non-negative definite tempered measure and we will refer to $f$ as the {\em correlation function} and its Fourier transform, $\widehat{f}(\xi) = \int_{\R^d} \exp(-i x \cdot \xi) f(x) \ud x$, will be denoted as the {\em spectral density}. 
	\begin{definition}
		The noise $\dot{W}$ has the following names under the following scenarios.
		\begin{enumerate}
			\item {\em Space-time white} : $f(x) = \delta_0(x)$ where $\delta_0$ is the {\em Dirac delta distribution},
			\item {\em White in time and colored in space} : When $f(x)$ is a non-negative and nonnegative definite function. For example, $f$ may take the form of any of the following well known kernels: {\em Riesz kernel, Poisson kernel, Ornstein-Uhlenbeck kernel, or Bessel kernel}.
			\item {\em Time-independent} : When the integral in \eqref{E:IntroCor} is indepnedent of $s$, in other words, 
				\[
						\E \left[ \dot{W}\left(\psi\right)\dot{W}\left(\phi\right) \right]
					= \int_{\R^d} \Gamma(\ud x)(\psi(\cdot)*\widetilde{\phi}(\cdot))(x),
				\]
			and $\Gamma(\ud x)$ can be as in either of the two cases above. 
		\end{enumerate}
	\end{definition}
...
\begin{figure}[t]
	...
	\begin{subfigure}[b]{0.475\textwidth}   
		\centering 
		\includegraphics[scale=.35]{figs/SHE0.png}
		\caption[]%
		{{\small $\lambda = 0$}} 
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.475\textwidth}   
		\centering 
		\includegraphics[scale=.35]{figs/SHE1.png}
		\caption[]%
		{{\small $\lambda = 1$}}    
	\end{subfigure}
	\begin{subfigure}[b]{0.475\textwidth}
	\centering
	\includegraphics[scale=.35]{figs/SHE1.8.png}
	\caption[]%
	{{\small $\lambda = 1.8$}}    
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.475\textwidth}  
	\centering 
	\includegraphics[scale=.35]{figs/SHE2.png}
	\caption[]%
	{{\small $\lambda = 2$}}    
\end{subfigure}
	\caption[] 
...
%	\begin{subfigure}[b]{0.475\textwidth}
%		\centering
%		\includegraphics[width=\textwidth]{figs/SHE1.8.png}
%		\caption[]%
%		{{\small $\lambda = 1.8$}}    
%	\end{subfigure}
%	\hfill
%	\begin{subfigure}[b]{0.475\textwidth}  
%		\centering 
%		\includegraphics[width=\textwidth]{figs/SHE2.png}
%		\caption[]%
%		{{\small $\lambda = 2$}}    
%	\end{subfigure}
%	\caption[] 
...
The noise term, $\dot{W}$, brings fundamental changes to the solution $u(t,x)$ which can be seen from the simulations in Figure \ref{Fig:SHEsim1}. In particular, one can notice that as we increase $\lambda$, or the level of noise that we allow in our system, then the formation of taller and taller peaks begin to form. This phenomena is referred to as {\em intermittency}, see \cite{chen.kim:19:nonlinear, khoshnevisan.kim:15:non-linear, carmona.molchanov:95:stationary}. This concept will be one of the main focuses of Chapter \ref{Ch:ISHWEmom}, which I will now introduce.

\subsection{A brief overview of Chapter \ref{Ch:ISHWEmom}}
In the following chapter we will study the a space-time fractional stochastic partial differential equation driven by a {\em time-independent noise}, $\dot{W}(x)$. The equation interpolates both the stochastic heat and wave equations and has the following form:
...
\]
The calculation of these limits is a very long and lengthly process so we hold any further discussion until Chapter \ref{Ch:ISHWEmom}.

However, as for the solvability of \eqref{E:IntroISHWE}, we prove that the solution may uniquely exist either {\em globally} or {\em locally}. We say a global solution exists if for all $p \ge 2$, $x \in \R^d$ and $t>0$, the $p^{th}$ moment, $\Norm{u(t,x)}_p$, exists. On the other hand, we say that a local solution exists when there exists two times $T_{1,p} < T_{2,p}$ such that for all $p \ge 2$ and $x \in \R^d$, the $p^{th}$ moment exists for $t<T_{1,p}$ and blows up for $t > T_{2,p}$. We prove that $T_{1,2} = T_{2,2}$ and in addition, for all $p \ge 2$, we calculate $T_{1,p}$ and give precise conditions on when a solution will be global or local. It is worth mentioning that the calculation of the cutoff time, $T_{1,p}$, is only possible due to our ability to express the solution in terms of its Wiener Chaos expansion as in \eqref{E:IntroISHWEwcexp}.

\subsection{A brief overview of Chapter \ref{Ch:SupLin}}
In this chapter we will study the same space-time fractional SPDE as in the previous chapter, however, in this case we will assume the noise is space-time white and that the diffusion term exhibits the following super-linear growth property as $|x|,\: |z| \to \infty$:
\[
		|\sigma(x)-\sigma(z)| \le \sigma_2|x-z|[\ln_+(|x-z|)]^{\delta},
\]
where $\sigma_2, \delta >0$ and $\ln_+(z) := \ln(z \vee e)$ for $z >0$. Moreover, we consider more general initial data, $u_0$ and $v_0$, than from the equation studied in Chapter \ref{Ch:ISHWEmom}. More precisely, we allow for any Borel-measurable initial conditions which satisfy for all $T>0$ the following:
	\[
		\sup_{(x,y) \in [0,T] \times \R^d} |J_0(t,x)| < \infty, 
	\]
where $J_0$ is the solution to the homogeneous equation (see \eqref{E:IntroGlobalMild} below). With these slight changes, the equation takes the following form:
\begin{equation} \label{E:IntroISHWEGlobal}
\begin{cases}
\left(\partial^b_t + \frac{\nu}{2}(-\Delta)^{a/2}\right)\: u(t,x) = I^r_t \left[\sigma( u(t,x))\: \dot W(t,x) \right] & \text{$x\in \R^d$, $t>0$}, \\
u(0,\cdot) = u_0(\cdot)                                                                                                           & b \in (0,1],               \\
u(0,\cdot) = u_0(\cdot), \quad \partial_t u(0,\cdot) = v_0(\cdot)                                                                            & b \in (1,2).
\end{cases}
\end{equation}

As before, this is purely notational, and Equation \eqref{E:IntroISHWEGlobal} is legitimately viewed as the following stochastic integral equation:
	\begin{equation}\label{E:IntroGlobalMild}
u(t,x) = J_0(t,x) + I(t,x),
	\end{equation}
where
\[
...
The goal of this chapter is to prove the existence and uniqueness of a global solution to \eqref{E:IntroISHWEGlobal}  (see Theorem \ref{T:MainSupLin} below). The work is highly motivated by the recent work by Millet and Sanz-Sol\'e \cite{millet.sanz-sole:21:global}. It is a well studied phenomena that either a super-linear drift or diffusion term may cause blow-up of the solution. As for the stochastic heat equation, we direct the reader to \cite{foondun.nualart:21:osgood, mueller.sowers:93:blowup, dalang.khoshnevisan.ea:19:global, bonder.groisman:09:global}. On the other hand, the only other work that we are aware of that is dedicated to proving some non-existence results of \eqref{E:IntroISHWEGlobal} is \cite{asogwa.mijena.ea:20:blow-up}. So, to the best of our knowledge, this is the first work on proving the global existence of a solution to \eqref{E:IntroISHWEGlobal} with a super-linear diffusion term. However, as we will see, the calculations given below are essentially identical to those performed in \cite{millet.sanz-sole:21:global}, which is the cause for our motivation.

We will end this section with some comments on trying to prove Theorem \ref{T:MainSupLin} below but for a noise that is white in time and colored in space. This problem was answered in \cite[Theorem 4.13]{millet.sanz-sole:21:global} for the stochastic wave equation where global existence and uniqueness is proven. The challenge with applying their techniques to the space-time fractional equation, as in \eqref{E:IntroISHWEGlobal}, comes from the increased complexity of the fundamental solutions. In particular, proving corresponding upper bounds for $R_1, \cdots, R_4$ from the proof of Theorem 4.10 {\em ibid} becomes less clear and will be saved for a future project. 

\subsection{A brief overview of Chapter \ref{Ch:InvMeas}}
In the final chapter, we will turn our focus towards proving the existence of an invariant measure for the stochastic heat equation \eqref{E:introSHE} on the whole space $\R^d$. Equations such as the SHE can be used by researchers to describe dynamical systems. Moreover, there often is a strong need to know the ergodic behavior of the dynamical system and no ergodic behavior can exist without an invariant measure. Hence proving the existence of such a measure is a crucial starting point. 

Since we work on the whole space, one needs to introduce a positive, bounded and continuous weight $\rho \in L^1(\R^d)$ and the corresponding $\rho$-weighted space of square integrable functions $L^2_{\rho}(\R^d)$ (see Section \ref{SS:Weight}). We will see below in Theorem \ref{T:Lrho}, that the solution starting from the initial condition $\mu$, which we denote as $u(t,\cdot; \mu)$, is almost surely in this weighted space for a broad range of initial conditions, which includes all bounded functions, some unbounded functions such as $|x|^{-\alpha}$ with $0 < \alpha < d/2$ and even some measures such as the Dirac delta measure.
...
We define the probability laws of the solution as
	...
	\]
where $\mathscr{B}(L^2_{\rho}(\R^d))$ denotes the Borel subsets of $L^2_{\rho}(\R^d)$ and $\zeta$ is the initial condition. Because of the Markov property of the solution, the laws form a family of {\em Markovian transition functions}, $P_t(\zeta, A) := \mathscr{L}(u(t,\cdot;\zeta))(A)$, for $\zeta \in L^2_{\rho}(\R^d)$. Moreover, the transition functions form a {\em transition semi-group} defined as follows:
	\[
...
Tessitore and Zabczyk laid forth a schematic to prove the existence of an invariant measure in their paper \cite{tessitore.zabczyk:98:invariant}, which revolves around showing the following:
\begin{enumerate}
...
\end{enumerate}
Moreover, when one verifies the above steps, then by applying the Krylov–Bogoliubov existence theorem (\cite[Theorem 11.7]{da-prato.zabczyk:14:stochastic}), it can be shown that the invariant measure, $\eta$, takes the following form for any $t_0 > 0$:
	...
	\end{equation}
where $\{T_n\}_{n\ge 1}$ is an appropriately chosen sequence with $T_n\uparrow \infty$ (see Theorem \ref{T:Main} below).

Note that we already discussed that item 1 above is satisfied in our setting. On the other hand, the second item will require some effort to show. A sufficient condition that is more easily verified which implies this boundedness condition is the following:
	\begin{align} \label{E:IntroBddMnt}
	\sup_{t>0} \E\left(\Norm{u(t,\cdot)}_{\rho}^2\right)
	<\infty.
	\end{align}
We remind the reader that for the SHE, moments usually grow exponentially in time (e.g. see \cite[Theorem 1.3]{chen.kim:19:nonlinear}), and so for the above finiteness to occur, we will need additional assumptions (e.g. see \eqref{E:U0finite} below). 
...
thanks to Bochner's theorem. Throughout this chapter, we will use $\mathcal{F}\phi(\xi) = \int_{\R^d}
...
\begin{remark} \label{R:Nonnegative}
	...
	we have the following four groups of sufficient conditions \footnote{Note that when $d\ge 1$,
		...
		$Y$, which is the fundamental solution $G$ in this chapter, is nonnegative provided $r=0$ or
...
of the current thesis. In this chapter, by working on a more general class of SPDEs, we are able to
...
overcome by studying the Wiener chaos expansion of the solution. In this chapter, we follow the same
...
Now let us state the main results of this chapter. The first main result deals with the well-posedness
...
The second main result of this chapter is about the moment asymptotics. We use $\Norm{\cdot}_p$ to
...
This chapter is organized as follows. In Section \ref{S:SolvEx}, we first give some concrete examples,
...
Section \ref{S:Pre}, we present some preliminaries, including the {\it Skorohod}
...
--- Section \ref{S:Appendix}, we list a few proofs of results that will be used.
...
examples for our results. To the best of our knowledge, all results in this chapter for either $b\ne
...
$\Norm{\widetilde{f_n}(\cdot,x,t)}_{\mathcal{H}^{\otimes n}}$ play an important role in this work.
...
\begin{proof}[Sketch of the proof of Theorem \ref{T:Variation}]
	...
	correspondence of parameters between Bass {\it et al} \cite{bass.chen.ea:09:large} and the current chapter is listed
...
chapter, we can no longer apply this inequality. Instead, we prove the following lemma using the {\it
	weak Young's inequality} (see, e.g., \cite[p.107]{lieb.loss:97:analysis}) as a generalization of Lemma 1.6
...
\begin{proof}
	...
	(see, e.g., \cite[eq. (7) on p. 107]{lieb.loss:97:analysis}), which says that for all  $p,q,r >1$ with $1/p +
...
\begin{remark}
	...
	the Riesz kernel, by \cite[(6) on p.  107]{lieb.loss:97:analysis}, we see that
	\begin{align*}
...
\chapter{\normalfont Global Solutions of the Interpolated Stochastic Heat and Wave Equation with a Super-linear Diffusion Term} \label{Ch:SupLin}
...
\]
where $\delta_0$ denotes the {\em Dirac delta distribution}.
...
\begin{proof}
	...
	\[
	\sigma_N(x) = \sigma(x) \textbf{1}_{|x| \le N} + \sigma(N)\textbf{1}_{x > N} + \sigma(-N)\textbf{1}_{x< -N}.
	\]
...
denoted by $\widehat{f}(\ud \xi)$ \footnote{See Remark \ref{R:Fourier} for the convention of Fourier
	transform that is used in this chapter.}. Moreover, in the case where $\Gamma$ has density $f$,
...
The aim of this chapter is to investigate the conditions required to guarantee the existence of an invariant measure for the solution to \eqref{E:SHE}, which is a crucial step towards the study of
...
\cite{tessitore.zabczyk:98:invariant}, with which we will follow closely in this chapter, a
few other papers that consider the whole space include \cite{assing.manthey:03:invariant, misiats.stanzhytskyi.ea:20:invariant,
	misiats.stanzhytskyi.ea:16:existence}. One thing to note is that the just mentioned papers use the theory of the stochastic integral developed by Daprato et al \cite{da-prato.zabczyk:14:stochastic} while we use the theory developed by Walsh \cite{walsh:86:introduction}. As far as we know, this is the first work which handles proving the existence of an invariant measure using Walsh's theory. Lastly,  since the space $\R^d$ is not compact, one needs to work on a
...
a drift (or reaction-diffusion) term $f(x,u)$ is included in \eqref{E:SHE} in a crucial way to help cancel the otherwise exponential growing moments. The absence of such a drift term in this chapter
...
Now we are ready to state our two main results of this chapter.
...
This chapter is organized as follows: In Section \ref{S:Example}, we will further discuss our main results and provide some examples. In particular, in Section \ref{SS:Weight}  we make some comments on the weight function and in Section \ref{SS:Init} we show that our results could include a wider class of initial conditions. Finally, the two main Theorems \ref{T:Lrho} and \ref{T:Main} will be proved in Sections \ref{S:Lrho}
and \ref{S:mainproof}, respectively.
\bigskip

We end this section by introducing some notation and formulas that we use throughout the paper. We will use $\Norm{X}_p $ to denote the $L^p(\Omega)$ norm, namely,
...
\end{equation}
which can be easily verified. Next, we remind the reader of the following spherical coordinate integration formula
\[
...
below. Lastly, $\Gamma(z)$ will be used to denote the Gamma function. 
...
The admissible condition \ref{E:aw} is needed in this chapter due to the following result:
...
L_b^{-2}> \frac{\Gamma(d/2 -1)2^{d/2-2}}{(2\pi)^{2d}} \int_{\R^d} \left(\left|  \mathcal{F}\left(\sqrt{ \widehat{f}\: }\right) \right| * \left| \mathcal{F}\left(\sqrt{ \widehat{f}\: }\right)\right|\right) (\zeta) |\zeta|^{2-d} \ud \zeta ,
\end{equation}
where the convention of the Fourier transform is given in Remark \ref{R:Fourier}. With these assumptions,
...
\begin{remark} \label{R:Fourier}
	...
	this chapter (as in \cite{chen.kim:19:nonlinear,chen.huang:19:comparison}), we use the convention
	that
	...
	corresponds to $\left(2\pi\right)^{-d/2}\widehat{f}$ in this chapter. Our equation \eqref{E:TZspec}, which is condition (3.4) \cite{tessitore.zabczyk:98:invariant}, takes into account this difference therefore explaining the slightly
...
One should note that the assumption that $\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x)$ is nonnegative, or equivalently that $\sqrt{\widehat{f}}$ is non-negative definite, is quite strong and may not be true even if $f$ is non-negative and non-negative definite. Indeed, suppose that $f$ was such that $\widehat{f}(\xi) = 2^{-2}\max\{2-|\xi|,0\}$. Then $f$ is non-negative and non-negative definite, which is shown in Example \ref{Ex:sqrt} below. We will now show that in this case, $\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(\zeta)$ takes on both positive and negative values. Since $\sqrt{\widehat{f}(\xi)} = 2^{-1}\sqrt{\max\{2-|\xi|,0\}}$ is even, the inverse Fourier transform takes the following form:
\[
\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x) = 2 \int_0^2 2^{-1}\sqrt{2-y} \cos(y\: x) \ud y.
\]
For simplicity, we will only consider the case $\zeta>0$ and real. Therefore 
\begin{align*}
\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x) &= 2 \int_0^{\sqrt{2}} y^2 \cos(2x- x y^2) \ud y \\
&= 2\cos(2x) \int_0^{\sqrt{2}} y^2 \cos(x y^2) \ud y  +2 \sin(2x) \int_0^{\sqrt{2}} y^2 \sin(x y^2) \ud y.
\end{align*}
Where in the first equality we used the change of variables, $y' = \sqrt{2 - y}$. We now do an integration by parts to see that 
\begin{align*}
\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x) &= - \cos(2x) \int_0 ^{\sqrt{2}} \frac{\sin(y^2 x)}{x} \ud y + \sin(2x) \int_0 ^{\sqrt{2}} \frac{\cos(y^2 x)}{x} \ud y,
\end{align*}
and lastly we apply the change of variables $y' = y\sqrt{2x/\pi}$ too see 
\[
\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x) = \frac{ \sqrt{\frac{\pi}{2}}\left(-\cos(2x)\mathcal{S}\left(\frac{2\sqrt{x}}{\sqrt{\pi}}\right) + \sin(2x) \mathcal{C}\left( \frac{2 \sqrt{x}}{\sqrt{\pi}} \right) \right)}{ x^{3/2} },
\]
	
	\begin{figure}
		\centering
		\includegraphics[scale=.9]{figs/fresnel.png}
		\caption{A plot of  $\mathcal{F}^{-1}\left(\sqrt{\widehat{f}}\right)(x)$ for $\widehat{f}(\xi) = 2^{-2}\max\{2-|\xi|,0\}$ with $-8 \le x \le 8$.}
	\end{figure}
