...
\begin{enumerate}
	...
	\end{equation}
\end{enumerate}
where $\mathscr{L}$ denotes the law and $L^2_{\rho}(\R^d)$ denotes the $\rho$-weighted space of square integrable functions for some positive, bounded and continuous function $\rho \in L^1(\R^d)$. Lastly, when one verifies the above steps, then by applying the The Krylov–Bogoliubov existence theorem (\cite[Theorem 11.7]{da-prato.zabczyk:14:stochastic}) we can show that the invariant measure, which we denote as $\eta$, takes the following form:
