...
\begin{NickQuestion}[Lemma 3.6 page 14]
	How do we get the inequality right above equation (3.13)?
\end{NickQuestion}

\begin{NickQuestion}[page 14]
	How does (1.22) imply the scaling property (3.15)?
\end{NickQuestion}

\begin{NickQuestion}[Proof of Theorem 3.1]
	
		In the first line of the proof of Theorem 3.1 it says that by theorem 2.2 it suffices to show that 
		\[
			\sum_{n \ge 0} \theta^nn!\Norm{\tilde{f}_n(\cdot,0,t)}_{\mathcal H^{\otimes n}}^2 < \infty 
		\]
		but this is not quite true since Theorem 2.2 says we need 
		\[
			\sum_{n \ge 0} \theta^nn!\Norm{\tilde{f}_n(\cdot,x,t)}_{\mathcal H^{\otimes n}}^2 < \infty 
		\]
		for every $x \in \R^d$. 
\end{NickQuestion}

\begin{NickQuestion}[Proof of Theorem 4.1]
\begin{enumerate}
	\item I am confused about the first line of the proof? Why does 
	\[
	\mathbb{E}(|u(t,x)|^2) = \sum_{n \ge 0} \theta^nn!\Norm{\tilde{f}_n(\cdot,0,t)}_{\mathcal H^{\otimes n}}^2
	\]
	According to theorem 2.2 it should be 
	\[
			\mathbb{E}(|u(t,x)|^2) = \sum_{n \ge 0} \theta^nn!\Norm{\tilde{f}_n(\cdot,x,t)}_{\mathcal H^{\otimes n}}^2
	\]
	but then the rest of this proof would be incorrect since te very next line you use the scaling property for $\Norm{\tilde{f}_n(\cdot,0,t)}_{\mathcal H^{\otimes n}}^2$. 
	\item 	In part (iii) there is a scaling property used for $\Norm{\tilde{f}_n(\cdot,x,t)}_{\mathcal H^{\otimes n}}^2$ but a scaling property has never been proved for this, only $\Norm{\tilde{f}_n(\cdot,0,t)}_{\mathcal H^{\otimes n}}^2$. You said that by the scaling property that 
	\[
			\mathbb{E}(|u(t,x)|^2) = \sum_{n \ge 0} \theta^nn!\Norm{\tilde{f}_n(\cdot,x,t)}_{\mathcal H^{\otimes n}}^2 =  \sum_{n \ge 0} \theta^nn!t^n\Norm{\tilde{f}_n(\cdot,0,1)}_{\mathcal H^{\otimes n}}^2
	\]
	but this doesnt seem correct since if this is supposed to be true for every $x \in \R^d$ then this would mean that 
	\[
	\Norm{\tilde{f}_n(\cdot,0,t)}_{\mathcal H^{\otimes n}}^2= 	t^n\Norm{\tilde{f}_n(\cdot,0,1)}_{\mathcal H^{\otimes n}}^2
	\]
	which is not the same as the scaling property (3.15). 
\end{enumerate}
\end{NickQuestion}

\begin{NickQuestion}[Lemma 6.2 proof]
	After doing the substitution, we have 
	\begin{align*}
	&\int_{(\R^d)^n} \prod_{k=1}^n \phi(\xi_k)\int_0^\infty e^{-t} \int_{[0,t]^n<} \prod_{k=1}^n \mathcal{F}G(s_k-s_{k-1})(\xi_k,...,\xi_n) \ud \textbf{s}\ud t\mu(\xi_1)...\mu(\xi_n)
		\\&=\int_{(\R^d)^n} \prod_{k=1}^n \phi(\xi_k)\left(\int_0^\infty e^{-u} \ud u\right)\prod_{k=1}^n\left(\int_0^\infty e^{-u_k}\mathcal{F}G(u_k,\cdot)(\xi_k,...,\xi_n) \ud u_k\right)\mu(\xi_1)...\mu(\xi_n)
	\end{align*}

Where does the $e^{-u_k}$ come from? Since we let $t-s_n=u$ shouldn't there only be a $e^{-u_n}$? 

Also I see how $u_i \in (-\infty,\infty)$  shows that $\textbf{s} \in [0,t]^n<$ but how could we see that $\textbf{s} \in [0,t]^n<$ shows that $u_i \in (-\infty,\infty)$?

\end{NickQuestion}

\begin{NickQuestion}[Prop 6.3 proof]
	At the equality above (6.6), why were we able to remove the $\frac{1}{n}\sum_{\sigma \in \Sigma_n}$?
\end{NickQuestion}

\begin{NickQuestion}
	In (6.9) where does the $\sum_{\sigma \in \Sigma_n}$ come from? 
\end{NickQuestion}

\begin{NickQuestion}[page 29]
	Three centered lines starting with $\rho(\mathcal{F}f)$ 
\end{NickQuestion}

\begin{NickQuestion}
	I might have something that shows that 
	\[
		H(\xi) := \int_{\R^d} \dfrac{h(\xi+\eta)h(\eta)}{\sqrt{1+|\xi + \eta|^2}\sqrt{1+|\eta|^2}} \ud \eta
	\]
	is even if and only if $h$ is even. 
	
	First suppose that $h$ is even. Then, 
	\begin{align*}
			H(-\xi) &= \int_{\R^d} \dfrac{h(-\xi+\eta)h(\eta)}{\sqrt{1+|-\xi + \eta|^2}\sqrt{1+|\eta|^2}} \ud \eta
			\\&= \int_{\R^d} \dfrac{h(\xi-\eta)h(\eta)}{\sqrt{1+|\xi - \eta|^2}\sqrt{1+|\eta|^2}} \ud \eta
			\\&= \int_{\R^d} \dfrac{h(\xi+u)h(-u)}{\sqrt{1+|\xi +u|^2}\sqrt{1+|-u|^2}} \ud u
			\\& = \int_{\R^d} \dfrac{h(\xi+u)h(u)}{\sqrt{1+|\xi +u|^2}\sqrt{1+|u|^2}} \ud u
			\\& = H(\xi)
	\end{align*}
and so $H$ is even. 

Now suppose that $h$ is not even. Then,
\begin{align}
		H(-\xi) &= \int_{\R^d} \dfrac{h(-\xi+\eta)h(\eta)}{\sqrt{1+|-\xi + \eta|^2}\sqrt{1+|\eta|^2}} \ud \eta \nonumber
		\\&= -\int_{\R^d} \dfrac{h(\xi+u)h(u+2\xi)}{\sqrt{1+|\xi + u|^2}\sqrt{1+|u+2\xi|^2}} \ud \eta \label{cont1}
\end{align}
\textcolor{blue}{This is incorrect, there should not be a minus sign here. This proof is incorrect.} 
where above we use the substitution $-\xi +\eta = \xi +u$. Recall that our problem deals with looking at 
\[
	\sup_{\Norm{h}_{L^2(\R^d)}=1} \int_{\R^d} \mathcal{F}{f}(\xi) \left[ \int_{\R^d} \dfrac{h(\xi+\eta)h(\eta)}{\sqrt{1+|\xi + \eta|^2}\sqrt{1+|\eta|^2}} \ud \eta \right] \ud \xi
\]
where $\mathcal{F}{f}(\cdot)$ is non-negative and non-negative definite. Since it is non-negative we can restrict our choices of $h$ to also be non-negative and in particular we have that 
\[
	\int_{\R^d} \dfrac{h(\xi+u)h(u+2\xi)}{\sqrt{1+|\xi + u|^2}\sqrt{1+|u+2\xi|^2}} \ud \eta \ge 0
\]
but then this means that 
\begin{align}
	-\int_{\R^d} \dfrac{h(\xi+u)h(u+2\xi)}{\sqrt{1+|\xi + u|^2}\sqrt{1+|u+2\xi|^2}} \ud \eta \le 0. \label{cont2}
\end{align}
But \eqref{cont1} and \eqref{cont2} then implies that 
\[ 
	H(-\xi) \le 0
\]
and since we assume $h \ge 0$ then $H(\xi) \ge 0$ for all $\xi \in \R^d$ and this mean $H(\xi)=0$ which can not always be the case. Thus we must have $h$ to be even in order to get a non-zero $H(\cdot)$. 
\end{NickQuestion}
\end{document}