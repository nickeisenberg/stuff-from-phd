...
% }}}
	\begin{frame}[t]% {{{ SHE and SWE
		...
		\begin{equation*}
		\begin{cases}
		\left(\partial^b_t  -\Delta \right)\: u(t,x) = \sqrt{\theta}\: u(t,x)\: \dot W(x) & \text{$x\in \R^d$, $t>0$} \\
	...
	\only<2>{
	\begin{center}
		\textcolor{red}{$b\in (1,2)$ and $a \in (0,2]$ ??}
...
\end{frame}% }}}
		...
		\begin{frame}[t]
	\frametitle{The Interpolated Stochastic Heat and Wave Equation (ISHWE)}
	\begin{equation*}
	\begin{cases}
...
\end{frame}% }}}
	...
	\]
where $G$ is defined through the Fox-H function.
\\ $\;$ \\
...
\begin{frame}
	...
	\begin{definition}
		$u(t,x)$ is a \textit{global solution} to the ISHWE if $\Norm{u(t,x)}_p < \infty$ for any $t>0$ and $x \in \R^d$.
	\end{definition}
	$\;$ \\
	\begin{theorem}[Theorem 1.6 Chen and Eisenberg '21+]
...
\begin{frame}
	...
	\begin{definition}
		$u(t,x)$ is a \textit{local solution} to the ISHWE if there exists $0 < T_a \le T_b < \infty$ such that $\Norm{u(t,x)}_2 < \infty$ when $0<t<T_a$ and $\Norm{u(t,x)}_2 $ D.N.E. (does not exist) for $t> T_b$.
	\end{definition}
	$\;$ \\
	\begin{theorem}[Theorem 1.6 Chen and Eisenberg '21+]
...
\begin{frame}[t]
	...
	\begin{theorem}[{\small Theorem 2.3 X. Chen '07}]
		\begin{align*}
...
Using this scaling property we see that
	...
	\begin{lemma}[Lemma 4.1 R.  Balan, L. Chen, X. Chen. '21]
...
\end{frame}
	...
	\begin{theorem}[R. Bass, X. Chen and M. Rosen '09]
		\[
...
\end{frame}
	...
	\begin{theorem}[Theorem 1.7 Chen and Eisenberg '21+]
...
\end{frame}
	...
	\begin{corollary}[Corollary 1.8 Chen and Eisenberg '21+] For $p \ge 2$ fixed, we have that
		\begin{align*}
...
\begin{frame}[t]
	...
	\begin{corollary}[Corollary 1.8 Chen and Eisenberg '21+] For $t>0$ fixed , we have that
		\begin{align*}
