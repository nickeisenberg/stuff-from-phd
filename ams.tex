...
\begin{frame}[t]
	...
	\\&= \sum_{n \ge 0} \frac{\theta^nt^{(4-\alpha)n}}{(n!)^{3-\alpha}} R_n
	\end{align*}
	and $R_n = (n!)^{4-\alpha}\Norm{\widetilde{f}_n(\cdot,0,1)}_{\mathcal{H}^{\otimes n}}^2$. When $\alpha = 3$, then the above reduces down to
	\[
...
The following scaling property can be easily derived by a change a variables.
	\[
