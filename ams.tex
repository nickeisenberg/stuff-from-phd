...
\end{frame}
	...
	\begin{theorem}
		Suppose a global solution to the ISHWE exists and $\dot{W}$ is given through the generalized Riesz kernel defined below. Then, 
		\begin{align*}
...
\end{frame}
	...
	\begin{itemize}
		...
		\item \textit{Local Solution:} If there exists $0 < T_a \le T_b < \infty$ such that $\Norm{u(t,x)}_2 < \infty$ when $0<t<T_a$ and $\Norm{u(t,x)}_2 $ D.N.E. (does not exist) for $t> T_b$.
	\end{itemize}
\end{frame}
